    ...
    vtkDataObject *&mesh)
{
  ...
  vtkSmartPointer<vtkOverlappingAMR> tmpMesh =
    vtkSmartPointer<vtkOverlappingAMR>::New();

  tmpMesh->Initialize(internals.sim->max_levels+1,
  ...
  bool *spacingSet = new bool[internals.sim->max_levels + 1];
  ...
  for(int i = 0; i < np; ++i)
    ...
    if(!spacingSet[patches_this_rank[i]->level])
      {
      tmpMesh->SetSpacing(patches_this_rank[i]->level, spacing);
    ...
    tmpMesh->GetLevelAndIndex(domain, mylevel, mypatch);
#ifdef DEBUG_GET_MESH
    if(f != NULL)
      {
    ...
    tmpMesh->SetAMRBox(mylevel, mypatch, box);
    tmpMesh->SetDataSet(mylevel, mypatch, p);
    p->Delete();
    }
...
#endif
    tmpMesh->SetRefinementRatio(i, internals.sim->refinement_ratio);
    }
#ifdef DEBUG_GET_MESH
    if(f != NULL)
      fclose(f);
#endif
  delete [] spacingSet;
  patch_free_flat_array(patches_this_rank);
  ...
  sensei::MeshMetadataPtr &metadata)
{
  ...
  metadata->NumLevels = internals.sim->max_levels;
  ...
  for(int i = 0; i < np; ++i)
    {
    if ((local_patches[i]->nowners > 1) &&
      (local_patches[i]->owners[0] != internals.sim->par_rank))
      continue;
    ...
    metadata->BlockLevel.push_back(local_patches[i]->level);
