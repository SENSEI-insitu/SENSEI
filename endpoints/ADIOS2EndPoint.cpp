...
using DataAdaptorPtr = vtkSmartPointer<sensei::ADIOS2DataAdaptor>;
 ...
 * workflow. It can read a data-stream using ADIOS2=SST. When enabled, this end point
...
int main(int argc, char **argv)
{
  ...
  std::string fileName;
  std::string readengine("BP4");
  std::string config_file;

  opts::Options ops(argc, argv);
  ops >> opts::Option('r', "readengine", readengine, "specify read method: BP3, BP4, or SST ")
  ...
  if (!showHelp && !haveInput && (rank == 0))
    SENSEI_ERROR("Missing ADIOS2 fileName stream")
  ...
  // open the ADIOS2 stream using the ADIOS2 adaptor
  DataAdaptorPtr dataAdaptor = DataAdaptorPtr::New();

  dataAdaptor->SetCommunicator(comm);
  dataAdaptor->SetReadEngine(readengine);
