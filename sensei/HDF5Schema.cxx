...
static void gGetNameStr(std::string &out, unsigned int meshID,
...
static void gGetArrayNameStr(std::string &out, unsigned int meshID,
...
//
//
HDF5VarGuard::HDF5VarGuard(hid_t varID) : m_VarID(varID) {
...
void HDF5VarGuard::ReadSlice(void *buf, int ndim, const hsize_t *start,
...
//
//
StreamHandler::StreamHandler(bool readmode, const std::string &hostFile,
                             BasicStream *client)
    : m_TimeStepId(-1), m_InReadMode(readmode), m_FileName(hostFile),
      m_Client(client) {}
...
DefaultStreamHandler::DefaultStreamHandler(const std::string &hostFile,
  ...
  if (m_HostFileId >= 0)
    client->ReadNativeAttr(senseiHDF5::ATTRNAME_NUM_TIMESTEP,
                           &(m_TimeStepTotal), H5T_NATIVE_UINT, m_HostFileId);
}

DefaultStreamHandler::DefaultStreamHandler(const std::string &hostFile,
                                           WriteStream *client)
    : StreamHandler(false, hostFile, client) {
  m_HostFileId = H5Fcreate(hostFile.c_str(), H5F_ACC_TRUNC, H5P_DEFAULT,
                           client->m_PropertyListId);
}
...
bool DefaultStreamHandler::AdvanceStream() {
  ...
  if (m_InReadMode) {
    ...
    if (m_TimeStepCounter > 0)
      CloseStream();
    m_TimeStepId = H5Gcreate2(m_HostFileId, stepName.c_str(), H5P_DEFAULT,
...
bool DefaultStreamHandler::Summary() {
  ...
  if (!writer->WriteNativeAttr(senseiHDF5::ATTRNAME_NUM_TIMESTEP,
                               &(m_TimeStepCounter), H5T_NATIVE_UINT,
                               m_HostFileId))
    return false;
...
bool PerStepStreamHandler::IsValid() { return (m_NumStepsWritten > 0); }
...
bool PerStepStreamHandler::OpenStream() {
  ...
  m_TimeStepId = H5Fopen(currStepFileName.c_str(), H5F_ACC_RDONLY,
...
bool PerStepStreamHandler::AdvanceStream() {
  ...
  if (m_InReadMode) {
    ...
    if (m_TimeStepCounter > 0)
      CloseStream();
    m_TimeStepId = H5Fcreate(stepName.c_str(), H5F_ACC_TRUNC, H5P_DEFAULT,
                             m_Client->m_PropertyListId);
  }
...
ReadStream::~ReadStream() { m_Streamer->Summary(); }
...
bool ReadStream::AdvanceTimeStep(unsigned long &time_step, double &time) {
  ...
  if (!ReadNativeAttr(senseiHDF5::ATTRNAME_TIMESTEP, &time_step,
                      H5T_NATIVE_ULONG, -1))
    return false;
...
bool ReadStream::ReadNativeAttr(const std::string &name, void *val,
                                hid_t h5Type, hid_t hid) {
  if (hid == -1)
...
bool ReadStream::ReadVar1D(const std::string &name, hsize_t s, hsize_t c,
                           void *data) {
...
bool ReadStream::ReadBinary(const std::string &name,
...
bool ReadStream::ReadMetadata(unsigned int &nMesh) {
  if (!ReadNativeAttr(senseiHDF5::ATTRNAME_NUM_MESH, &(nMesh), H5T_NATIVE_UINT,
                      -1))
    return false;
...
bool ReadStream::ReadMeshMetaData(unsigned int i,
                                  sensei::MeshMetadataPtr &ptr) {
...
bool ReadStream::ReadMesh(std::string name, vtkDataObject *&dobj,
                          bool structure_only) {
...
bool MeshFlow::ReadArray(ReadStream *reader, const std::string &array_name,
                         int association) {
...
bool MeshFlow::Initialize(const sensei::MeshMetadataPtr &md,
                          ReadStream *input) {
  //
...
bool WorkerCollection::load(unsigned int block_id, vtkCompositeDataIterator *it,
                            ReadStream *reader) {
  for (size_t i = 0; i < m_Workers.size(); i++) {
...
//
//
ArrayFlow::ArrayFlow(const sensei::MeshMetadataPtr &md, unsigned int meshID,
...
bool ArrayFlow::load(unsigned int block_id, vtkCompositeDataIterator *it,
                     ReadStream *reader) {
...
bool ArrayFlow::unload(unsigned int block_id, vtkCompositeDataIterator *it,
                       WriteStream *output) {
  vtkDataSet *ds = dynamic_cast<vtkDataSet *>(it->GetCurrentDataObject());
  if (!ds) {
    SENSEI_ERROR("Failed to get block " << block_id);
  ...
  output->WriteVar1D(m_ArrayPath, arraySpace, h5TypeCurrArray,
                     da->GetVoidPointer(0));
...
bool PolydataCellFlow::load(unsigned int block_id, vtkCompositeDataIterator *it,
                            ReadStream *reader) {
  ...
  if (!reader->ReadVar1D(m_CellTypeVarName, ct_start, ct_count,
                         cell_types.data()))
    return false;
  ...
  ;
  if (!reader->ReadVar1D(m_CellArrayVarName, ca_start, ca_count,
                         cell_array.data()))
    return false;
...
bool PolydataCellFlow::unload(unsigned int block_id,
  ...
  if (nv) {
    ...
    cells.insert(cells.end(), pv,
                 pv + pd->GetVerts()->GetData()->GetNumberOfTuples());
  }
  ...
  if (nl) {
    ...
    cells.insert(cells.end(), pl,
                 pl + pd->GetLines()->GetData()->GetNumberOfTuples());
  }
  ...
  if (np) {
    ...
    cells.insert(cells.end(), pp,
                 pp + pd->GetPolys()->GetData()->GetNumberOfTuples());
  }
  ...
  if (ns) {
    ...
    cells.insert(cells.end(), ps,
                 ps + pd->GetStrips()->GetData()->GetNumberOfTuples());
  }

  HDF5SpaceGuard cellArraySpace(m_TotalArraySize, m_CellArrayBlockOffset,
                                cell_array_size_local);
  output->WriteVar1D(m_CellArrayVarName, cellArraySpace, h5TypeCellArray,
                     cells.data());

  HDF5SpaceGuard cellTypeSpace(m_TotalCell, m_CellTypesBlockOffset,
                               num_cells_local);
  output->WriteVar1D(m_CellTypeVarName, cellTypeSpace, h5TypeCellType,
                     types.data());
...
bool PointFlow::load(unsigned int block_id, vtkCompositeDataIterator *it,
                     ReadStream *reader) {
  ...
  if (!reader->ReadVar1D(m_PointVarName, start, count,
...
bool PointFlow::unload(unsigned int block_id, vtkCompositeDataIterator *it,
                       WriteStream *output) {
  uint64_t start = 3 * m_BlockOffset;
  uint64_t count = 3 * m_Metadata->BlockNumPoints[block_id];
  ...
  output->WriteVar1D(m_PointVarName, space, m_PointType,
...
LogicallyCartesianFlow::LogicallyCartesianFlow(
    const sensei::MeshMetadataPtr &md, unsigned int meshID)
    : VTKObjectFlow(md, meshID) {
...
bool LogicallyCartesianFlow::unload(unsigned int block_id,
  ...
  case VTK_RECTILINEAR_GRID:
    output->WriteVar1D(m_ExtentPath, space, H5T_NATIVE_INT,
...
bool StretchedCartesianFlow::load(unsigned int block_id,
  ...
  if (!reader->ReadVar1D(m_XPath, m_BlockOffset[0], local[0],
                         x_coords->GetVoidPointer(0)))
    return false;
  if (!reader->ReadVar1D(m_YPath, m_BlockOffset[1], local[1],
                         y_coords->GetVoidPointer(0)))
    return false;
  if (!reader->ReadVar1D(m_ZPath, m_BlockOffset[2], local[2],
                         z_coords->GetVoidPointer(0)))
    return false;
...
bool WriteStream::AdvanceTimeStep(unsigned long &time_step, double &time) {
  if (m_Streamer->m_TimeStepCounter > 0)
    WriteNativeAttr(senseiHDF5::ATTRNAME_NUM_MESH, &(m_MeshCounter),
                    H5T_NATIVE_UINT, -1);

  m_MeshCounter = 0;
  m_Streamer->AdvanceStream();

  WriteNativeAttr(senseiHDF5::ATTRNAME_TIMESTEP, &time_step, H5T_NATIVE_ULONG,
                  -1);
...
bool WriteStream::WriteNativeAttr(const std::string &name, void *val,
                                  hid_t h5Type, hid_t owner) {
  if (owner == -1)
...
bool WriteStream::WriteVar1D(const std::string &name,
                             const HDF5SpaceGuard &space, hid_t h5Type,
                             void *data) {
  hid_t varID =
      H5Dcreate(m_Streamer->m_TimeStepId, name.c_str(), h5Type,
                space.m_FileSpaceID, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);

  H5Dwrite(varID, h5Type, space.m_MemSpaceID, space.m_FileSpaceID,
           m_CollectiveTxf, data);
...
bool WriteStream::WriteBinary(const std::string &name,
  ...
  hid_t varID = H5Dcreate(m_Streamer->m_TimeStepId, name.c_str(), h5Type,
...
bool WriteStream::WriteMesh(sensei::MeshMetadataPtr &md,
  ...
  hid_t meshID = H5Gcreate2(m_Streamer->m_TimeStepId, meshName.c_str(),
