...
namespace senseiHDF5
{
  ...
  static void gGetNameStr(std::string& out, unsigned int meshID, const std::string& suffix)
  {
  ...
  static void gGetArrayNameStr(std::string& out, unsigned int meshID, unsigned int array_id)
  {
    std::ostringstream ons;
  ...
  hid_t gGetHDF5Type(vtkDataArray* da)
  {
	...
	return gHDF5_IDType();
      ...
      {
	SENSEI_ERROR("the HDF5  type for data array \"" << da->GetClassName()
  ...
  hid_t  gVTKToH5Type(int vtkt)
  {
	...
	break;
      default:
	{
	  SENSEI_ERROR("the HDF5 type for vtk type enumeration " << vtkt
		       << " is currently not implemented");
	  MPI_Abort(MPI_COMM_WORLD, -1);
	}
      }
...
vtkDataObject *newDataObject(int code)
{
  ...
  void HDF5VarGuard::ReadSlice(void* buf, int ndim, const hsize_t *start, const hsize_t *stride, const hsize_t *count, const hsize_t *block)
  {
    hid_t memDataSpace = H5Screate_simple(ndim, count, NULL);

    hid_t ret = H5Sselect_hyperslab(m_VarSpace, H5S_SELECT_SET, start, stride, count, block);
  ...
  //
  //
  StreamHandler::StreamHandler(bool readmode, const std::string& hostFile, BasicStream* client)
    :m_TimeStepId(-1), m_InReadMode(readmode), m_FileName(hostFile), m_Client(client)
  {}

  //
  //
  //
  DefaultStreamHandler::DefaultStreamHandler(const std::string& hostFile, ReadStream* client)
    :StreamHandler(true, hostFile, client)
  {
    m_HostFileId = H5Fopen(hostFile.c_str(),  H5F_ACC_RDONLY, client->m_PropertyListId);

    if (m_HostFileId >=  0)
      client->ReadNativeAttr(senseiHDF5::ATTRNAME_NUM_TIMESTEP, &(m_TimeStepTotal), H5T_NATIVE_UINT, m_HostFileId);
  }


  DefaultStreamHandler::DefaultStreamHandler(const std::string& hostFile, WriteStream* client)
    :StreamHandler(false, hostFile, client)
  {
    m_HostFileId = H5Fcreate(hostFile.c_str(), H5F_ACC_TRUNC, H5P_DEFAULT, client->m_PropertyListId);       
  }

  DefaultStreamHandler::~DefaultStreamHandler()
  {
    if (m_HostFileId >= 0) H5Fclose(m_HostFileId);	
  }

  bool DefaultStreamHandler::IsValid()
  {
    if (m_HostFileId < 0) return false;
    
    if (m_TimeStepTotal == 0) return false;
  ...
  */
  ...
  bool DefaultStreamHandler::AdvanceStream()
  {
    ...
    {      
      CloseStream();
      if (m_TimeStepCounter == m_TimeStepTotal) return false;	     
      m_TimeStepId = H5Gopen(m_HostFileId, stepName.c_str(), H5P_DEFAULT);      
    } 
    else 
    {
      if (m_TimeStepCounter > 0)  CloseStream();      
      m_TimeStepId = H5Gcreate2(m_HostFileId, stepName.c_str(), H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);      
    }
  ...
  bool DefaultStreamHandler::Summary()
  {
    ...
    if (!writer->WriteNativeAttr(senseiHDF5::ATTRNAME_NUM_TIMESTEP, &(m_TimeStepCounter), H5T_NATIVE_UINT, m_HostFileId))
      return false;
  ...
  //if using char, do 11111 and use 0 to indicate end... so only need to check whether the last char is 0? 

  //
  //
  //
  PerStepStreamHandler::PerStepStreamHandler(const std::string& hostFile, ReadStream* client)
    :StreamHandler(true, hostFile, client)
  {
  ...
  PerStepStreamHandler::PerStepStreamHandler(const std::string& hostFile, WriteStream* client)
    :StreamHandler(false, hostFile, client)
  {
  ...
  bool PerStepStreamHandler::OpenStream()
  {
    ...
    m_TimeStepId = H5Fopen(currStepFileName.c_str(),  H5F_ACC_RDONLY, m_Client->m_PropertyListId);
  ...
  {
    if (m_NumStepsWritten > m_TimeStepCounter) 
      return false;
  
    if (m_AllStepsWritten && (m_NumStepsWritten == m_TimeStepCounter)) 
      return true;
    ...
    while (true) {
      GetCurrAvailStep();
      if ((m_NumStepsWritten < 0) && (m_TimeStepCounter > 0)) return true; // all finished

      if (m_NumStepsWritten >  m_TimeStepCounter)
	return false;

      // either nothing is written ((m_timestepcounter == 0) && ( m_numstepswritten <0))
      // or no new steps (numstepswritten = counter)
      sleep(1);
      counter ++;
      if (counter > MAXWAIT) return true;
    }
    return true;
  }
  ...
  bool PerStepStreamHandler::CloseStream()
  {
    ...
    m_TimeStepId = -1;
    if (!m_InReadMode) UpdateAvailStep();
  
    if ((m_InReadMode) && (m_Client->m_Rank == 0)) {
      //std::cout<<" ... [TODO]: will remove this file when read is done ... "<<name<<std::endl;
      std::remove(name);
    }
  ...
  bool PerStepStreamHandler::AdvanceStream()
  {
    ...
    {      
      CloseStream();
      if (NoMoreStep()) return false;	      
      m_TimeStepId = H5Fopen(stepName.c_str(),  H5F_ACC_RDONLY, m_Client->m_PropertyListId);
    } 
    else 
    {
      if (m_TimeStepCounter > 0)  CloseStream();      
      m_TimeStepId = H5Fcreate(stepName.c_str(), H5F_ACC_TRUNC, H5P_DEFAULT, m_Client->m_PropertyListId);   
    }
  ...
  BasicStream::BasicStream(MPI_Comm comm, bool streaming)
    :m_Comm(comm), m_StreamingOn(streaming)
  {
  ...
  BasicStream::~BasicStream()
  {
      ...
      if (H5P_DEFAULT != m_CollectiveTxf)  H5Pclose(m_CollectiveTxf);	

      CloseTimeStep();
      if (m_Streamer != nullptr) delete m_Streamer;      
  }

  void BasicStream::CloseTimeStep() 
  {
    if (m_Streamer) m_Streamer->CloseStream();
  }
  ...
  //
  //
  ReadStream::ReadStream(MPI_Comm comm, bool streaming):BasicStream(comm, streaming)
  {
    m_TimeStepTotal = 0;
  }
  ...
  {
    m_AllMeshInfo.Clear();
    if (!m_Streamer->AdvanceStream()) return false;

    if (!ReadNativeAttr(senseiHDF5::ATTRNAME_TIMESTEP, &time_step, H5T_NATIVE_ULONG, -1)) return false;
    if (!ReadNativeAttr(senseiHDF5::ATTRNAME_TIME, &time, H5T_NATIVE_DOUBLE, -1)) return false;

    return true;
  }

  bool ReadStream::ReadNativeAttr(const std::string& name, void* val, hid_t h5Type, hid_t hid)
  {
    if (hid == -1) hid = m_Streamer->m_TimeStepId;
  ...
  bool ReadStream::ReadVar1D(const std::string& name, hsize_t& s, hsize_t& c, void* data)
  {
  ...
  {    
    if (!ReadNativeAttr(senseiHDF5::ATTRNAME_NUM_MESH, &(nMesh), H5T_NATIVE_UINT, -1)) return false;
	...
	if (!ReadBinary(path, bs)) return false;
  ...
  bool ReadStream::ReadMesh(std::string name, vtkDataObject *&dobj, bool structure_only)
  {
  ...
  bool ReadStream::ReadInArray(const std::string &meshName, int association,
			       const std::string &array_name, vtkDataObject *dobj)
  {
	...
	SENSEI_ERROR("Failed to read "
		     ...
		     << " data array \"" << array_name << "\" from object \"" << meshName
		     << "\"");
	return false;
    }
    return  true;
  ...
  bool MeshFlow::ReadArray(ReadStream* reader, const std::string& array_name, int association)
  {
    ...
    for (unsigned int i = 0; i < num_arrays; ++i) {
      ...
      if ((association != md->ArrayCentering[i]) || (array_name != md->ArrayName[i]))
	continue;
...
//
//
WorkerCollection::WorkerCollection(const sensei::MeshMetadataPtr& md, unsigned int meshID)
{
    if (sensei::VTKUtils::Unstructured(md) || sensei::VTKUtils::Structured(md) || sensei::VTKUtils::Polydata(md)) {
...
bool WorkerCollection::load(unsigned int block_id, vtkCompositeDataIterator *it, ReadStream* reader)
{
  for (size_t i=0; i<m_Workers.size(); i++)
  {
...
bool WorkerCollection::unload(unsigned int block_id, vtkCompositeDataIterator *it,WriteStream* writer)
{
  for (size_t i=0; i<m_Workers.size(); i++)
  {
...
//
//
ArrayFlow::ArrayFlow(const sensei::MeshMetadataPtr& md, unsigned int meshID, unsigned int arrayID)
  :VTKObjectFlow(md, meshID), m_BlockOffset(0), m_ArrayID(arrayID)
{
...
bool ArrayFlow::load(unsigned int block_id, vtkCompositeDataIterator *it, ReadStream* reader)
{
  unsigned long long num_elem_local = m_NumArrayComponent * 
    (m_ArrayCenter == vtkDataObject::POINT ? m_Metadata->BlockNumPoints[block_id] : m_Metadata->BlockNumCells[block_id]);
     
  uint64_t start = m_BlockOffset;
  uint64_t count = num_elem_local;;
  
  vtkDataArray *array = vtkDataArray::CreateDataArray(m_Metadata->ArrayType[m_ArrayID]);
  ...
  if (!reader->ReadVar1D(m_ArrayPath, start, count, array->GetVoidPointer(0))) return false;
  ...
  vtkDataSetAttributes *dsa = (m_ArrayCenter == vtkDataObject::POINT) ?
...
bool ArrayFlow::unload(unsigned int block_id, vtkCompositeDataIterator *it,WriteStream* output)
{
  vtkDataSet *ds = dynamic_cast<vtkDataSet*>(it->GetCurrentDataObject());
  if (!ds) {
      SENSEI_ERROR("Failed to get block " << block_id);
  ...
  unsigned long long num_elem_local = m_NumArrayComponent * getLocalElement(block_id);
  HDF5SpaceGuard arraySpace(m_ElementTotal, m_BlockOffset, num_elem_local);
  output->WriteVar1D(m_ArrayPath, arraySpace, h5TypeCurrArray,  da->GetVoidPointer(0));
...
{
  return (m_ArrayCenter == vtkDataObject::POINT ? m_Metadata->BlockNumPoints[block_id] : m_Metadata->BlockNumCells[block_id]);
...
//
//
PolydataCellFlow::PolydataCellFlow(const sensei::MeshMetadataPtr& md, unsigned int meshID)
...
bool PolydataCellFlow::load(unsigned int block_id, vtkCompositeDataIterator *it, ReadStream* reader )
{
    // /data_object_<id>/cell_types
    unsigned long long cell_array_size_local = m_Metadata->BlockCellArraySize[block_id];
    ...
    if (!reader->ReadVar1D(m_CellTypeVarName, ct_start, ct_count, cell_types.data())) return false;

    uint64_t ca_start = m_CellArrayBlockOffset;
    uint64_t ca_count = cell_array_size_local;;    
    if (!reader->ReadVar1D(m_CellArrayVarName, ca_start, ca_count, cell_array.data())) return false;
...
bool PolydataCellFlow::unload(unsigned int block_id, vtkCompositeDataIterator *it,WriteStream* output)
{
  ...
  HDF5SpaceGuard cellArraySpace(m_TotalArraySize, m_CellArrayBlockOffset, cell_array_size_local);
  output->WriteVar1D(m_CellArrayVarName, cellArraySpace, h5TypeCellArray, cells.data());
  
  HDF5SpaceGuard cellTypeSpace(m_TotalCell, m_CellTypesBlockOffset, num_cells_local);
  output->WriteVar1D(m_CellTypeVarName, cellTypeSpace, h5TypeCellType,  types.data());
...
//
//
UnstructuredCellFlow::UnstructuredCellFlow(const sensei::MeshMetadataPtr& md, unsigned int meshID)
  :VTKObjectFlow(md, meshID)
{
  m_CellTypesBlockOffset = 0;
  m_CellArrayBlockOffset = 0;
...
bool UnstructuredCellFlow::load(unsigned int block_id, vtkCompositeDataIterator *it, ReadStream* reader)
{
    // /data_object_<id>/cell_types
    unsigned long long cell_array_size_local = m_Metadata->BlockCellArraySize[block_id];
    unsigned long long num_cells_local = m_Metadata->BlockNumCells[block_id];
    ...
    if (!reader->ReadVar1D(m_CellTypeVarName, ct_start, ct_count, cell_types->GetVoidPointer(0))) return false;
    ...
    if (!reader->ReadVar1D(m_CellArrayVarName, ca_start, ca_count, cell_array->GetVoidPointer(0))) return false;

    vtkUnstructuredGrid *ds = dynamic_cast<vtkUnstructuredGrid*>(it->GetCurrentDataObject());
...
bool UnstructuredCellFlow::unload(unsigned int block_id, vtkCompositeDataIterator *it,WriteStream* output)
{
  hid_t h5TypeCellArray = senseiHDF5::gHDF5_IDType(); 
  hid_t h5TypeCellType = H5T_NATIVE_CHAR; 
  ...
  HDF5SpaceGuard cellArraySpace(m_TotalArraySize, m_CellArrayBlockOffset, cell_array_size_local);
  output->WriteVar1D(m_CellArrayVarName, cellArraySpace, h5TypeCellArray, ds->GetCells()->GetData()->GetVoidPointer(0)); 

  HDF5SpaceGuard cellTypeSpace(m_TotalCell, m_CellTypesBlockOffset, num_cells_local);
  output->WriteVar1D(m_CellTypeVarName, cellTypeSpace, h5TypeCellType,  ds->GetCellTypesArray()->GetVoidPointer(0));
...
//
//
  VTKObjectFlow::VTKObjectFlow(const sensei::MeshMetadataPtr& md, unsigned int meshID)
    :m_Metadata(md), m_MeshID(meshID)
{
...
PointFlow::PointFlow(const sensei::MeshMetadataPtr& md, unsigned int meshID)
  :VTKObjectFlow(md, meshID), m_BlockOffset(0), m_GlobalTotal(0)
{
  unsigned int num_blocks = md->NumBlocks;
  // calc global size                                                                                                                                                                                     
...
bool PointFlow::load(unsigned int block_id, vtkCompositeDataIterator *it, ReadStream* reader)
{
  uint64_t start = 3 * m_BlockOffset;
  uint64_t count = 3 * m_Metadata->BlockNumPoints[block_id];
  
  vtkDataArray *points = vtkDataArray::CreateDataArray(m_Metadata->CoordinateType);
  ...
  if (!reader->ReadVar1D(m_PointVarName, start, count, points->GetVoidPointer(0))) return false;
...
bool PointFlow::unload(unsigned int block_id, vtkCompositeDataIterator *it,WriteStream* output)
{
  uint64_t start = 3 * m_BlockOffset;
  uint64_t count = 3 * m_Metadata->BlockNumPoints[block_id];
  ...
  output->WriteVar1D(m_PointVarName,  space, m_PointType, ds->GetPoints()->GetData()->GetVoidPointer(0));
...
UniformCartesianFlow::UniformCartesianFlow(const sensei::MeshMetadataPtr& md, unsigned int meshID)
  :VTKObjectFlow(md, meshID)
{
...
bool UniformCartesianFlow::load(unsigned int block_id, vtkCompositeDataIterator *it, ReadStream* reader)
{
  ...
  double x0[3] = {0.0};
  if (!reader->ReadVar1D(m_OriginPath, triplet_start, triplet_count, x0)) return false;

  double dx[3] = {0.0};
  if (!reader->ReadVar1D(m_SpacingPath, triplet_start, triplet_count, dx)) return false;
...
bool UniformCartesianFlow::unload(unsigned int block_id, vtkCompositeDataIterator *it, WriteStream* output)
{
...
//
//
LogicallyCartesianFlow::LogicallyCartesianFlow(const sensei::MeshMetadataPtr& md, unsigned int meshID)
  :VTKObjectFlow(md, meshID)
{
...
bool LogicallyCartesianFlow::unload(unsigned int block_id, vtkCompositeDataIterator *it, WriteStream* output)
{
  vtkDataObject *dobj = it->GetCurrentDataObject();
  if (!dobj) {    
    SENSEI_ERROR("Failed to get logically cartesian data from block" << block_id);
    return false;
  }
  ...
  switch (m_Metadata->BlockType)
    {
    case VTK_RECTILINEAR_GRID:
      output->WriteVar1D(m_ExtentPath, space, H5T_NATIVE_INT, dynamic_cast<vtkRectilinearGrid*>(dobj)->GetExtent());		       
      break;
    case VTK_IMAGE_DATA:
      output->WriteVar1D(m_ExtentPath, space, H5T_NATIVE_INT, dynamic_cast<vtkImageData*>(dobj)->GetExtent());		       
      break;
    case VTK_STRUCTURED_GRID:
      output->WriteVar1D(m_ExtentPath, space, H5T_NATIVE_INT, dynamic_cast<vtkStructuredGrid*>(dobj)->GetExtent());			 
      break;
    }
...
bool LogicallyCartesianFlow::load(unsigned int block_id, vtkCompositeDataIterator *it, ReadStream* reader)
{
  uint64_t hexlet_start = 6*block_id;
  uint64_t hexlet_count = 6;
    
  int ext[6] = {0};
  if (!reader->ReadVar1D(m_ExtentPath, hexlet_start, hexlet_count, ext)) return false;
...
//
//
StretchedCartesianFlow::StretchedCartesianFlow(const sensei::MeshMetadataPtr& md, unsigned int meshID)
  :VTKObjectFlow(md, meshID)
{
...
void StretchedCartesianFlow::GetLocal(int block_id, unsigned long long (&out)[3])
{  
...
bool StretchedCartesianFlow::load(unsigned int block_id, vtkCompositeDataIterator *it, ReadStream* reader)
{
  ...
  vtkDataArray *x_coords = vtkDataArray::CreateDataArray(m_Metadata->CoordinateType);
  x_coords->SetNumberOfComponents(1);
  x_coords->SetNumberOfTuples(local[0]);
  x_coords->SetName("x_coords");

  vtkDataArray *y_coords = vtkDataArray::CreateDataArray(m_Metadata->CoordinateType);
  y_coords->SetNumberOfComponents(1);
  y_coords->SetNumberOfTuples(local[1]);
  y_coords->SetName("y_coords");

  vtkDataArray *z_coords = vtkDataArray::CreateDataArray(m_Metadata->CoordinateType);
  z_coords->SetNumberOfComponents(1);
  z_coords->SetNumberOfTuples(local[2]);
  z_coords->SetName("z_coords");

  if (!reader->ReadVar1D(m_XPath, m_BlockOffset[0], local[0], x_coords->GetVoidPointer(0))) return false;
  if (!reader->ReadVar1D(m_YPath, m_BlockOffset[1], local[1], y_coords->GetVoidPointer(0))) return false;
  if (!reader->ReadVar1D(m_ZPath, m_BlockOffset[2], local[2], z_coords->GetVoidPointer(0))) return false;
...
bool StretchedCartesianFlow::unload(unsigned int block_id, vtkCompositeDataIterator *it, WriteStream* output)
{
  unsigned long long local[3];
  GetLocal(block_id, local);
...

  {
    ...
    output->WriteVar1D(m_XPath, spaceX, m_PointType, ds->GetXCoordinates()->GetVoidPointer(0));
  }
  {
    HDF5SpaceGuard spaceY(m_Total[1], m_BlockOffset[1], local[1]);
    output->WriteVar1D(m_YPath, spaceY, m_PointType, ds->GetYCoordinates()->GetVoidPointer(0));
  }
  {
    HDF5SpaceGuard spaceZ(m_Total[2], m_BlockOffset[2], local[2]);
    output->WriteVar1D(m_ZPath, spaceZ, m_PointType, ds->GetZCoordinates()->GetVoidPointer(0));
  }
...
//
//
WriteStream::WriteStream(MPI_Comm comm, bool streaming):BasicStream(comm, streaming)
{
  m_MeshCounter = 0;
}
...
bool WriteStream::AdvanceTimeStep(unsigned long &time_step, double &time)
{
  if (m_Streamer->m_TimeStepCounter > 0)
        WriteNativeAttr(senseiHDF5::ATTRNAME_NUM_MESH, &(m_MeshCounter), H5T_NATIVE_UINT, -1);        

  m_MeshCounter = 0;
  m_Streamer->AdvanceStream();

  WriteNativeAttr(senseiHDF5::ATTRNAME_TIMESTEP, &time_step, H5T_NATIVE_ULONG, -1);
...
bool WriteStream::WriteNativeAttr(const std::string& name, void* val, hid_t h5Type, hid_t owner)
{
  if (owner == -1) owner = m_Streamer->m_TimeStepId;

  hid_t s = H5Screate(H5S_SCALAR);
  hid_t attr = H5Acreate(owner, name.c_str(), h5Type, s, H5P_DEFAULT, H5P_DEFAULT);
...
bool WriteStream::WriteVar1D(const std::string& name, const HDF5SpaceGuard& space, hid_t h5Type, void* data)
{  
  hid_t varID  = H5Dcreate(m_Streamer->m_TimeStepId, name.c_str(), h5Type, space.m_FileSpaceID,  H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  
  H5Dwrite(varID, h5Type, space.m_MemSpaceID, space.m_FileSpaceID, m_CollectiveTxf, data);

  //H5Dwrite(varID, h5Type, space.m_MemSpaceID, space.m_FileSpaceID, H5P_DEFAULT, data);
...
bool WriteStream::WriteBinary(const std::string& name, sensei::BinaryStream &str)
{
    ...
    hid_t varID = H5Dcreate(m_Streamer->m_TimeStepId, name.c_str(), h5Type, fileSpace, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
...
bool WriteStream::WriteMesh(sensei::MeshMetadataPtr& md, vtkCompositeDataSet * vtkPtr) 
{
  std::string meshName;
  gGetNameStr(meshName, m_MeshCounter, "");

  hid_t meshID = H5Gcreate2(m_Streamer->m_TimeStepId, meshName.c_str(), H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
