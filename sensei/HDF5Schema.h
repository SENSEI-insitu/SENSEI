...
class HDF5VarGuard
{
 public:
  ...
  void ReadSlice(void* buf, int ndim, const hsize_t *start, const hsize_t *stride, const hsize_t *count, const hsize_t *block);
  ...
  HDF5SpaceGuard(hsize_t global, hsize_t s, hsize_t c)
    {
      ...
      H5Sselect_hyperslab(m_FileSpaceID, H5S_SELECT_SET, offset, NULL, count, NULL);
...
class BasicStream
{
 public:
  ...
  sensei::MeshMetadataMap m_AllMeshInfo;
...
class WriteStream: public BasicStream
{
 public:
  ...
  bool WriteNativeAttr(const std::string& name, void* val, hid_t h5Type, hid_t owner);
  bool WriteVar1D(const std::string& name, const HDF5SpaceGuard& space, hid_t h5Type, void* data);
...
class ReadStream: public BasicStream
{
 public:
  ...
  bool ReadNativeAttr(const std::string& name, void* val, hid_t h5Type, hid_t hid);
...
class MeshFlow
{
 public:
  ...
  bool ReadArray(ReadStream* input, const std::string& array_name, int association);
...
class VTKObjectFlow
{
 public:
  ...
  virtual bool load(unsigned int block_id, vtkCompositeDataIterator *it, ReadStream* input ) = 0;
  virtual bool update(unsigned int block_id) = 0;
  virtual bool unload(unsigned int block_id, vtkCompositeDataIterator *it,WriteStream* output) = 0;
...
public:
  ...
  bool load(unsigned int block_id, vtkCompositeDataIterator *it, ReadStream* input );
  bool unload(unsigned int block_id, vtkCompositeDataIterator *it, WriteStream* input );
...
{
 public:
  ArrayFlow(const sensei::MeshMetadataPtr& md, unsigned int meshID, unsigned int arrayID);
  ~ArrayFlow() {};
  bool load(unsigned int block_id, vtkCompositeDataIterator *it, ReadStream* );
  bool unload(unsigned int block_id, vtkCompositeDataIterator *it,WriteStream* output);
  bool update(unsigned int block_id);
...
{
 public:
    LogicallyCartesianFlow(const sensei::MeshMetadataPtr& md, unsigned int meshID);
...
{
 public:
    StretchedCartesianFlow(const sensei::MeshMetadataPtr& md, unsigned int meshID);
