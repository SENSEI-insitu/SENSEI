...
struct ConfigurableAnalysis::InternalsType {
  ...
  int TimeInitialization(AnalysisAdaptorPtr adaptor,
...
int ConfigurableAnalysis::InternalsType::TimeInitialization(
  ...
  if (logEnabled) {
    ...
    initName << adaptor->GetClassName() << "::" << analysisNumber
             << "::Initialize";
    execName << adaptor->GetClassName() << "::" << analysisNumber
             << "::Execute";
    finiName << adaptor->GetClassName() << "::" << analysisNumber
             << "::Finalize";
...
int ConfigurableAnalysis::InternalsType::AddHistogram(pugi::xml_node node) {
  if (XMLUtils::RequireAttribute(node, "mesh") ||
      XMLUtils::RequireAttribute(node, "array")) {
  ...
  SENSEI_STATUS("Configured histogram with "
                << bins << " bins on " << assocStr << " data array \"" << array
...
int ConfigurableAnalysis::InternalsType::AddVTKmContour(pugi::xml_node node) {
  ...
  (void)node;
  SENSEI_ERROR(
      "vtkAcceleratorsVtkm was requested but is disabled in this build")
  return -1;
#else
...
// --------------------------------------------------------------------------
int ConfigurableAnalysis::InternalsType::AddVTKmVolumeReduction(
    pugi::xml_node node) {
#ifndef ENABLE_VTKM
  (void)node;
  ...
  if (XMLUtils::RequireAttribute(node, "mesh") ||
      ...
      XMLUtils::RequireAttribute(node, "reduction")) {
  ...
  SENSEI_STATUS("Configured VTKmVolumeReductionAnalysis " << mesh << "/"
                                                          << field)
...
int ConfigurableAnalysis::InternalsType::AddVTKmCDF(pugi::xml_node node) {
  ...
  std::string workDir =
      haveWorkDir ? node.attribute("working-directory").as_string() : ".";

  auto analysis = vtkSmartPointer<VTKmCDFAnalysis>::New();
  this->TimeInitialization(analysis, [&]() {
    analysis->Initialize(mesh, field, assoc, workDir, quantiles, exchangeSize,
                         this->Comm);
    return 0;
  });
...
int ConfigurableAnalysis::InternalsType::AddAdios(pugi::xml_node node) {
  ...
  SENSEI_STATUS("Configured ADIOSAnalysisAdaptor \""
...
int ConfigurableAnalysis::InternalsType::AddCatalyst(pugi::xml_node node) {
  ...
  if (strcmp(node.attribute("pipeline").value(), "slice") == 0) {
    ...
    if (node.attribute("slice-normal") &&
        (std::sscanf(node.attribute("slice-normal").value(), "%lg,%lg,%lg",
                     &tmp[0], &tmp[1], &tmp[2]) == 3)) {
      slice->SetSliceNormal(tmp[0], tmp[1], tmp[2]);
    }

    if (node.attribute("slice-origin") &&
        (std::sscanf(node.attribute("slice-origin").value(), "%lg,%lg,%lg",
                     &tmp[0], &tmp[1], &tmp[2]) == 3)) {
    ...
    if (node.attribute("color-range") &&
        (std::sscanf(node.attribute("color-range").value(), "%lg,%lg", &tmp[0],
                     &tmp[1]) == 2)) {
    ...
    if (node.attribute("image-filename")) {
      slice->SetImageParameters(node.attribute("image-filename").value(),
    ...
    if (node.attribute("particle-radius") &&
        (std::sscanf(node.attribute("particle-radius").value(), "%lg",
                     &tmp[0]) == 1)) {
      particle->SetParticleRadius(tmp[0]);
    }
    if (node.attribute("camera-position") &&
        (std::sscanf(node.attribute("camera-position").value(), "%lg,%lg,%lg",
                     &tmp[0], &tmp[1], &tmp[2]) == 3)) {
      particle->SetCameraPosition(tmp);
    }

    if (node.attribute("camera-focus") &&
        (std::sscanf(node.attribute("camera-focus").value(), "%lg,%lg,%lg",
                     &tmp[0], &tmp[1], &tmp[2]) == 3)) {
    ...
    if (node.attribute("image-filename")) {
      particle->SetImageParameters(node.attribute("image-filename").value(),
                                   node.attribute("image-width").as_int(800),
                                   node.attribute("image-height").as_int(800));
    }
  ...
  SENSEI_STATUS(
      "Configured CatalystAnalysisAdaptor "
...
int ConfigurableAnalysis::InternalsType::AddLibsim(pugi::xml_node node) {
  ...
  if (!this->LibsimAdaptor) {
    ...
    if (node.attribute("visitdir"))
      this->LibsimAdaptor->SetVisItDirectory(
          node.attribute("visitdir").value());
  ...
  if (node.attribute("slice-origin") != NULL) {
    double tmp[3];
    if (sscanf(node.attribute("slice-origin").value(), "%lg,%lg,%lg", &tmp[0],
               &tmp[1], &tmp[2]) == 3) {
      slice = true;
      origin[0] = tmp[0];
      origin[1] = tmp[1];
  ...
  if (node.attribute("slice-normal") != NULL) {
    double tmp[3];
    if (sscanf(node.attribute("slice-normal").value(), "%lg,%lg,%lg", &tmp[0],
               &tmp[1], &tmp[2]) == 3) {
      slice = true;
      normal[0] = tmp[0];
      normal[1] = tmp[1];
...
// --------------------------------------------------------------------------
int ConfigurableAnalysis::InternalsType::AddAutoCorrelation(
    pugi::xml_node node) {
  if (XMLUtils::RequireAttribute(node, "mesh") ||
      XMLUtils::RequireAttribute(node, "array")) {
  ...
  SENSEI_STATUS("Configured Autocorrelation "
                << assocStr << " data array \"" << arrayName << "\" on mesh \""
                << meshName << "\" window " << window << " k-max " << kMax
...
// --------------------------------------------------------------------------
int ConfigurableAnalysis::InternalsType::AddPythonAnalysis(
    pugi::xml_node node) {
  ...
  SENSEI_STATUS("Configured python with " << scriptType << " \"" << scriptName
                                          << "\"")
...
//----------------------------------------------------------------------------
ConfigurableAnalysis::~ConfigurableAnalysis() { delete this->Internals; }
...
int ConfigurableAnalysis::Initialize(const pugi::xml_node &root) {
  ...
  for (pugi::xml_node node = root.child("analysis"); node;
       node = node.next_sibling("analysis")) {
    ...
    if (!(((type == "histogram") && !this->Internals->AddHistogram(node)) ||
          ((type == "autocorrelation") &&
           !this->Internals->AddAutoCorrelation(node)) ||
          ...
          ((type == "VTKAmrWriter") &&
           !this->Internals->AddVTKAmrWriter(node)) ||
          ((type == "vtkmcontour") && !this->Internals->AddVTKmContour(node)) ||
          ((type == "vtkmhaar") &&
           !this->Internals->AddVTKmVolumeReduction(node)) ||
