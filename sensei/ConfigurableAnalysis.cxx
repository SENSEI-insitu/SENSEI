...
int ConfigurableAnalysis::InternalsType::AddVTKmCDF(pugi::xml_node node)
{
  ...
  this->TimeInitialization(analysis, [&]() {
    analysis->Initialize(
      mesh, field, assoc, workDir, quantiles, exchangeSize, this->Comm);
    return 0;
  });
  ...
  if (strcmp(node.attribute("pipeline").value(), "slice") == 0)
    {
      ...
      if (node.attribute("color-range") &&
          (std::sscanf(node.attribute("color-range").value(),
                       "%lg,%lg",
                       &tmp[0],
                       &tmp[1]) == 2))
        {
  ...
  else if (strcmp(node.attribute("pipeline").value(), "particle") == 0)
    {
      ...
      if (node.attribute("particle-radius") &&
          (std::sscanf(
             node.attribute("particle-radius").value(), "%lg", &tmp[0]) == 1))
        {
  ...
  else if (strcmp(node.attribute("pipeline").value(), "pythonscript") == 0)
    {
      ...
      SENSEI_ERROR(
        "Catalyst Python was requested but is disabled in this build")
#else
  ...
  if (doExport)
    {
      ...
      if (!this->LibsimAdaptor->AddExport(frequency,
                                          session,
                                          plots,
                                          plotVars,
                                          slice,
  ...
  else
    {
      ...
      if (!this->LibsimAdaptor->AddRender(frequency,
                                          session,
                                          plots,
                                          plotVars,
                                          slice,
  ...
  for (pugi::xml_node node = root.child("analysis"); node;
       node = node.next_sibling("analysis"))
    {
      ...
      if (!(((type == "histogram") && !this->Internals->AddHistogram(node)) ||
            ...
            ((type == "vtkmcontour") &&
             !this->Internals->AddVTKmContour(node)) ||
            ((type == "vtkmhaar") &&
