    ...
    for (int i = 0; i < n_tuple; i++) {
      ...
      if (array->GetTuple1(i) != i * typeSize) {
        std::cout << "............. oh no at " << i << " th.. "
                  << array->GetTuple1(i) << " != " << i * typeSize << std::endl
                  << std::endl;
        return -1;
      }
    }
...
// request each array
      ...
      for (unsigned int j = 0; j < n_arrays; j++) {
        ...
        da->AddArray(mesh, meshName, /*assoc_ids[k],*/ mmd->ArrayCentering[j],
                     arrayName);
      }
#endif
      ...
      */
      ...
      if (ds == NULL) {
        std::cerr << "Unexpected mesh!! Not multiblock, but got this class: "
      ...
      while (!it->IsDoneWithTraversal()) {
        ...
        // classname="<<curr->GetClassName()<<"  numArrays = "<<n_arrays<<"
        // totalBlocks="<<mmd->NumBlocks<<std::endl;  std::cout<<"  .. info:
...
vtkPolyData *points_to_polydata(std::vector<float> &x, std::vector<float> &y,
                                std::vector<float> &z) {
...
template <class T>
vtkDataArray *generate_array(const char *name, unsigned long size,
                             vtkDataArray *result) {
...
void get_data_arrays(unsigned long size, vtkDataSetAttributes *dsa) {
  ...
  temp = generate_array<unsigned char>("unsigned_char_array", size,
                                       vtkUnsignedCharArray::New());
  dsa->AddArray(temp);
  temp->Delete();

  temp = generate_array<unsigned int>("unsigned_int_array", size,
                                      vtkUnsignedIntArray::New());
  dsa->AddArray(temp);
  temp->Delete();

  temp = generate_array<unsigned long>("unsigned_long_array", size,
                                       vtkUnsignedLongArray::New());
  dsa->AddArray(temp);
  temp->Delete();
}

vtkImageData *get_image(unsigned long i0, unsigned long i1, unsigned long j0,
                        unsigned long j1, unsigned long k0, unsigned long k1) {
...
TimedAdaptorWrap *GetReadAdaptor(const std::string &file_name,
                                 const std::string &method, MPI_Comm &comm) {
