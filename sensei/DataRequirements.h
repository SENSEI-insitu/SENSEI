...
/** This is a helper class that handles the common task of specifying the set
 * of meshes and arrays rqeuired to perform a specific analysis. An analysis
 * would typically intialize the data requirement from XML and then probe the
 * requirements during execution to get the minimal set of data required to
...
public:
  ...
  /** This is a convenience method that fills in all of data that the data
   * adaptor makes avaialable.
   *
  ...
  /**  Adds a mesh. The requirement consists of a mesh name and weather it is
   * structure only
   *
  ...
  /**  Adds a set of arrays on a specific mesh.  The requirement consists of a
   * mesh name and a list of arrays
   *
   *  @param[in] meshName name of the mesh
   ...
   *
   *  @param[in] meshName name of the mesh
   *  @param[in] association type of the required array
   *  @param[in] array name of the required array
   *  @returns zero if successful
   */
  int AddRequirement(const std::string &meshName, int association,
...
/**  Iterate over the mesh's arrays. One can iterate either over associations
 * and access all of an associations arrays or one by one over the arrays.
 */
class ArrayRequirementsIterator
{
public:
