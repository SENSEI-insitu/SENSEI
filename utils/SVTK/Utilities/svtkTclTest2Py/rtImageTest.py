...
# Mock class that overrides the Start() method from svtkRenderWindowInteractor
# to do nothing. This allows SVTK's python tests to be standard SVTK scripts that
# call Start() on the interactor.
class svtkTestingInteractor(svtk.svtkRenderWindowInteractor):
    def Start(self):
        pass
...
rtTempObject = svtk.svtkObject()

rtExMath = svtk.svtkMath()
...
SVTK_DATA_ROOT = rtTester.GetDataRoot()

if rtTester.IsInteractiveModeSpecified() == 0:
    svtk.svtkRenderWindowInteractor = svtkTestingInteractor
