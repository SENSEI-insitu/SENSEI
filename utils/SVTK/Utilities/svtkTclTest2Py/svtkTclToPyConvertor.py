...
# 5) define procedures before using them or setting them on SVTK objects
#    as callbacks.
...
import svtkTclParser

reVariable = re.compile("^([+\-])?\$([^\$\{\}]+)$")
reCompoundVariable = re.compile("\$(?:([^\$\}\{]+)|\{([^\$\}]+)\})")

class svtkTclToPyConvertor(svtkTclParser.svtkTclParser):

    def __init__(self):
        svtkTclParser.svtkTclParser.__init__(self)
        self.output = ""
        self.indent = ""
        self._procedure_list = []
        self.class_list = []
        self.name_space = "svtk"
...
%s""" % (prefix_content))
    ...
    def reset(self):
        self.output = ""
        self.indent = ""
        svtkTclParser.svtkTclParser.reset(self)
        self._procedure_list = []

    def _get_block_parser(self):
        p = svtkTclToPyConvertor()
    ...
    def translate_command(self, command, arguments):
        ...
        if len(command) > 0 and command[0] == "#":
            ...
            # makes no harm.
            proc_args.append("__svtk__temp0=0")
            proc_args.append("__svtk__temp1=0")
            i = False
            pair = 0
            for pa in proc_args:
                if pa.strip() == "{":
            ...
            #translate a SVTK object create command.
            if len(arguments) == 1:
            ...
            if arguments[0].strip() == "Print" :
                translated_cmd = "#"
            #translate a    SVTK object command invocation.
            translated_cmd += "%s.%s(" % (command, arguments[0])
            if len(arguments) > 9: #typically, methods taking more than 9 args in SVTK take them as lists.
                translated_cmd += "["
...
if __name__ == "__main__":
    ...
    if (not input_file or not output_file) and not convert_file_list_file:
        print "Usage: %s  [-o <output tcl test> -i <input tcl test>]"\
                ...
                "[-k <svtk*Kit.cmake file path>] [-k ...] ..." % sys.argv[0]
    ...
    if class_file:
        ...
        try:
            import svtkClassList
            class_list = svtkClassList.get_svtk_classes()
            pass
        except:
            print "ERROR: Failed to load module svtkClassList."
            sys.exit(1)
