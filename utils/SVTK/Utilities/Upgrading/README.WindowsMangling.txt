As of December 15, 2004 SVTK no longer includes windows.h by default in
...
windows.h.  All SVTK code that still includes windows.h does so through
svtkWindows.h which includes a minimal part of the real windows.h
header.  This also avoids contaminating user code with the windows.h
API just because it included a SVTK header.
...
windows.h is included it invisibly renames methods in SVTK like
...
1.) SVTK includes windows.h everywhere (as it did previously).  In this
    case the methods are always renamed for all SVTK code and all user
    code that includes a SVTK header.  The method GetClassNameA is
    actually visible as an export from the DLL.  Since the renaming is
    consistent then user code links correctly to SVTK libraries.

    However, applications and other libraries do not necessarily
    include SVTK or windows.h in all of their sources.  When this
    ...
    other reason than that it included a SVTK header!

    Also, when SVTK is built without UNICODE defined then all the
    methods are renamed to have "A" at the end.  If the user
    application defines UNICODE and includes a SVTK header then the
    methods get renamed to have "W" at the end, which results in
    unresolved symbols like "GetClassNameW".  Then the user complains
    and is told he/she has to rebuild SVTK from scratch with UNICODE
    defined.  Basically SVTK must be built separately for ASCII and
    UNICODE applications.

2.) SVTK does not include windows.h everywhere.  In this case the
    methods are not renamed when SVTK is built.  The method
    GetClassName is visible as an export from the DLL.  User code that
    includes windows.h before a SVTK header will cause the method to be
...
Here is how all three names can be provided in a SVTK header regardless
of whether windows.h has been included before it:

// See svtkWin32Header.h for definition of SVTK_WORKAROUND_WINDOWS_MANGLE.
...
by using this cmake script:
  ...
  SVTK/Utilities/Upgrading/FindWindowsMangledMethods.cmake

There are three backward-compatibility issues:

1.) User code that used the windows API without including windows.h
    that worked before because SVTK included it will now break until
    ...
    quick-fix, users can define SVTK_INCLUDE_WINDOWS_H in their
    application and SVTK will include windows.h as it did before.
...
3.) All functions that fall victim to mangling have been deprecated.
    ...
    the new non-mangled versions of the methods.  SVTK 5.0 includes
    support for the original names but it will be removed in a future
    version.  Use the SVTK_LEGACY_REMOVE setting when building SVTK to
...
1.) Use #undef to avoid the name mangling altogether.
    ...
    If SVTK includes windows.h and then does the #undef then user code
    will not be able to access the windows API through the standard
    means.  If SVTK does not include windows.h and uses #undef just in
    case the user included windows.h first then user code can still
    include windows.h after the SVTK header and then their calls to SVTK
    methods will be mangled and will not compile.

2.) Do not include windows.h but instead provide the same mangled
    names by defining the macros in SVTK the same way windows.h does.
    ...
    solution does not address the problems when users build SVTK
    without UNICODE and then build their application with UNICODE.  It
    defines macros in SVTK that are supposed to be defined in a system
...
3.) Use the above solution but change SVTK_INCLUDE_WINDOWS_H to a
    SVTK_DO_NOT_INCLUDE_WINDOWS_H so that the previous default behavior
    of including windows.h is preserved for user applications.  SVTK
    can define SVTK_DO_NOT_INCLUDE_WINDOWS_H when it is building
    itself.
    ...
    trying to achieve is that including a SVTK header should not do
    anything but define SVTK... and svtk... symbols to avoid namespacing
...
were actually applied to it in SVTK.  All changes were applied to other
...
svtkTypeMacro anyway this should not require many changes.
