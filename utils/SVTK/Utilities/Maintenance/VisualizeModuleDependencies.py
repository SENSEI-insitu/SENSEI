...
Typical usage would be as follows:
 VisualizeModuleDependencies.py SVTKSourceDir svtkFiltersSources,svtkInteractionStyle,svtkRenderingOpenGL2
...
def GetProgramParameters():
    ...
    parser.add_argument('svtkSourceDir', help='The path to the svtk Source Directory.')
...
def GetProgramParametersOld():
    ...
    '''
    if len(sys.argv) < 3:
        s = 'Usage: ' + sys.argv[0] + ' svtkSourceDir moduleList [moduleTreeDepth]'
        print(s)
        exit(0)
    args = dict()
    args['svtkSourceDir'] = sys.argv[1]
...
def ParseModuleFile(fileName):
    ...
    state = "START";
    for item in lines:
        if state == "START" and item.startswith("svtk_module("):
...
def GenerateSVTKGraph(graph):
    '''
    Take the vertices and edge list in the graph parameter
    and return a SVTK graph.
    '''
    g = svtk.svtkMutableDirectedGraph()
    # Label the vertices
...
def main():
    ...
    if ver >= 2.7:
        svtkSourceDir, moduleList, moduleTreeDepth = GetProgramParameters()
    else:
        svtkSourceDir, moduleList, moduleTreeDepth = GetProgramParametersOld()
    ...
    # Now build a svtk graph.
    g = GenerateSVTKGraph(graph)
