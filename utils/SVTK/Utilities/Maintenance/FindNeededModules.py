...
def get_program_parameters():
    import argparse
    description = 'Generate a find_package(SVTK COMPONENTS ...) command for CMake.'
    epilogue = '''
Use modules.json and your source files to generate a
  find_package(SVTK COMPONENTS ...) command listing all the svtk modules
...
If it is unable to find modules for your headers then
  ...
  so you can manually add the corresponding modules or rebuild SVTK
...
You will need to manually add any third-party modules
   (if used) to the find_package command.
    ...
    parser.add_argument('-j', '--json', nargs='+', default=["modules.json"],
                        help='The path to the SVTK JSON file (modules.json).')
...
class Constants:
    ...
    def __init__(self):
        ...
        self.svtk_include_pattern = re.compile(r'^(svtk\S+)')
        self.valid_ext = ['.h', '.hxx', '.cxx', '.cpp', '.txx']


def get_users_headers(path):
    """
    Get the set of SVTK headers in all the user's files in the path.
                    ...
                    with open(os.path.join(root, f)) as data:
                        ...
                        for line in data:
                            ...
                            if m:
                                ...
                                m = c.svtk_include_pattern.match(header_parts[1])
                                if m:
...
def get_users_file_headers(path):
    """
    Get the set of SVTK headers in a user's file or files with a common name in the path.
...
def main():
    ...
    if inc_no_mod:
        ...
        nohead += 'Or you may need to rebuild SVTK to include the missing modules.\n'
        nohead += '\nHere are the svtk headers and corresponding files:'
