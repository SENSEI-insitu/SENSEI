...
# The URLs where data is stored for SVTK.
readonly urlbases="https://www.svtk.org/files/ExternalData/ALGO/HASH"

# Move to the top of the SVTK tree.
...
find_data_objects () {
    ...
    git ls-tree --full-tree -r "$revision" | \
        ...
        while read mode type obj path; do
            ...
            if git check-attr svtk-is-large-data -- $path | grep -q -v -e " set$"; then
                if test "$largedata" != "SVTK_USE_LARGE_DATA"; then
                    # Build the path to the object.
                    echo "$algo/$( git cat-file blob $obj )"
                fi
            elif test "$largedata" = "SVTK_USE_LARGE_DATA"; then
                echo "$algo/$( git cat-file blob $obj )"
            fi
        done | \
            sort | \
...
for format in $formats; do
    git_archive "$format" "$tree" "SVTK-$version" || \
        result=1
done
...
for format in $formats; do
    git_archive "$format" "$tree" "SVTKData-$version" "SVTK-$version" || \
        result=1
done
...
rm -f "$GIT_INDEX_FILE"
load_data_objects "$commit" SVTK_USE_LARGE_DATA
load_data_files "$commit"
tree="$( git write-tree )"

info "Generating large data archive(s)..."
for format in $formats; do
    git_archive "$format" "$tree" "SVTKLargeData-$version" "SVTK-$version" || \
        result=1
done
