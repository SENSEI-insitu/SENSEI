#!/usr/bin/env python
'''
This program takes one mandatory parameter, the SVTK base folder, and up to
...
These are the additional options:
  ...
  <program file Name> <path to SVTK> Python Cxx
  <program file Name> <path to SVTK> Python Cxx -p
  <program file Name> <path to SVTK> Python -n -p
  <program file Name> <path to SVTK> -d -p
...
class FindTests(object):
    '''
    Search through the SVTK folder and create a dictionary of all the tests.
...
def GetProgramParameters():
    description = 'Review the testing programs and scripts in SVTK.'
    epilogue = '''
        This program produces statistics about the number of tests in SVTK.
        It can also list programs/scripts that are in one language but not
         in other languages.
    '''
    parser = argparse.ArgumentParser(description=description, epilog=epilogue)
    parser.add_argument('inputPath',
        help='The path to the SVTK source folder.', action='store')
...
def main():
    ...
    if not(os.path.exists(inputPath) and os.path.isdir(inputPath)):
        print(
    'Input path: {:s}\n should be the SVTK top-level folder.'.format(inputPath))
        Ok = False
