    ...
    def get_child_folder(parent, child_name):
        ...
        for folder in children['folders']:
            if folder['name'] == child_name:
                return folder
        return None
    svtk_community = communicator.get_community_by_name('SVTK')
    svtk_public = get_child_folder(svtk_community, 'Public')
    svtk_folder = get_child_folder(svtk_public, 'SVTK')
...
def run(svtk_source_dir, externaldata_object_store,
        email=None, api_key=None):
    session, communicator = connect_to_midas(email, api_key)

    md5files = []
    for root, dirnames, filenames in os.walk(svtk_source_dir):
    ...
    if repository_dir == None:
        sys.stderr.write('Could not find SVTK repository directory.\n')
        sys.exit(1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('svtk_source_dir',
            help='Path to the SVTK source tree.')
    ...
    run(args.svtk_source_dir, args.externaldata_object_store,
