...
echo "       the SVTK source tree. Add the relative path from your location at the TOP "
...
echo "   "
echo "   "
echo "       This will run the selected tests in SVTK and generate code coverage for   "
...
lcov --quiet --remove app.info '*ThirdParty*' 'svtkType*Array.*'  '*Python.cxx' '*Wrapping*' '*Examples*' '*Testing*'  '*Utilities*' '*_s.cxx' '*_vs*.cxx' '*_fs*.cxx' '*GS.cxx' '*VS.cxx' '*FS.cxx' '*FP*.cxx' '*VP*.cxx' 'svtkgl.cxx' '/usr/*' --output-file  app.info2
echo "Done"
