...
void svtkRTTestSequence::GetSequenceNumbers(int& xdim)
{
...
void svtkRTTestSequence::GetSequenceNumbers(int& xdim, int& ydim)
{
...
void svtkRTTestSequence::GetSequenceNumbers(int& xdim, int& ydim, int& zdim)
{
...
void svtkRTTestSequence::GetSequenceNumbers(int& xdim, int& ydim, int& zdim, int& wdim)
{
...
void svtkRTTestSequence::Run()
{
  ...
  svtkNew<svtkDoubleArray> summary;
  svtkNew<svtkDoubleArray> secondSummary;
  if (this->ChartResults)
  {
    ...
    chart->GetAxis(svtkAxis::LEFT)->SetTitle(this->Test->GetSummaryResultName());
    chart->GetAxis(svtkAxis::LEFT)->LogScaleOn();
    chart->GetAxis(svtkAxis::BOTTOM)->SetTitle(this->Test->GetSecondSummaryResultName());
    chart->GetAxis(svtkAxis::BOTTOM)->LogScaleOn();
    ...
    (sequenceEnd == 0 || this->SequenceCount <= sequenceEnd) && lastRunTime < stepLimit)
  {
    ...
    svtkRTTestResult tr = this->Test->Run(this, argc, argv);
    ...
    lastRunTime = svtkTimerLog::GetUniversalTime() - startTime;
    remainingTime -= lastRunTime;
    this->SequenceCount++;
  }
}

void svtkRTTestSequence::ReportSummaryResults(ostream& ost)
{
  double result = 0.0;
  svtkRTTestResult* bestTestResult = nullptr;
  bool initialized = false;
  std::vector<svtkRTTestResult>::iterator trItr;
...
svtkRenderTimings::svtkRenderTimings()
{
  this->TargetTime = 600.0; // 10 minutes
  svtksys::SystemInformation si;
  si.RunOSCheck();
...
int svtkRenderTimings::RunTests()
{
  // what tests to run?
  bool useRegex = false;
  ...
  std::vector<svtkRTTest*>::iterator testItr;
  if (useRegex)
  {
    testCount = 0;
  ...
  for (testItr = this->TestsToRun.begin(); testItr != this->TestsToRun.end(); ++testItr)
  {
    ...
    {
      svtkRTTestSequence* ats = new svtkRTTestSequence(this);
...
int svtkRenderTimings::ParseCommandLineArguments(int argc, char* argv[])
{
  ...
  if (this->DisplayHelp)
  {
    cerr << "Usage" << endl
         << endl
         << "  SVTKRenderTimings [options]" << endl
         << endl
         << "Options" << endl;
