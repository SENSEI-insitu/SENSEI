#!/usr/bin/env python
#
# tcl2py.py - try to Pythonize a svtk tcl script
#
# This code is based on tcl2py.py from SVTK 3.2 which was written by
# Randy Heiland.
#
# Changes:
#
...
#  code, fixed a few issues, using new SVTK package structure, added
#  support for SVTK_DATA_ROOT and set keyword, primitive support for
...
def processLine(line, output, keepTcl=1):
    ...
    if line[0] == '#':
        ...
        # Detect svtk class instance; Pythonize it.
        elif line[:3] == 'svtk':
            output.write( keys[1] + ' = svtk.' + keys[0] + '()\n' )
        else:
            lparen = 0
            finishedFlag = 0
            ...
            for i in range(len(keys)):
                ...
                # continuation mark
                    ...
                    if keys[i+1][0:1] == '"':
                        f_name = re.sub('"SVTK_DATA_ROOT/', \
                                        'SVTK_DATA_ROOT + "/', keys[i+1])
                        output.write( keys[i] + "(" + f_name + ")")
                    else:
                        f_name = re.sub('SVTK_DATA_ROOT/', \
                                        'SVTK_DATA_ROOT + "/', keys[i+1])
                        if f_name[:13] == 'SVTK_DATA_ROOT':
                            output.write(keys[i] + "(" + f_name + "\")")
                        else:
...
This script attempts to convert a SVTK-Tcl script to Python.
...
def main():
    ...
    # Standard stuff; import useful things and handle SVTK_DATA_ROOT.
    output.write('#!/usr/bin/env python\n')
    output.write('\nimport svtk\n')
    output.write('from svtk.util.misc import svtkGetDataRoot\n')
    output.write('SVTK_DATA_ROOT = svtkGetDataRoot()\n\n')
