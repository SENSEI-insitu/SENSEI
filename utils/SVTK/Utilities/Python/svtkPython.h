...
*/
#if defined(_DEBUG) && !defined(SVTK_WINDOWS_PYTHON_DEBUGGABLE)
#define SVTK_PYTHON_UNDEF_DEBUG
...
To avoid re-definitions warnings, ensure "svtkPython.h" is included
...
#ifdef SVTK_PYTHON_UNDEF_DEBUG
#define _DEBUG 1
#undef SVTK_PYTHON_UNDEF_DEBUG
#endif
...
/* This logic is borrowed from mpi4py/svtkmpi4py/src/atimport.h */
...
// Note: behaviour of this class depends on SVTK_PYTHON_FULL_THREADSAFE.
class svtkPythonScopeGilEnsurer
{
public:
  // Description:
  // If force is TRUE, lock/unlock even if SVTK_PYTHON_FULL_THREADSAFE is not defined.
  // If force is FALSE, lock/unlock is only performed if SVTK_PYTHON_FULL_THREADSAFE is
  // defined.
  ...
  svtkPythonScopeGilEnsurer(bool force = false, bool noRelease = false)
    : State(PyGILState_UNLOCKED)
  {
#ifdef SVTK_PYTHON_FULL_THREADSAFE
    // Force is always true with SVTK_PYTHON_FULL_THREADSAFE
    force = true;
#endif
    this->Force = force;
...
private:
  ...
  svtkPythonScopeGilEnsurer(const svtkPythonScopeGilEnsurer&) = delete;
  void operator=(const svtkPythonScopeGilEnsurer&) = delete;
};

#endif // __cplusplus

#endif
// SVTK-HeaderTest-Exclude: svtkPython.h
