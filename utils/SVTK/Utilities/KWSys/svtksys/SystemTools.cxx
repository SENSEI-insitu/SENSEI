...
#define SVTK_URL_PROTOCOL_REGEX "([a-zA-Z0-9]*)://(.*)"
#define SVTK_URL_REGEX                                                         \
  "([a-zA-Z0-9]*)://(([A-Za-z0-9]+)(:([^:@]+))?@)?([^:@/]*)(:([0-9]+))?/"     \
  "(.+)?"
#define SVTK_URL_BYTE_REGEX "%[0-9a-fA-F][0-9a-fA-F]"
#ifdef _MSC_VER
#  include <sys/utime.h>
#else
#  include <utime.h>
                                   ...
                                   std::string& dataglom, bool decode)
{
  ...
  kwsys::RegularExpression urlRe(SVTK_URL_PROTOCOL_REGEX);
...
{
  kwsys::RegularExpression urlRe(SVTK_URL_REGEX);
  if (!urlRe.find(URL))
    return false;
...
{
  kwsys::RegularExpression urlByteRe(SVTK_URL_BYTE_REGEX);
  std::string ret;
