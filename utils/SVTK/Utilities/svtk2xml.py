...
def getReaderWriter(file_name, out_dir=None):
    ...
    for i in ['StructuredPoints', 'StructuredGrid', 'RectilinearGrid',
        ...
        if eval('r.IsFile%s()'%i):
            reader = eval('svtk.svtk%sReader()'%i)
            if i == 'StructuredPoints':
                writer = eval('svtk.svtkXMLImageDataWriter()')
            else:
                writer = eval('svtk.svtkXML%sWriter()'%i)
            xmlsuffix = map[i]
            break
    if not reader:
        return None, None
...
def usage():
    msg = """usage: svtk2xml.py [options] svtk_file1 svtk_file2 ...\n
This program converts SVTK's old file format to the new XML format.
...
This code requires SVTK 4.x or above to run.
