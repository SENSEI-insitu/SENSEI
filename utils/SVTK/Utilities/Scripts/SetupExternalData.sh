...
say_store() {
  if test -f "$config_store"; then
    echo 'A default for SVTK_DATA_STORE is configured as:' &&
    echo &&
    sed 's/^/  /' < "$config_store" &&
    echo
  else
    echo 'No default for SVTK_DATA_STORE is configured.' &&
    echo
  fi
}
...
select a default for SVTK_DATA_STORE [n/h/s]: ' ans &&
    case "$ans" in
      n|N|no) ans='no' ;;
...
store_sibling="$(native_path "${PWD%/*}/SVTKExternalData")" &&
echo 'SVTK may download data objects into a local "store" using
the CMake ExternalData module.  The store is content-addressed
and can be shared across multiple projects and build trees.
SVTK build trees have a SVTK_DATA_STORE CMake cache entry to set
...
say_exclude() {
  if test -f "$config_exclude"; then
    echo 'A default for SVTK_DATA_EXCLUDE_FROM_ALL is configured as:' &&
    echo &&
    sed 's/^/  /' < "$config_exclude" &&
    echo
  else
    echo 'No default for SVTK_DATA_EXCLUDE_FROM_ALL is configured.' &&
    echo
  fi
}
...
ask_exclude() {
 ...
 n,no,off    = Include SVTKData target in default build
 y,yes,on    = Exclude SVTKData target from default build

select a default for SVTK_DATA_EXCLUDE_FROM_ALL [d/n/y]: ' ans &&
    case "$ans" in
      d|D|delete) ans='D' ;;
      n|N|no|off) ans='OFF' ;;
      y|Y|yes|on) ans='ON' ;;
...
echo 'SVTK defines a "SVTKData" build target to download data
objects at build time to make them available for running tests.
SVTK build trees have a SVTK_DATA_EXCLUDE_FROM_ALL CMake
cache option to exclude the "SVTKData" target from being built
