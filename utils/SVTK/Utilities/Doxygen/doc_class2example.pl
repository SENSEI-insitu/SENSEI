...
#
# 0.55 (barre) :
#   - change default --to to '../svtk-doxygen' to comply with Kitware's doxyfile
#
# 0.54 (barre) :
...
#
# 0.4 (barre) :
#   - change (warning) default --to to '../svtk2' because I ruined my own
#     SVTK distrib too many times :(
...
#
# 0.2 (barre)
#   - use svtkVersion.h to fill the main page (@mainpage) with version/revision
...
my %default =
  (
   ...
   linksuffix => "",
   project => "SVTK",
   store => "doc_SVTK_class2examples.dox",
...
my $eliminate_matcher = '^svtkCommand$';
...
sub parse {
    my $ref = shift;
    my @matches = $$ref =~ /\b(svtk[A-Z0-9][A-Za-z0-9]+)[^"]/gms;
...
# Ex: xref{"svtkNormals"}{"C++"}{"test.cxx"}
...
#   ex: $headers{"svtkPoints"} = "Common/svtkPoints.h"
...
find sub {
    ...
    } elsif (-f $_ && $_ =~ /^(svtk[A-Z0-9][A-Za-z0-9]+)\.h$/) {
