...
#
# 0.22 (barre) :
#   - add more (useful) links to various SVTK documentation resources
#
# 0.21 (barre) :
#   - no more --logo defaults
#
# 0.2 (barre) :
#   - update to match the new SVTK 4.0 tree
...
#
# 0.16 (barre) :
#   - change default --to to '../svtk-doxygen' to comply with Kitware
#   - update SVTK home page URL.
#
# 0.15 (barre) :
...
my %default =
  (
   version_file => "../../CMake/svtkVersion.cmake",
   store => "doc_SVTK_version.dox",
   to => "../../../SVTK-doxygen"
  );
...
while (<FILE>) {
    if ($_ =~ /SVTK_MAJOR_VERSION\s+(\d+)/) {
        $major_version = $1;
        print " major => $major_version\n";
    } elsif ($_ =~ /SVTK_MINOR_VERSION\s+(\d+)/) {
        $minor_version = $1;
        print " minor => $minor_version\n";
    } elsif ($_ =~ /SVTK_BUILD_VERSION\s+(\d+)/) {
        $build_version = $1;
...
print DEST_FILE
  "/*! \@mainpage SVTK $major_version.$minor_version.$build_version Documentation\n\n";
...
print DEST_FILE
  ...
  "   communities. \n",
  "  \@par Useful links:\n",
  "  \@li SVTK Home: https://svtk.org/\n",
  "  \@li SVTK Source: https://gitlab.kitware.com/svtk/svtk\n",
  "  \@li SVTK Discourse Forum: https://discourse.svtk.org\n",
  "  \@li SVTK FAQ: http://www.svtk.org/Wiki/SVTK_FAQ\n",
  "  \@li SVTK Wiki: http://www.svtk.org/Wiki/\n",
  "  \@li SVTK Dashboard: https://open.cdash.org/index.php?project=SVTK\n",
