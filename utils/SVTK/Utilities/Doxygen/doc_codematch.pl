...
my %default =
  (
   ...
   store => "doc_SVTK_events.dox",
   store2 => "doc_SVTK_events2.dox",
...
# Example: $match2classes{"svtkCommand::Event"}{"svtkMarchingCubes"} = 2
...
foreach my $source (@files) {
    ...
    # Skip if not a SVTK header file or has no implementation file

    next if $source !~ /(\w?svtk[^\\\/]*)\.$args{"headerext"}\Z/;
