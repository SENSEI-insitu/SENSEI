/*=========================================================================
  ...
  Module:    svtkAmoebaMinimizer.cxx
...
=========================================================================*/
#include "svtkAmoebaMinimizer.h"
#include "svtkObjectFactory.h"

#define N_STEPS_NO_VALUE_IMPROVEMENT 2
#define N_STEPS_NO_PARAM_IMPROVEMENT 18

svtkStandardNewMacro(svtkAmoebaMinimizer);

//----------------------------------------------------------------------------
svtkAmoebaMinimizer::svtkAmoebaMinimizer()
{
...
//----------------------------------------------------------------------------
void svtkAmoebaMinimizer::PrintSelf(ostream& os, svtkIndent indent)
{
...
//----------------------------------------------------------------------------
void svtkAmoebaMinimizer::SetFunction(void (*f)(void*), void* arg)
{
...
//----------------------------------------------------------------------------
void svtkAmoebaMinimizer::SetFunctionArgDelete(void (*f)(void*))
{
...
//----------------------------------------------------------------------------
double svtkAmoebaMinimizer::GetParameterValue(const char* name)
{
  for (int i = 0; i < this->NumberOfParameters; i++)
  {
  ...
  svtkErrorMacro("GetParameterValue: no parameter named " << name);
  return 0.0;
}

//----------------------------------------------------------------------------
void svtkAmoebaMinimizer::SetParameterValue(const char* name, double val)
{
  int i;
...
//----------------------------------------------------------------------------
void svtkAmoebaMinimizer::SetParameterValue(int i, double val)
{
...
//----------------------------------------------------------------------------
double svtkAmoebaMinimizer::GetParameterScale(const char* name)
{
  for (int i = 0; i < this->NumberOfParameters; i++)
  {
    if (this->ParameterNames[i] && strcmp(name, this->ParameterNames[i]) == 0)
  ...
  svtkErrorMacro("GetParameterScale: no parameter named " << name);
  return 1.0;
}

//----------------------------------------------------------------------------
void svtkAmoebaMinimizer::SetParameterScale(const char* name, double scale)
{
  for (int i = 0; i < this->NumberOfParameters; i++)
  {
    if (this->ParameterNames[i] && strcmp(name, this->ParameterNames[i]) == 0)
  ...
  svtkErrorMacro("SetParameterScale: no parameter named " << name);
}

//----------------------------------------------------------------------------
void svtkAmoebaMinimizer::SetParameterScale(int i, double scale)
{
  if (i < 0 || i > this->NumberOfParameters)
  {
    svtkErrorMacro("SetParameterScale: parameter number out of range: " << i);
    return;
  }
...
void svtkAmoebaMinimizer::Initialize()
{
  if (this->ParameterNames)
  {
    for (int i = 0; i < this->NumberOfParameters; i++)
...
//----------------------------------------------------------------------------
void svtkAmoebaMinimizer::EvaluateFunction()
{
  if (this->Function)
  {
...
//----------------------------------------------------------------------------
int svtkAmoebaMinimizer::CheckParameterTolerance()
{
...
//----------------------------------------------------------------------------
int svtkAmoebaMinimizer::Iterate()
{
  if (this->Iterations == 0)
  {
    if (!this->Function)
    {
      svtkErrorMacro("Iterate: Function is nullptr");
      return 0;
    }
...
//----------------------------------------------------------------------------
void svtkAmoebaMinimizer::Minimize()
{
  if (this->Iterations == 0)
  {
    if (!this->Function)
    {
      svtkErrorMacro("Minimize: Function is nullptr");
      return;
    }
    this->InitializeAmoeba();
  }
...
/* ------------------------------------------------
  ...
  have been expanded, functions have been renamed to match SVTK
  conventions, and the formatting has been changed.
*/

/* ----------------------------- MNI Header -----------------------------------
@NAME       : svtkAmoebaNumericallyClose
@INPUT      : n1
              n2
              threshold_ratio
@OUTPUT     :
...
#define SVTK_AMOEBA_SMALLEST 1.0e-20

static int svtkAmoebaNumericallyClose(double n1, double n2, double threshold_ratio)
{
  ...
  if (abs_n1 < SVTK_AMOEBA_SMALLEST || abs_n2 < SVTK_AMOEBA_SMALLEST)
  {
...
void svtkAmoebaMinimizer::InitializeAmoeba()
{
  int i, j;
...
void svtkAmoebaMinimizer::GetAmoebaParameterValues()
{
  int i, j, low;
...
void svtkAmoebaMinimizer::TerminateAmoeba()
{
...
double svtkAmoebaMinimizer::TryAmoeba(double sum[], int high, double fac)
{
  int j;
...
int svtkAmoebaMinimizer::PerformAmoeba()
{
  ...
  if (this->AmoebaValues[high] == this->AmoebaHighValue ||
    svtkAmoebaNumericallyClose(this->AmoebaValues[low], this->AmoebaValues[high], this->Tolerance))
  {
