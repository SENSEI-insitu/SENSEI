...
#include "svtkIOStream.h" // For streaming operators
...
public:
   ...
   */
  explicit svtkTuple(const T& scalar)
  {
   ...
   */
  explicit svtkTuple(const T* init)
  {
    for (int i = 0; i < Size; ++i)
    {
      this->Data[i] = init[i];
   ...
   */
  bool Compare(const svtkTuple<T, Size>& other, const T& tol) const
  {
  ...
  svtkTuple<TR, Size> Cast() const
  {
    svtkTuple<TR, Size> result;
    for (int i = 0; i < Size; ++i)
    {
...
ostream& operator<<(ostream& out, const svtkTuple<A, Size>& t)
{
  out << "(";
  bool first = true;
  for (int i = 0; i < Size; ++i)
...
template <int Size>
ostream& operator<<(ostream& out, const svtkTuple<unsigned char, Size>& t)
{
  out << "(";
  bool first = true;
  for (int i = 0; i < Size; ++i)
 ...
 */
template <typename A, int Size>
bool operator==(const svtkTuple<A, Size>& t1, const svtkTuple<A, Size>& t2)
{
  for (int i = 0; i < Size; ++i)
  {
    if (t1[i] != t2[i])
...
#endif // svtkTuple_h
// SVTK-HeaderTest-Exclude: svtkTuple.h
