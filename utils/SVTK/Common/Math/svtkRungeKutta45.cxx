...
svtkStandardNewMacro(svtkRungeKutta45);

//----------------------------------------------------------------------------
// Cash-Karp parameters
double svtkRungeKutta45::A[5] = { 1.0 / 5.0, 3.0 / 10.0, 3.0 / 5.0, 1.0, 7.0 / 8.0 };
double svtkRungeKutta45::B[5][5] = { { 1.0 / 5.0, 0, 0, 0, 0 }, { 3.0 / 40.0, 9.0 / 40.0, 0, 0, 0 },
  { 3.0 / 10.0, -9.0 / 10.0, 6.0 / 5.0, 0, 0 },
  { -11.0 / 54.0, 5.0 / 2.0, -70.0 / 27.0, 35.0 / 27.0, 0 },
  { 1631.0 / 55296.0, 175.0 / 512.0, 575.0 / 13824.0, 44275.0 / 110592.0, 253.0 / 4096.0 } };
double svtkRungeKutta45::C[6] = { 37.0 / 378.0, 0, 250.0 / 621.0, 125.0 / 594.0, 0, 512.0 / 1771.0 };
double svtkRungeKutta45::DC[6] = { 37.0 / 378.0 - 2825.0 / 27648.0, 0,
  250.0 / 621.0 - 18575.0 / 48384.0, 125.0 / 594.0 - 13525.0 / 55296.0, -277.0 / 14336.0,
  512.0 / 1771.0 - 1.0 / 4.0 };

//----------------------------------------------------------------------------
svtkRungeKutta45::svtkRungeKutta45()
{
  for (int i = 0; i < 6; i++)
  {
...
//----------------------------------------------------------------------------
void svtkRungeKutta45::Initialize()
{
  this->svtkInitialValueProblemSolver::Initialize();
...
//----------------------------------------------------------------------------
int svtkRungeKutta45::ComputeNextStep(double* xprev, double* dxprev, double* xnext, double t,
  double& delT, double& delTActual, double minStep, double maxStep, double maxError, double& estErr,
  void* userData)
{
  estErr = SVTK_DOUBLE_MAX;
  ...
  while (estErr > maxError)
  {
    ...
    if (tmp2 == t)
    {
      svtkWarningMacro("Step size underflow. You must choose a larger "
...
int svtkRungeKutta45::ComputeAStep(double* xprev, double* dxprev, double* xnext, double t,
...
//----------------------------------------------------------------------------
void svtkRungeKutta45::PrintSelf(ostream& os, svtkIndent indent)
{
