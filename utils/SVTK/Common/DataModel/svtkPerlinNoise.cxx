...
svtkStandardNewMacro(svtkPerlinNoise);
...
// assumes 32 bit ints, but so it seems does SVTK
static double frand(unsigned int s)
{
  s = (s << 13) ^ s;
  s = (s * (s * s * 15731 + 789221) + 1376312589) & SVTK_INT_MAX;

  return 1.0 - double(s) / (SVTK_INT_MAX / 2 + 1);
}
...
svtkPerlinNoise::svtkPerlinNoise()
{
...
double svtkPerlinNoise::EvaluateFunction(double x[3])
{
  double xd[3];
  double noise[4];
...
void svtkPerlinNoise::EvaluateGradient(double* svtkNotUsed(x), // Was x[3]
  double n[3])
{
...
void svtkPerlinNoise::PrintSelf(ostream& os, svtkIndent indent)
{
