/*=========================================================================
  ...
  Module:    svtkIncrementalOctreeNode.cxx
...
svtkStandardNewMacro(svtkIncrementalOctreeNode);

svtkCxxSetObjectMacro(svtkIncrementalOctreeNode, PointIdSet, svtkIdList);
svtkCxxSetObjectMacro(svtkIncrementalOctreeNode, Parent, svtkIncrementalOctreeNode);
...
// function svtkIncreemntalOctreeNode::InsertPoint(). The caller inserts the
// point index to the svtkIdList maintained by a leaf node, without inserting
// the point (coordinate) to svtkPoints at all.
static void OctreeNodeGetPointId(
  svtkPoints* svtkNotUsed(points), svtkIdType* svtkNotUsed(pntIdx), const double* svtkNotUsed(coords))
{
  // the 3D point coordinate is not inserted to svtkPoints at all
}

//----------------------------------------------------------------------------
// Insert a point, with a specified point index, to a svtkPoints object by
// calling svtkPoints::InsertPoint().
static void OctreeNodeInsertPoint(svtkPoints* points, svtkIdType* pntIdx, const double* coords)
{
  points->InsertPoint(*pntIdx, coords);
}

//----------------------------------------------------------------------------
// Insert a point to a svtkPoints by calling svtkPoints::InsertNextPoint().
static void OctreeNodeInsertNextPoint(svtkPoints* points, svtkIdType* pntIdx, const double* coords)
{
...
typedef void (*OCTREENODE_INSERTPOINT_FUNCTION)(
  svtkPoints* points, svtkIdType* pntIdx, const double* coords);
...
// ---------------------------------------------------------------------------
// ------------------------- svtkIncrementalOctreeNode ------------------------
// ---------------------------------------------------------------------------

//----------------------------------------------------------------------------
svtkIncrementalOctreeNode::svtkIncrementalOctreeNode()
{
  this->Parent = nullptr;
  ...
  for (int i = 0; i < 3; i++)
  {
    this->MinBounds[i] = this->MinDataBounds[i] = SVTK_DOUBLE_MIN;
    this->MaxBounds[i] = this->MaxDataBounds[i] = SVTK_DOUBLE_MAX;
  }
}
...
//----------------------------------------------------------------------------
void svtkIncrementalOctreeNode::DeleteChildNodes()
{
  if (this->Children)
  {
    for (int i = 0; i < 8; i++)
...
//----------------------------------------------------------------------------
void svtkIncrementalOctreeNode::CreatePointIdSet(int initSize, int growSize)
{
  if (this->PointIdSet == nullptr)
  {
    this->PointIdSet = svtkIdList::New();
    this->PointIdSet->Allocate(initSize, growSize);
  }
}

//----------------------------------------------------------------------------
void svtkIncrementalOctreeNode::DeletePointIdSet()
{
  if (this->PointIdSet)
  {
...
//----------------------------------------------------------------------------
void svtkIncrementalOctreeNode::SetBounds(
...
//----------------------------------------------------------------------------
void svtkIncrementalOctreeNode::GetBounds(double bounds[6]) const
{
...
//----------------------------------------------------------------------------
int svtkIncrementalOctreeNode::UpdateCounterAndDataBounds(
...
//----------------------------------------------------------------------------
void svtkIncrementalOctreeNode::SeperateExactlyDuplicatePointsFromNewInsertion(svtkPoints* points,
  svtkIdList* pntIds, const double newPnt[3], svtkIdType* pntIdx, int maxPts, int ptMode)
{
  ...
  svtkIncrementalOctreeNode* ocNode = nullptr;
  svtkIncrementalOctreeNode* duplic = this;
  svtkIncrementalOctreeNode* single = this;
  ...
  while (duplic == single) // as long as separation has not been achieved
  {
    ...
    ocNode->Children = new svtkIncrementalOctreeNode*[8];
    for (i = 0; i < 8; i++)
    {
      // x-bound: axis 0
      ...
      ocNode->Children[i] = svtkIncrementalOctreeNode::New();
  ...
  // create a svtkIdList object for the new point
...
//----------------------------------------------------------------------------
int svtkIncrementalOctreeNode::CreateChildNodes(svtkPoints* points, svtkIdList* pntIds,
  const double newPnt[3], svtkIdType* pntIdx, int maxPts, int ptMode)
{
  ...
  if (this->ContainsDuplicatePointsOnly(sample) == 1)
  {
    ...
    // notify svtkIncrementalOctreeNode::InsertPoint() that pntIds just needs
  ...
  // create eight child nodes
  this->Children = new svtkIncrementalOctreeNode*[8];
  for (i = 0; i < 8; i++)
  {
    // x-bound: axis 0
    octMin[0] = boxPtr[OCTREE_CHILD_BOUNDS_LUT[i][0][0]][0];
    ...
    this->Children[i] = svtkIncrementalOctreeNode::New();
  ...
  // Now it is time to reclaim those un-used svtkIdList objects, of which each
  // either is empty or still needs to be deleted due to further division of
  // the child node. This post-deallocation of the un-used svtkIdList objects
  // (of some child nodes) is based on the assumption that retrieving the
  // 'maxPts' points from svtkPoints and the associated 'maxPts' point-indices
  // from svtkIdList is more expensive than reclaiming at most 8 svtkIdList
  // objects at hand.
  for (i = 0; i < 8; i++)
  {
  ...
  // notify svtkIncrementalOctreeNode::InsertPoint() to destroy pntIds
  return 1;
}

//----------------------------------------------------------------------------
int svtkIncrementalOctreeNode::InsertPoint(
  svtkPoints* points, const double newPnt[3], int maxPts, svtkIdType* pntId, int ptMode)
{
  if (this->PointIdSet)
  {
...
//----------------------------------------------------------------------------
double svtkIncrementalOctreeNode::GetDistance2ToBoundary(const double point[3], double closest[3],
  int innerOnly, svtkIncrementalOctreeNode* rootNode, int checkData)
{
  ...
  double minDist = SVTK_DOUBLE_MAX; // minimum distance to the boundaries
  if (checkData)
  {
  ...
  switch (xyzFlag)
  {
    case 0: // withinZ = 0; withinY = 0;  withinX = 0
      ...
      minDist = svtkMath::Distance2BetweenPoints(point, closest);
      break;
...
//----------------------------------------------------------------------------
double svtkIncrementalOctreeNode::GetDistance2ToBoundary(
  const double point[3], svtkIncrementalOctreeNode* rootNode, int checkData)
{
  double dumbPnt[3];
...
//----------------------------------------------------------------------------
double svtkIncrementalOctreeNode::GetDistance2ToBoundary(
  const double point[3], double closest[3], svtkIncrementalOctreeNode* rootNode, int checkData)
{
  return (checkData == 1 && this->GetNumberOfPoints() == 0)
    ? SVTK_DOUBLE_MAX
...
//----------------------------------------------------------------------------
double svtkIncrementalOctreeNode::GetDistance2ToInnerBoundary(
  const double point[3], svtkIncrementalOctreeNode* rootNode)
{
  double dumbPnt[3];
...
//----------------------------------------------------------------------------
void svtkIncrementalOctreeNode::ExportAllPointIdsByInsertion(svtkIdList* idList)
{
  if (this->Children == nullptr)
  {
    for (svtkIdType localId = 0; localId < this->NumberOfPoints; localId++)
    {
...
//----------------------------------------------------------------------------
void svtkIncrementalOctreeNode::ExportAllPointIdsByDirectSet(svtkIdType* pntIdx, svtkIdList* idList)
{
  if (this->Children == nullptr)
  {
    for (svtkIdType localId = 0; localId < this->NumberOfPoints; localId++)
...
//----------------------------------------------------------------------------
void svtkIncrementalOctreeNode::PrintSelf(ostream& os, svtkIndent indent)
{
