 ...
 *
 * svtkCellLocator is a spatial search object to quickly locate cells in 3D.
 * svtkCellLocator uses a uniform-level octree subdivision, where each octant
 ...
 * candidate cells, or intersection with another svtkCellLocator to return
 * candidate cells.
 *
 * @warning
 * Many other types of spatial locators have been developed, such as
 * variable depth octrees and kd-trees. These are often more efficient
 * for the operations described here. svtkCellLocator has been designed
 ...
 * more efficient generic implementation, please use svtkStaticCellLocator
 *
 * @sa
 * svtkLocator svtkPointLocator svtkOBBTree svtkStaticCellLocator
 */

#ifndef svtkCellLocator_h
#define svtkCellLocator_h
...
class SVTKCOMMONDATAMODEL_EXPORT svtkCellLocator : public svtkAbstractCellLocator
{
public:
  svtkTypeMacro(svtkCellLocator, svtkAbstractCellLocator);
   ...
   * FindClosestPointWithinRadius signatures, see svtkAbstractCellLocator.
   * THIS METHOD IS NOT THREAD SAFE.
   */
   ...
   */
  virtual svtkIdList* GetCells(int bucket);
...
protected:
  ...
  void ClearCellHasBeenVisited(svtkIdType id);
...
private:
  svtkCellLocator(const svtkCellLocator&) = delete;
  void operator=(const svtkCellLocator&) = delete;
};
