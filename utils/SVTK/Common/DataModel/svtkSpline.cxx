...
// ClampValueOff
svtkSpline::svtkSpline()
{
  this->ComputeTime = 0;
  this->ClampValue = 0;
  this->PiecewiseFunction = svtkPiecewiseFunction::New();
...
//----------------------------------------------------------------------------
void svtkSpline::SetParametricRange(double tMin, double tMax)
{
...
//----------------------------------------------------------------------------
void svtkSpline::GetParametricRange(double tRange[2]) const
{
...
//----------------------------------------------------------------------------
double svtkSpline::ComputeLeftDerivative()
{
...
//----------------------------------------------------------------------------
double svtkSpline::ComputeRightDerivative()
{
  double* dptr = this->PiecewiseFunction->GetDataPointer();
  int size = this->PiecewiseFunction->GetSize();
  if (dptr == nullptr || size < 2)
...
//----------------------------------------------------------------------------
int svtkSpline::GetNumberOfPoints()
{
...
void svtkSpline::AddPoint(double t, double x)
{
  if (this->ParametricRange[0] != this->ParametricRange[1])
  {
...
void svtkSpline::RemovePoint(double t)
{
  if (this->ParametricRange[0] != this->ParametricRange[1])
  {
    t = (t < this->ParametricRange[0]
...
void svtkSpline::RemoveAllPoints()
{
  this->PiecewiseFunction->RemoveAllPoints();
}

//----------------------------------------------------------------------------
void svtkSpline::DeepCopy(svtkSpline* s)
{
  svtkSpline* spline = svtkSpline::SafeDownCast(s);
...
svtkMTimeType svtkSpline::GetMTime()
{
...
//----------------------------------------------------------------------------
int svtkSpline::FindIndex(int size, double t)
{
  int index = 0;
...
//----------------------------------------------------------------------------
void svtkSpline::PrintSelf(ostream& os, svtkIndent indent)
{
