 ...
 *
 *
 *    This class converts between the svtkKdTree
 *    representation of a tree of svtkKdNodes (used by svtkDistributedDataFilter)
 ...
 *
 * @sa
 *      svtkKdTree svtkKdNode svtkDistributedDataFilter
 */

#ifndef svtkBSPCuts_h
#define svtkBSPCuts_h
...
class SVTKCOMMONDATAMODEL_EXPORT svtkBSPCuts : public svtkDataObject
{
public:
  static svtkBSPCuts* New();
  svtkTypeMacro(svtkBSPCuts, svtkDataObject);
  ...
  /**
   * Initialize the cuts from a tree of svtkKdNode's
   */

  void CreateCuts(svtkKdNode* kd);

  /**
   * Return a tree of svtkKdNode's representing the cuts specified
   * in this object.  This is our copy, don't delete it.
   */

  svtkKdNode* GetKdNodeTree() { return this->Top; }
   ...
   */
  int Equals(svtkBSPCuts* other, double tolerance = 0.0);
   ...
   */
  static svtkBSPCuts* GetData(svtkInformation* info);
  static svtkBSPCuts* GetData(svtkInformationVector* v, int i = 0);
  //@}
...
protected:
  svtkBSPCuts();
  ~svtkBSPCuts() override;

  static void DeleteAllDescendants(svtkKdNode* kd);

  static int CountNodes(svtkKdNode* kd);
  static void SetMinMaxId(svtkKdNode* kd);
  static void _PrintTree(svtkKdNode* kd, int depth);

  void BuildTree(svtkKdNode* kd, int idx);
  int WriteArray(svtkKdNode* kd, int loc);
  ...
  svtkBSPCuts(const svtkBSPCuts&) = delete;
  void operator=(const svtkBSPCuts&) = delete;
};
