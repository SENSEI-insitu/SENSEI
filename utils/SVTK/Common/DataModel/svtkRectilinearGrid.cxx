/*=========================================================================
  ...
  Module:    svtkRectilinearGrid.cxx
...
svtkStandardNewMacro(svtkRectilinearGrid);

svtkCxxSetObjectMacro(svtkRectilinearGrid, XCoordinates, svtkDataArray);
svtkCxxSetObjectMacro(svtkRectilinearGrid, YCoordinates, svtkDataArray);
svtkCxxSetObjectMacro(svtkRectilinearGrid, ZCoordinates, svtkDataArray);

//----------------------------------------------------------------------------
svtkRectilinearGrid::svtkRectilinearGrid()
{
...
//----------------------------------------------------------------------------
void svtkRectilinearGrid::Cleanup()
{
...
//----------------------------------------------------------------------------
void svtkRectilinearGrid::Initialize()
{
...
// object.
void svtkRectilinearGrid::CopyStructure(svtkDataSet* ds)
{
  svtkRectilinearGrid* rGrid = static_cast<svtkRectilinearGrid*>(ds);
  int i;
  this->Initialize();
...
//----------------------------------------------------------------------------
svtkCell* svtkRectilinearGrid::GetCell(svtkIdType cellId)
{
...
//----------------------------------------------------------------------------
svtkCell* svtkRectilinearGrid::GetCell(int iMin, int jMin, int kMin)
{
  svtkCell* cell = nullptr;
  svtkIdType idx, npts;
  int loc[3];
...
//----------------------------------------------------------------------------
void svtkRectilinearGrid::GetCell(svtkIdType cellId, svtkGenericCell* cell)
{
  svtkIdType idx, npts;
  int loc[3];
  int iMin, iMax, jMin, jMax, kMin, kMax;
...
// constructing a cell.
void svtkRectilinearGrid::GetCellBounds(svtkIdType cellId, double bounds[6])
{
  int loc[3];
  int iMin, iMax, jMin, jMax, kMin, kMax;
  double x[3];
...
//----------------------------------------------------------------------------
void svtkRectilinearGrid::GetPoints(svtkPoints* pnts)
{
...
//----------------------------------------------------------------------------
double* svtkRectilinearGrid::GetPoint(svtkIdType ptId)
{
  int loc[3];
  ...
  switch (this->DataDescription)
  {
    case SVTK_EMPTY:
      ...
      this->PointReturn[1] = 0.0;
      this->PointReturn[2] = 0.0;
      svtkErrorMacro("Requesting a point from an empty data set.");
    ...
    default:
      svtkErrorMacro(<< "Unexpected value for DataDescription (" << this->DataDescription
                    << ") in svtkRectilinearGrid::GetPoint");
      loc[0] = loc[1] = loc[2] = 0;
      break;
  }
...
void svtkRectilinearGrid::GetPoint(svtkIdType ptId, double x[3])
{
  int loc[3];
...
//----------------------------------------------------------------------------
void svtkRectilinearGrid::GetPoint(const int i, const int j, const int k, double p[3])
{
  int ijk[3];
  ijk[0] = i;
  ijk[1] = j;
  ...
  svtkIdType pntIdx = this->ComputePointId(ijk);
  this->GetPoint(pntIdx, p);
}

//----------------------------------------------------------------------------
svtkIdType svtkRectilinearGrid::FindPoint(double x[3])
{
  int i, j, loc[3];
  double xPrev, xNext;
  svtkDataArray* scalars[3];
...
svtkIdType svtkRectilinearGrid::FindCell(double x[3], svtkCell* svtkNotUsed(cell),
...
//----------------------------------------------------------------------------
svtkCell* svtkRectilinearGrid::FindAndGetCell(double x[3], svtkCell* svtkNotUsed(cell),
  svtkIdType svtkNotUsed(cellId), double svtkNotUsed(tol2), int& subId, double pcoords[3],
  double* weights)
{
  int loc[3];
  ...
  return svtkRectilinearGrid::GetCell(cellId);
}

//----------------------------------------------------------------------------
int svtkRectilinearGrid::GetCellType(svtkIdType svtkNotUsed(cellId))
{
  switch (this->DataDescription)
  {
    case SVTK_EMPTY:
...
//----------------------------------------------------------------------------
void svtkRectilinearGrid::ComputeBounds()
{
  double tmp;
...
void svtkRectilinearGrid::SetDimensions(int i, int j, int k)
{
...
//----------------------------------------------------------------------------
// Set dimensions of rectilinear grid dataset.
void svtkRectilinearGrid::SetDimensions(const int dim[3])
{
  this->SetExtent(0, dim[0] - 1, 0, dim[1] - 1, 0, dim[2] - 1);
}

//----------------------------------------------------------------------------
void svtkRectilinearGrid::SetExtent(int extent[6])
{
  int description;
...
//----------------------------------------------------------------------------
void svtkRectilinearGrid::SetExtent(int xMin, int xMax, int yMin, int yMax, int zMin, int zMax)
{
  int extent[6];
...
int svtkRectilinearGrid::ComputeStructuredCoordinates(double x[3], int ijk[3], double pcoords[3])
{
  int i, j;
...
//----------------------------------------------------------------------------
unsigned long svtkRectilinearGrid::GetActualMemorySize()
{
...
//----------------------------------------------------------------------------
void svtkRectilinearGrid::GetCellNeighbors(svtkIdType cellId, svtkIdList* ptIds, svtkIdList* cellIds)
{
  ...
  switch (numPtIds)
  {
    ...
    case 4: // vertex, edge, face neighbors
      svtkStructuredData::GetCellNeighbors(cellId, ptIds, cellIds, this->Dimensions);
      break;
...
//----------------------------------------------------------------------------
void svtkRectilinearGrid::ShallowCopy(svtkDataObject* dataObject)
{
  svtkRectilinearGrid* grid = svtkRectilinearGrid::SafeDownCast(dataObject);
...
//----------------------------------------------------------------------------
void svtkRectilinearGrid::DeepCopy(svtkDataObject* dataObject)
{
  svtkRectilinearGrid* grid = svtkRectilinearGrid::SafeDownCast(dataObject);
...
//----------------------------------------------------------------------------
void svtkRectilinearGrid::Crop(const int* updateExtent)
{
  ...
  else
  {
    ...
    {
      newCoords->InsertComponent(idx - static_cast<svtkIdType>(uExt[0]), 0,
        coords->GetComponent(idx - static_cast<svtkIdType>(ext[0]), 0));
    }
    ...
    {
      newCoords->InsertComponent(idx - static_cast<svtkIdType>(uExt[2]), 0,
        coords->GetComponent(idx - static_cast<svtkIdType>(ext[2]), 0));
    }
    ...
    {
      newCoords->InsertComponent(idx - static_cast<svtkIdType>(uExt[4]), 0,
        coords->GetComponent(idx - static_cast<svtkIdType>(ext[4]), 0));
    }
...
//----------------------------------------------------------------------------
svtkRectilinearGrid* svtkRectilinearGrid::GetData(svtkInformation* info)
{
  return info ? svtkRectilinearGrid::SafeDownCast(info->Get(DATA_OBJECT())) : nullptr;
}

//----------------------------------------------------------------------------
svtkRectilinearGrid* svtkRectilinearGrid::GetData(svtkInformationVector* v, int i)
{
  return svtkRectilinearGrid::GetData(v->GetInformationObject(i));
}

//----------------------------------------------------------------------------
void svtkRectilinearGrid::PrintSelf(ostream& os, svtkIndent indent)
{
