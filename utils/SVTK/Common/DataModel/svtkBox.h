 ...
 *
 * svtkBox computes the implicit function and/or gradient for a axis-aligned
 ...
 * the superclass svtkImplicitFunction supports a transformation matrix.)
 * svtkBox is a concrete implementation of svtkImplicitFunction.
 *
 * @sa
 * svtkCubeSource svtkImplicitFunction
 */
...
class SVTKCOMMONDATAMODEL_EXPORT svtkBox : public svtkImplicitFunction
{
public:
  svtkTypeMacro(svtkBox, svtkImplicitFunction);
  ...
  /**
   * Same method as svtkBox::IntersectWithLine, except that t1 and t2 can be outside of [0,1].
   ...
   * In svtkBox::IntersectWithInLine, it is assumed that [p1,p2] is a segment, here, it is
   ...
   * otherwise.
   */
  static svtkTypeBool IntersectWithPlane(double bounds[6], double origin[3], double normal[3]);
   ...
   * svtkPoints array layout and is organized as (xyz, xyz, xyz, xyz, xyz,
   * xyz).
   */
  static svtkTypeBool IntersectWithPlane(
...
private:
  svtkBox(const svtkBox&) = delete;
  void operator=(const svtkBox&) = delete;
};

inline void svtkBox::SetXMin(double p[3])
{
  this->SetXMin(p[0], p[1], p[2]);
}

inline void svtkBox::SetXMax(double p[3])
{
