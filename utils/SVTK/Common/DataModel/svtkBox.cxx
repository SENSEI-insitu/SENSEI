...
void svtkBox::SetBounds(double xMin, double xMax, double yMin, double yMax, double zMin, double zMax)
{
...
//----------------------------------------------------------------------------
void svtkBox::SetBounds(const double bounds[6])
{
  this->SetBounds(bounds[0], bounds[1], bounds[2], bounds[3], bounds[4], bounds[5]);
}

//----------------------------------------------------------------------------
void svtkBox::SetXMin(double x, double y, double z)
{
  svtkDebugMacro(<< this->GetClassName() << " (" << this << "): setting XMin to (" << x << "," << y
                << "," << z << ")");
...
//----------------------------------------------------------------------------
void svtkBox::SetXMax(double x, double y, double z)
{
  svtkDebugMacro(<< this->GetClassName() << " (" << this << "): setting XMax to (" << x << "," << y
                << "," << z << ")");
...
//----------------------------------------------------------------------------
void svtkBox::GetBounds(double bounds[6])
{
  this->BBox->GetBounds(bounds);
}

//----------------------------------------------------------------------------
double* svtkBox::GetBounds()
{
...
//----------------------------------------------------------------------------
void svtkBox::AddBounds(const double bounds[6])
{
  svtkBoundingBox bbox(*(this->BBox));
...
//----------------------------------------------------------------------------
// Evaluate box equation. This differs from the similar svtkPlanes
// (with six planes) because of the "rounded" nature of the corners.
double svtkBox::EvaluateFunction(double x[3])
{
  double diff, dist, minDistance = (-SVTK_DOUBLE_MAX), t, distance = 0.0;
  int inside = 1;
  const double* minP = this->BBox->GetMinPoint();
  const double* maxP = this->BBox->GetMaxPoint();
...
void svtkBox::EvaluateGradient(double x[3], double n[3])
{
  int i, loc[3], minAxis = 0;
  double dist, minDist = SVTK_DOUBLE_MAX, center[3];
  ...
  const double bounds[6], const double origin[3], const double dir[3], double coord[3], double& t)
{
  ...
  for (i = 0; i < 3; i++)
  {
    ...
    {
      quadrant[i] = SVTK_LEFT;
      candidatePlane[i] = bounds[2 * i];
      inside = false;
    }
    else if (origin[i] > bounds[2 * i + 1])
    {
      quadrant[i] = SVTK_RIGHT;
      candidatePlane[i] = bounds[2 * i + 1];
      inside = false;
    }
    else
    {
      quadrant[i] = SVTK_MIDDLE;
    }
  }
  ...
  for (i = 0; i < 3; i++)
  {
    if (quadrant[i] != SVTK_MIDDLE && dir[i] != 0.0)
    {
...
int svtkBox::IntersectWithLine(const double bounds[6], const double p1[3], const double p2[3],
...
//----------------------------------------------------------------------------
bool svtkBox::IntersectWithInfiniteLine(const double bounds[6], const double p1[3],
  const double p2[3], double& t1, double& t2, double x1[3], double x2[3], int& plane1, int& plane2)
{
  ...
  for (int j = 0; j < 3; j++)
  {
    for (int k = 0; k < 2; k++)
    {
      ...
      int i = 2 * j + k;
      double t =
        std::abs(bounds[i] - p1[j]) < SVTK_DBL_MIN ? 0.0 : (bounds[i] - p1[j]) / (p2[j] - p1[j]);
...
//----------------------------------------------------------------------------
svtkTypeBool svtkBox::IntersectWithPlane(double bounds[6], double origin[3], double normal[3])
{
  ...
  for (z = 4; z <= 5; ++z)
  {
    ...
    for (y = 2; y <= 3; ++y)
    {
      ...
      {
        p[0] = bounds[x];
        d = svtkPlane::Evaluate(normal, origin, p);
        if (firstOne)
        {
          sign = (d >= 0 ? 1 : -1);
          firstOne = 0;
...
// is consistent with the svtkPoints array and is organized as (xyz, xyz, xyz,
// xyz, xyz, xyz).
svtkTypeBool svtkBox::IntersectWithPlane(
  double bounds[6], double origin[3], double normal[3], double xout[18])
{
  ...
  for (vertNum = 0, z = 4; z <= 5; ++z)
  {
    for (y = 2; y <= 3; ++y)
    {
      for (x = 0; x <= 1; ++x)
      {
        ...
        scalars[vertNum] = svtkPlane::Evaluate(n, origin, p[vertNum]);
        vertNum++;
      } // x
    }   // y
  }     // z
  ...
  svtkMath::Normalize(Vx);
  svtkMath::Cross(n, Vx, Vy);
  svtkMath::Normalize(Vy);
  ...
  for (i = 1; i < numInts; ++i)
  {
    ...
    dx = svtkMath::Dot(v, Vx);
    dy = svtkMath::Dot(v, Vy);
    t = atan2(dy, dx);
    t = (t >= 0 ? t : (2.0 * svtkMath::Pi() + t));
...
//----------------------------------------------------------------------------
void svtkBox::PrintSelf(ostream& os, svtkIndent indent)
{
...
//----------------------------------------------------------------------------
void svtkBox::GetXMin(double p[3])
{
  this->BBox->GetMinPoint(p[0], p[1], p[2]);
}

//----------------------------------------------------------------------------
void svtkBox::GetXMin(double& x, double& y, double& z)
{
  this->BBox->GetMinPoint(x, y, z);
}

//----------------------------------------------------------------------------
void svtkBox::GetXMax(double p[3])
{
  this->BBox->GetMaxPoint(p[0], p[1], p[2]);
}

//----------------------------------------------------------------------------
void svtkBox::GetXMax(double& x, double& y, double& z)
{
