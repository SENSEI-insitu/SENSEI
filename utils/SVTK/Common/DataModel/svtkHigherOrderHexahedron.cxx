/*=========================================================================
  ...
  Module:    svtkHigherOrderHexahedron.cxx
...
svtkHigherOrderHexahedron::svtkHigherOrderHexahedron()
{
  this->Approx = nullptr;
...
svtkHigherOrderHexahedron::~svtkHigherOrderHexahedron() = default;

void svtkHigherOrderHexahedron::PrintSelf(ostream& os, svtkIndent indent)
{
...
void svtkHigherOrderHexahedron::SetEdgeIdsAndPoints(int edgeId,
...
void svtkHigherOrderHexahedron::SetFaceIdsAndPoints(svtkHigherOrderQuadrilateral* result, int faceId,
  const std::function<void(const svtkIdType&)>& set_number_of_ids_and_points,
  const std::function<void(const svtkIdType&, const svtkIdType&)>& set_ids_and_points)
{
  ...
  const int* order = this->GetOrder();
  svtkVector2i faceParams = svtkHigherOrderInterpolation::GetVaryingParametersOfHexFace(faceId);
  const int* corners = svtkHigherOrderInterpolation::GetPointIndicesBoundingHexFace(faceId);
  ...
  int offset;
  const int* faceEdges = svtkHigherOrderInterpolation::GetEdgeIndicesBoundingHexFace(faceId);
  for (int ii = 0; ii < 4; ++ii)
  {
    ...
    if (!flipFace)
    {
      ...
      const int pp = svtkHigherOrderInterpolation::GetVaryingParameterOfHexEdge(edgeId);
      if (pp == 2)
      {
        offset += 4 * (order[0] + order[1] - 2);
        offset += (edgeId - 8) * (order[2] - 1);
  ...
  {
    svtkVector2i tmp = svtkHigherOrderInterpolation::GetVaryingParametersOfHexFace(ff);
...
void svtkHigherOrderHexahedron::Initialize() {}

int svtkHigherOrderHexahedron::CellBoundary(
...
int svtkHigherOrderHexahedron::EvaluatePosition(const double x[3], double closestPoint[3],
  int& subId, double pcoords[3], double& minDist2, double weights[])
{
  ...
  svtkIdType nhex = svtkHigherOrderInterpolation::NumberOfIntervals<3>(this->GetOrder());
  for (int subCell = 0; subCell < nhex; ++subCell)
  {
    svtkHexahedron* approx = this->GetApproximateHex(subCell, nullptr, nullptr);
...
void svtkHigherOrderHexahedron::EvaluateLocation(
...
void svtkHigherOrderHexahedron::Contour(double value, svtkDataArray* cellScalars,
...
void svtkHigherOrderHexahedron::Clip(double value, svtkDataArray* cellScalars,
...
int svtkHigherOrderHexahedron::IntersectWithLine(
...
int svtkHigherOrderHexahedron::Triangulate(int svtkNotUsed(index), svtkIdList* ptIds, svtkPoints* pts)
{
  ptIds->Reset();
  pts->Reset();
  ...
  for (int i = 0; i < nhex; ++i)
  {
    svtkHexahedron* approx = this->GetApproximateHex(i);
...
void svtkHigherOrderHexahedron::Derivatives(
...
void svtkHigherOrderHexahedron::SetParametricCoords()
{
  if (!this->PointParametricCoordinates)
  {
  ...
  if (static_cast<int>(this->PointParametricCoordinates->GetNumberOfPoints()) != this->GetOrder(3))
  {
    ...
    svtkHigherOrderInterpolation::AppendHexahedronCollocationPoints(
      this->PointParametricCoordinates, this->Order);
  }
}

double* svtkHigherOrderHexahedron::GetParametricCoords()
{
...
double svtkHigherOrderHexahedron::GetParametricDistance(const double pcoords[3])
{
...
svtkHexahedron* svtkHigherOrderHexahedron::GetApprox()
{
  if (!this->Approx)
  {
    this->Approx = svtkSmartPointer<svtkHexahedron>::New();
 ...
 */
void svtkHigherOrderHexahedron::PrepareApproxData(
...
bool svtkHigherOrderHexahedron::SubCellCoordinatesFromId(svtkVector3i& ijk, int subId)
{
 ...
 * up to date.
 */
bool svtkHigherOrderHexahedron::SubCellCoordinatesFromId(int& i, int& j, int& k, int subId)
{
  if (subId < 0)
  {
    return false;
 ...
 */
int svtkHigherOrderHexahedron::PointIndexFromIJK(int i, int j, int k)
{
  return svtkHigherOrderHexahedron::PointIndexFromIJK(i, j, k, this->Order);
}
 ...
 */
int svtkHigherOrderHexahedron::PointIndexFromIJK(int i, int j, int k, const int* order)
{
...
svtkIdType svtkHigherOrderHexahedron::NodeNumberingMappingFromSVTK8To9(
  const int order[3], const svtkIdType node_id_svtk8)
{
  ...
  if ((node_id_svtk8 < offset) || (node_id_svtk8 >= offset + 2 * numPtsPerEdgeWithoutCorners[2]))
    return node_id_svtk8;
  else if (node_id_svtk8 < offset + numPtsPerEdgeWithoutCorners[2])
    return node_id_svtk8 + numPtsPerEdgeWithoutCorners[2];
  else
    return node_id_svtk8 - numPtsPerEdgeWithoutCorners[2];
}
 ...
 */
bool svtkHigherOrderHexahedron::TransformApproxToCellParams(int subCell, double* pcoords)
{
  svtkVector3i ijk;
 ...
 * the order is up to date.
 */
bool svtkHigherOrderHexahedron::TransformFaceToCellParams(int bdyFace, double* pcoords)
{
  ...
  svtkVector2i faceParams = svtkHigherOrderInterpolation::GetVaryingParametersOfHexFace(bdyFace);
  ...
  pp = svtkHigherOrderInterpolation::GetFixedParameterOfHexFace(bdyFace);
 ...
 */
void svtkHigherOrderHexahedron::SetOrderFromCellData(
...
void svtkHigherOrderHexahedron::SetUniformOrderFromNumPoints(svtkIdType numPts)
{
...
void svtkHigherOrderHexahedron::SetOrder(int s, int t, int u)
{
...
const int* svtkHigherOrderHexahedron::GetOrder()
{
