/*=========================================================================
  ...
  Module:    svtkIncrementalOctreeNode.h
...
#ifndef svtkIncrementalOctreeNode_h
#define svtkIncrementalOctreeNode_h
...
class SVTKCOMMONDATAMODEL_EXPORT svtkIncrementalOctreeNode : public svtkObject
{
public:
  svtkTypeMacro(svtkIncrementalOctreeNode, svtkObject);
  void PrintSelf(ostream& os, svtkIndent indent) override;

  static svtkIncrementalOctreeNode* New();
   ...
   */
  svtkGetObjectMacro(PointIdSet, svtkIdList);
  //@}
   ...
   */
  svtkGetVector3Macro(MinBounds, double);
  //@}
   ...
   */
  svtkGetVector3Macro(MaxBounds, double);
  //@}
   ...
   */
  svtkIncrementalOctreeNode* GetChild(int i) { return this->Children[i]; }
   ...
   */
  svtkTypeBool ContainsPoint(const double pnt[3]);

  /**
   * A point is in a node, in terms of data, if and only if MinDataBounds[i]
   * <= p[i] <= MaxDataBounds[i].
   */
  svtkTypeBool ContainsPointByData(const double pnt[3]);
   ...
   * the point index is provided upon 0, the point is inserted via svtkPoints::
   * InsertPoint() upon 1, or it is inserted via svtkPoints::InsertNextPoint()
   ...
   */
  double GetDistance2ToInnerBoundary(const double point[3], svtkIncrementalOctreeNode* rootNode);
   ...
   * inserting them to an allocated svtkIdList via svtkIdList::InsertNextId().
   */
  void ExportAllPointIdsByInsertion(svtkIdList* idList);

  /**
   * Export all the indices of the points (contained in or under this node) by
   * directly setting them in an allocated svtkIdList object. pntIdx indicates
   * the starting location (in terms of svtkIdList) from which new point indices
   * are added to svtkIdList by svtkIdList::SetId().
   */
  void ExportAllPointIdsByDirectSet(svtkIdType* pntIdx, svtkIdList* idList);

protected:
  svtkIncrementalOctreeNode();
  ~svtkIncrementalOctreeNode() override;
...
private:
   ...
   * inserted via svtkPoints::InsertPoint() upon 1, or the point is inserted by
   * svtkPoints::InsertNextPoint() upon 2. The returned value of this function
   ...
   */
  int CreateChildNodes(svtkPoints* points, svtkIdList* pntIds, const double newPnt[3],
    svtkIdType* pntIdx, int maxPts, int ptMode);

  /**
   * Create a svtkIdList object for storing point indices. Two arguments
  ...
  int UpdateCounterAndDataBoundsRecursively(
    const double point[3], int nHits, int updateData, svtkIncrementalOctreeNode* endNode);
   ...
   * the point index is provided upon 0, the point is inserted via svtkPoints::
   * InsertPoint() upon 1, or this point is instead inserted through svtkPoints::
   * InsertNextPoint() upon 2.
   */
  void SeperateExactlyDuplicatePointsFromNewInsertion(svtkPoints* points, svtkIdList* pntIds,
  ...
  double GetDistance2ToBoundary(const double point[3], double closest[3], int innerOnly,
    svtkIncrementalOctreeNode* rootNode, int checkData = 0);

  svtkIncrementalOctreeNode(const svtkIncrementalOctreeNode&) = delete;
  void operator=(const svtkIncrementalOctreeNode&) = delete;
};

// In-lined for performance
inline int svtkIncrementalOctreeNode::GetChildIndex(const double point[3])
{
...
// In-lined for performance
inline svtkTypeBool svtkIncrementalOctreeNode::ContainsPoint(const double pnt[3])
{
  return (
...
// In-lined for performance
inline svtkTypeBool svtkIncrementalOctreeNode::ContainsPointByData(const double pnt[3])
{
...
// In-lined for performance
inline int svtkIncrementalOctreeNode::ContainsDuplicatePointsOnly(const double pnt[3])
{
  return ((this->MinDataBounds[0] == pnt[0] && pnt[0] == this->MaxDataBounds[0] &&
            this->MinDataBounds[1] == pnt[1] && pnt[1] == this->MaxDataBounds[1] &&
            this->MinDataBounds[2] == pnt[2] && pnt[2] == this->MaxDataBounds[2])
...
// In-lined for performance
inline void svtkIncrementalOctreeNode::UpdateCounterAndDataBounds(const double point[3])
{
  this->NumberOfPoints++;
...
// In-lined for performance
inline int svtkIncrementalOctreeNode::UpdateCounterAndDataBoundsRecursively(
  const double point[3], int nHits, int updateData, svtkIncrementalOctreeNode* endNode)
{
