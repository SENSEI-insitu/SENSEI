...
svtkStandardNewMacro(svtkPlanes);
svtkCxxSetObjectMacro(svtkPlanes, Points, svtkPoints);

//----------------------------------------------------------------------------
svtkPlanes::svtkPlanes()
{
  int i;
...
//----------------------------------------------------------------------------
void svtkPlanes::SetNormals(svtkDataArray* normals)
{
  svtkDebugMacro(<< this->GetClassName() << " (" << this << "): setting Normals to " << normals);

  if (normals && normals->GetNumberOfComponents() != 3)
  {
    svtkWarningMacro("This array does not have 3 components. Ignoring normals.");
    return;
  }
...
double svtkPlanes::EvaluateFunction(double x[3])
{
  int numPlanes, i;
  double val, maxVal;
  ...
  {
    svtkErrorMacro(<< "Please define points and/or normals!");
    return SVTK_DOUBLE_MAX;
  }

  if ((numPlanes = this->Points->GetNumberOfPoints()) != this->Normals->GetNumberOfTuples())
  {
    svtkErrorMacro(<< "Number of normals/points inconsistent!");
    return SVTK_DOUBLE_MAX;
  }

  for (maxVal = -SVTK_DOUBLE_MAX, i = 0; i < numPlanes; i++)
  {
...
void svtkPlanes::EvaluateGradient(double x[3], double n[3])
{
  int numPlanes, i;
  double val, maxVal;
  double nTemp[3];
...
//----------------------------------------------------------------------------
void svtkPlanes::SetFrustumPlanes(double planes[24])
{
  int i;
...
//----------------------------------------------------------------------------
void svtkPlanes::SetBounds(const double bounds[6])
{
  int i;
  double n[3], x[3];
...
//----------------------------------------------------------------------------
int svtkPlanes::GetNumberOfPlanes()
{
  if (this->Points && this->Normals)
  {
...
//----------------------------------------------------------------------------
svtkPlane* svtkPlanes::GetPlane(int i)
{
  double normal[3];
  double point[3];
...
//----------------------------------------------------------------------------
void svtkPlanes::GetPlane(int i, svtkPlane* plane)
{
  if (i >= 0 && i < this->GetNumberOfPlanes())
  {
    double normal[3];
...
//----------------------------------------------------------------------------
void svtkPlanes::PrintSelf(ostream& os, svtkIndent indent)
{
