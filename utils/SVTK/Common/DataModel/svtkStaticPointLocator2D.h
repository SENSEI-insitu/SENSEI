/*=========================================================================
  ...
  Module:    svtkStaticPointLocator2D.h
 ...
 *
 * svtkStaticPointLocator2D is a spatial search object to quickly locate points
 * in 2D.  svtkStaticPointLocator2D works by dividing a specified region of
 ...
 * points. (Note that the more general svtkStaticPointLocator is available
 * for 3D operations.) Other specialized methods for 2D have also been provided.
 *
 * svtkStaticPointLocator2D is an accelerated version of svtkPointLocator. It is
 ...
 * incrementally insert points, use the svtkPointLocator or its kin to do so.
 *
 ...
 * svtkAbstactPointLocator and svtkLocator. In particular the Automatic
 ...
 *
 * @sa
 * svtkStaticPointLocator svtkPointLocator svtkCellLocator svtkLocator
 * svtkAbstractPointLocator
 */

#ifndef svtkStaticPointLocator2D_h
#define svtkStaticPointLocator2D_h
...
class SVTKCOMMONDATAMODEL_EXPORT svtkStaticPointLocator2D : public svtkAbstractPointLocator
{
public:
  /**
   ...
   * 5 points per bucket.
   */
  static svtkStaticPointLocator2D* New();

  //@{
  /**
   * Standard type and print methods.
   */
  svtkTypeMacro(svtkStaticPointLocator2D, svtkAbstractPointLocator);
   ...
   */
  svtkSetVector2Macro(Divisions, int);
  svtkGetVectorMacro(Divisions, int, 2);
  //@}

  // Re-use any superclass signatures that we don't override.
  using svtkAbstractPointLocator::FindClosestNPoints;
  using svtkAbstractPointLocator::FindClosestPoint;
  using svtkAbstractPointLocator::FindPointsWithinRadius;
  using svtkAbstractPointLocator::GetBounds;
  ...
  //@{
  /**
   * Special method for 2D operations (e.g., svtkVoronoi2D). The method
   ...
   * mergeMap.
   */
  void MergePoints(double tol, svtkIdType* mergeMap);

  //@{
  /**
   * See svtkLocator and svtkAbstractPointLocator interface documentation.
   ...
   */
  svtkIdType GetNumberOfPointsInBucket(svtkIdType bNum);

  /**
   * Given a bucket number bNum between 0 <= bNum < this->GetNumberOfBuckets(),
   * return a list of point ids contained within the bucket. The user must
   * provide an instance of svtkIdList to contain the result.
   */
  void GetBucketIds(svtkIdType bNum, svtkIdList* bList);
...
protected:
  svtkStaticPointLocator2D();
  ~svtkStaticPointLocator2D() override;
  ...
  svtkBucketList2D* Buckets;     // Lists of point ids in each bucket
...
private:
  svtkStaticPointLocator2D(const svtkStaticPointLocator2D&) = delete;
  void operator=(const svtkStaticPointLocator2D&) = delete;
};
