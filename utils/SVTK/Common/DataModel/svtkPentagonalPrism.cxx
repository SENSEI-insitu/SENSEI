/*=========================================================================
  ...
  Module:    svtkPentagonalPrism.cxx
...
svtkStandardNewMacro(svtkPentagonalPrism);

static const double SVTK_DIVERGED = 1.e6;

//----------------------------------------------------------------------------
// Construct the prism with ten points.
svtkPentagonalPrism::svtkPentagonalPrism()
{
  int i;
...
//
static const int SVTK_PENTA_MAX_ITERATION = 10;
static const double SVTK_PENTA_CONVERGED = 1.e-03;

//----------------------------------------------------------------------------
int svtkPentagonalPrism::EvaluatePosition(const double x[3], double closestPoint[3], int& subId,
  double pcoords[3], double& dist2, double weights[])
{
  ...
  for (iteration = converged = 0; !converged && (iteration < SVTK_PENTA_MAX_ITERATION); iteration++)
  {
    ...
    if (((fabs(pcoords[0] - params[0])) < SVTK_PENTA_CONVERGED) &&
      ((fabs(pcoords[1] - params[1])) < SVTK_PENTA_CONVERGED) &&
      ((fabs(pcoords[2] - params[2])) < SVTK_PENTA_CONVERGED))
    {
      converged = 1;
    }
...
void svtkPentagonalPrism::InterpolationFunctions(const double pcoords[3], double weights[10])
{
...
// http://dilbert.engr.ucdavis.edu/~suku/nem/papers/polyelas.pdf
//
void svtkPentagonalPrism::InterpolationDerivs(const double pcoords[3], double derivs[30])
{
  // SVTK needs parametric coordinates to be between [0,1]. Isoparametric
  // shape functions are formulated between [-1,1]. Here we do a
  // coordinate system conversion from [0,1] to [-1,1].
...
//----------------------------------------------------------------------------
void svtkPentagonalPrism::EvaluateLocation(
...
static constexpr svtkIdType edges[svtkPentagonalPrism::NumberOfEdges][2] = {
  { 0, 1 }, // 0
  { 1, 2 }, // 1
  { 2, 3 }, // 2
  { 3, 4 }, // 3
...
static constexpr svtkIdType
  faces[svtkPentagonalPrism::NumberOfFaces][svtkPentagonalPrism::MaximumFaceSize + 1] = {
    { 0, 4, 3, 2, 1, -1 },  // 0
    { 5, 6, 7, 8, 9, -1 },  // 1
    { 0, 1, 6, 5, -1, -1 }, // 2
    { 1, 2, 7, 6, -1, -1 }, // 3
...
static constexpr svtkIdType edgeToAdjacentFaces[svtkPentagonalPrism::NumberOfEdges][2] = {
  { 0, 2 }, // 0
  { 0, 3 }, // 1
  { 0, 4 }, // 2
  { 0, 5 }, // 3
...
static constexpr svtkIdType
  faceToAdjacentFaces[svtkPentagonalPrism::NumberOfFaces][svtkPentagonalPrism::MaximumFaceSize] = {
    { 6, 5, 4, 3, 2 },  // 0
    { 2, 3, 4, 5, 6 },  // 1
    { 0, 3, 1, 6, -1 }, // 2
    { 0, 4, 1, 2, -1 }, // 3
...
static constexpr svtkIdType
  pointToIncidentEdges[svtkPentagonalPrism::NumberOfPoints][svtkPentagonalPrism::MaximumValence] = {
    { 0, 10, 4 }, // 0
    { 0, 1, 11 }, // 1
    { 1, 2, 12 }, // 2
    { 2, 3, 13 }, // 3
...
static constexpr svtkIdType
  pointToIncidentFaces[svtkPentagonalPrism::NumberOfPoints][svtkPentagonalPrism::MaximumValence] = {
    { 2, 6, 0 }, // 0
    { 0, 3, 2 }, // 1
    { 0, 4, 3 }, // 2
    { 0, 5, 4 }, // 3
...
static constexpr svtkIdType
  pointToOneRingPoints[svtkPentagonalPrism::NumberOfPoints][svtkPentagonalPrism::MaximumValence] = {
    { 1, 5, 4 }, // 0
    { 0, 2, 6 }, // 1
    { 1, 3, 7 }, // 2
    { 2, 4, 8 }, // 3
...
static constexpr svtkIdType numberOfPointsInFace[svtkPentagonalPrism::NumberOfFaces] = {
  5, // 0
  5, // 1
  4, // 2
  4, // 3
...
//----------------------------------------------------------------------------
bool svtkPentagonalPrism::GetCentroid(double centroid[3]) const
{
  return svtkPentagonalPrism::ComputeCentroid(this->Points, nullptr, centroid);
}

//----------------------------------------------------------------------------
bool svtkPentagonalPrism::ComputeCentroid(
  svtkPoints* points, const svtkIdType* pointIds, double centroid[3])
{
  ...
  {
    svtkIdType facePointsIds[5] = { pointIds[faces[0][0]], pointIds[faces[0][1]],
...
//----------------------------------------------------------------------------
bool svtkPentagonalPrism::IsInsideOut()
{
  double n0[3], n1[3];
...
// parametrically.
int svtkPentagonalPrism::CellBoundary(int subId, const double pcoords[3], svtkIdList* pts)
{
  // load coordinates
...
//----------------------------------------------------------------------------
const svtkIdType* svtkPentagonalPrism::GetEdgeToAdjacentFacesArray(svtkIdType edgeId)
{
  assert(edgeId < svtkPentagonalPrism::NumberOfEdges && "edgeId too large");
  return edgeToAdjacentFaces[edgeId];
}

//----------------------------------------------------------------------------
const svtkIdType* svtkPentagonalPrism::GetFaceToAdjacentFacesArray(svtkIdType faceId)
{
  assert(faceId < svtkPentagonalPrism::NumberOfFaces && "faceId too large");
  return faceToAdjacentFaces[faceId];
}

//----------------------------------------------------------------------------
const svtkIdType* svtkPentagonalPrism::GetPointToIncidentEdgesArray(svtkIdType pointId)
{
  assert(pointId < svtkPentagonalPrism::NumberOfPoints && "pointId too large");
  return pointToIncidentEdges[pointId];
}

//----------------------------------------------------------------------------
const svtkIdType* svtkPentagonalPrism::GetPointToIncidentFacesArray(svtkIdType pointId)
{
  assert(pointId < svtkPentagonalPrism::NumberOfPoints && "pointId too large");
...
//----------------------------------------------------------------------------
const svtkIdType* svtkPentagonalPrism::GetPointToOneRingPointsArray(svtkIdType pointId)
{
  assert(pointId < svtkPentagonalPrism::NumberOfPoints && "pointId too large");
...
//----------------------------------------------------------------------------
const svtkIdType* svtkPentagonalPrism::GetEdgeArray(svtkIdType edgeId)
{
  assert(edgeId < svtkPentagonalPrism::NumberOfEdges && "edgeId too large");
  return edges[edgeId];
}
...
//----------------------------------------------------------------------------
svtkCell* svtkPentagonalPrism::GetEdge(int edgeId)
{
  const svtkIdType* verts;
...
//----------------------------------------------------------------------------
const svtkIdType* svtkPentagonalPrism::GetFaceArray(svtkIdType faceId)
{
  assert(faceId < svtkPentagonalPrism::NumberOfFaces && "faceId too large");
  return faces[faceId];
}
...
//----------------------------------------------------------------------------
svtkCell* svtkPentagonalPrism::GetFace(int faceId)
{
  const svtkIdType* verts;
...
//
int svtkPentagonalPrism::IntersectWithLine(const double p1[3], const double p2[3], double tol,
...
//----------------------------------------------------------------------------
int svtkPentagonalPrism::Triangulate(int svtkNotUsed(index), svtkIdList* ptIds, svtkPoints* pts)
{
  ptIds->Reset();
  pts->Reset();
  int i, p[4];
...
//
void svtkPentagonalPrism::Derivatives(
...
void svtkPentagonalPrism::JacobianInverse(
...
//----------------------------------------------------------------------------
svtkIdType svtkPentagonalPrism::GetPointToOneRingPoints(svtkIdType pointId, const svtkIdType*& pts)
{
  assert(pointId < svtkPentagonalPrism::NumberOfPoints && "pointId too large");
  pts = pointToOneRingPoints[pointId];
  return svtkPentagonalPrism::MaximumValence;
}

//----------------------------------------------------------------------------
svtkIdType svtkPentagonalPrism::GetPointToIncidentFaces(svtkIdType pointId, const svtkIdType*& faceIds)
{
  assert(pointId < svtkPentagonalPrism::NumberOfPoints && "pointId too large");
...
//----------------------------------------------------------------------------
svtkIdType svtkPentagonalPrism::GetPointToIncidentEdges(svtkIdType pointId, const svtkIdType*& edgeIds)
{
  assert(pointId < svtkPentagonalPrism::NumberOfPoints && "pointId too large");
...
//----------------------------------------------------------------------------
svtkIdType svtkPentagonalPrism::GetFaceToAdjacentFaces(svtkIdType faceId, const svtkIdType*& faceIds)
{
  assert(faceId < svtkPentagonalPrism::NumberOfFaces && "faceId too large");
...
//----------------------------------------------------------------------------
void svtkPentagonalPrism::GetEdgeToAdjacentFaces(svtkIdType edgeId, const svtkIdType*& pts)
{
  assert(edgeId < svtkPentagonalPrism::NumberOfEdges && "edgeId too large");
...
//----------------------------------------------------------------------------
void svtkPentagonalPrism::GetEdgePoints(int edgeId, int*& pts)
{
  SVTK_LEGACY_REPLACED_BODY(svtkPentagonalPrism::GetEdgePoints(int, int*&), "SVTK 9.0",
    svtkPentagonalPrism::GetEdgePoints(svtkIdType, const svtkIdType*&));
  static std::vector<int> tmp(std::begin(faces[edgeId]), std::end(faces[edgeId]));
  pts = tmp.data();
}

//----------------------------------------------------------------------------
void svtkPentagonalPrism::GetFacePoints(int faceId, int*& pts)
{
  SVTK_LEGACY_REPLACED_BODY(svtkPentagonalPrism::GetFacePoints(int, int*&), "SVTK 9.0",
    svtkPentagonalPrism::GetFacePoints(svtkIdType, const svtkIdType*&));
...
//----------------------------------------------------------------------------
void svtkPentagonalPrism::GetEdgePoints(svtkIdType edgeId, const svtkIdType*& pts)
{
  assert(edgeId < svtkPentagonalPrism::NumberOfEdges && "edgeId too large");
...
//----------------------------------------------------------------------------
svtkIdType svtkPentagonalPrism::GetFacePoints(svtkIdType faceId, const svtkIdType*& pts)
{
  assert(faceId < svtkPentagonalPrism::NumberOfFaces && "faceId too large");
...
static double svtkPentagonalPrismCellPCoords[30] = {
  0.654508, 0.975528, 0,  //
...
//----------------------------------------------------------------------------
double* svtkPentagonalPrism::GetParametricCoords()
{
  return svtkPentagonalPrismCellPCoords;
}

//----------------------------------------------------------------------------
void svtkPentagonalPrism::PrintSelf(ostream& os, svtkIndent indent)
{
