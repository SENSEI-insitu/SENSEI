...
//------------------------------------------------------------------------------
void svtkAMRUtilities::PrintSelf(std::ostream& os, svtkIndent indent)
{
  this->Superclass::PrintSelf(os, indent);
}

//------------------------------------------------------------------------------
bool svtkAMRUtilities::HasPartiallyOverlappingGhostCells(svtkOverlappingAMR* amr)
{
  ...
  for (; levelIdx > 0; --levelIdx)
  {
    ...
    {
      const svtkAMRBox& myBox = amr->GetAMRInfo()->GetAMRBox(levelIdx, dataIdx);
      const int* lo = myBox.GetLoCorner();
      int hi[3];
      myBox.GetValidHiCorner(hi);
      svtkAMRBox coarsenedBox = myBox;
...
//------------------------------------------------------------------------------
void svtkAMRUtilities::CopyFieldData(
  svtkFieldData* target, svtkIdType targetIdx, svtkFieldData* source, svtkIdType srcIdx)
{
  ...
  for (int arrayIdx = 0; arrayIdx < source->GetNumberOfArrays(); ++arrayIdx)
  {
    ...
    svtkDataArray* srcArray = source->GetArray(arrayIdx);
...
//------------------------------------------------------------------------------
void svtkAMRUtilities::CopyFieldsWithinRealExtent(
  int realExtent[6], svtkUniformGrid* ghostedGrid, svtkUniformGrid* strippedGrid)
{
  ...
  int dataDescription = svtkStructuredData::GetDataDescriptionFromExtent(realExtent);
  ...
  assert("pre: description of ghosted and non-ghosted grid mismatch!" &&
    (dataDescription == svtkStructuredData::GetDataDescription(ghostedGrid->GetDimensions())));

  // STEP 3: Get the corresponding cell-extent for accessing cell fields
  int realCellExtent[6];
  svtkStructuredData::GetCellExtentFromPointExtent(realExtent, realCellExtent, dataDescription);
  ...
  for (int i = IMIN(realExtent); i <= IMAX(realExtent); ++i)
  {
    for (int j = JMIN(realExtent); j <= JMAX(realExtent); ++j)
    {
      for (int k = KMIN(realExtent); k <= KMAX(realExtent); ++k)
      {
        ...
        svtkStructuredData::GetLocalStructuredCoordinates(ijk, realExtent, lijk, dataDescription);

        // Compute the source index w.r.t. the ghosted grid dimensions
        svtkIdType sourceIdx =
          svtkStructuredData::ComputePointId(ghostedGrid->GetDimensions(), ijk, dataDescription);

        // Compute the target index w.r.t the real extent
        svtkIdType targetIdx =
          svtkStructuredData::ComputePointIdForExtent(realExtent, ijk, dataDescription);

        // Copy node-centered data
        svtkAMRUtilities::CopyFieldData(
          ...
          (k >= KMIN(realCellExtent)) && (k <= KMAX(realCellExtent)))
        {
          ...
          svtkIdType sourceCellIdx =
            svtkStructuredData::ComputeCellId(ghostedGrid->GetDimensions(), ijk, dataDescription);

          // Compute the target cell index w.r.t. the un-ghosted grid
          svtkIdType targetCellIdx =
            svtkStructuredData::ComputeCellId(strippedGrid->GetDimensions(), lijk, dataDescription);
...
//------------------------------------------------------------------------------
svtkUniformGrid* svtkAMRUtilities::StripGhostLayersFromGrid(svtkUniformGrid* grid, int ghost[6])
{
  ...
  svtkUniformGrid* myGrid = svtkUniformGrid::New();
  myGrid->Initialize();
  ...
  svtkAMRUtilities::CopyFieldsWithinRealExtent(copyExtent, grid, myGrid);
  return (myGrid);
}

//------------------------------------------------------------------------------
void svtkAMRUtilities::StripGhostLayers(
  svtkOverlappingAMR* ghostedAMRData, svtkOverlappingAMR* strippedAMRData)
{
  ...
  {
    svtkUniformGrid* grid = ghostedAMRData->GetDataSet(0, dataIdx);
    const svtkAMRBox& box = ghostedAMRData->GetAMRBox(0, dataIdx);
  ...
  for (; levelIdx < ghostedAMRData->GetNumberOfLevels(); ++levelIdx)
  {
    ...
    {
      svtkUniformGrid* grid = ghostedAMRData->GetDataSet(levelIdx, dataIdx);
      int r = ghostedAMRData->GetRefinementRatio(levelIdx);
      svtkAMRBox myBox = ghostedAMRData->GetAMRBox(levelIdx, dataIdx);
      svtkAMRBox strippedBox = myBox;
      ...
      if (grid != nullptr)
      {
        ...
        svtkUniformGrid* strippedGrid = svtkAMRUtilities::StripGhostLayersFromGrid(grid, ghost);

        assert(strippedBox ==
          svtkAMRBox(strippedGrid->GetOrigin(), strippedGrid->GetDimensions(),
...
//------------------------------------------------------------------------------
void svtkAMRUtilities::BlankCells(svtkOverlappingAMR* amr)
{
...
//------------------------------------------------------------------------------
void svtkAMRUtilities::BlankGridsAtLevel(svtkOverlappingAMR* amr, int levelIdx,
  std::vector<std::vector<unsigned int> >& children, const std::vector<int>& processMap)
{
  ...
  {
    const svtkAMRBox& box = amr->GetAMRBox(levelIdx, dataSetIdx);
    svtkUniformGrid* grid = amr->GetDataSet(levelIdx, dataSetIdx);
    if (grid == nullptr)
    {
      continue;
    }
        ...
        if (amr->GetAMRInfo()->GetCoarsenedAMRBox(levelIdx + 1, *iter, ibox))
        {
          ...
          for (int iz = loCorner[2]; iz <= hi[2]; iz++)
          {
            for (int iy = loCorner[1]; iy <= hi[1]; iy++)
            {
              ...
              {
                svtkIdType id =
                  svtkAMRBox::GetCellLinearIndex(box, ix, iy, iz, grid->GetDimensions());
                ghosts->SetValue(id, ghosts->GetValue(id) | svtkDataSetAttributes::REFINEDCELL);
              } // END for x
            }   // END for y
          }     // END for z
        }
