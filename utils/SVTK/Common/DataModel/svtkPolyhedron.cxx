...
svtkStandardNewMacro(svtkPolyhedron);

// Special typedef
typedef vector<svtkIdType> svtkIdVectorType;
class svtkPointIdMap : public map<svtkIdType, svtkIdType>
{
};
...
struct Edge : public std::pair<svtkIdType, svtkIdType>
{
public:
  Edge() = default;
  Edge(svtkIdType a, svtkIdType b)
    : std::pair<svtkIdType, svtkIdType>(a, b)
  {
  }
  Edge(svtkCell* edge)
    : std::pair<svtkIdType, svtkIdType>(edge->GetPointId(0), edge->GetPointId(1))
  {
  }
...
typedef unordered_map<Edge, set<svtkIdType>, hash_fn, equal_fn> EdgeFaceSetMap;

typedef unordered_multimap<svtkIdType, Edge> PointIndexEdgeMultiMap;
typedef unordered_map<Edge, svtkIdType, hash_fn, equal_fn> EdgePointIndexMap;

typedef unordered_set<Edge, hash_fn, equal_fn> EdgeSet;

typedef svtkIdVectorType Face;
typedef vector<Face> FaceVector;

//// Special class for iterating through polyhedron faces
////----------------------------------------------------------------------------
class svtkPolyhedronFaceIterator
{
public:
  ...
  svtkPolyhedronFaceIterator(svtkIdType numFaces, svtkIdType* t)
  {
  ...
  svtkIdType* operator++()
  {
    ...
    {
      this->CurrentPolygonSize = SVTK_ID_MAX;
    }
    return this->Current;
  }
};
...
svtkPolyhedron::svtkPolyhedron()
{
  ...
  this->GlobalFaces = svtkIdTypeArray::New();
  this->FaceLocations = svtkIdTypeArray::New();
  this->PointIdMap = new svtkPointIdMap;

  this->EdgesGenerated = 0;
  this->EdgeTable = svtkEdgeTable::New();
  this->Edges = svtkIdTypeArray::New();
  this->Edges->SetNumberOfComponents(2);
  this->EdgeFaces = svtkIdTypeArray::New();
...
//----------------------------------------------------------------------------
void svtkPolyhedron::ComputeBounds()
{
...
//----------------------------------------------------------------------------
void svtkPolyhedron::ConstructPolyData()
{
  ...
  // of the array. Other than that,it's a svtkCellArray. So we play games
  // with the pointers.
  this->GenerateFaces();
  ...
  const svtkIdType numCells = *this->Faces->GetPointer(0);
  const svtkIdType connSize = this->Faces->GetNumberOfValues() - numCells - 1;
...
svtkPolyData* svtkPolyhedron::GetPolyData()
{
  if (!this->PolyDataConstructed)
  {
...
//----------------------------------------------------------------------------
void svtkPolyhedron::ConstructLocator()
{
...
//----------------------------------------------------------------------------
void svtkPolyhedron::ComputeParametricCoordinate(const double x[3], double pc[3])
{
  this->ComputeBounds();
...
//----------------------------------------------------------------------------
void svtkPolyhedron::ComputePositionFromParametricCoordinate(const double pc[3], double x[3])
{
  this->ComputeBounds();
  double* bounds = this->Bounds;
...
void svtkPolyhedron::Initialize()
{
  ...
  svtkIdType i, id, numPointIds = this->PointIds->GetNumberOfIds();
...
//----------------------------------------------------------------------------
int svtkPolyhedron::GetNumberOfEdges()
{
...
svtkCell* svtkPolyhedron::GetEdge(int edgeId)
{
  // Make sure edges have been generated.
  if (!this->EdgesGenerated)
  {
  ...
  svtkIdType numEdges = this->Edges->GetNumberOfTuples();
...
//----------------------------------------------------------------------------
int svtkPolyhedron::GenerateEdges()
{
  if (this->EdgesGenerated)
  {
  ...
  svtkIdType* faces = this->GlobalFaces->GetPointer(0);
  svtkIdType nfaces = faces[0];
  svtkIdType* face = faces + 1;
  svtkIdType fid, i, edge[2], npts, edgeFaces[2], edgeId;
  edgeFaces[1] = -1;
...
//----------------------------------------------------------------------------
int svtkPolyhedron::GetNumberOfFaces()
{
...
//----------------------------------------------------------------------------
void svtkPolyhedron::GenerateFaces()
{
  if (this->FacesGenerated)
  {
    return;
  ...
  svtkIdType* gFaces = this->GlobalFaces->GetPointer(0);
  svtkIdType* faces = this->Faces->GetPointer(0);
  svtkIdType nfaces = gFaces[0];
  faces[0] = nfaces;
  svtkIdType* gFace = gFaces + 1;
  svtkIdType* face = faces + 1;
  svtkIdType fid, i, id, npts;
...
//----------------------------------------------------------------------------
svtkCell* svtkPolyhedron::GetFace(int faceId)
{
  ...
  svtkIdType i, p, loc = this->FaceLocations->GetValue(faceId);
  svtkIdType* face = this->GlobalFaces->GetPointer(loc);
...
void svtkPolyhedron::SetFaces(svtkIdType* faces)
{
  ...
  svtkIdType* face = faces + 1;
  svtkIdType faceLoc = 1;
  svtkIdType i, fid, npts;
...
svtkIdType* svtkPolyhedron::GetFaces()
{
...
//----------------------------------------------------------------------------
int svtkPolyhedron::IntersectWithLine(const double p1[3], const double p2[3], double tol,
  double& tMin, double xMin[3], double pc[3], int& subId)
{
  ...
  svtkIdType* face = this->Faces->GetPointer(0);
  svtkIdType nfaces = *face++;
  svtkIdType npts, i, fid, numHits = 0;
  double t = SVTK_FLOAT_MAX;
  double x[3];
...
#define SVTK_VOTE_THRESHOLD 3

//----------------------------------------------------------------------------
// Shoot random rays and count the number of intersections
int svtkPolyhedron::IsInside(const double x[3], double tolerance)
{
  ...
  svtkIdType* faceArray = this->Faces->GetPointer(0);
  svtkIdType nfaces = *faceArray++;
  if (nfaces > 25)
  {
...
bool svtkPolyhedron::IsConvex()
{
  double x[2][3], n[3], c[3], c0[3], c1[3], c0p[3], c1p[3], n0[3], n1[3];
  double np[3], tmp0, tmp1;
  svtkIdType i, w[2], edgeId, edgeFaces[2], loc, v, *face, r = 0;
  const double eps = FLT_EPSILON;

  std::vector<double> p(this->PointIds->GetNumberOfIds());
  svtkIdVectorType d(this->PointIds->GetNumberOfIds(), 0);
  ...
  while ((edgeId = this->EdgeTable->GetNextEdge(w[0], w[1])) >= 0)
  {
    ...
    svtkPolygon::ComputeCentroid(this->Points, face[0], face + 1, c0);
    svtkPolygon::ComputeNormal(this->Points, face[0], face + 1, n0);
    ...
    svtkPolygon::ComputeCentroid(this->Points, face[0], face + 1, c1);
    svtkPolygon::ComputeNormal(this->Points, face[0], face + 1, n1);
    ...
    if (svtkMath::Dot(n0, c0p) > 0. || svtkMath::Dot(n1, c1p) > 0.)
    {
      return false;
    }
    ...
    tmp0 = svtkMath::Dot(np, c0p);
    tmp1 = svtkMath::Dot(np, c1p);
...
//----------------------------------------------------------------------------
int svtkPolyhedron::CellBoundary(int svtkNotUsed(subId), const double pcoords[3], svtkIdList* pts)
{
  double x[3], n[3], o[3], v[3];
  double dist, minDist = SVTK_DOUBLE_MAX;
  svtkIdType numFacePts = -1;
  svtkIdType* facePts = nullptr;

  // compute coordinates
  this->ComputePositionFromParametricCoordinate(pcoords, x);

  svtkPolyhedronFaceIterator faceIter(this->GetNumberOfFaces(), this->Faces->GetPointer(1));
  while (faceIter.Id < faceIter.NumberOfPolygons)
  {
    if (faceIter.CurrentPolygonSize < 3)
    {
      svtkErrorMacro("Find a face with "
        << faceIter.CurrentPolygonSize
        << " vertices. Cannot return CellBoundary due to this degenerate case.");
      break;
    }

    svtkPolygon::ComputeNormal(this->Points, faceIter.CurrentPolygonSize, faceIter.Current, n);
    svtkMath::Normalize(n);
    this->Points->GetPoint(faceIter.Current[0], o);
    v[0] = x[0] - o[0];
    v[1] = x[1] - o[1];
    v[2] = x[2] - o[2];
    dist = fabs(svtkMath::Dot(v, n));
    if (dist < minDist)
    {
      minDist = dist;
  ...
  if (numFacePts > 0)
  {
    for (svtkIdType i = 0; i < numFacePts; i++)
    {
...
//----------------------------------------------------------------------------
int svtkPolyhedron::EvaluatePosition(const double x[3], double closestPoint[3],
  int& svtkNotUsed(subId), double pcoords[3], double& minDist2, double weights[])
{
  // compute parametric coordinates
...
//----------------------------------------------------------------------------
void svtkPolyhedron::EvaluateLocation(
...
//----------------------------------------------------------------------------
void svtkPolyhedron::Derivatives(
...
//----------------------------------------------------------------------------
double* svtkPolyhedron::GetParametricCoords()
{
  return nullptr;
}

//----------------------------------------------------------------------------
void svtkPolyhedron::InterpolateFunctions(const double x[3], double* sf)
{
  // construct polydata, the result is stored in this->PolyData,
  // the cell array is stored in this->Polys
  this->ConstructPolyData();
...
//----------------------------------------------------------------------------
void svtkPolyhedron::InterpolateDerivs(const double x[3], double* derivs)
{
  (void)x;
  (void)derivs;
}
...
//----------------------------------------------------------------------------
int svtkPolyhedron::Triangulate(int svtkNotUsed(index), svtkIdList* ptIds, svtkPoints* pts)
{
  ptIds->Reset();
  pts->Reset();
  ...
  svtkSmartPointer<svtkOrderedTriangulator> triangulator =
    svtkSmartPointer<svtkOrderedTriangulator>::New();
  ...
  svtkIdType* ids = ptIds->GetPointer(0);
...
bool IntersectWithContour(svtkCell* cell, svtkDataArray* pointScalars, svtkPointIdMap* pointIdMap,
  double value, function<bool(double, double)>& compare, bool& allTrue)
{
  ...
  {
    svtkIdType globalPid = cell->GetPointId(i);
    svtkIdType localPid = pointIdMap->find(globalPid)->second;
...
bool CheckWatertightNonManifoldPolyhedron(svtkPolyhedron* cell, EdgeSet& originalEdges)
{
  ...
  for (int i = 0; i < nFaces; ++i)
  {
    ...
    for (int j = 0; j < face->GetNumberOfEdges(); ++j)
    {
      ...
      if (at == directMap.end())
      {
        set<svtkIdType> facesOfEdge;
        facesOfEdge.insert(i);
        directMap.insert(make_pair(e, facesOfEdge));
      }
      else
      {
        set<svtkIdType>& facesOfEdge = at->second;
        facesOfEdge.insert(i);
      }
    }
  }
  ...
  {
    const set<svtkIdType>& facesOfEdge = entry.second;
...
int FindLowestIndex(svtkIdType n, svtkIdType* arr)
{
  int lowest(-1);
  svtkIdType min(SVTK_ID_MAX);
  for (int i = 0; i < n; ++i)
  {
    if (arr[i] < min)
    {
...
void FindLowestNeighbor(svtkIdType n, svtkIdType* arr, int idx, bool& mustReverse)
{
  idx += n; // add n to prevent negative remainders
  svtkIdType left = arr[(idx - 1) % n];
  svtkIdType right = arr[(idx + 1) % n];
  if (left < right)
  {
    mustReverse = true;
  }
...
int TriangulatePolygonAt(svtkCell* polygon, int offset, svtkIdList* triIds)
{
  triIds->Reset();
...
void CalculateAngles(const svtkIdType* tri, svtkPoints* phPoints, const svtkPointIdMap* pointIdMap,
  double& minAngle, double& maxAngle)
{
  svtkIdType idx0 = tri[0];
  svtkIdType idx1 = tri[1];
  svtkIdType idx2 = tri[2];
  ...
  for (int i = 0; i < 3; ++i)
  {
    ...
    // rounding errors can occur in the svtkVector3d::Dot function,
    // clamp to [-1, 1] (i.e. the input range for the acos function)
    dot = min(1.0, dot);
    dot = max(-1.0, dot);

    double angle = acos(dot) * 180.0 / svtkMath::Pi();
...
void TriangulatePolygon(svtkCell* polygon, FaceVector& faces, svtkIdList* triIds, svtkPoints* phPoints,
...
void TriangulateFace(svtkCell* face, FaceVector& faces, svtkIdList* triIds, svtkPoints* phPoints,
  svtkPointIdMap* pointIdMap)
{
  switch (face->GetCellType())
  {
    ...
    default:
    {
      svtkGenericWarningMacro(<< "Unable to triangulate face cell type " << face->GetCellType());
    }
  }
}
...
bool GetContourPoints(double value, svtkPolyhedron* cell,
  svtkPointIdMap* pointIdMap, // from global id to local cell id
  FaceEdgesVector& faceEdgesVector, EdgeFaceSetMap& edgeFaceMap, EdgeSet& originalEdges,
  vector<vector<svtkIdType> >& oririginalFaceTriFaceMap,
  PointIndexEdgeMultiMap& contourPointEdgeMultiMap, EdgePointIndexMap& edgeContourPointMap,
  svtkIncrementalPointLocator* locator, svtkDataArray* pointScalars, svtkPointData* inPd,
  svtkPointData* outPd)
{
  ...
  for (svtkIdType i = 0; i < nFaces; ++i)
  {
    ...
    vector<svtkIdType> trisOfFace;
    for (size_t j = nTris; j < faces.size(); ++j)
    {
      trisOfFace.push_back((svtkIdType)j);
    }
  ...
  {
    svtkGenericWarningMacro(<< "A cell with a non-manifold triangulation has been encountered. This "
                              "cell cannot be contoured.");
    return false;
  }

  svtkPoints* cellPoints = cell->GetPoints();
  ...
  for (const auto& entry : edgeFaceMap)
  {
    ...
    {
      svtkGenericWarningMacro(<< "Could not find global id " << edge.first << " or " << edge.second);
      continue;
    }

    svtkIdType id0 = at0->second;
    svtkIdType id1 = at1->second;
...
int CreateContours(EdgeFaceSetMap& edgeFaceMap, FaceEdgesVector& faceEdgesVector,
  ...
  function<void(svtkIdList*)> contourCallback)
{
  ...
  svtkNew<svtkIdList> poly;
  EdgeSet visited;
  while (!availableContourEdges.empty())
  {
    Edge start = *availableContourEdges.begin();
    Edge at(start);
    svtkIdType lastFace(-1);

    do
    {
      svtkIdType cp = edgeContourPointMap.find(at)->second;
      ...
      const set<svtkIdType>& facesOfEdge = edgeFaceMap[at];

      svtkIdType face(lastFace);
      for (const svtkIdType& faceOfEdge : facesOfEdge)
      {
      ...
      if (face == lastFace)
      {
        svtkGenericWarningMacro(<< "Face navigation failed in polyhedral contouring");
        return EXIT_FAILURE;
      }
...
void svtkPolyhedron::Contour(double value, svtkDataArray* pointScalars,
  ...
  svtkCellData* outCd)
{
  ...
  function<void(svtkIdList*)> cb = [=](svtkIdList* poly) {
    if (!poly)
      return;
...
void PolygonAsEdges(vector<svtkIdType>& polygon, vector<Edge>& edges,
...
void EdgesToPolygon(vector<Edge>& edges, vector<svtkIdType>& polygon)
{
...
void EdgesToPolygons(vector<vector<Edge> >& edgePolygons, vector<vector<svtkIdType> >& polygons)
{
  for (auto it = edgePolygons.begin(); it != edgePolygons.end(); ++it)
  {
    vector<Edge>& edgePolygon = *it;
    vector<svtkIdType> polygon;
    EdgesToPolygon(edgePolygon, polygon);
    polygons.push_back(polygon);
  }
}

void PruneContourPoints(vector<vector<svtkIdType> >& merged, EdgeSet& originalEdges,
  PointIndexEdgeMultiMap& contourPointEdgeMultiMap)
{
  for (auto it = merged.begin(); it != merged.end(); ++it)
  {
    vector<svtkIdType>& polygon = *it;
...
void MergeTriFacePolygons(vector<vector<svtkIdType> >& toMerge, vector<vector<svtkIdType> >& merged,
  EdgeSet& originalEdges, PointIndexEdgeMultiMap& contourPointEdgeMultiMap)
{
  // this is a five-step procedure:

  // 1) convert from vector<svtkIdType> to vector<Edge>
  // 2) remove duplicate edges;
  // 3) order the remaining edges head-to-tail;
  // 4) convert back from vector<Edge> to vector<svtkIdType>
  // 5) prune contour points that are not on original edges.

  // step 1: convert from vector<svtkIdType> to vector<Edge>
  ...
  // step 4: convert back to vector<svtkIdType> polygons
...
void MergeTriFacePolygons(svtkPolyhedron* cell,
  unordered_map<svtkIdType, vector<svtkIdType> >& triFacePolygonMap,
  vector<vector<svtkIdType> >& oririginalFaceTriFaceMap,
  PointIndexEdgeMultiMap& contourPointEdgeMultiMap, EdgeSet& originalEdges,
  vector<vector<svtkIdType> >& polygons)
{
  ...
  for (int i = 0; i < nFaces; ++i)
  {
    const vector<svtkIdType>& triFacesOfOriginalFace = oririginalFaceTriFaceMap[i];

    vector<vector<svtkIdType> > facePolygons;
    for (auto it = triFacesOfOriginalFace.begin(); it != triFacesOfOriginalFace.end(); ++it)
    {
      svtkIdType triFace = *it;
    ...
    {
      vector<vector<svtkIdType> > mergedPolygons;
...
void svtkPolyhedron::Clip(double value, svtkDataArray* pointScalars,
  ...
  svtkPointData* outPd, svtkCellData* inCd, svtkIdType cellId, svtkCellData* outCd, int insideOut)
{
  ...
  if (!intersect && all)
  {
    ...
    if (nFaces > 0)
    {
      svtkIdType newCellId = connectivity->InsertNextCell(faceStream);
      outCd->CopyData(inCd, cellId, newCellId);
    }
    return;
  }
  ...
  svtkPoints* cellPoints = this->GetPoints();
  ...
  for (size_t i = 0; i < faceEdgesVector.size(); ++i)
  {
    ...
    for (auto edgeIt = edges.begin(); edgeIt != edges.end(); ++edgeIt)
    {
      ...
      svtkIdType v0 = edge.first;
      auto localIdIt = this->PointIdMap->find(v0);
      if (localIdIt == this->PointIdMap->end())
      {
        svtkGenericWarningMacro(<< "Could not find global id " << v0);
        continue;
      }
      svtkIdType localId = localIdIt->second;
    ...
    if (!polygon.empty())
    {
      triFacePolygonMap.insert(make_pair(static_cast<svtkIdType>(i), polygon));
    }
  }

  std::vector<std::vector<svtkIdType> > polygons;
  ...
  // variables
  std::vector<std::vector<svtkIdType> >* pPolygons = &polygons;

  function<void(svtkIdList*)> cb = [=](svtkIdList* poly) {
    svtkIdType nIds = poly->GetNumberOfIds();
    vector<svtkIdType> polygon;
  ...
  while (!polygons.empty())
  {
    ...
    unordered_set<svtkIdType> polyhedralIdSet;
    ...
    std::vector<std::vector<svtkIdType> > polyhedralFaceList;
    ...
    while (add)
    {
      ...
      while (it != polygons.end())
      {
        ...
        const vector<svtkIdType>& nextPolygon = *it;
    ...
    if (polyhedralFaceList.size())
    {
      ...
      svtkNew<svtkIdList> polyhedron;
      // first entry: # of faces:
      polyhedron->InsertNextId(static_cast<svtkIdType>(polyhedralFaceList.size()));
      for (const auto& polyFace : polyhedralFaceList)
      {
        // each face entry starts with # points in that face
        polyhedron->InsertNextId(static_cast<svtkIdType>(polyFace.size()));
      ...
      svtkIdType newCellId = connectivity->InsertNextCell(polyhedron);
...
//----------------------------------------------------------------------------
void svtkPolyhedron::PrintSelf(ostream& os, svtkIndent indent)
{
