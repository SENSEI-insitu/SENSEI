/*=========================================================================
  ...
  Module:    svtkGenericCellTessellator.cxx
...
svtkCxxSetObjectMacro(svtkGenericCellTessellator, ErrorMetrics, svtkCollection);

//-----------------------------------------------------------------------------
// Create the tessellator helper with a default of 0.25 for threshold
svtkGenericCellTessellator::svtkGenericCellTessellator()
{
  this->DataSet = nullptr;
  this->ErrorMetrics = svtkCollection::New();
...
//-----------------------------------------------------------------------------
void svtkGenericCellTessellator::PrintSelf(ostream& os, svtkIndent indent)
{
...
int svtkGenericCellTessellator::RequiresEdgeSubdivision(
  double* leftPoint, double* midPoint, double* rightPoint, double alpha)
{
  ...
  svtkGenericSubdivisionErrorMetric* e =
    static_cast<svtkGenericSubdivisionErrorMetric*>(this->ErrorMetrics->GetNextItemAsObject());
  ...
  while (!result && e != nullptr)
  {
    ...
    e = static_cast<svtkGenericSubdivisionErrorMetric*>(this->ErrorMetrics->GetNextItemAsObject());
  }
...
// \pre valid_size: sizeof(leftPoint)=sizeof(midPoint)=sizeof(rightPoint)
//          =GetAttributeCollection()->GetNumberOfPointCenteredComponents()+6
void svtkGenericCellTessellator::UpdateMaxError(
  double* leftPoint, double* midPoint, double* rightPoint, double alpha)
{
  assert("pre: leftPoint_exists" && leftPoint != nullptr);
  assert("pre: midPoint_exists" && midPoint != nullptr);
...
void svtkGenericCellTessellator::InitErrorMetrics(svtkGenericDataSet* ds)
{
  this->Initialize(ds);
  this->ErrorMetrics->InitTraversal();
  svtkGenericSubdivisionErrorMetric* e =
...
void svtkGenericCellTessellator::ResetMaxErrors()
{
...
void svtkGenericCellTessellator::GetMaxErrors(double* errors)
{
...
void svtkGenericCellTessellator::SetGenericCell(svtkGenericAdaptorCell* cell)
{
