...
svtkStandardNewMacro(svtkTriangleStrip);

//----------------------------------------------------------------------------
svtkTriangleStrip::svtkTriangleStrip()
{
...
//----------------------------------------------------------------------------
int svtkTriangleStrip::EvaluatePosition(const double x[3], double closestPoint[3], int& subId,
  double pcoords[3], double& minDist2, double weights[])
{
  ...
  return_status = 0;
  for (minDist2 = SVTK_DOUBLE_MAX, i = 0; i < this->Points->GetNumberOfPoints() - 2; i++)
  {
    weights[i] = 0.0;
...
//----------------------------------------------------------------------------
void svtkTriangleStrip::EvaluateLocation(
...
//----------------------------------------------------------------------------
int svtkTriangleStrip::CellBoundary(int subId, const double pcoords[3], svtkIdList* pts)
{
  static const int idx[2][3] = { { 0, 1, 2 }, { 1, 0, 2 } };
  int order;
...
//----------------------------------------------------------------------------
void svtkTriangleStrip::Contour(double value, svtkDataArray* cellScalars,
  ...
  svtkCellData* outCd)
{
  ...
  svtkDataArray* triScalars = cellScalars->NewInstance();
...
//----------------------------------------------------------------------------
svtkCell* svtkTriangleStrip::GetEdge(int edgeId)
{
  int id1, id2;
...
//
int svtkTriangleStrip::IntersectWithLine(const double p1[3], const double p2[3], double tol,
...
//----------------------------------------------------------------------------
int svtkTriangleStrip::Triangulate(int svtkNotUsed(index), svtkIdList* ptIds, svtkPoints* pts)
{
...
//----------------------------------------------------------------------------
void svtkTriangleStrip::Derivatives(
...
void svtkTriangleStrip::DecomposeStrip(int npts, const svtkIdType* pts, svtkCellArray* polys)
{
  int p1, p2, p3, i;
...
//----------------------------------------------------------------------------
void svtkTriangleStrip::Clip(double value, svtkDataArray* cellScalars,
  svtkIncrementalPointLocator* locator, svtkCellArray* tris, svtkPointData* inPd, svtkPointData* outPd,
...
int svtkTriangleStrip::GetParametricCenter(double pcoords[3])
{
...
//----------------------------------------------------------------------------
void svtkTriangleStrip::PrintSelf(ostream& os, svtkIndent indent)
{
