/*=========================================================================
  ...
  Module:    svtkPiecewiseFunction.cxx
...
svtkStandardNewMacro(svtkPiecewiseFunction);

// The Node structure
class svtkPiecewiseFunctionNode
{
public:
  double X;
  double Y;
...
class svtkPiecewiseFunctionCompareNodes
{
public:
  bool operator()(const svtkPiecewiseFunctionNode* node1, const svtkPiecewiseFunctionNode* node2)
  {
...
class svtkPiecewiseFunctionFindNodeEqual
{
public:
  double X;
  bool operator()(const svtkPiecewiseFunctionNode* node) { return node->X == this->X; }
};

// A find method for finding nodes inside a specified range
class svtkPiecewiseFunctionFindNodeInRange
{
public:
  double X1;
  double X2;
  bool operator()(const svtkPiecewiseFunctionNode* node)
  {
...
class svtkPiecewiseFunctionFindNodeOutOfRange
{
public:
  double X1;
  double X2;
...
class svtkPiecewiseFunctionInternals
{
public:
  std::vector<svtkPiecewiseFunctionNode*> Nodes;
  svtkPiecewiseFunctionCompareNodes CompareNodes;
  svtkPiecewiseFunctionFindNodeEqual FindNodeEqual;
  svtkPiecewiseFunctionFindNodeInRange FindNodeInRange;
  svtkPiecewiseFunctionFindNodeOutOfRange FindNodeOutOfRange;
};

// Construct a new svtkPiecewiseFunction with default values
svtkPiecewiseFunction::svtkPiecewiseFunction()
{
  this->Clamping = 1;
  this->Range[0] = 0;
  this->Range[1] = 0;
  ...
  this->Internal = new svtkPiecewiseFunctionInternals;
}

// Destruct a svtkPiecewiseFunction
svtkPiecewiseFunction::~svtkPiecewiseFunction()
{
...
void svtkPiecewiseFunction::DeepCopy(svtkDataObject* o)
{
  svtkPiecewiseFunction* f = svtkPiecewiseFunction::SafeDownCast(o);
...
void svtkPiecewiseFunction::ShallowCopy(svtkDataObject* o)
{
  svtkPiecewiseFunction* f = svtkPiecewiseFunction::SafeDownCast(o);
  ...
  // Do the superclass
  this->svtkDataObject::ShallowCopy(o);
}

// This is a legacy method that is no longer needed
void svtkPiecewiseFunction::Initialize()
{
  this->RemoveAllPoints();
}

// Return the number of points which specify this function
int svtkPiecewiseFunction::GetSize()
{
...
//
const char* svtkPiecewiseFunction::GetType()
{
  unsigned int i;
  double value;
...
double* svtkPiecewiseFunction::GetDataPointer()
{
...
double svtkPiecewiseFunction::GetFirstNonZeroValue()
{
  ...
  else // A point was found with a non-zero value
  {
    ...
    // value is clamping is off, otherwise SVTK_DOUBLE_MIN if
    // clamping is on.
    {
      if (this->Clamping)
      {
...
int svtkPiecewiseFunction::GetNodeValue(int index, double val[4])
{
  int size = static_cast<int>(this->Internal->Nodes.size());
...
// For a specified index value, get the node parameters
int svtkPiecewiseFunction::SetNodeValue(int index, double val[4])
{
  int size = static_cast<int>(this->Internal->Nodes.size());
...
// sharpness = 0.0
int svtkPiecewiseFunction::AddPoint(double x, double y)
{
  return this->AddPoint(x, y, 0.5, 0.0);
}

// Adds a point to the function and returns the array index of the point.
int svtkPiecewiseFunction::AddPoint(double x, double y, double midpoint, double sharpness)
{
  // Error check
  ...
  // Create the new node
  svtkPiecewiseFunctionNode* node = new svtkPiecewiseFunctionNode;
  node->X = x;
  node->Y = y;
...
// the Range
void svtkPiecewiseFunction::SortAndUpdateRange()
{
  std::sort(
...
//----------------------------------------------------------------------------
bool svtkPiecewiseFunction::UpdateRange()
{
  double oldRange[2];
...
// remains the same.
int svtkPiecewiseFunction::RemovePoint(double x)
{
  ...
  std::vector<svtkPiecewiseFunctionNode*>::iterator iter = std::find_if(
...
void svtkPiecewiseFunction::RemoveAllPoints()
{
  for (unsigned int i = 0; i < this->Internal->Nodes.size(); i++)
  {
    delete this->Internal->Nodes[i];
...
void svtkPiecewiseFunction::AddSegment(double x1, double y1, double x2, double y2)
{
  int done;
...
double svtkPiecewiseFunction::GetValue(double x)
{
  double table[1];
...
int svtkPiecewiseFunction::AdjustRange(double range[2])
{
  if (!range)
  {
    return 0;
  ...
  while (!done)
  {
    ...
    std::vector<svtkPiecewiseFunctionNode*>::iterator iter =
...
//--------------------------------------------------------------------------
int svtkPiecewiseFunction::EstimateMinNumberOfSamples(double const& x1, double const& x2)
{
...
//----------------------------------------------------------------------------
double svtkPiecewiseFunction::FindMinimumXDistance()
{
  std::vector<svtkPiecewiseFunctionNode*> const& nodes = this->Internal->Nodes;
...
void svtkPiecewiseFunction::GetTable(
...
void svtkPiecewiseFunction::BuildFunctionFromTable(
...
void svtkPiecewiseFunction::FillFromDataPointer(int nb, double* ptr)
{
  if (nb <= 0 || !ptr)
  {
    return;
...
//----------------------------------------------------------------------------
svtkPiecewiseFunction* svtkPiecewiseFunction::GetData(svtkInformation* info)
{
  return info ? svtkPiecewiseFunction::SafeDownCast(info->Get(DATA_OBJECT())) : nullptr;
}

//----------------------------------------------------------------------------
svtkPiecewiseFunction* svtkPiecewiseFunction::GetData(svtkInformationVector* v, int i)
{
  return svtkPiecewiseFunction::GetData(v->GetInformationObject(i));
}

// Print method for tkPiecewiseFunction
void svtkPiecewiseFunction::PrintSelf(ostream& os, svtkIndent indent)
{
