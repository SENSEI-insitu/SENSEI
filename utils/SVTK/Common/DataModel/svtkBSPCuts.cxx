...
svtkStandardNewMacro(svtkBSPCuts);

//----------------------------------------------------------------------------
svtkBSPCuts::svtkBSPCuts()
{
  this->Top = nullptr;
  this->NumberOfCuts = 0;
  this->Dim = nullptr;
...
svtkBSPCuts::~svtkBSPCuts()
{
  if (this->Top)
  {
    svtkBSPCuts::DeleteAllDescendants(this->Top);
    this->Top->Delete();
  }
...
//----------------------------------------------------------------------------
void svtkBSPCuts::AllocateArrays(int nNodes)
{
...
//----------------------------------------------------------------------------
void svtkBSPCuts::DeleteAllDescendants(svtkKdNode* nd)
{
  ...
  {
    svtkBSPCuts::DeleteAllDescendants(left);
  }

  if (right && right->GetLeft())
  {
    svtkBSPCuts::DeleteAllDescendants(right);
  }
...
//----------------------------------------------------------------------------
void svtkBSPCuts::ShallowCopy(svtkDataObject* src)
{
  this->Superclass::ShallowCopy(src);
  svtkBSPCuts* srcCuts = svtkBSPCuts::SafeDownCast(src);
...
//----------------------------------------------------------------------------
void svtkBSPCuts::DeepCopy(svtkDataObject* src)
{
...
//----------------------------------------------------------------------------
void svtkBSPCuts::CreateCuts(svtkKdNode* kd)
{
  // Given a tree of svtkKdNodes, create the arrays that describe this
  ...
  int nNodes = svtkBSPCuts::CountNodes(kd);
  ...
  // keep a copy of svtkKdNode tree
...
//----------------------------------------------------------------------------
int svtkBSPCuts::CountNodes(svtkKdNode* kd)
{
  int leftCount = 0;
  int rightCount = 0;
  ...
  if (kd->GetLeft())
  {
    leftCount = svtkBSPCuts::CountNodes(kd->GetLeft());
    rightCount = svtkBSPCuts::CountNodes(kd->GetRight());
  }

  return leftCount + rightCount + 1;
}
//----------------------------------------------------------------------------
int svtkBSPCuts::WriteArray(svtkKdNode* kd, int loc)
{
  int nextloc = loc + 1;
  ...
  if (kd->GetLeft())
  {
    ...
    svtkKdNode* left = kd->GetLeft();
    svtkKdNode* right = kd->GetRight();
...
//----------------------------------------------------------------------------
void svtkBSPCuts::CreateCuts(double* bnds, int ncuts, int* dim, double* coord, int* lower,
  int* upper, double* lowerDataCoord, double* upperDataCoord, int* npoints)
{
  // Keep a copy of these arrays

  svtkBSPCuts::ResetArrays();
  svtkBSPCuts::AllocateArrays(ncuts);
  ...
  svtkBSPCuts::SetMinMaxId(this->Top);
}
//----------------------------------------------------------------------------
void svtkBSPCuts::BuildTree(svtkKdNode* kd, int idx)
{
...
//----------------------------------------------------------------------------
void svtkBSPCuts::SetMinMaxId(svtkKdNode* kd)
{
  if (kd->GetLeft())
  {
    svtkBSPCuts::SetMinMaxId(kd->GetLeft());
    svtkBSPCuts::SetMinMaxId(kd->GetRight());
  }
  else
  {
...
//----------------------------------------------------------------------------
int svtkBSPCuts::GetArrays(int len, int* dim, double* coord, int* lower, int* upper,
...
//-----------------------------------------------------------------------------
int svtkBSPCuts::Equals(svtkBSPCuts* other, double tolerance)
{
...
void svtkBSPCuts::PrintTree()
{
  ...
  svtkBSPCuts::_PrintTree(this->Top, 0);
}
//----------------------------------------------------------------------------
void svtkBSPCuts::_PrintTree(svtkKdNode* kd, int depth)
{
  kd->PrintNode(depth);

  if (kd->GetLeft())
  {
    svtkBSPCuts::_PrintTree(kd->GetLeft(), depth + 1);
    svtkBSPCuts::_PrintTree(kd->GetRight(), depth + 1);
  }
}

//----------------------------------------------------------------------------
svtkBSPCuts* svtkBSPCuts::GetData(svtkInformation* info)
{
  return svtkBSPCuts::SafeDownCast(svtkDataObject::GetData(info));
}
//----------------------------------------------------------------------------
svtkBSPCuts* svtkBSPCuts::GetData(svtkInformationVector* v, int i)
{
  return svtkBSPCuts::SafeDownCast(svtkDataObject::GetData(v, i));
}

//----------------------------------------------------------------------------
void svtkBSPCuts::PrintSelf(ostream& os, svtkIndent indent)
{
