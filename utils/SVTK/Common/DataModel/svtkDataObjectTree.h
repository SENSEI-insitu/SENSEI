/*=========================================================================

  Program:   Visualization Toolkit
  Module:    svtkDataObjectTree.h

  Copyright (c) Ken Martin, Will Schroeder, Bill Lorensen
  All rights reserved.
  See Copyright.txt or http://www.kitware.com/Copyright.htm for details.

     This software is distributed WITHOUT ANY WARRANTY; without even
     the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR
     PURPOSE.  See the above copyright notice for more information.

=========================================================================*/
/**
 * @class   svtkDataObjectTree
 * @brief   provides implementation for most abstract
 * methods in the superclass svtkCompositeDataSet
 *
 * svtkDataObjectTree is represents a collection
 * of datasets (including other composite datasets). It
 * provides an interface to access the datasets through iterators.
 * svtkDataObjectTree provides methods that are used by subclasses to store the
 * datasets.
 * svtkDataObjectTree provides the datastructure for a full tree
 * representation. Subclasses provide the semantics for it and control how
 * this tree is built.
 *
 * @sa
 * svtkDataObjectTreeIterator
 */

#ifndef svtkDataObjectTree_h
#define svtkDataObjectTree_h

#include "svtkCommonDataModelModule.h" // For export macro
#include "svtkCompositeDataSet.h"

class svtkCompositeDataIterator;
class svtkDataObjectTreeIterator;
class svtkDataObjectTreeInternals;
class svtkInformation;
class svtkInformationStringKey;
class svtkDataObject;

class SVTKCOMMONDATAMODEL_EXPORT svtkDataObjectTree : public svtkCompositeDataSet
{
public:
  svtkTypeMacro(svtkDataObjectTree, svtkCompositeDataSet);
  void PrintSelf(ostream& os, svtkIndent indent) override;

  /**
   * Return a new iterator (the iterator has to be deleted by user).
   */
  SVTK_NEWINSTANCE virtual svtkDataObjectTreeIterator* NewTreeIterator();

  /**
   * Return a new iterator (the iterator has to be deleted by user).

   * Use NewTreeIterator when you have a pointer to a svtkDataObjectTree
   * and NewIterator when you have a pointer to a svtkCompositeDataSet;
   * NewIterator is inherited and calls NewTreeIterator internally.
   */
  SVTK_NEWINSTANCE svtkCompositeDataIterator* NewIterator() override;

  /**
   * Copies the tree structure from the input. All pointers to non-composite
   * data objects are initialized to nullptr. This also shallow copies the meta data
   * associated with all the nodes.
   */
  void CopyStructure(svtkCompositeDataSet* input) override;

  /**
   * Sets the data set at the location pointed by the iterator.
   * The iterator does not need to be iterating over this dataset itself. It can
   * be any composite datasite with similar structure (achieved by using
   * CopyStructure).
   */
  void SetDataSet(svtkCompositeDataIterator* iter, svtkDataObject* dataObj) override;

  /**
   * Sets the data at the location provided by a svtkDataObjectTreeIterator
   */
  void SetDataSetFrom(svtkDataObjectTreeIterator* iter, svtkDataObject* dataObj);

  /**
   * Returns the dataset located at the positiong pointed by the iterator.
   * The iterator does not need to be iterating over this dataset itself. It can
   * be an iterator for composite dataset with similar structure (achieved by
   * using CopyStructure).
   */
  svtkDataObject* GetDataSet(svtkCompositeDataIterator* iter) override;

  /**
   * Returns the meta-data associated with the position pointed by the iterator.
   * This will create a new svtkInformation object if none already exists. Use
   * HasMetaData to avoid creating the svtkInformation object unnecessarily.
   * The iterator does not need to be iterating over this dataset itself. It can
   * be an iterator for composite dataset with similar structure (achieved by
   * using CopyStructure).
   */
  virtual svtkInformation* GetMetaData(svtkCompositeDataIterator* iter);

  /**
   * Returns if any meta-data associated with the position pointed by the iterator.
   * The iterator does not need to be iterating over this dataset itself. It can
   * be an iterator for composite dataset with similar structure (achieved by
   * using CopyStructure).
   */
  virtual int HasMetaData(svtkCompositeDataIterator* iter);

  /**
   * Return the actual size of the data in kibibytes (1024 bytes). This number
   * is valid only after the pipeline has updated.
   */
  unsigned long GetActualMemorySize() override;

  /**
   * Restore data object to initial state,
   */
  void Initialize() override;

  //@{
  /**
   * Shallow and Deep copy.
   */
  void ShallowCopy(svtkDataObject* src) override;
  void DeepCopy(svtkDataObject* src) override;
  //@}

  /**
   * Returns the total number of points of all blocks. This will
   * iterate over all blocks and call GetNumberOfPoints() so it
   * might be expansive.
   */
  svtkIdType GetNumberOfPoints() override;

  /**
   * Returns the total number of cells of all blocks. This will
   * iterate over all blocks and call GetNumberOfPoints() so it
   * might be expensive.
   */
  svtkIdType GetNumberOfCells() override;

  //@{
  /**
   * Retrieve an instance of this class from an information object.
   */
  static svtkDataObjectTree* GetData(svtkInformation* info);
  static svtkDataObjectTree* GetData(svtkInformationVector* v, int i = 0);
  //@}

protected:
  svtkDataObjectTree();
  ~svtkDataObjectTree() override;

  /**
   * Set the number of children.
   */
  void SetNumberOfChildren(unsigned int num);

  /**
   * Get the number of children.
   */
  unsigned int GetNumberOfChildren();

  /**
   * Set child dataset at a given index. The number of children is adjusted to
   * to be greater than the index specified.
   */
  void SetChild(unsigned int index, svtkDataObject*);

  /**
   * Remove the child at a given index.
   */
  void RemoveChild(unsigned int index);

  /**
   * Returns a child dataset at a given index.
   */
  svtkDataObject* GetChild(unsigned int num);

  /**
   * Returns the meta-data at a given index. If the index is valid, however, no
   * information object is set, then a new one will created and returned.
   * To avoid unnecessary creation, use HasMetaData().
   */
  svtkInformation* GetChildMetaData(unsigned int index);

  /**
   * Sets the meta-data at a given index.
   */
  void SetChildMetaData(unsigned int index, svtkInformation* info);

  /**
   * Returns if meta-data information is available for the given child index.
   * Returns 1 is present, 0 otherwise.
   */
  int HasChildMetaData(unsigned int index);

  // The internal datastructure. Subclasses need not access this directly.
  svtkDataObjectTreeInternals* Internals;

  friend class svtkDataObjectTreeIterator;

private:
  svtkDataObjectTree(const svtkDataObjectTree&) = delete;
  void operator=(const svtkDataObjectTree&) = delete;
};

#endif
