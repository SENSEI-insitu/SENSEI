...
svtkStandardNewMacro(svtkCellLocator);

#define SVTK_CELL_OUTSIDE 0
#define SVTK_CELL_INSIDE 1

typedef svtkIdList* svtkIdListPtr;

//----------------------------------------------------------------------------
class svtkNeighborCells
{
public:
  svtkNeighborCells(const int sz, const int ext = 1000)
  {
    this->P = svtkIntArray::New();
    this->P->Allocate(3 * sz, 3 * ext);
  }
  ~svtkNeighborCells() { this->P->Delete(); }
...
inline int svtkNeighborCells::InsertNextPoint(int* x)
{
...
// 25 cells per bucket.
svtkCellLocator::svtkCellLocator()
{
  this->MaxLevel = 8;
  this->Level = 8;
  ...
  this->Buckets = new svtkNeighborCells(10, 10);
  ...
  this->OctantBounds[0] = this->OctantBounds[2] = this->OctantBounds[4] = SVTK_DOUBLE_MAX;
  this->OctantBounds[1] = this->OctantBounds[3] = this->OctantBounds[5] = SVTK_DOUBLE_MIN;
}
...
//----------------------------------------------------------------------------
void svtkCellLocator::FreeSearchStructure()
{
  svtkIdList* cellIds;
  int i;
  ...
  if (this->Tree)
  {
    ...
    {
      cellIds = this->Tree[i];
      if (cellIds == reinterpret_cast<void*>(SVTK_CELL_INSIDE))
      {
        cellIds = nullptr;
      }
      if (cellIds)
...
int svtkCellLocator::GenerateIndex(int offset, int numDivs, int i, int j, int k, svtkIdType& idx)
{
...
//----------------------------------------------------------------------------
void svtkCellLocator::ComputeOctantBounds(int i, int j, int k)
{
...
// the same instance of svtkCellLocator). This is the due to the use of the
...
// alternative implementation, see svtkStaticCellLocator which is thread safe.
//
int svtkCellLocator::IntersectWithLine(const double a0[3], const double a1[3], double tol, double& t,
  double x[3], double pcoords[3], int& subId, svtkIdType& cellId, svtkGenericCell* cell)
{
  ...
  if (svtkBox::IntersectBox(bounds2, origin, direction2, hitPosition, result))
  {
      ...
      (pos[2] <= this->NumberOfDivisions) && (currDist < stopDist))
    {
      if (this->Tree[idx])
      {
        ...
        for (tMax = SVTK_DOUBLE_MAX, cellId = 0; cellId < this->Tree[idx]->GetNumberOfIds();
             cellId++)
        {
          ...
          if (this->CellHasBeenVisited[cId] != this->QueryNumber)
          {
            ...
            {
              hitCellBounds = svtkBox::IntersectBox(
            ...
            else
            {
              ...
              hitCellBounds =
                svtkBox::IntersectBox(cellBounds, a0, direction1, hitCellBoundsPosition, result);
            }
...
void svtkCellLocator::FindClosestPoint(const double x[3], double closestPoint[3],
  svtkGenericCell* cell, svtkIdType& cellId, int& subId, double& dist2)
{
  int i;
  svtkIdType j;
  int* nei;
  svtkIdType closestCell = -1;
  ...
  {
    // empty tree, most likely there are no cells in the input data set
    svtkErrorMacro("svtkCellLocator::FindClosestPoint failed: no cells in the input data set");
    return;
  }
  ...
  // init
  dist2 = -1.0;
  refinedRadius2 = SVTK_DOUBLE_MAX;
  ...
  //
  for (closestCell = (-1), minDist2 = SVTK_DOUBLE_MAX, level = 0;
...
//----------------------------------------------------------------------------
svtkIdType svtkCellLocator::FindClosestPointWithinRadius(double x[3], double radius,
...
//
void svtkCellLocator::GetBucketNeighbors(int ijk[3], int ndivs, int level)
{
...
// in the bucket list.
//
void svtkCellLocator::GetOverlappingBuckets(
  const double x[3], int svtkNotUsed(ijk)[3], double dist, int prevMinLevel[3], int prevMaxLevel[3])
{
...
int svtkCellLocator::GetNumberOfBuckets()
{
  if (this->Tree)
  {
  ...
  {
    svtkWarningMacro(<< "Attempting to access Tree before Locator has been built");
    return 0;
  }
}
...
svtkIdList* svtkCellLocator::GetCells(int octantId)
{
  // handle parents ?
...
//---------------------------------------------------------------------------
void svtkCellLocator::BuildLocator()
{
...
//---------------------------------------------------------------------------
void svtkCellLocator::BuildLocatorIfNeeded()
{
  if (this->LazyEvaluation)
  {
...
//---------------------------------------------------------------------------
void svtkCellLocator::ForceBuildLocator()
{
  //
...
void svtkCellLocator::BuildLocatorInternal()
{
  ...
  svtkDebugMacro(<< "Subdividing octree...");

  if (!this->DataSet || (numCells = this->DataSet->GetNumberOfCells()) < 1)
  {
    svtkErrorMacro(<< "No cells to subdivide");
    return;
  }
  ...
  this->Tree = new svtkIdListPtr[numOctants];
  memset(this->Tree, 0, numOctants * sizeof(svtkIdListPtr));
    ...
    for (k = ijkMin[2]; k <= ijkMax[2]; k++)
    {
      for (j = ijkMin[1]; j <= ijkMax[1]; j++)
      {
        for (i = ijkMin[0]; i <= ijkMax[0]; i++)
        {
          ...
          this->MarkParents(reinterpret_cast<void*>(SVTK_CELL_INSIDE), i, j, k, ndivs, this->Level);
          octant = this->Tree[idx];
          if (!octant)
          {
            octant = svtkIdList::New();
...
//----------------------------------------------------------------------------
void svtkCellLocator::MarkParents(void* a, int i, int j, int k, int ndivs, int level)
{
  int offset, prod, ii;
  svtkIdType parentIdx;
  ...
  while (level > 0)
  {
    ...
    this->Tree[parentIdx] = static_cast<svtkIdList*>(a);
...
//----------------------------------------------------------------------------
void svtkCellLocator::GenerateRepresentation(int level, svtkPolyData* pd)
{
  svtkPoints* pts;
  svtkCellArray* polys;
  int l, i, j, k, ii, boundary[3];
  svtkIdType idx = 0;
  svtkIdList *inside, *Inside[3] = { nullptr, nullptr, nullptr };
  int numDivs = 1;
...
//----------------------------------------------------------------------------
void svtkCellLocator::GenerateFace(
  int face, int numDivs, int i, int j, int k, svtkPoints* pts, svtkCellArray* polys)
{
  int ii;
  svtkIdType ids[4];
  double origin[3], x[3];
...
//----------------------------------------------------------------------------
void svtkCellLocator::ClearCellHasBeenVisited()
{
...
//----------------------------------------------------------------------------
void svtkCellLocator::ClearCellHasBeenVisited(svtkIdType id)
{
...
//
double svtkCellLocator::Distance2ToBucket(const double x[3], int nei[3])
{
  double bounds[6];
...
double svtkCellLocator::Distance2ToBounds(const double x[3], double bounds[6])
{
  double distance;
  double deltas[3];
...
//----------------------------------------------------------------------------
static bool svtkCellLocator_Inside(const double bounds[6], const double point[3])
{
...
//----------------------------------------------------------------------------
svtkIdType svtkCellLocator::FindCell(
  double x[3], double svtkNotUsed(tol2), svtkGenericCell* cell, double pcoords[3], double* weights)
{
  svtkIdList* cellIds;
  int ijk[3];
  int subId;
         ...
         ijk[2] * this->NumberOfDivisions * this->NumberOfDivisions]) != nullptr)
  {
    ...
    {
      // get the cell
      svtkIdType cellId = cellIds->GetId(j);
      // check whether we could be close enough to the cell by
      // testing the cell bounds
      if (this->CacheCellBounds)
      {
      ...
      {
        this->DataSet->GetCellBounds(cellId, cellBounds);
        if (svtkCellLocator_Inside(cellBounds, x))
        {
          this->DataSet->GetCell(cellId, cell);
          if (cell->EvaluatePosition(x, nullptr, subId, pcoords, dist2, weights) == 1)
          {
...
//----------------------------------------------------------------------------
void svtkCellLocator::FindCellsWithinBounds(double* bbox, svtkIdList* cells)
{
  this->BuildLocatorIfNeeded();
...
//----------------------------------------------------------------------------
void svtkCellLocator::FindCellsAlongLine(
  const double p1[3], const double p2[3], double svtkNotUsed(tol), svtkIdList* cells)
{
  this->BuildLocatorIfNeeded();
        ...
        for (cellId = 0; cellId < this->Tree[idx]->GetNumberOfIds(); cellId++)
        {
          ...
          if (this->CellHasBeenVisited[cId] != this->QueryNumber)
          {
            ...
            else
            {
              this->DataSet->GetCellBounds(cId, cellBounds);
              hitCellBounds =
                svtkBox::IntersectBox(cellBounds, p1, direction1, hitCellBoundsPosition, result);
            }
...
//----------------------------------------------------------------------------
void svtkCellLocator::PrintSelf(ostream& os, svtkIndent indent)
{
