/*=========================================================================
  ...
  Module:    svtkDataSetAttributesFieldList.cxx
...
struct FieldInfo
{
   ...
   */
  std::vector<std::array<bool, svtkDataSetAttributes::NUM_ATTRIBUTES> > AttributeTypes;

  /**
   * Location of this field in the input svtkDataSetAttributes instance at the
  ...
  void PrintSelf(ostream& os, svtkIndent indent) const
  {
    if (this->IsEmpty())
    {
  ...
  bool IsEmpty() const { return this->Type == SVTK_VOID; }

  static FieldInfo Create(svtkAbstractArray* array, int loc)
  {
    FieldInfo info;
    if (array)
    {
      ...
      if (auto da = svtkDataArray::SafeDownCast(array))
      {
  ...
  void InitializeArray(svtkAbstractArray* array, svtkIdType sz, svtkIdType ext) const
  {
    if (array)
    {
      ...
      if (auto darray = svtkDataArray::SafeDownCast(array))
      {
  ...
  void ExtendForUnion()
  {
    ...
    std::array<bool, svtkDataSetAttributes::NUM_ATTRIBUTES> curattrs;
...
std::multimap<std::string, FieldInfo> GetFields(svtkDataSetAttributes* dsa)
{
  std::array<int, svtkDataSetAttributes::NUM_ATTRIBUTES> attribute_indices;
 ...
 */
std::array<const detail::FieldInfo*, svtkDataSetAttributes::NUM_ATTRIBUTES> GetAttributes(
  const std::multimap<std::string, FieldInfo>& mmap)
{
  std::array<const detail::FieldInfo*, svtkDataSetAttributes::NUM_ATTRIBUTES> attrs;
  ...
  for (auto& pair : mmap)
  {
    ...
    std::array<bool, svtkDataSetAttributes::NUM_ATTRIBUTES> accumulated_attrs;
...
class svtkDataSetAttributesFieldList::svtkInternals
{
public:
  enum FieldListMode
  {
  ...
  {
    if (i >= 0 && i < svtkDataSetAttributes::NUM_ATTRIBUTES)
    {
      auto attrs = detail::GetAttributes(this->Fields);
      auto finfo = attrs[i];
      return finfo;
    }
    else if (i >= svtkDataSetAttributes::NUM_ATTRIBUTES &&
      i < svtkDataSetAttributes::NUM_ATTRIBUTES + static_cast<int>(this->Fields.size()))
    {
      const auto attrs_ptrs = detail::GetAttributes(this->Fields);

      auto iter = this->Fields.begin();
      std::advance(iter, i - svtkDataSetAttributes::NUM_ATTRIBUTES);
...
//----------------------------------------------------------------------------
svtkDataSetAttributesFieldList::svtkDataSetAttributesFieldList(int svtkNotUsed(number_of_inputs))
  : Internals(new svtkDataSetAttributesFieldList::svtkInternals())
{
}

//----------------------------------------------------------------------------
svtkDataSetAttributesFieldList::~svtkDataSetAttributesFieldList() {}

//----------------------------------------------------------------------------
void svtkDataSetAttributesFieldList::Reset()
{
  this->Internals->Reset();
}

//----------------------------------------------------------------------------
void svtkDataSetAttributesFieldList::InitializeFieldList(svtkDataSetAttributes* dsa)
{
  this->Internals->Reset();
...
//----------------------------------------------------------------------------
void svtkDataSetAttributesFieldList::IntersectFieldList(svtkDataSetAttributes* dsa)
{
  ...
  if (internals.NumberOfInputs == -1)
  {
    ...
    internals.Mode = svtkInternals::INTERSECTION;
    return;
  }

  if (internals.Mode == svtkInternals::UNION)
  {
    svtkGenericWarningMacro("Mixing of `IntersectFieldList` and `UnionFieldList` "
...
//----------------------------------------------------------------------------
void svtkDataSetAttributesFieldList::UnionFieldList(svtkDataSetAttributes* dsa)
{
  auto& internals = *this->Internals;
  if (internals.NumberOfInputs == -1)
  {
    // called without calling InitializeFieldList, just call it.
    this->InitializeFieldList(dsa);
    internals.Mode = svtkInternals::UNION;
    return;
  }

  if (internals.Mode == svtkInternals::INTERSECTION)
  {
    svtkGenericWarningMacro("Mixing of `IntersectFieldList` and `UnionFieldList` "
                           "calls is not supported!");
    return;
...
//----------------------------------------------------------------------------
void svtkDataSetAttributesFieldList::CopyAllocate(
  svtkDataSetAttributes* output, int ctype, svtkIdType sz, svtkIdType ext) const
{
  auto& internals = *this->Internals;
  ...
  for (auto& pair : internals.Fields)
  {
    ...
    for (int attrType = 0; attrType < svtkDataSetAttributes::NUM_ATTRIBUTES; ++attrType)
    {
    ...
    if (!is_attribute)
    {
      ...
      // (this is directly copied over from svtkDataSetAttributes::FieldList,
    ...
    if (array)
    {
      ...
      for (int attrType = 0;
           is_attribute && attrType < svtkDataSetAttributes::NUM_ATTRIBUTES && index != -1;
           ++attrType)
      {
        if (attribute_ptrs[attrType] == &fieldInfo)
        {
...
//----------------------------------------------------------------------------
void svtkDataSetAttributesFieldList::CopyData(int inputIndex, svtkDataSetAttributes* input,
  svtkIdType fromId, svtkDataSetAttributes* output, svtkIdType toId) const
{
  auto& internals = *this->Internals;
  for (auto& pair : internals.Fields)
  {
    ...
    {
      svtkGenericWarningMacro("Incorrect/unknown inputIndex specified : " << inputIndex);
      return;
    }
...
//----------------------------------------------------------------------------
void svtkDataSetAttributesFieldList::CopyData(int inputIndex, svtkDataSetAttributes* input,
  svtkIdType inputStart, svtkIdType numValues, svtkDataSetAttributes* output, svtkIdType outStart) const
{
  auto& internals = *this->Internals;
  for (auto& pair : internals.Fields)
  {
...
//----------------------------------------------------------------------------
void svtkDataSetAttributesFieldList::InterpolatePoint(int inputIndex, svtkDataSetAttributes* input,
  svtkIdList* inputIds, double* weights, svtkDataSetAttributes* output, svtkIdType toId) const
{
  auto& internals = *this->Internals;
  for (auto& pair : internals.Fields)
  {
    ...
    else if (fieldInfo.OutputLocation != -1 && fieldInfo.Location[inputIndex] != -1)
    {
      ...
      if (attrIndex != -1 &&
        output->GetCopyAttribute(attrIndex, svtkDataSetAttributes::INTERPOLATE) == 2)
      {
        svtkIdType numIds = inputIds->GetNumberOfIds();
        svtkIdType maxId = inputIds->GetId(0);
...
//----------------------------------------------------------------------------
void svtkDataSetAttributesFieldList::TransformData(int inputIndex, svtkDataSetAttributes* input,
  svtkDataSetAttributes* output, std::function<void(svtkAbstractArray*, svtkAbstractArray*)> op) const
{
  auto& internals = *this->Internals;
  for (auto& pair : internals.Fields)
  {
...
//----------------------------------------------------------------------------
svtkSmartPointer<svtkAbstractArray> svtkDataSetAttributesFieldList::CreateArray(int type) const
{
  return svtkSmartPointer<svtkAbstractArray>::Take(svtkAbstractArray::CreateArray(type));
}

//----------------------------------------------------------------------------
void svtkDataSetAttributesFieldList::PrintSelf(ostream& os, svtkIndent indent)
{
  os << indent << "svtkDataSetAttributesFieldList (" << this << ")\n";
  auto& internals = *this->Internals;
  for (auto& pair : internals.Fields)
  {
