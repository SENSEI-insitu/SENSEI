/*=========================================================================
  ...
  Module:    svtkGenericAdaptorCell.h
 ...
 *
 * In SVTK, spatial-temporal data is defined in terms of a dataset which is
 ...
 *
 * Currently most algorithms in SVTK use svtkCell and svtkDataSet, which make
 ...
 * are linear, or products of linear functions. Further, SVTK implements
 ...
 * increases.
 *
 * svtkGenericAdaptorCell addresses these issues by providing more general
 * abstraction for cells. It also adopts modern C++ practices including using
 * iterators. The svtkGenericAdaptorCell is designed to fit within the adaptor
 * framework; meaning that it is meant to adapt SVTK to external simulation
...
#ifndef svtkGenericAdaptorCell_h
#define svtkGenericAdaptorCell_h
...
class SVTKCOMMONDATAMODEL_EXPORT svtkGenericAdaptorCell : public svtkObject
{
public:
  svtkTypeMacro(svtkGenericAdaptorCell, svtkObject);
   ...
   * \post positive_result: result>=0
   */
  virtual int GetAttributeOrder(svtkGenericAttribute* a) = 0;
   ...
   */
  virtual int GetHighestOrderAttribute(svtkGenericAttributeCollection* ac);
   ...
   * \pre it_exists: it!=0
   */
  virtual void GetPointIterator(svtkGenericPointIterator* it) = 0;
   ...
   */
  SVTK_NEWINSTANCE
  virtual svtkGenericCellIterator* NewCellIterator() = 0;
   ...
   */
  virtual void GetBoundaryIterator(svtkGenericCellIterator* boundaries, int dim = -1) = 0;
   ...
   * \post positive_result: result>=0
   */
  virtual int CountNeighbors(svtkGenericAdaptorCell* boundary) = 0;
   ...
   */
  virtual void GetNeighbors(svtkGenericAdaptorCell* boundary, svtkGenericCellIterator* neighbors) = 0;
  ...
  virtual int FindClosestBoundary(
    int subId, double pcoords[3], svtkGenericCellIterator*& boundary) = 0;
   ...
   */
  virtual void InterpolateTuple(svtkGenericAttribute* a, double pcoords[3], double* val) = 0;
  ...
  virtual void InterpolateTuple(
    svtkGenericAttributeCollection* c, double pcoords[3], double* val) = 0;
   ...
   */
  virtual void Contour(svtkContourValues* values, svtkImplicitFunction* f,
   ...
   * \pre secondaryCd_exists: secondaryCd!=0
   */
  virtual void Clip(double value, svtkImplicitFunction* f, svtkGenericAttributeCollection* attributes,
    svtkGenericCellTessellator* tess, int insideOut, svtkIncrementalPointLocator* locator,
    svtkCellArray* connectivity, svtkPointData* outPd, svtkCellData* outCd, svtkPointData* internalPd,
    svtkPointData* secondaryPd, svtkCellData* secondaryCd);
  ...
  virtual void Derivatives(
    int subId, double pcoords[3], svtkGenericAttribute* attribute, double* derivs) = 0;
   ...
   */
  virtual int IsFaceOnBoundary(svtkIdType faceId) = 0;
   ...
   */
  virtual void GetPointIds(svtkIdType* id) = 0;
   ...
   */
  virtual const svtkIdType* GetFaceArray(svtkIdType faceId) = 0;
   ...
   */
  virtual const svtkIdType* GetEdgeArray(svtkIdType edgeId) = 0;

protected:
  svtkGenericAdaptorCell();
  ~svtkGenericAdaptorCell() override;
  ...
  // points for
  // Be careful the use of a svtkLocator in conjunction with the table fast
  // tessellator is very sensitive, we need to keep all the points we used
  svtkDoubleArray* InternalPoints;
  svtkCellArray* InternalCellArray;
  svtkDoubleArray* InternalScalars;
  svtkDoubleArray* PointDataScalars;

  svtkIdList* InternalIds; // used by Tessellate() and TriangulateFace()

  // Attributes to mimic the svtk cell look and feel, internal use only
...
private:
  svtkGenericAdaptorCell(const svtkGenericAdaptorCell&) = delete;
  void operator=(const svtkGenericAdaptorCell&) = delete;
};
