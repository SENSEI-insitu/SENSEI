/*=========================================================================
  ...
  Module:    svtkStaticPointLocator.cxx
...
svtkStandardNewMacro(svtkStaticPointLocator);
...
// in svtkPointLocator and svtkStaticPointLocator and causing weird faults.
...
struct svtkBucketList
{
  svtkStaticPointLocator* Locator; // locater
  ...
  // Construction
  svtkBucketList(svtkStaticPointLocator* loc, svtkIdType numPts, int numBuckets)
  {
    this->Locator = loc;
    this->NumPts = numPts;
  ...
  virtual ~svtkBucketList() = default;
  ...
  void GetBucketIndices(const double* x, int ijk[3]) const
  {
    ...
    svtkIdType tmp2 = static_cast<svtkIdType>(((x[2] - bZ) * fZ));
...
struct NeighborBuckets
{
  ...
  int* GetPoint(svtkIdType i) { return this->P + 3 * i; }

  svtkIdType InsertNextBucket(const int x[3])
  {
    // Re-allocate if beyond the current max size.
    // (Increase by SVTK_INITIAL_BUCKET_SIZE)
    int* tmp;
    svtkIdType offset = this->Count * 3;
...
protected:
  ...
  int InitialBuffer[SVTK_INITIAL_BUCKET_SIZE * 3];
  int* P;
  svtkIdType Count;
  svtkIdType MaxSize;
};
...
//
void svtkBucketList::GetBucketNeighbors(
  NeighborBuckets* buckets, const int ijk[3], const int ndivs[3], int level)
{
...
//-----------------------------------------------------------------------------
void svtkBucketList::GenerateFace(int face, int i, int j, int k, svtkPoints* pts, svtkCellArray* polys)
{
  svtkIdType ids[4];
  double origin[3], x[3];
...
//
double svtkBucketList::Distance2ToBucket(const double x[3], const int nei[3])
{
  double bounds[6];
...
// WARNING!!!!! Be very careful altering this routine.  Simple changes to this
// routine can make is 25% slower!!!!
double svtkBucketList::Distance2ToBounds(const double x[3], const double bounds[6])
{
  double distance;
  double deltas[3];
...
struct BucketList : public svtkBucketList
{
  ...
  // Construction
  BucketList(svtkStaticPointLocator* loc, svtkIdType numPts, int numBuckets)
    : svtkBucketList(loc, numPts, numBuckets)
  {
  ...
  struct MapDataSet
  {
    ...
    MapDataSet(BucketList<T>* blist, svtkDataSet* ds)
      : BList(blist)
      , DataSet(ds)
    {
    }
  ...
  struct MergePrecise
  {
    ...
    MergePrecise(BucketList<T>* blist, svtkIdType* mergeMap)
      : BList(blist)
      , MergeMap(mergeMap)
    {
  ...
  struct MergeClose
  {
    ...
    MergeClose(BucketList<T>* blist, double tol, svtkIdType* mergeMap)
      : BList(blist)
      , MergeMap(mergeMap)
      , Tol(tol)
    {
...
template <typename TIds>
svtkIdType BucketList<TIds>::FindClosestPoint(const double x[3])
{
  int i, j;
  double minDist2;
  ...
  //  closest point found.
  //
  for (closest = (-1), minDist2 = SVTK_DOUBLE_MAX, level = 0; (closest == -1) &&
    ...
    for (i = 0; i < buckets.GetNumberOfNeighbors(); i++)
    {
      ...
      if ((numIds = this->GetNumberOfIds(cno)) > 0)
      {
        ...
        for (j = 0; j < numIds; j++)
        {
          ...
          if ((dist2 = svtkMath::Distance2BetweenPoints(x, pt)) < minDist2)
          {
            closest = ptId;
            minDist2 = dist2;
          }
...
//-----------------------------------------------------------------------------
template <typename TIds>
svtkIdType BucketList<TIds>::FindClosestPointWithinRadius(
  double radius, const double x[3], double inputDataLength, double& dist2)
{
  int i, j;
  double pt[3];
  ...
  //
  svtkIdType numIds;
  svtkIdType cno = ijk[0] + ijk[1] * this->xD + ijk[2] * this->xyD;
  if ((numIds = this->GetNumberOfIds(cno)) > 0)
  {
    ids = this->GetIds(cno);
    for (j = 0; j < numIds; j++)
...
//-----------------------------------------------------------------------------
template <typename TIds>
void BucketList<TIds>::FindClosestNPoints(int N, const double x[3], svtkIdList* result)
{
  int i, j;
  double dist2;
  double pt[3];
    ...
    for (i = 0; i < buckets.GetNumberOfNeighbors(); i++)
    {
      ...
      if ((numIds = this->GetNumberOfIds(cno)) > 0)
      {
        ...
        for (j = 0; j < numIds; j++)
        {
          ptId = ids[j].PtId;
          this->DataSet->GetPoint(ptId, pt);
          dist2 = svtkMath::Distance2BetweenPoints(x, pt);
          if (currentCount < N)
          {
...
// touch.
template <typename TIds>
void BucketList<TIds>::FindPointsWithinRadius(double R, const double x[3], svtkIdList* result)
{
  double dist2;
  double pt[3];
  svtkIdType ptId, cno, numIds;
  ...
  double lineX[3], double ptX[3], svtkIdType& ptId)
{
  ...
  svtkIdType prod = ndivs[0] * ndivs[1];
  double* h = this->H;
  TIds ii, numPtsInBin;
  ...
  if (svtkBox::IntersectBox(bounds, a0, rayDir, curPos, curT))
  {
    ...
    tMax[2] = (rayDir[2] != 0.0) ? (next[2] - curPos[2]) / rayDir[2] : SVTK_FLOAT_MAX;

    tDelta[0] = (rayDir[0] != 0.0) ? (h[0] / rayDir[0]) * step[0] : SVTK_FLOAT_MAX;
    tDelta[1] = (rayDir[1] != 0.0) ? (h[1] / rayDir[1]) * step[1] : SVTK_FLOAT_MAX;
    tDelta[2] = (rayDir[2] != 0.0) ? (h[2] / rayDir[2]) * step[2] : SVTK_FLOAT_MAX;
...
template <typename TIds>
void BucketList<TIds>::MergePoints(double tol, svtkIdType* mergeMap)
{
...
template <typename TIds>
void BucketList<TIds>::GenerateRepresentation(int svtkNotUsed(level), svtkPolyData* pd)
{
  svtkPoints* pts;
  svtkCellArray* polys;
...
// 5 points per bucket.
svtkStaticPointLocator::svtkStaticPointLocator()
{
...
//-----------------------------------------------------------------------------
void svtkStaticPointLocator::Initialize()
{
  this->FreeSearchStructure();
}

//-----------------------------------------------------------------------------
void svtkStaticPointLocator::FreeSearchStructure()
{
  if (this->Buckets)
  {
    delete this->Buckets;
...
//
void svtkStaticPointLocator::BuildLocator()
{
  int ndivs[3];
  int i;
  svtkIdType numPts;
  ...
  this->NumberOfBuckets = numBuckets = static_cast<svtkIdType>(ndivs[0]) *
    static_cast<svtkIdType>(ndivs[1]) * static_cast<svtkIdType>(ndivs[2]);
  ...
  {
    this->LargeIds = true;
    this->Buckets = new BucketList<svtkIdType>(this, numPts, numBuckets);
  }
  else
  {
    this->LargeIds = false;
...
//  The result is directly addressable and of uniform subdivision.
//
void svtkStaticPointLocator::BuildLocator(const double* bds)
{
  int ndivs[3];
  int i;
  svtkIdType numPts;
...
//-----------------------------------------------------------------------------
// These methods satisfy the svtkStaticPointLocator API. The implementation is
...
//-----------------------------------------------------------------------------
// Given a position x, return the id of the point closest to it.
svtkIdType svtkStaticPointLocator::FindClosestPoint(const double x[3])
{
  ...
  if (this->LargeIds)
  {
    return static_cast<BucketList<svtkIdType>*>(this->Buckets)->FindClosestPoint(x);
  }
  else
  {
...
//-----------------------------------------------------------------------------
svtkIdType svtkStaticPointLocator::FindClosestPointWithinRadius(
  double radius, const double x[3], double inputDataLength, double& dist2)
{
  this->BuildLocator(); // will subdivide if modified; otherwise returns
  if (!this->Buckets)
  ...
  if (this->LargeIds)
  {
    return static_cast<BucketList<svtkIdType>*>(this->Buckets)
...
//-----------------------------------------------------------------------------
void svtkStaticPointLocator::FindClosestNPoints(int N, const double x[3], svtkIdList* result)
{
  this->BuildLocator(); // will subdivide if modified; otherwise returns
  if (!this->Buckets)
  {
  ...
  if (this->LargeIds)
  {
    return static_cast<BucketList<svtkIdType>*>(this->Buckets)->FindClosestNPoints(N, x, result);
  }
  else
  {
...
//-----------------------------------------------------------------------------
void svtkStaticPointLocator::FindPointsWithinRadius(double R, const double x[3], svtkIdList* result)
{
  this->BuildLocator(); // will subdivide if modified; otherwise returns
  if (!this->Buckets)
  {
  ...
  if (this->LargeIds)
  {
    return static_cast<BucketList<svtkIdType>*>(this->Buckets)->FindPointsWithinRadius(R, x, result);
  }
  else
  {
...
int svtkStaticPointLocator::IntersectWithLine(double a0[3], double a1[3], double tol, double& t,
  double lineX[3], double ptX[3], svtkIdType& ptId)
{
  this->BuildLocator(); // will subdivide if modified; otherwise returns
  if (!this->Buckets)
...
void svtkStaticPointLocator::GenerateRepresentation(int level, svtkPolyData* pd)
{
  this->BuildLocator(); // will subdivide if modified; otherwise returns
  if (!this->Buckets)
  {
  ...
  if (this->LargeIds)
  {
    return static_cast<BucketList<svtkIdType>*>(this->Buckets)->GenerateRepresentation(level, pd);
  }
  else
  {
...
svtkIdType svtkStaticPointLocator::GetNumberOfPointsInBucket(svtkIdType bNum)
{
  this->BuildLocator(); // will subdivide if modified; otherwise returns
  if (!this->Buckets)
  {
  ...
  if (this->LargeIds)
  {
    return static_cast<BucketList<svtkIdType>*>(this->Buckets)->GetNumberOfIds(bNum);
  }
  else
  {
...
void svtkStaticPointLocator::GetBucketIds(svtkIdType bNum, svtkIdList* bList)
{
  this->BuildLocator(); // will subdivide if modified; otherwise returns
  if (!this->Buckets)
  {
  ...
  if (this->LargeIds)
  {
    return static_cast<BucketList<svtkIdType>*>(this->Buckets)->GetIds(bNum, bList);
  }
  else
  {
...
//-----------------------------------------------------------------------------
// Given a bucket, return the ids in the bucket.
void svtkStaticPointLocator::MergePoints(double tol, svtkIdType* pointMap)
{
  this->BuildLocator(); // will subdivide if modified; otherwise returns
  if (!this->Buckets)
  {
  ...
  if (this->LargeIds)
  {
    return static_cast<BucketList<svtkIdType>*>(this->Buckets)->MergePoints(tol, pointMap);
  }
  else
  {
...
//-----------------------------------------------------------------------------
void svtkStaticPointLocator::PrintSelf(ostream& os, svtkIndent indent)
{
