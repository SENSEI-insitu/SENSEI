 ...
 *
 * svtkTriangle is a concrete implementation of svtkCell to represent a triangle
 * located in 3-space.
 */

#ifndef svtkTriangle_h
#define svtkTriangle_h

#include "svtkCell.h"
#include "svtkCommonDataModelModule.h" // For export macro

#include "svtkMath.h" // Needed for inline methods

class svtkLine;
class svtkQuadric;
class svtkIncrementalPointLocator;

class SVTKCOMMONDATAMODEL_EXPORT svtkTriangle : public svtkCell
{
public:
  static svtkTriangle* New();
  svtkTypeMacro(svtkTriangle, svtkCell);
   ...
   */
  int GetCellType() override { return SVTK_TRIANGLE; }
  ...
  /**
   * A convenience function to compute the area of a svtkTriangle.
   */
  double ComputeArea();
  ...
  {
    svtkTriangle::InterpolationFunctions(pcoords, sf);
  }
  void InterpolateDerivs(const double pcoords[3], double derivs[6]) override
  {
    svtkTriangle::InterpolationDerivs(pcoords, derivs);
  }
  //@}
  /**
   ...
   * See also svtkTriangle::ComputeArea()
   */
   ...
   * quadric as a 4x4 matrix or a svtkQuadric.  (from Peter
...
protected:
  svtkTriangle();
  ~svtkTriangle() override;

  svtkLine* Line;

private:
  svtkTriangle(const svtkTriangle&) = delete;
  void operator=(const svtkTriangle&) = delete;
};

//----------------------------------------------------------------------------
inline int svtkTriangle::GetParametricCenter(double pcoords[3])
{
...
//----------------------------------------------------------------------------
inline void svtkTriangle::ComputeNormalDirection(
  const double v1[3], const double v2[3], const double v3[3], double n[3])
{
...
//----------------------------------------------------------------------------
inline void svtkTriangle::ComputeNormal(
  const double v1[3], const double v2[3], const double v3[3], double n[3])
{
  double length;

  svtkTriangle::ComputeNormalDirection(v1, v2, v3, n);
...
//----------------------------------------------------------------------------
inline void svtkTriangle::TriangleCenter(
  const double p1[3], const double p2[3], const double p3[3], double center[3])
{
...
//----------------------------------------------------------------------------
inline double svtkTriangle::TriangleArea(const double p1[3], const double p2[3], const double p3[3])
{
  double n[3];
  svtkTriangle::ComputeNormalDirection(p1, p2, p3, n);

  return 0.5 * svtkMath::Norm(n);
}
