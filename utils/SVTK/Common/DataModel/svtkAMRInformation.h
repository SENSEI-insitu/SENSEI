 ...
 *
 *
 * svtkAMRInformation encaspulates the following meta information for an AMR data set
 * - a list of svtkAMRBox objects
...
#ifndef svtkAMRInformation_h
#define svtkAMRInformation_h

#include "svtkAMRBox.h"                //for storing AMR Boxes
#include "svtkCommonDataModelModule.h" // For export macro
#include "svtkObject.h"
#include "svtkSmartPointer.h" //for ivars
#include <vector>            //for storing AMR Boxes

typedef std::vector<svtkAMRBox> svtkAMRBoxList;

class svtkUnsignedIntArray;
class svtkIntArray;
class svtkDoubleArray;
class svtkAMRIndexIterator;

class SVTKCOMMONDATAMODEL_EXPORT svtkAMRInformation : public svtkObject
{
public:
  static svtkAMRInformation* New();
  svtkTypeMacro(svtkAMRInformation, svtkObject);

  void PrintSelf(ostream& os, svtkIndent indent) override;

  bool operator==(const svtkAMRInformation& other);
  ...
  //@{
  /**
   * returns the value of svtkUniformGrid::GridDescription() of any block
   */
  svtkGetMacro(GridDescription, int);
   ...
   */
  void SetAMRBox(unsigned int level, unsigned int id, const svtkAMRBox& box);
  const svtkAMRBox& GetAMRBox(unsigned int level, unsigned int id) const;
  //@}

  /**
   * return the amr box coarsened to the previous level
   */
  bool GetCoarsenedAMRBox(unsigned int level, unsigned int id, svtkAMRBox& box) const;
  ...
  void DeepCopy(svtkAMRInformation* other);

private:
  svtkAMRInformation();
  ~svtkAMRInformation() override;
  svtkAMRInformation(const svtkAMRInformation&) = delete;
  void operator=(const svtkAMRInformation&) = delete;
  ...
  //-------------------------------------------------------------------------
  int GridDescription; // example: SVTK_XYZ_GRID
  double Origin[3];    // the origin of the whole data set
  svtkAMRBoxList Boxes; // svtkAMRBoxes, one per data set
  std::vector<int>
    NumBlocks; // NumBlocks[i] stores the total number of blocks from level 0 to level i-1

  svtkSmartPointer<svtkIntArray>
    SourceIndex; // Typically, this maps to a file block index used by the reader
  svtkSmartPointer<svtkDoubleArray> Spacing; // The grid spacing for all levels
  double Bounds[6];                        // the bounds of the entire domain

  //-------------------------------------------------------------------------
  // Auxiliary information that be computed
  //-------------------------------------------------------------------------
  svtkSmartPointer<svtkIntArray> Refinement;         // refinement ratio between two adjacent levels
  svtkSmartPointer<svtkUnsignedIntArray> BlockLevel; // only necessary if need to call
                                                   // ComputeIndexPair
