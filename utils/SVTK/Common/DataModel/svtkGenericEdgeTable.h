 ...
 *
 * svtkGenericEdgeTable is used to indicate the existence of and hold
 * information about edges. Similar to svtkEdgeTable, this class is
 * more sophisticated in that it uses reference counting to keep track
 * of when information about an edge should be deleted.
 *
 * svtkGenericEdgeTable is a helper class used in the adaptor framework.  It
...
#ifndef svtkGenericEdgeTable_h
#define svtkGenericEdgeTable_h
...
class SVTKCOMMONDATAMODEL_EXPORT svtkGenericEdgeTable : public svtkObject
{
public:
  /**
   * Instantiate an empty edge table.
   */
  static svtkGenericEdgeTable* New();

  //@{
  /**
   * Standard SVTK type and print macros.
   */
  svtkTypeMacro(svtkGenericEdgeTable, svtkObject);
   ...
   */
  void InsertEdge(svtkIdType e1, svtkIdType e2, svtkIdType cellId, int ref, svtkIdType& ptId);

  /**
   * Insert an edge but do not split it.
   */
  void InsertEdge(svtkIdType e1, svtkIdType e2, svtkIdType cellId, int ref = 1);

  /**
   * Method to remove an edge from the table. The method returns the
   * current reference count.
   */
  int RemoveEdge(svtkIdType e1, svtkIdType e2);
   ...
   */
  int CheckEdge(svtkIdType e1, svtkIdType e2, svtkIdType& ptId);

  /**
   * Method that increments the referencecount and returns it.
   */
  int IncrementEdgeReferenceCount(svtkIdType e1, svtkIdType e2, svtkIdType cellId);

  /**
   * Return the edge reference count.
   */
  int CheckEdgeReferenceCount(svtkIdType e1, svtkIdType e2);
   ...
   */
  void Initialize(svtkIdType start);
   ...
   */
  int CheckPoint(svtkIdType ptId);
   ...
   */
  int CheckPoint(svtkIdType ptId, double point[3], double* scalar);

  //@{
  /**
   * Insert point associated with an edge.
   */
  void InsertPoint(svtkIdType ptId, double point[3]);
  // \pre: sizeof(s)==GetNumberOfComponents()
  void InsertPointAndScalar(svtkIdType ptId, double pt[3], double* s);
  //@}

  /**
   * Remove a point from the point table.
   */
  void RemovePoint(svtkIdType ptId);

  /**
   * Increment the reference count for the indicated point.
   */
  void IncrementPointReferenceCount(svtkIdType ptId);
  ...
  public:
    ...
    svtkIdType PtId;
    svtkIdType CellId; // CellId the edge refer to at a step in tessellation
...
protected:
  svtkGenericEdgeTable();
  ~svtkGenericEdgeTable() override;
  ...
  svtkEdgeTableEdge* EdgeTable;

  // At end of process we should be able to retrieve points coord based on pointid
  svtkEdgeTablePoints* HashPoints;

  // Main hash functions
  // For edge table:
  svtkIdType HashFunction(svtkIdType e1, svtkIdType e2);

  // For point table:
  svtkIdType HashFunction(svtkIdType ptId);

  // Keep track of the last point id we inserted, increment it each time:
  svtkIdType LastPointId;

  svtkIdType NumberOfComponents;

private:
  svtkGenericEdgeTable(const svtkGenericEdgeTable&) = delete;
  void operator=(const svtkGenericEdgeTable&) = delete;
};
