/*=========================================================================
  ...
  Module:    svtkStaticCellLocator.cxx
...
svtkStandardNewMacro(svtkStaticCellLocator);

//----------------------------------------------------------------------------
// Helper classes to support efficient computing, and threaded execution.
//
// Note that are two key classes: the svtkCellBinner and the svtkCellProcessor.
...
// fragment count, a templated class of either int or svtkIdType is
...
{
  svtkStaticCellLocator* Locator; // locater
  svtkIdType NumCells;            // the number of cells to bin
  svtkIdType NumBins;
  svtkIdType NumFragments; // total number of (cellId,binId) tuples
  ...
  // Construction
  svtkCellBinner(svtkStaticCellLocator* loc, svtkIdType numCells, svtkIdType numBins)
  {
    this->Locator = loc;
    ...
    this->Counts = new svtkIdType[numCells + 1]; // one extra holds total count
  ...
  void GetBinIndices(svtkIdType binId, int ijk[3]) const
  {
    ijk[0] = binId % xD;
    svtkIdType tmp = binId / xD;
    ijk[1] = tmp % yD;
    ijk[2] = tmp / yD;
  }
  ...
  svtkIdType GetBinIndex(int ijk[3]) const { return ijk[0] + ijk[1] * xD + ijk[2] * xyD; }

  // These are helper functions
  svtkIdType CountBins(const int ijkMin[3], const int ijkMax[3])
  {
    // Ensure all temporary values are svtkIdType:
    svtkIdType result = ijkMax[0] - ijkMin[0] + 1;
  ...
  void operator()(svtkIdType cellId, svtkIdType endCellId)
  {
    ...
    svtkIdType* counts = this->Counts + cellId;
  ...
  {
    // Perform prefix sum
    svtkIdType* counts = this->Counts;
    svtkIdType numBins, total = 0, numCells = this->NumCells;
...
{
  svtkCellBinner* Binner;
  svtkDataSet* DataSet;
  double* CellBounds;
  svtkIdType* Counts;
  svtkIdType NumFragments;
  svtkIdType NumCells;
  svtkIdType NumBins;
  int BatchSize;
  int NumBatches;
  ...
  svtkCellProcessor(svtkCellBinner* cb)
    : Binner(cb)
  {
  ...
  virtual ~svtkCellProcessor() = default;

  // Satisfy cell locator API
  virtual svtkIdType FindCell(
    const double pos[3], svtkGenericCell* cell, double pcoords[3], double* weights) = 0;
  virtual void FindCellsWithinBounds(double* bbox, svtkIdList* cells) = 0;
  virtual void FindCellsAlongLine(
    const double p1[3], const double p2[3], double tol, svtkIdList* cells) = 0;
  virtual void FindCellsAlongPlane(
    const double o[3], const double n[3], double tolerance, svtkIdList* cells) = 0;
  virtual int IntersectWithLine(const double a0[3], const double a1[3], double tol, double& t,
    double x[3], double pcoords[3], int& subId, svtkIdType& cellId, svtkGenericCell* cell) = 0;
  virtual svtkIdType FindClosestPointWithinRadius(const double x[3], double radius,
    double closestPoint[3], svtkGenericCell* cell, svtkIdType& cellId, int& subId, double& dist2,
    int& inside) = 0;

  // Convenience for computing
  virtual int IsEmpty(svtkIdType binId) = 0;
};
...
struct CellProcessor : public svtkCellProcessor
{
  ...
  T GetNumberOfIds(svtkIdType binNum) { return (this->Offsets[binNum + 1] - this->Offsets[binNum]); }

  // Given a bin number, return the cells ids in that bin.
  const CellFragments<T>* GetIds(svtkIdType binNum) { return this->Map + this->Offsets[binNum]; }
  ...
  // Methods to satisfy svtkCellProcessor virtual API
  svtkIdType FindCell(
    const double pos[3], svtkGenericCell* cell, double pcoords[3], double* weights) override;
  void FindCellsWithinBounds(double* bbox, svtkIdList* cells) override;
  void FindCellsAlongLine(
    const double p1[3], const double p2[3], double tol, svtkIdList* cells) override;
  void FindCellsAlongPlane(
    const double o[3], const double n[3], double tolerance, svtkIdList* cells) override;
  ...
  svtkIdType FindClosestPointWithinRadius(const double x[3], double radius, double closestPoint[3],
    svtkGenericCell* cell, svtkIdType& cellId, int& subId, double& dist2, int& inside) override;
  int IsEmpty(svtkIdType binId) override
  {
...
//-----------------------------------------------------------------------------
template <typename T>
svtkIdType CellProcessor<T>::FindCell(
  const double pos[3], svtkGenericCell* cell, double pcoords[3], double* weights)
{
  svtkIdType binId = this->Binner->GetBinIndex(pos);
  ...
  else
  {
    ...
    for (int j = 0; j < numIds; j++)
    {
      ...
      if (svtkMath::PointIsWithinBounds(pos, bounds, delta))
      {
...
//-----------------------------------------------------------------------------
template <typename T>
void CellProcessor<T>::FindCellsWithinBounds(double* bbox, svtkIdList* cells)
{
  svtkIdType binNum, numIds, jOffset, kOffset;
...
// cells in intersected bins are placed into the output cellId svtkIdList. See
// the IntersectWithLine method for more information on voxel traversal.
template <typename T>
void CellProcessor<T>::FindCellsAlongLine(
  const double a0[3], const double a1[3], double svtkNotUsed(tol), svtkIdList* cells)
{
  ...
  if (svtkBox::IntersectBox(bounds, a0, rayDir, curPos, curT) == 0)
  {
    return;
  }
    ...
    if ((numCellsInBin = this->GetNumberOfIds(idx)) > 0) // there are some cell here
    {
      ...
      for (i = 0; i < numCellsInBin; i++)
      {
        ...
        if (cellHasBeenVisited[cId] == 0)
        {
          ...
          int hitCellBounds = svtkBox::IntersectBox(
...
struct CellPlaneCandidates
{
  ...
  CellPlaneCandidates(CellProcessor<TId>* p, const svtkCellBinner* b, const double* o,
    ...
    , CellVisited(visited)
  {
    ...
    x[1] = -this->BinOffsetY;
    x[2] = -this->BinOffsetZ;
    d = svtkMath::Dot(x, this->Normal); // simplified because plane passes through origin
    dMax = (d > dMax ? d : dMax);

    x[0] = this->BinOffsetX;
    x[1] = -this->BinOffsetY;
    x[2] = -this->BinOffsetZ;
    d = svtkMath::Dot(x, this->Normal);
    dMax = (d > dMax ? d : dMax);
  ...
  void operator()(svtkIdType k, svtkIdType kEnd)
  {
    double *o = this->Origin, *n = this->Normal;
    double center[3], d, *bounds;
    svtkIdType i, j, iEnd = this->Binner->Divisions[0], jEnd = this->Binner->Divisions[1];
    ...
    for (; k < kEnd; ++k)
    {
      ...
      for (j = 0; j < jEnd; ++j)
      {
        ...
        for (i = 0; i < iEnd; ++i)
        {
          ...
          d = svtkPlane::DistanceToPlane(center, n, o);
          if (d <= this->BinRadius)
          {
            svtkIdType cId, ii, numCellsInBin;
            svtkIdType bin = i + j * this->Binner->xD + k * this->Binner->xyD;
              ...
              0) // there are some cell here
            {
              ...
              for (ii = 0; ii < numCellsInBin; ii++)
              {
                ...
                if (!this->CellVisited[cId])
                {
                  ...
                  this->CellVisited[cId] = (svtkBox::IntersectWithPlane(bounds, o, n) ? 2 : 1);
                }
              }
            }
          } // if bin intersects
...
void CellProcessor<T>::FindCellsAlongPlane(
  const double o[3], const double n[3], double svtkNotUsed(tol), svtkIdList* cells)
{
  // Initialize the list of cells
  cells->Reset();
  ...
  normal[1] = n[1];
  normal[2] = n[2];
  if (svtkBox::IntersectWithPlane(bounds, origin, normal) == 0)
  {
    return;
  }
  ...
  svtkSMPTools::For(0, this->Binner->Divisions[2], cellCandidates);

  // Populate the output list.
  for (svtkIdType cellId = 0; cellId < this->NumCells; ++cellId)
  {
...
// Return closest point (if any) AND the cell on which this closest point lies
template <typename T>
svtkIdType CellProcessor<T>::FindClosestPointWithinRadius(const double x[3], double radius,
  double closestPoint[3], svtkGenericCell* cell, svtkIdType& closestCellId, int& closestSubId,
  double& minDist2, int& inside)
{
  ...
  using node = std::pair<double, svtkIdType>;
  std::priority_queue<node, std::vector<node>, std::greater<node> > queue;

  // first get ijk containing point
  svtkIdType binId = this->Binner->GetBinIndex(x);
  ...
  double& t, double x[3], double pcoords[3], int& subId, svtkIdType& cellId, svtkGenericCell* cell)
{
  ...
  double rayDir[3];
  svtkMath::Subtract(a1, a0, rayDir);
  double curPos[3], curT, tMin = SVTK_FLOAT_MAX;
  int ijk[3];
  svtkIdType idx, cId, bestCellId = (-1);
  double hitCellBoundsPosition[3], tHitCell;
  double step[3], next[3], tMax[3], tDelta[3];
...
//-----------------------------------------------------------------------------
svtkStaticCellLocator::svtkStaticCellLocator()
{
...
//-----------------------------------------------------------------------------
void svtkStaticCellLocator::FreeSearchStructure()
{
  if (this->Binner)
  {
    delete this->Binner;
...
//-----------------------------------------------------------------------------
svtkIdType svtkStaticCellLocator::FindCell(
...
//----------------------------------------------------------------------------
void svtkStaticCellLocator::FindClosestPoint(const double x[3], double closestPoint[3],
  svtkGenericCell* cell, svtkIdType& cellId, int& subId, double& dist2)
{
  int inside;
  double radius = svtkMath::Inf();
...
//----------------------------------------------------------------------------
svtkIdType svtkStaticCellLocator::FindClosestPointWithinRadius(double x[3], double radius,
  double closestPoint[3], svtkGenericCell* cell, svtkIdType& cellId, int& subId, double& dist2,
  int& inside)
{
  this->BuildLocator();
...
//-----------------------------------------------------------------------------
void svtkStaticCellLocator::FindCellsWithinBounds(double* bbox, svtkIdList* cells)
{
  this->BuildLocator();
  if (!this->Processor)
  {
...
//-----------------------------------------------------------------------------
void svtkStaticCellLocator::FindCellsAlongLine(
  const double p1[3], const double p2[3], double tol, svtkIdList* cells)
{
  this->BuildLocator();
  if (!this->Processor)
  {
...
//-----------------------------------------------------------------------------
void svtkStaticCellLocator::FindCellsAlongPlane(
  const double o[3], const double n[3], double tol, svtkIdList* cells)
{
  this->BuildLocator();
  if (!this->Processor)
  {
...
//-----------------------------------------------------------------------------
int svtkStaticCellLocator::IntersectWithLine(const double p1[3], const double p2[3], double tol,
  double& t, double x[3], double pcoords[3], int& subId, svtkIdType& cellId, svtkGenericCell* cell)
{
  this->BuildLocator();
  if (!this->Processor)
...
//-----------------------------------------------------------------------------
void svtkStaticCellLocator::BuildLocator()
{
  svtkDebugMacro(<< "Building static cell locator");
  ...
  {
    svtkErrorMacro(<< "No cells to build");
    return;
  }
  ...
  svtkIdType numBins = static_cast<svtkIdType>(
  ...
  numBins = static_cast<svtkIdType>(ndivs[0]) * static_cast<svtkIdType>(ndivs[1]) *
    static_cast<svtkIdType>(ndivs[2]);
  ...
  delete this->Binner;
  delete this->Processor;
  this->Binner = new svtkCellBinner(this, numCells, numBins);
  svtkSMPTools::For(0, numCells, *(this->Binner));

  // Create sorted cell fragments tuples of (cellId,binId). Depending
  // on problem size, different types are used.
  svtkIdType numFragments = this->Binner->NumFragments;
  if (numFragments >= SVTK_INT_MAX)
  {
    this->LargeIds = true;
    CellProcessor<svtkIdType>* processor = new CellProcessor<svtkIdType>(this->Binner);
    svtkSMPTools::For(0, numCells, *processor);
    svtkSMPTools::Sort(processor->Map, processor->Map + numFragments);
    MapOffsets<svtkIdType> mapOffsets(processor);
    svtkSMPTools::For(0, processor->NumBatches, mapOffsets);
...
// ignored.
void svtkStaticCellLocator::GenerateRepresentation(int svtkNotUsed(level), svtkPolyData* pd)
{
  ...
  int i, j, k;
  svtkIdType idx, kOffset, jOffset, kSlice = dims[0] * dims[1], pIds[8];
  double* s = this->H;
...
//-----------------------------------------------------------------------------
void svtkStaticCellLocator::PrintSelf(ostream& os, svtkIndent indent)
{
