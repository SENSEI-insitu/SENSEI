 ...
 *
 * svtkExplicitStructuredGrid is a data object that is a concrete implementation
 * of svtkDataSet. svtkExplicitStructuredGrid represents a geometric structure
 ...
 *
 * Like unstructured grid, svtkExplicitStructuredGrid has explicit point coordinates
 * and cell to point indexing.
 * Unlike unstructured grid, svtkExplicitStructuredGrid does not keep a cell type
 * list as all visible cells are known to be hexahedra.
 * svtkExplicitStructuredGrid can take advantage of its layout to perform operations
 ...
 * of the grid minus 1, because in svtk structured datasets the dimensions
...
#ifndef svtkExplicitStructuredGrid_h
#define svtkExplicitStructuredGrid_h

#include "svtkCommonDataModelModule.h" // For export macro
#include "svtkNew.h"                   // for svtkNew
#include "svtkPointSet.h"
#include "svtkStructuredData.h" // For static method usage
...
class SVTKCOMMONDATAMODEL_EXPORT svtkExplicitStructuredGrid : public svtkPointSet
{
public:
  //@{
  /**
   ...
   */
  static svtkExplicitStructuredGrid* New();
  svtkTypeMacro(svtkExplicitStructuredGrid, svtkPointSet);
   ...
   */
  int GetDataObjectType() override { return SVTK_EXPLICIT_STRUCTURED_GRID; }
   ...
   */
  void SetCells(svtkCellArray* cells);
  svtkGetObjectMacro(Cells, svtkCellArray);
  //@}
   ...
   */
  void BuildLinks();
  svtkGetObjectMacro(Links, svtkAbstractCellLinks);
  //@}

  /**
   * Get direct raw pointer to the 8 points indices of an hexahedra.
   */
  svtkIdType* GetCellPoints(svtkIdType cellId);

  /**
   * More efficient method to obtain cell points.
   */
  void GetCellPoints(svtkIdType cellId, svtkIdType& npts, svtkIdType*& pts);

  /**
   * Get cell neighbors of the cell for every faces.
   */
  void GetCellNeighbors(svtkIdType cellId, svtkIdType neighbors[6], int* wholeExtent = nullptr);
  ...
  void ComputeCellStructuredCoords(
    svtkIdType cellId, int& i, int& j, int& k, bool adjustForExtent = true);
   ...
   * The default adjustForExtent is true.
   */
  svtkIdType ComputeCellId(int i, int j, int k, bool adjustForExtent = true);
   ...
   */
  svtkSetStringMacro(FacesConnectivityFlagsArrayName);
  svtkGetStringMacro(FacesConnectivityFlagsArrayName);
  //@}
   ...
   * grid are set.
   */
  void BlankCell(svtkIdType cellId);
  void UnBlankCell(svtkIdType cellId);
  //@}
   ...
   * grid are set.
   */
  unsigned char IsCellGhost(svtkIdType cellId);
   ...
   */
  static svtkExplicitStructuredGrid* GetData(svtkInformation* info);
  static svtkExplicitStructuredGrid* GetData(svtkInformationVector* v, int i = 0);
  //@}
...
protected:
   ...
   */
  virtual void InternalCopy(svtkExplicitStructuredGrid* src);

  /**
   * Internal method used by GetCell.
   */
  void GetCell(svtkIdType, svtkCell*);
  ...
  svtkNew<svtkHexahedron> Hexahedron;
  svtkNew<svtkEmptyCell> EmptyCell;

  svtkCellArray* Cells;
  svtkAbstractCellLinks* Links;
  int Extent[6];
  char* FacesConnectivityFlagsArrayName;

private:
  svtkExplicitStructuredGrid(const svtkExplicitStructuredGrid&) = delete;
  void operator=(const svtkExplicitStructuredGrid&) = delete;
};

//----------------------------------------------------------------------------
inline void svtkExplicitStructuredGrid::GetDimensions(int dim[3])
{
  svtkStructuredData::GetDimensionsFromExtent(this->Extent, dim);
}

//----------------------------------------------------------------------------
inline void svtkExplicitStructuredGrid::GetCellDims(int cellDims[3])
{
  svtkStructuredData::GetCellDimensionsFromExtent(this->Extent, cellDims);
}

//----------------------------------------------------------------------------
inline void svtkExplicitStructuredGrid::ComputeCellStructuredCoords(
  svtkIdType cellId, int& i, int& j, int& k, bool adjustForExtent)
{
  int ijk[3];
  if (adjustForExtent)
  {
    svtkStructuredData::ComputeCellStructuredCoordsForExtent(cellId, this->Extent, ijk);
  }
  else
  {
    int dims[3];
    this->GetDimensions(dims);
    svtkStructuredData::ComputeCellStructuredCoords(cellId, dims, ijk);
  }
  i = ijk[0];
  j = ijk[1];
  k = ijk[2];
...
//----------------------------------------------------------------------------
inline svtkIdType svtkExplicitStructuredGrid::ComputeCellId(int i, int j, int k, bool adjustForExtent)
{
  int ijk[] = { i, j, k };
  if (adjustForExtent)
  {
