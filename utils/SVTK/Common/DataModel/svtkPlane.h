 ...
 *
 * svtkPlane provides methods for various plane computations. These include
 * projecting points onto a plane, evaluating the plane equation, and
 * returning plane normal. svtkPlane is a concrete implementation of the
 * abstract class svtkImplicitFunction.
 */
...
class SVTKCOMMONDATAMODEL_EXPORT svtkPlane : public svtkImplicitFunction
{
public:
  /**
   * Construct plane passing through origin and normal to z-axis.
   */
  static svtkPlane* New();

  svtkTypeMacro(svtkPlane, svtkImplicitFunction);
  ...
  void EvaluateFunction(svtkDataArray* input, svtkDataArray* output) override;
   ...
   * this instance of svtkPlane (and its associated origin and normal).
   */
...
private:
  svtkPlane(const svtkPlane&) = delete;
  void operator=(const svtkPlane&) = delete;
};

// Generally the normal should be normalized
inline double svtkPlane::Evaluate(double normal[3], double origin[3], double x[3])
{
...
inline double svtkPlane::DistanceToPlane(double x[3], double n[3], double p0[3])
{
#define svtkPlaneAbs(x) ((x) < 0 ? -(x) : (x))
  return (svtkPlaneAbs(n[0] * (x[0] - p0[0]) + n[1] * (x[1] - p0[1]) + n[2] * (x[2] - p0[2])));
}
