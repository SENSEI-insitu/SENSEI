...
int svtkStructuredData::GetDataDimension(int dataDescription)
{
...
int svtkStructuredData::GetDataDescription(int dims[3])
{
  ...
  return svtkStructuredData::SetDimensions(dims, tempDims);
}

//------------------------------------------------------------------------------
// Given the extent, returns the data description given the dimensions
// (eg. SVTK_SINGLE_POINT,SVTK_X_LINE, SVTK_XY_PLANE etc.)
int svtkStructuredData::GetDataDescriptionFromExtent(int ext[6])
{
  int dims[3];
  svtkStructuredExtent::GetDimensions(ext, dims);
  return (svtkStructuredData::GetDataDescription(dims));
}
...
int svtkStructuredData::SetDimensions(int inDim[3], int dim[3])
{
  int dataDim, i;
  int dataDescription = SVTK_UNCHANGED;
  ...
  if (inDim[0] != dim[0] || inDim[1] != dim[1] || inDim[2] != dim[2])
  {
    ...
    if (dataDim == 3)
    {
      dataDescription = SVTK_XYZ_GRID;
    }
    else if (dataDim == 2)
    {
      if (inDim[0] == 1)
      {
        dataDescription = SVTK_YZ_PLANE;
      }
      else if (inDim[1] == 1)
      {
        dataDescription = SVTK_XZ_PLANE;
      }
      else
      {
        dataDescription = SVTK_XY_PLANE;
      }
    }
    else if (dataDim == 1)
    {
      if (inDim[0] != 1)
      {
        dataDescription = SVTK_X_LINE;
      }
      else if (inDim[1] != 1)
      {
        dataDescription = SVTK_Y_LINE;
      }
      else
      {
        dataDescription = SVTK_Z_LINE;
      }
    }
    else
    {
      dataDescription = SVTK_SINGLE_POINT;
    }
  }
...
// improperly specified a -1 is returned. If the dimensions are unchanged, a
// value of 100 is returned.
int svtkStructuredData::SetExtent(int inExt[6], int ext[6])
{
  int dataDim, i;
  int dataDescription;
...
void svtkStructuredData::GetCellPoints(
  svtkIdType cellId, svtkIdList* ptIds, int dataDescription, int dim[3])
{
  int loc[3];
  svtkIdType idx, npts;
  int iMin, iMax, jMin, jMax, kMin, kMax;
  svtkIdType d01 = static_cast<svtkIdType>(dim[0]) * dim[1];
  ...
  switch (dataDescription)
  {
    ...
    case SVTK_XYZ_GRID:
      ...
      jMax = jMin + 1;
      kMin = cellId / (static_cast<svtkIdType>(dim[0] - 1) * (dim[1] - 1));
      kMax = kMin + 1;
      break;
    default:
  ...
  for (npts = 0, loc[2] = kMin; loc[2] <= kMax; loc[2]++)
  {
    for (loc[1] = jMin; loc[1] <= jMax; loc[1]++)
    {
      ...
      {
        idx = loc[0] + loc[1] * static_cast<svtkIdType>(dim[0]) + loc[2] * d01;
...
void svtkStructuredData::GetPointCells(svtkIdType ptId, svtkIdList* cellIds, int dim[3])
{
  svtkIdType cellDim[3];
  ...
  ptLoc[2] = ptId / (static_cast<svtkIdType>(dim[0]) * dim[1]);
...
//------------------------------------------------------------------------------
void svtkStructuredData::GetCellNeighbors(
  svtkIdType cellId, svtkIdList* ptIds, svtkIdList* cellIds, int dim[3], int seedLoc[3])
{
  ...
  svtkIdType id0 = seedLoc[0] + seedLoc[1] * dim[0] + seedLoc[2] * dim[0] * dim[1];
  ...
  for (int j = 0; j < 8; j++)
  {
    ...
    if (i >= 3) // add cell
    {
      svtkIdType id = cellLoc[0] + cellLoc[1] * cellDim[0] + cellLoc[2] * cellDim[0] * cellDim[1];
      if (id != cellId)
      {
...
//------------------------------------------------------------------------------
void svtkStructuredData::GetCellNeighbors(
  svtkIdType cellId, svtkIdList* ptIds, svtkIdList* cellIds, int dim[3])
{
  int j, seedLoc[3], ptLoc[3], cellLoc[3];
  svtkIdType cellDim[3];
  int offset[8][3];
  svtkIdType numPts = ptIds->GetNumberOfIds(), id, i;
  ...
  for (i = 1; i < numPts; i++)
  {
    ...
    ptLoc[2] = id / (static_cast<svtkIdType>(dim[0]) * dim[1]);
