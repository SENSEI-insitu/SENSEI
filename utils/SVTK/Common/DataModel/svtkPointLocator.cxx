...
svtkStandardNewMacro(svtkPointLocator);

static const int SVTK_INITIAL_SIZE = 1000;
...
public:
  svtkNeighborPoints()
  {
    ...
    this->MaxSize = SVTK_INITIAL_SIZE;
  }
  ~svtkNeighborPoints()
  {
    this->Count = 0;
...
protected:
  ...
  int InitialBuffer[SVTK_INITIAL_SIZE * 3];
  int* P;
  int Count;
  int MaxSize;
};
...
// 3 points per bucket.
svtkPointLocator::svtkPointLocator()
{
  this->Points = nullptr;
...
//-----------------------------------------------------------------------------
void svtkPointLocator::Initialize()
{
  if (this->Points)
  {
    this->Points->UnRegister(this);
...
//-----------------------------------------------------------------------------
void svtkPointLocator::FreeSearchStructure()
{
  svtkIdList* ptIds;
  svtkIdType i;
...
void svtkPointLocator::ComputePerformanceFactors()
{
  this->HX = this->H[0];
  this->HY = this->H[1];
  this->HZ = this->H[2];
...
svtkIdType svtkPointLocator::FindClosestPoint(const double x[3])
{
  int i, j;
  double minDist2;
  ...
  double pt[3];
  int level;
  svtkIdType ptId, closest, cno, nids;
  svtkIdList* ptIds;
  int ijk[3], *nei;
  svtkNeighborPoints buckets;
...
//-----------------------------------------------------------------------------
svtkIdType svtkPointLocator::FindClosestPointWithinRadius(
  ...
  double radius, const double x[3], double inputDataLength, double& dist2)
{
  int i, j;
  double pt[3];
  svtkIdType ptId, nids, closest = -1;
  svtkIdList* ptIds;
  int ijk[3], *nei;
  double minDist2;
  ...
  svtkPointSet* pointSet = static_cast<svtkPointSet*>(this->DataSet);
  if (!pointSet)
  {
    return closest;
  }
  svtkPoints* points = pointSet->GetPoints();
  if (!points)
  {
    return closest;
  }
...
//-----------------------------------------------------------------------------
void svtkPointLocator::FindDistributedPoints(
  int N, double x, double y, double z, svtkIdList* result, int M)
{
  double p[3];
  p[0] = x;
  p[1] = y;
...
//-----------------------------------------------------------------------------
void svtkPointLocator::FindDistributedPoints(int N, const double x[3], svtkIdList* result, int M)
{
  int i, j;
  double dist2;
  double pt[3];
  int level;
  svtkIdType ptId, nids, cno;
  svtkIdList* ptIds;
  int ijk[3], *nei;
  int oct;
  int pointsChecked = 0;
...
//-----------------------------------------------------------------------------
void svtkPointLocator::FindClosestNPoints(int N, const double x[3], svtkIdList* result)
{
  int i, j;
  double dist2;
  double pt[3];
...
//-----------------------------------------------------------------------------
void svtkPointLocator::FindPointsWithinRadius(double R, const double x[3], svtkIdList* result)
{
  int i, j;
  double dist2;
  double pt[3];
...
//
void svtkPointLocator::BuildLocator()
{
  int ndivs[3];
  svtkIdType idx;
  svtkIdList* bucket;
  ...
  {
    svtkErrorMacro(<< "No points to subdivide");
    return;
  }
  //
  ...
  this->HashTable = new svtkIdListPtr[numBuckets];
  memset(this->HashTable, 0, numBuckets * sizeof(svtkIdListPtr));
...
//
void svtkPointLocator::GetBucketNeighbors(
...
void svtkPointLocator::GetOverlappingBuckets(
...
// Internal method to find those buckets that are within distance specified
// only those buckets outside of level radiuses of ijk are returned
void svtkPointLocator::GetOverlappingBuckets(svtkNeighborPoints* buckets, const double x[3],
  double dist, int prevMinLevel[3], int prevMaxLevel[3])
{
  int i, j, k, nei[3], minLevel[3], maxLevel[3];
  svtkIdType kFactor, jFactor;
...
int svtkPointLocator::InitPointInsertion(svtkPoints* newPts, const double bounds[6])
{
...
// point coordinates into which incremental insertion methods place their
// data. Bounds are the box that the points lie in.
int svtkPointLocator::InitPointInsertion(
  svtkPoints* newPts, const double bounds[6], svtkIdType estNumPts)
{
  int i;
  int maxDivs;
  typedef svtkIdList* svtkIdListPtr;
  ...
  {
    numBuckets = static_cast<svtkIdType>(
  ...
  this->HashTable = new svtkIdListPtr[this->NumberOfBuckets];
  memset(this->HashTable, 0, this->NumberOfBuckets * sizeof(svtkIdListPtr));
  ...
  for (maxDivs = 0, hmin = SVTK_DOUBLE_MAX, i = 0; i < 3; i++)
  {
...
svtkIdType svtkPointLocator::InsertNextPoint(const double x[3])
{
  svtkIdType idx;
  svtkIdList* bucket;
...
void svtkPointLocator::InsertPoint(svtkIdType ptId, const double x[3])
{
  svtkIdType idx;
  svtkIdList* bucket;
...
// -1.
svtkIdType svtkPointLocator::IsInsertedPoint(const double x[3])
{
  int i, j, ijk[3];
  svtkNeighborPoints buckets;
  svtkIdType nids;
  ...
  for (lsvtk = 0; lsvtk <= this->InsertionLevel; lsvtk++)
  {
    this->GetBucketNeighbors(&buckets, ijk, this->Divisions, lsvtk);
    ...
    for (i = 0; i < buckets.GetNumberOfNeighbors(); i++)
    {
      ...
      if ((ptIds = this->HashTable[cno]) != nullptr)
      {
        ...
        for (j = 0; j < nids; j++)
        {
          ...
          if (svtkMath::Distance2BetweenPoints(x, pt) <= this->InsertionTol2)
          {
            return ptId;
          }
        }
...
//-----------------------------------------------------------------------------
int svtkPointLocator::InsertUniquePoint(const double x[3], svtkIdType& id)
{
  svtkIdType ptId;
...
svtkIdType svtkPointLocator::FindClosestInsertedPoint(const double x[3])
{
  int i;
  double minDist2, dist2;
  double pt[3];
  int level;
  svtkIdType closest, j;
  svtkIdType ptId, nids, cno;
  svtkIdList* ptIds;
  int ijk[3], *nei;
  int MULTIPLES;
  ...
  //  until closest point found.
  //
  for (closest = 0, minDist2 = SVTK_DOUBLE_MAX, level = 0; (closest == 0) &&
       (level < this->Divisions[0] || level < this->Divisions[1] || level < this->Divisions[2]);
       level++)
  {
    this->GetBucketNeighbors(&buckets, ijk, this->Divisions, level);
...
svtkIdList* svtkPointLocator::GetPointsInBucket(const double x[3], int ijk[3])
{
  int i;
...
void svtkPointLocator::GenerateRepresentation(int svtkNotUsed(level), svtkPolyData* pd)
{
  svtkPoints* pts;
  svtkCellArray* polys;
  ...
  {
    svtkErrorMacro(<< "Can't build representation...no data!");
    return;
  }
...
//-----------------------------------------------------------------------------
void svtkPointLocator::GenerateFace(
  int face, int i, int j, int k, svtkPoints* pts, svtkCellArray* polys)
{
  svtkIdType ids[4];
  double origin[3], x[3];
...
//
double svtkPointLocator::Distance2ToBucket(const double x[3], const int nei[3])
{
  double bounds[6];
...
// WARNING!!!!! Be very careful altering this routine.  Simple changes to this
// routine can make is 25% slower!!!!
double svtkPointLocator::Distance2ToBounds(const double x[3], const double bounds[6])
{
  double distance;
  double deltas[3];
...
//-----------------------------------------------------------------------------
void svtkPointLocator::PrintSelf(ostream& os, svtkIndent indent)
{
