/*=========================================================================
  ...
  Module:    svtkImplicitSelectionLoop.cxx
...
=========================================================================*/
#include "svtkImplicitSelectionLoop.h"
...
svtkStandardNewMacro(svtkImplicitSelectionLoop);
svtkCxxSetObjectMacro(svtkImplicitSelectionLoop, Loop, svtkPoints);

//----------------------------------------------------------------------------
// Instantiate object with no initial loop.
svtkImplicitSelectionLoop::svtkImplicitSelectionLoop()
{
  this->Loop = nullptr;
...
//----------------------------------------------------------------------------
#define SVTK_DELTA 0.0001
// Generate plane equations only once to avoid a lot of extra work
void svtkImplicitSelectionLoop::Initialize()
{
  int i, numPts;
  double x[3], xProj[3];
  ...
  if (this->AutomaticNormalGeneration)
  {
    ...
    svtkPolygon::ComputeNormal(this->Loop, this->Normal);
    if (this->Normal[0] == 0.0 && this->Normal[1] == 0.0 && this->Normal[2] == 0.0)
    {
      svtkErrorMacro(<< "Cannot determine inside/outside of loop");
    }
  }
  ...
  {
    this->Loop->GetPoint(i, x);
    svtkPlane::ProjectPoint(x, this->Origin, this->Normal, xProj);
    this->Polygon->Points->SetPoint(i, xProj);
  }

  this->Polygon->GetBounds(this->Bounds);
  this->DeltaX = SVTK_DELTA * (this->Bounds[1] - this->Bounds[0]);
  this->DeltaY = SVTK_DELTA * (this->Bounds[3] - this->Bounds[2]);
  this->DeltaZ = SVTK_DELTA * (this->Bounds[5] - this->Bounds[4]);
  this->InitializationTime.Modified();
}

//----------------------------------------------------------------------------
// Evaluate plane equations. Return smallest absolute value.
double svtkImplicitSelectionLoop::EvaluateFunction(double x[3])
{
  int i, numPts;
  double xProj[3];
  ...
  if (xProj[0] >= this->Bounds[0] && xProj[0] <= this->Bounds[1] && xProj[1] >= this->Bounds[2] &&
    ...
    this->Polygon->PointInPolygon(xProj, numPts,
      svtkArrayDownCast<svtkDoubleArray>(this->Polygon->Points->GetData())->GetPointer(0),
  ...
  for (minDist2 = SVTK_DOUBLE_MAX, i = 0; i < numPts; i++)
  {
    ...
    dist2 = svtkLine::DistanceToLine(xProj, p1, p2, t, closest);
    if (dist2 < minDist2)
    {
      minDist2 = dist2;
    }
...
// It's damn slow.
void svtkImplicitSelectionLoop::EvaluateGradient(double x[3], double n[3])
{
...
//----------------------------------------------------------------------------
svtkMTimeType svtkImplicitSelectionLoop::GetMTime()
{
...
//----------------------------------------------------------------------------
void svtkImplicitSelectionLoop::PrintSelf(ostream& os, svtkIndent indent)
{
