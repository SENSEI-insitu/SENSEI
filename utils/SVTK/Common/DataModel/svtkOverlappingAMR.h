 ...
 * @brief   hierarchical dataset of svtkUniformGrids
 *
 *
 * svtkOverlappingAMR extends svtkUniformGridAMR by exposing access to the
 * amr meta data, which stores all structural information represented
 * by an svtkAMRInformation object
 *
 * @sa
 * svtkAMRInformation
 */

#ifndef svtkOverlappingAMR_h
#define svtkOverlappingAMR_h
...
class SVTKCOMMONDATAMODEL_EXPORT svtkOverlappingAMR : public svtkUniformGridAMR
{
public:
  static svtkOverlappingAMR* New();

  /**
   * Return class name of data type (see svtkType.h for definitions).
   */
  int GetDataObjectType() override { return SVTK_OVERLAPPING_AMR; }

  svtkTypeMacro(svtkOverlappingAMR, svtkUniformGridAMR);
  ...
  const svtkAMRBox& GetAMRBox(unsigned int level, unsigned int id);
  //@}
  ...
  static svtkInformationIdTypeKey* NUMBER_OF_BLANKED_POINTS();

  /**
   * Retrieve an instance of this class from an information object.
   */
  static svtkOverlappingAMR* GetData(svtkInformation* info)
  {
    return svtkOverlappingAMR::SafeDownCast(Superclass::GetData(info));
  }
  static svtkOverlappingAMR* GetData(svtkInformationVector* v, int i = 0)
  {
    return svtkOverlappingAMR::SafeDownCast(Superclass::GetData(v, i));
  }
   ...
   * the blocks (svtkUniformGrid) does not match the one described
   ...
   */
  int GetRefinementRatio(svtkCompositeDataIterator* iter);
   ...
   */
  svtkAMRInformation* GetAMRInfo() override { return Superclass::GetAMRInfo(); }
  void SetAMRInfo(svtkAMRInformation* info) override { return Superclass::SetAMRInfo(info); }
...
protected:
  svtkOverlappingAMR();
  ~svtkOverlappingAMR() override;
  //@}

private:
  svtkOverlappingAMR(const svtkOverlappingAMR&) = delete;
  void operator=(const svtkOverlappingAMR&) = delete;
};
