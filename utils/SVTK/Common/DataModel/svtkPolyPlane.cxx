...
//----------------------------------------------------------------------------
svtkStandardNewMacro(svtkPolyPlane);
svtkCxxSetObjectMacro(svtkPolyPlane, PolyLine, svtkPolyLine);

//----------------------------------------------------------------------------
svtkPolyPlane::svtkPolyPlane()
{
...
//----------------------------------------------------------------------------
svtkMTimeType svtkPolyPlane::GetMTime()
{
  svtkMTimeType mTime = this->Superclass::GetMTime();

  if (this->PolyLine)
  {
    svtkMTimeType p1Time;
...
//----------------------------------------------------------------------------
void svtkPolyPlane::ComputeNormals()
{
  if (!this->PolyLine)
  {
    return;
  ...
  if (this->GetMTime() > this->NormalComputeTime.GetMTime())
  {
    ...
    svtkPoints* points = this->PolyLine->GetPoints();
    const svtkIdType nPoints = points->GetNumberOfPoints();
    const svtkIdType nLines = nPoints - 1;

    // Allocate an array to store the normals

    this->Normals = svtkDoubleArray::New();
    ...
    for (int pIdx = 0; pIdx < nLines; ++pIdx)
    {
      ...
      svtkMath::Cross(v1, this->ExtrusionDirection, n);
...
double svtkPolyPlane::EvaluateFunction(double x[3])
{
  // Sanity check
  ...
  double minDistance2 = SVTK_DOUBLE_MAX, distance2, signedDistance = SVTK_DOUBLE_MAX, sign = 1;
  ...
  for (int pIdx = 0; pIdx < nLines; ++pIdx)
  {
    ...
    distance2 = svtkLine::DistanceToLine(xFlat, p1, p2, t, closest);
    ...
    else if (t < 0)
    {
      ...
      distance2 = svtkMath::Distance2BetweenPoints(p1, xFlat);
    ...
    else if (t > 1)
    {
      ...
      distance2 = svtkMath::Distance2BetweenPoints(p2, xFlat);
...
// Y Axis.
void svtkPolyPlane::EvaluateGradient(double svtkNotUsed(x)[3], double n[3])
{
  n[0] = 0;
  n[1] = 1;
  n[2] = 0;
...
//----------------------------------------------------------------------------
void svtkPolyPlane::PrintSelf(ostream& os, svtkIndent indent)
{
