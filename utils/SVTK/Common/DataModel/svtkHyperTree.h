...
#ifndef svtkHyperTree_h
#define svtkHyperTree_h
...
struct svtkHyperTreeData
{
  ...
  // of cells.
  svtkIdType GlobalIndexStart;
};

//=============================================================================
class SVTKCOMMONDATAMODEL_EXPORT svtkHyperTree : public svtkObject
{
public:
  svtkTypeMacro(svtkHyperTree, svtkObject);
   ...
   * this hypertree which is a svtkBitArray.
   */
  virtual void InitializeForReader(svtkIdType numberOfLevels, svtkIdType nbVertices,
    svtkIdType nbVerticesOfLastLevel, svtkBitArray* isParent, svtkBitArray* isMasked,
    svtkBitArray* outIsMasked) = 0;
   ...
   * @param isParent: modify the svtkBitArray, a binary decomposition
   ...
   * @param isMasked: modify the svtkBitArray, a binary mask
   * corresponding. It is useless to declare all the latest values
   * to False.
   */
  virtual void GetByLevelForWriter(svtkBitArray* inIsMasked, svtkUnsignedLongArray* nbVerticesbyLevel,
    svtkBitArray* isParent, svtkBitArray* isMasked, svtkIdList* ids) = 0;
   ...
   */
  void CopyStructure(svtkHyperTree* ht);
   ...
   */
  virtual svtkHyperTree* Freeze(const char* mode) = 0;
   ...
   */
  void SetTreeIndex(svtkIdType treeIndex) { this->Datas->TreeIndex = treeIndex; }
  svtkIdType GetTreeIndex() const { return this->Datas->TreeIndex; }
  //@}
   ...
   */
  svtkIdType GetNumberOfVertices() const { return this->Datas->NumberOfVertices; }

  /**
   * Return the number of nodes (coarse) in the tree.
   */
  svtkIdType GetNumberOfNodes() const { return this->Datas->NumberOfNodes; }

  /**
   * Return the number of leaf (fine) in the tree.
   */
  svtkIdType GetNumberOfLeaves() const
  {
   ...
   */
  svtkIdType GetNumberOfChildren() const { return this->NumberOfChildren; }
   ...
   * the symetric filter.
   */
  std::shared_ptr<svtkHyperTreeGridScales> InitializeScales(
   ...
   */
  SVTK_NEWINSTANCE
  static svtkHyperTree* CreateInstance(unsigned char branchFactor, unsigned char dimension);
  /**
   ...
   */
  virtual void SetGlobalIndexStart(svtkIdType start) = 0;

  /**
   * Get the start global index for the current tree for implicit global
   * index mapping.
   */
  svtkIdType GetGlobalIndexStart() const { return this->Datas->GlobalIndexStart; }
   ...
   */
  virtual void SetGlobalIndexFromLocal(svtkIdType index, svtkIdType global) = 0;
   ...
   */
  virtual svtkIdType GetGlobalIndexFromLocal(svtkIdType index) const = 0;

  /**
   * Return the maximum value reached by global index mapping
   * (implicit or explicit).
   */
  virtual svtkIdType GetGlobalNodeIndexMax() const = 0;

  /**
   * Return if a vertice identified by index in tree as being leaf.
   * \pre not_valid_index
   */
  virtual bool IsLeaf(svtkIdType index) const = 0;
   ...
   * \pre not_leaf
   */
  virtual void SubdivideLeaf(svtkIdType index, unsigned int level) = 0;
   ...
   */
  virtual bool IsTerminalNode(svtkIdType index) const = 0;
   ...
   * Public only for entry: svtkHyperTreeGridEntry,
   * svtkHyperTreeGridGeometryEntry, svtkHyperTreeGridGeometryLevelEntry
   */
  virtual svtkIdType GetElderChildIndex(unsigned int index_parent) const = 0;
   ...
   */
  void SetScales(std::shared_ptr<svtkHyperTreeGridScales> scales) const { this->Scales = scales; }
  //@}
   ...
   * Return all scales.
   */
  std::shared_ptr<svtkHyperTreeGridScales> GetScales() const
  {
...
protected:
  ...
  virtual ~svtkHyperTree() override {}

  virtual void InitializePrivate() = 0;
  virtual void PrintSelfPrivate(ostream& os, svtkIndent indent) = 0;
  virtual void CopyStructurePrivate(svtkHyperTree* ht) = 0;
  ...
  //-- Local information
  std::shared_ptr<svtkHyperTreeData> Datas;
...
private:
  svtkHyperTree(const svtkHyperTree&) = delete;
  void operator=(const svtkHyperTree&) = delete;
};
