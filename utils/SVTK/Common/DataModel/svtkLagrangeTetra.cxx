...
svtkStandardNewMacro(svtkLagrangeTetra);
//----------------------------------------------------------------------------
svtkLagrangeTetra::svtkLagrangeTetra()
  : svtkHigherOrderTetra()
{
}

//----------------------------------------------------------------------------
svtkLagrangeTetra::~svtkLagrangeTetra() = default;

void svtkLagrangeTetra::PrintSelf(ostream& os, svtkIndent indent)
{
  this->Superclass::PrintSelf(os, indent);
}

svtkCell* svtkLagrangeTetra::GetEdge(int edgeId)
{
...
svtkCell* svtkLagrangeTetra::GetFace(int faceId)
{
  svtkLagrangeTriangle* result = FaceCell;
  const auto set_number_of_ids_and_points = [&](const svtkIdType& npts) -> void {
    result->Points->SetNumberOfPoints(npts);
    result->PointIds->SetNumberOfIds(npts);
  };
...
//----------------------------------------------------------------------------
void svtkLagrangeTetra::InterpolateFunctions(const double pcoords[3], double* weights)
{
  ...
  else
  {
    ...
    for (svtkIdType idx = 0; idx < nPoints; idx++)
    {
      ...
      weights[idx] = (svtkLagrangeTriangle::eta(n, lambda[0], tau[0]) *
        svtkLagrangeTriangle::eta(n, lambda[1], tau[1]) *
        svtkLagrangeTriangle::eta(n, lambda[2], tau[2]) *
        svtkLagrangeTriangle::eta(n, lambda[3], tau[3]));
    }
  }
}

//----------------------------------------------------------------------------
void svtkLagrangeTetra::InterpolateDerivs(const double pcoords[3], double* derivs)
{
  ...
  else
  {
    ...
    for (svtkIdType idx = 0; idx < nPoints; idx++)
    {
      ...
      double eta_alpha = svtkLagrangeTriangle::eta(n, lambda[0], tau[0]);
      double eta_beta = svtkLagrangeTriangle::eta(n, lambda[1], tau[1]);
      double eta_gamma = svtkLagrangeTriangle::eta(n, lambda[2], tau[2]);
      double eta_delta = svtkLagrangeTriangle::eta(n, lambda[3], tau[3]);

      double d_eta_alpha = svtkLagrangeTriangle::d_eta(n, lambda[0], tau[0]);
      double d_eta_beta = svtkLagrangeTriangle::d_eta(n, lambda[1], tau[1]);
      double d_eta_gamma = svtkLagrangeTriangle::d_eta(n, lambda[2], tau[2]);
      double d_eta_delta = svtkLagrangeTriangle::d_eta(n, lambda[3], tau[3]);
...
svtkHigherOrderCurve* svtkLagrangeTetra::getEdgeCell()
{
  return EdgeCell;
}
svtkHigherOrderTriangle* svtkLagrangeTetra::getFaceCell()
{
  return FaceCell;
}
