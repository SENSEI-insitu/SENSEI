...
svtkStandardNewMacro(svtkSphere);

//----------------------------------------------------------------------------
// Construct sphere with center at (0,0,0) and radius=0.5.
svtkSphere::svtkSphere()
{
  this->Radius = 0.5;
...
double svtkSphere::EvaluateFunction(double x[3])
{
...
void svtkSphere::EvaluateGradient(double x[3], double n[3])
{
...
//
#define SVTK_ASSIGN_POINT(_x, _y)                                                                   \
  {                                                                                                \
    _x[0] = _y[0];                                                                                 \
    _x[1] = _y[1];                                                                                 \
    _x[2] = _y[2];                                                                                 \
...
// bounding sphere.
template <class T>
void svtkSphereComputeBoundingSphere(T* pts, svtkIdType numPts, T sphere[4], svtkIdType hints[2])
{
  ...
  {
    p = pts + 3 * hints[0];
    SVTK_ASSIGN_POINT(d1, p);
    p = pts + 3 * hints[1];
    SVTK_ASSIGN_POINT(d2, p);
  }
  else // no hints provided, compute an initial guess
  {
    T xMin[3], xMax[3], yMin[3], yMax[3], zMin[3], zMax[3];
    xMin[0] = xMin[1] = xMin[2] = SVTK_FLOAT_MAX;
    yMin[0] = yMin[1] = yMin[2] = SVTK_FLOAT_MAX;
    zMin[0] = zMin[1] = zMin[2] = SVTK_FLOAT_MAX;
    xMax[0] = xMax[1] = xMax[2] = -SVTK_FLOAT_MAX;
    yMax[0] = yMax[1] = yMax[2] = -SVTK_FLOAT_MAX;
    zMax[0] = zMax[1] = zMax[2] = -SVTK_FLOAT_MAX;
    ...
    {
      if (p[0] < xMin[0])
        SVTK_ASSIGN_POINT(xMin, p);
      if (p[0] > xMax[0])
        SVTK_ASSIGN_POINT(xMax, p);
      if (p[1] < yMin[1])
        SVTK_ASSIGN_POINT(yMin, p);
      if (p[1] > yMax[1])
        SVTK_ASSIGN_POINT(yMax, p);
      if (p[2] < zMin[2])
        SVTK_ASSIGN_POINT(zMin, p);
      if (p[2] > zMax[2])
        SVTK_ASSIGN_POINT(zMax, p);
    }
    ...
    if (xSpan > ySpan)
    {
      if (xSpan > zSpan)
      {
        SVTK_ASSIGN_POINT(d1, xMin);
        SVTK_ASSIGN_POINT(d2, xMax);
      }
      else
      {
        SVTK_ASSIGN_POINT(d1, zMin);
        SVTK_ASSIGN_POINT(d2, zMax);
      }
    }
    else // ySpan > xSpan
    {
      if (ySpan > zSpan)
      {
        SVTK_ASSIGN_POINT(d1, yMin);
        SVTK_ASSIGN_POINT(d2, yMax);
      }
      else
      {
        SVTK_ASSIGN_POINT(d1, zMin);
  ...
  T r2 = svtkMath::Distance2BetweenPoints(d1, d2) / 4.0;
  sphere[3] = sqrt(r2);
  ...
  for (p = pts, i = 0; i < numPts; ++i, p += 3)
  {
    dist2 = svtkMath::Distance2BetweenPoints(p, sphere);
    if (dist2 > r2)
    {
      dist = sqrt(dist2);
...
#undef SVTK_ASSIGN_POINT

#define SVTK_ASSIGN_SPHERE(_x, _y)                                                                  \
  {                                                                                                \
    _x[0] = _y[0];                                                                                 \
    _x[1] = _y[1];                                                                                 \
    _x[2] = _y[2];                                                                                 \
...
//----------------------------------------------------------------------------
template <class T>
void svtkSphereComputeBoundingSphere(
  T** spheres, svtkIdType numSpheres, T sphere[4], svtkIdType hints[2])
{
  if (numSpheres < 1)
  {
    sphere[0] = sphere[1] = sphere[2] = sphere[3] = 0.0;
  ...
  {
    SVTK_ASSIGN_SPHERE(sphere, spheres[0]);
    return;
  }
  ...
  {
    s = spheres[hints[0]];
    SVTK_ASSIGN_SPHERE(s1, s);
    s = spheres[hints[1]];
    SVTK_ASSIGN_SPHERE(s2, s);
  }
  else // no hints provided, compute an initial guess
  {
    ...
    for (i = 0; i < numSpheres; ++i)
    {
      ...
      if ((s[0] - s[3]) < (xMin[0] - xMin[3]))
        SVTK_ASSIGN_SPHERE(xMin, s);
      if ((s[0] + s[3]) > (xMax[0] + xMax[3]))
        SVTK_ASSIGN_SPHERE(xMax, s);
      if ((s[1] - s[3]) < (yMin[1] - yMin[3]))
        SVTK_ASSIGN_SPHERE(yMin, s);
      if ((s[1] + s[3]) > (yMax[1] + yMax[3]))
        SVTK_ASSIGN_SPHERE(yMax, s);
      if ((s[2] - s[3]) < (zMin[2] - zMin[3]))
        SVTK_ASSIGN_SPHERE(zMin, s);
      if ((s[2] + s[3]) > (zMax[2] + zMax[3]))
        SVTK_ASSIGN_SPHERE(zMax, s);
    }
    ...
    if (xSpan > ySpan)
    {
      if (xSpan > zSpan)
      {
        SVTK_ASSIGN_SPHERE(s1, xMin);
        SVTK_ASSIGN_SPHERE(s2, xMax);
      }
      else
      {
        SVTK_ASSIGN_SPHERE(s1, zMin);
        SVTK_ASSIGN_SPHERE(s2, zMax);
      }
    }
    else // ySpan > xSpan
    {
      if (ySpan > zSpan)
      {
        SVTK_ASSIGN_SPHERE(s1, yMin);
        SVTK_ASSIGN_SPHERE(s2, yMax);
      }
      else
      {
        SVTK_ASSIGN_SPHERE(s1, zMin);
  ...
  T tmp, v[3], r2 = svtkMath::Distance2BetweenPoints(s1, s2) / 4.0;
  sphere[3] = sqrt(r2);
  ...
  r2 = svtkMath::Distance2BetweenPoints(s1, s2) / 4.0;
  sphere[3] = sqrt(r2);
  ...
  for (i = 0; i < numSpheres; ++i)
  {
    s = spheres[i];
    sR2 = s[3] * s[3];
    dist2 = svtkMath::Distance2BetweenPoints(s, sphere);
...
{
  svtkSphereComputeBoundingSphere(spheres, numSpheres, sphere, hints);
}
//----------------------------------------------------------------------------
void svtkSphere::ComputeBoundingSphere(
...
//----------------------------------------------------------------------------
void svtkSphere::PrintSelf(ostream& os, svtkIndent indent)
{
