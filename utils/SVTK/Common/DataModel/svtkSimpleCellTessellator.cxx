/*=========================================================================
  ...
  Module:    svtkSimpleCellTessellator.cxx
...
static signed char svtkTessellatorTriangleCases[9][4][3] = {
...
// The last case is only a sentinel to avoid stepping out of table
// [case][tetra][vertex]
static signed char svtkTessellatorTetraCasesRight[65][8][4] = {
  // Index = 0, Case where no edges are split
...
//
static signed char svtkTessellatorTetraCasesLeft[65][8][4] = {
  // Index = 0, Case where no edges are split
  { { 0, 1, 2, 3 }, NO_TETRA, NO_TETRA, NO_TETRA, NO_TETRA, NO_TETRA, NO_TETRA, NO_TETRA },
  // Index = 1, Case where edges: 4 are split
  { { 0, 2, 3, 4 }, { 1, 2, 4, 3 }, NO_TETRA, NO_TETRA, NO_TETRA, NO_TETRA, NO_TETRA, NO_TETRA },
...
svtkStandardNewMacro(svtkSimpleCellTessellator);
//-----------------------------------------------------------------------------
//
// svtkTriangleTile
//
...
public:
  ...
  void SetPointId(int i, svtkIdType id) { this->PointId[i] = id; }

  void SetPointIds(svtkIdType id[3])
  {
    this->PointId[0] = id[0];
    this->PointId[1] = id[1];
    this->PointId[2] = id[2];
  ...
  svtkIdType GetPointId(int i) { return this->PointId[i]; }

  // Return true if (e1, e2) is an edge of the tri:
  svtkTypeBool IsAnEdge(svtkIdType e1, svtkIdType e2)
  {
    int sum = 0;
    for (int i = 0; i < 3; i++)
    {
  ...
  void CopyPoint(int i, svtkTriangleTile* source, int j)
  {
  ...
  //  svtkTriangleTile res[4]
  int Refine(svtkSimpleCellTessellator* tess, svtkTriangleTile* res);
...
public:
  ...
  void SetPointIds(svtkIdType id[4])
  {
    this->PointId[0] = id[0];
    this->PointId[1] = id[1];
    this->PointId[2] = id[2];
  ...
  // Description:
  // Copy point j of source into point i of the current tile.
  void CopyPoint(int i, svtkTetraTile* source, int j)
  {
  ...
  void CopyEdgeAndFaceIds(svtkTetraTile* source)
  {
    assert("pre: source_exists" && source != nullptr);
  ...
  // of all edges
  // svtkTetraTile res[8]
  int Refine(svtkSimpleCellTessellator* tess, svtkTetraTile* res);

  // Description:
  // Initialize the Edges and Faces arrays as for a root tetrahedron
  void SetOriginal(svtkIdType order[4],
    int* edgeIds, // 6
    int* faceIds) // 4
  {
...
//-----------------------------------------------------------------------------
int svtkTriangleTile::Refine(svtkSimpleCellTessellator* tess, svtkTriangleTile* res)
{
  // The output will contain a maximum of 4 svtkTriangleTiles
  int i, index;
  ...
  if (this->SubdivisionLevel < tess->GetMaxSubdivisionLevel())
  {
    ...
    if (index)
    {
      ...
      signed char* cases = **(svtkTessellatorTriangleCases + index);
...
// point data.
void svtkSimpleCellTessellator::CopyPoint(svtkIdType pointId)
{
  double point[3];
...
//-----------------------------------------------------------------------------
static void Reorder(svtkIdType in[4], svtkIdType order[4])
{
  ...
  svtkIdType min1 = in[0];
  svtkIdType min2 = in[1];
  svtkIdType idx1 = 0;
  svtkIdType idx2 = 1;
  for (int i = 1; i < 4; i++)
  {
...
//-----------------------------------------------------------------------------
int svtkTetraTile::Refine(svtkSimpleCellTessellator* tess, svtkTetraTile* res)
{
  // The output will contains a maximum of 8 svtkTetraTiles
  int i, index;
  ...
  if (this->SubdivisionLevel < tess->GetMaxSubdivisionLevel())
  {
    ...
    if (index)
    {
      ...
      {
        cases = **(svtkTessellatorTetraCasesRight + index);
      }
      else
      {
        cases = **(svtkTessellatorTetraCasesLeft + index);
      }
...
//
svtkSimpleCellTessellator::svtkSimpleCellTessellator()
{
  ...
  this->EdgeTable = svtkGenericEdgeTable::New();
  ...
  this->TriangleIds = svtkIdList::New();
  this->TriangleIds->Allocate(SVTK_CELL_SIZE);
}
...
void svtkSimpleCellTessellator::InsertPointsIntoEdgeTable(svtkTriangleTile& tri)
{
  double global[3];
...
//-----------------------------------------------------------------------------
//
void svtkSimpleCellTessellator::InsertEdgesIntoEdgeTable(svtkTriangleTile& tri)
{
  double* local = nullptr;
  svtkIdType tmp;
  svtkIdType l, r;
  svtkIdType cellId = this->GenericCell->GetId();
  ...
  for (int j = 0; j < 3; j++)
  {
    ...
    svtkIdType leftId = tri.GetPointId(l);
    svtkIdType rightId = tri.GetPointId(r);
...
//-----------------------------------------------------------------------------
//
void svtkSimpleCellTessellator::InsertEdgesIntoEdgeTable(svtkTetraTile& tetra)
{
  double* local = nullptr;

  svtkIdType tmp;
  svtkIdType l, r;
  const svtkIdType cellId = this->GenericCell->GetId();
  ...
  for (int j = 0; j < 6; j++)
  {
    ...
    svtkIdType leftId = tetra.GetPointId(l);
    svtkIdType rightId = tetra.GetPointId(r);
    ...
    // svtkDebugMacro( << "InsertEdgesIntoEdgeTable:" << leftId << "," << rightId );
    ...
    else
    {
      ...
      // svtkDebugMacro( << "IncrementEdgeReferenceCount:" << ptId );
...
//-----------------------------------------------------------------------------
void svtkSimpleCellTessellator::RemoveEdgesFromEdgeTable(svtkTriangleTile& tri)
{
  svtkIdType l, r;
  int i;
...
//-----------------------------------------------------------------------------
void svtkSimpleCellTessellator::RemoveEdgesFromEdgeTable(svtkTetraTile& tetra)
{
  svtkIdType l, r;
  int i;
  ...
  for (i = 0; i < 6; i++)
  {
    ...
    svtkIdType ll = tetra.GetPointId(l);
    svtkIdType rr = tetra.GetPointId(r);
...
//-----------------------------------------------------------------------------
void svtkSimpleCellTessellator::Reset()
{
...
void svtkSimpleCellTessellator::Initialize(svtkGenericDataSet* ds)
{
  this->DataSet = ds;
...
//-----------------------------------------------------------------------------
void svtkSimpleCellTessellator::Tessellate(svtkGenericAdaptorCell* cell,
  svtkGenericAttributeCollection* att, svtkDoubleArray* points, svtkCellArray* cellArray,
  svtkPointData* internalPd)
{
  ...
  int complexCell = cell->GetType() != SVTK_HIGHER_ORDER_TETRAHEDRON;
  ...
  std::queue<svtkTetraTile> work;
  svtkTetraTile roots[10]; // up to 10 top-level sub-tetra
  ...
  if (complexCell)
  {
    ...
    while (this->Connectivity->GetNextCell(npts, pts))
    {
      ...
      //      int edgeIds[6];
      const svtkIdType* originalEdge;
      svtkIdType edge[2];
      j = 0;
      while (j < 6)
      {
        edge[0] = pts[svtkTetra::GetEdgeArray(j)[0]];
        edge[1] = pts[svtkTetra::GetEdgeArray(j)[1]];
        int k = 0;
        edgeIds[j] = -1;
      ...
      //      int faceIds[4];
      const svtkIdType* originalFace;
      svtkIdType face[3];
      j = 0;
      while (j < 4)
      {
        face[0] = pts[svtkTetra::GetFaceArray(j)[0]];
        face[1] = pts[svtkTetra::GetFaceArray(j)[1]];
        face[2] = pts[svtkTetra::GetFaceArray(j)[2]];
        int k = 0;
        faceIds[j] = -1;
  ...
  {
    svtkIdType pts[4] = { 0, 1, 2, 3 }; // from sub-tetra tessellation
    ...
    {
      edge[0] =
        svtkTetra::GetEdgeArray(j)[0]; // faster that edge[0]=pts[svtkTetra::GetEdgeArray(j)[0]]
      edge[1] = svtkTetra::GetEdgeArray(j)[1];
      int k = 0;
      edgeIds[j] = -1;
      while (edgeIds[j] == -1)
      {
...
//-----------------------------------------------------------------------------
void svtkSimpleCellTessellator::InitTetraTile(
...
//-----------------------------------------------------------------------------
void svtkSimpleCellTessellator::PrintSelf(ostream& os, svtkIndent indent)
{
...
//-----------------------------------------------------------------------------
void svtkSimpleCellTessellator::TessellateFace(svtkGenericAdaptorCell* cell,
  svtkGenericAttributeCollection* att, svtkIdType index, svtkDoubleArray* points,
  svtkCellArray* cellArray, svtkPointData* internalPd)
{
  ...
  {
    svtkIdType pts[3]; // from sub-tetra tessellation

    this->AllocatePointIds(4); // tetra
    cell->GetPointIds(this->PointIds);

    const svtkIdType* facepts = cell->GetFaceArray(index);
...
//-----------------------------------------------------------------------------
void svtkSimpleCellTessellator::Triangulate(svtkGenericAdaptorCell* cell,
  svtkGenericAttributeCollection* att, svtkDoubleArray* points, svtkCellArray* cellArray,
  svtkPointData* internalPd)
{
  assert("pre: cell_exists" && cell != nullptr);
  ...
  if (cell->GetType() != SVTK_HIGHER_ORDER_TRIANGLE)
  {
    // build a linear polygon, call tessellate() on it and iterate over each triangle
    // by sending it to the tessellator
  ...
  {
    svtkIdType pts[3] = { 0, 1, 2 };
    const svtkIdType edgeIds[3] = { 0, 1, 2 };
    this->AllocatePointIds(cell->GetNumberOfBoundaries(0));
    cell->GetPointIds(this->PointIds);
...
//-----------------------------------------------------------------------------
void svtkSimpleCellTessellator::TriangulateTriangle(svtkGenericAdaptorCell* cell, svtkIdType* localIds,
  svtkIdType* ids, const svtkIdType* edgeIds, svtkGenericAttributeCollection* att,
  svtkDoubleArray* points, svtkCellArray* cellArray, svtkPointData* internalPd)
{
  ...
  std::queue<svtkTriangleTile> work;
  svtkTriangleTile begin = svtkTriangleTile(root);
  work.push(begin);

  while (!work.empty())
  {
    svtkTriangleTile piece[4];
    svtkTriangleTile curr = work.front();
    work.pop();
...
int svtkSimpleCellTessellator::GetNumberOfCellsUsingEdge(int edgeId)
{
...
int svtkSimpleCellTessellator::GetNumberOfCellsUsingFace(int faceId)
{
#if SLOW_API
  int result = 0;
...
void svtkSimpleCellTessellator::AllocateScalars(int size)
{
...
int svtkSimpleCellTessellator::GetFixedSubdivisions()
{
...
int svtkSimpleCellTessellator::GetMaxSubdivisionLevel()
{
...
int svtkSimpleCellTessellator::GetMaxAdaptiveSubdivisions()
{
...
void svtkSimpleCellTessellator::SetFixedSubdivisions(int level)
{
...
void svtkSimpleCellTessellator::SetMaxSubdivisionLevel(int level)
{
...
void svtkSimpleCellTessellator::SetSubdivisionLevels(int fixed, int maxLevel)
{
...
// \pre positive_size: size>0
void svtkSimpleCellTessellator::AllocatePointIds(int size)
{
  assert("pre: positive_size" && size > 0);
  ...
  {
    delete[] this->PointIds;
    this->PointIds = new svtkIdType[size];
...
int svtkSimpleCellTessellator::FacesAreEqual(const svtkIdType* originalFace, const svtkIdType face[3])
{
