/*=========================================================================
  ...
  Module:    svtkStaticPointLocator2D.cxx
...
// 2) svtkSMPTools::Sort() is used to sort the index array. Note that the sort
...
// mistakenly linking the hidden, scoped classes (svtkNeighborBuckets) found
// in svtkPointLocator and svtkStaticPointLocator2D and causing weird faults.
...
struct svtkBucketList2D
{
  svtkStaticPointLocator2D* Locator; // locater
  svtkIdType NumPts;                 // the number of points to bucket
  ...
  // Construction
  svtkBucketList2D(svtkStaticPointLocator2D* loc, svtkIdType numPts, int numBuckets)
  {
    this->Locator = loc;
    this->NumPts = numPts;
  ...
  virtual ~svtkBucketList2D() = default;
  ...
  void GetBucketIndices(const double* x, int ij[2]) const
  {
    ...
    svtkIdType tmp0 = static_cast<svtkIdType>(((x[0] - bX) * fX));
    svtkIdType tmp1 = static_cast<svtkIdType>(((x[1] - bY) * fY));
...
struct NeighborBuckets2D
{
  ...
  int InitialBuffer[SVTK_INITIAL_BUCKET_SIZE * 2];
  int* P;
  svtkIdType Count;
  svtkIdType MaxSize;
  ...
  NeighborBuckets2D()
  {
    ...
    this->Count = 0;
    this->MaxSize = SVTK_INITIAL_BUCKET_SIZE;
  }
  ...
  int* GetPoint(svtkIdType i) { return this->P + 2 * i; }

  svtkIdType InsertNextBucket(const int x[2])
  {
    // Re-allocate if beyond the current max size.
    // (Increase by SVTK_INITIAL_BUCKET_SIZE)
    int* tmp;
    svtkIdType offset = this->Count * 2;
...
//
void svtkBucketList2D::GetBucketNeighbors(
  NeighborBuckets2D* buckets, const int ij[2], const int ndivs[2], int level)
{
...
//-----------------------------------------------------------------------------
void svtkBucketList2D::GenerateFace(
  int svtkNotUsed(face), int i, int j, int svtkNotUsed(k), svtkPoints* pts, svtkCellArray* polys)
{
  svtkIdType ids[4];
  double origin[3], x[3];
...
//
double svtkBucketList2D::Distance2ToBucket(const double x[3], const int nei[3])
{
  double bounds[6];
...
double svtkBucketList2D::Distance2ToBounds(const double x[3], const double bounds[6])
{
  double distance;
  double deltas[3];
  ...
  distance = svtkMath::Dot(deltas, deltas);
  return distance;
}
...
// to svtkSMPTools for threaded processesing.
template <typename TIds>
struct BucketList2D : public svtkBucketList2D
{
  ...
  // Construction
  BucketList2D(svtkStaticPointLocator2D* loc, svtkIdType numPts, int numBuckets)
    : svtkBucketList2D(loc, numPts, numBuckets)
  {
  ...
  svtkIdType FindClosestPoint(const double x[3]);
  svtkIdType FindClosestPointWithinRadius(
    double radius, const double x[3], double inputDataLength, double& dist2);
  void FindClosestNPoints(int N, const double x[3], svtkIdList* result);
  void FindPointsWithinRadius(double R, const double x[3], svtkIdList* result);
  int IntersectWithLine(double a0[3], double a1[3], double tol, double& t, double lineX[3],
    double ptX[3], svtkIdType& ptId);
  double FindCloseNBoundedPoints(int N, const double x[3], svtkIdList* result);

  void MergePoints(double tol, svtkIdType* pointMap);
  void GenerateRepresentation(int svtkNotUsed(level), svtkPolyData* pd);
  ...
  struct MapDataSet
  {
    ...
    MapDataSet(BucketList2D<T>* blist, svtkDataSet* ds)
      : BList(blist)
      , DataSet(ds)
    {
    }
  ...
  struct MergePrecise
  {
    ...
    MergePrecise(BucketList2D<T>* blist, svtkIdType* mergeMap)
      : BList(blist)
      , MergeMap(mergeMap)
    {
    ...
    void operator()(svtkIdType bucket, svtkIdType endBucket)
    {
      BucketList2D<T>* bList = this->BList;
      svtkIdType* mergeMap = this->MergeMap;
      int i, j;
      const LocatorTuple<TIds>* ids;
      double p[3], p2[3];
      svtkIdType ptId, ptId2, numIds;
  ...
  struct MergeClose
  {
    ...
    MergeClose(BucketList2D<T>* blist, double tol, svtkIdType* mergeMap)
      : BList(blist)
      , MergeMap(mergeMap)
      , Tol(tol)
    {
    ...
    void operator()(svtkIdType ptId, svtkIdType endPtId)
    {
      ...
      int i;
      double p[3];
      svtkIdType nearId, numIds;
      svtkIdList*& nearby = this->PIds.Local();
  ...
  void BuildLocator() override
  {
    ...
    //
    svtkPointSet* ps = static_cast<svtkPointSet*>(this->DataSet);
    int mapped = 0;
    if (ps)
...
template <typename TIds>
svtkIdType BucketList2D<TIds>::FindClosestPoint(const double x[3])
{
  int i, j;
  double minDist2;
  double dist2 = SVTK_DOUBLE_MAX;
  double pt[3];
  int closest, level;
  svtkIdType ptId, cno, numIds;
  int ij[2], *nei;
  ...
  //  closest point found.
  //
  for (closest = (-1), minDist2 = SVTK_DOUBLE_MAX, level = 0;
...
//-----------------------------------------------------------------------------
template <typename TIds>
svtkIdType BucketList2D<TIds>::FindClosestPointWithinRadius(
  double radius, const double x[3], double inputDataLength, double& dist2)
{
  int i, j;
  double pt[3];
  svtkIdType ptId, closest = -1;
  int ij[2], *nei;
  double minDist2;
  ...
  svtkDataArray* pointData = static_cast<svtkPointSet*>(this->DataSet)->GetPoints()->GetData();
  ...
  //
  svtkIdType numIds;
  svtkIdType cno = ij[0] + ij[1] * this->xD;
  if ((numIds = this->GetNumberOfIds(cno)) > 0)
  {
    ids = this->GetIds(cno);
    for (j = 0; j < numIds; j++)
...
struct IdTuple
{
  ...
  IdTuple(svtkIdType ptId, double dist2)
    : PtId(ptId)
    , Dist2(dist2)
  {
  }
...
//-----------------------------------------------------------------------------
template <typename TIds>
void BucketList2D<TIds>::FindClosestNPoints(int N, const double x[3], svtkIdList* result)
{
  int i = 0, j = 0;
  double dist2;
  double pt[3];
...
// touch.
template <typename TIds>
void BucketList2D<TIds>::FindPointsWithinRadius(double R, const double x[3], svtkIdList* result)
{
  double dist2;
  double pt[3];
  svtkIdType ptId, cno, numIds;
...
// svtkStaticCellLocator for references) and update the current voxels/bins at
...
int BucketList2D<TIds>::IntersectWithLine(double a0[3], double a1[3], double tol, double& t,
  double lineX[3], double ptX[3], svtkIdType& ptId)
{
  ...
  if (a0[2] != a1[2]) // not in locator plane
  {
    ...
    if (svtkPlane::IntersectWithLine(a0, a1, n, minPnt, t, lineX) && bbox.ContainsPoint(lineX))
    {
  ...
  svtkIdType idx, pId, bestPtId = (-1);
  double step[2], next[2], tMax[2], tDelta[2];
  double curPos[3], curT, tHit, tMin = SVTK_FLOAT_MAX;
  double tol2 = tol * tol;
  unsigned char* bucketHasBeenVisited = nullptr;
  svtkMath::Subtract(a1, a0, rayDir);
  ...
  if (svtkBox::IntersectBox(bounds, a0, rayDir, curPos, curT))
  {
    ...
    tMax[0] = (rayDir[0] != 0.0) ? (next[0] - curPos[0]) / rayDir[0] : SVTK_FLOAT_MAX;
    tMax[1] = (rayDir[1] != 0.0) ? (next[1] - curPos[1]) / rayDir[1] : SVTK_FLOAT_MAX;

    tDelta[0] = (rayDir[0] != 0.0) ? (h[0] / rayDir[0]) * step[0] : SVTK_FLOAT_MAX;
    tDelta[1] = (rayDir[1] != 0.0) ? (h[1] / rayDir[1]) * step[1] : SVTK_FLOAT_MAX;
          ...
          if (!bucketHasBeenVisited[idx])
          {
            ...
            if ((numPtsInBin = this->GetNumberOfIds(idx)) > 0) // there are some points here
            {
              ...
              for (ii = 0; ii < numPtsInBin; ii++)
              {
                ...
                if (svtkLine::DistanceToLine(x, a0, a1, tHit, xl) <= tol2 && t < tMin)
                {
                  tMin = t;
                  bestPtId = pId;
  ...
  if (bestPtId >= 0)
  {
    ...
    ptId = bestPtId;
    this->DataSet->GetPoint(ptId, ptX);
    svtkLine::DistanceToLine(ptX, a0, a1, t, lineX);
...
//-----------------------------------------------------------------------------
template <typename TIds>
double BucketList2D<TIds>::FindCloseNBoundedPoints(int N, const double x[3], svtkIdList* result)
{
  int i, j;
  double dist2;
  double pt[3];
...
template <typename TIds>
void BucketList2D<TIds>::MergePoints(double tol, svtkIdType* mergeMap)
{
  ...
  if (tol <= 0.0)
  {
    ...
    svtkSMPTools::For(0, this->NumBuckets, merge);
  }
  ...
  else
  {
    ...
    svtkSMPTools::For(0, this->NumPts, merge);
  }
}
...
template <typename TIds>
void BucketList2D<TIds>::GenerateRepresentation(int svtkNotUsed(level), svtkPolyData* pd)
{
  svtkPoints* pts;
  svtkCellArray* polys;
...
//-----------------------------------------------------------------------------
// Here is the SVTK class proper. It's implemented with the templated
// BucketList2D class.

//-----------------------------------------------------------------------------
// Construct with automatic computation of divisions, averaging
// 5 points per bucket.
svtkStaticPointLocator2D::svtkStaticPointLocator2D()
{
  this->NumberOfPointsPerBucket = 5;
  this->Divisions[0] = this->Divisions[1] = 50;
  this->H[0] = this->H[1] = 0.0;
  this->Buckets = nullptr;
  this->MaxNumberOfBuckets = SVTK_INT_MAX;
  this->LargeIds = false;
}
...
//-----------------------------------------------------------------------------
void svtkStaticPointLocator2D::Initialize()
{
  this->FreeSearchStructure();
}

//-----------------------------------------------------------------------------
void svtkStaticPointLocator2D::FreeSearchStructure()
{
  if (this->Buckets)
  {
    delete this->Buckets;
...
//
void svtkStaticPointLocator2D::BuildLocator()
{
  int ndivs[3];
  int i;
  svtkIdType numPts;
  ...
  svtkIdType numBuckets = static_cast<svtkIdType>(
  ...
  this->NumberOfBuckets = numBuckets =
    static_cast<svtkIdType>(ndivs[0]) * static_cast<svtkIdType>(ndivs[1]);
  ...
  // memory savings.
  //
  if (numPts >= SVTK_INT_MAX || numBuckets >= SVTK_INT_MAX)
  {
    this->LargeIds = true;
    this->Buckets = new BucketList2D<svtkIdType>(this, numPts, numBuckets);
  }
  else
  {
    this->LargeIds = false;
...
//-----------------------------------------------------------------------------
// These methods satisfy the svtkStaticPointLocator2D API. The implementation is
// with the templated BucketList2D class. Note that a lot of the complexity here
// is due to the desire to use different id types (int versus svtkIdType) for the
...
//-----------------------------------------------------------------------------
// Given a position x, return the id of the point closest to it.
svtkIdType svtkStaticPointLocator2D::FindClosestPoint(const double x[3])
{
  ...
  if (this->LargeIds)
  {
    return static_cast<BucketList2D<svtkIdType>*>(this->Buckets)->FindClosestPoint(x);
  }
  else
  {
...
//-----------------------------------------------------------------------------
svtkIdType svtkStaticPointLocator2D::FindClosestPointWithinRadius(
  double radius, const double x[3], double inputDataLength, double& dist2)
{
  this->BuildLocator(); // will subdivide if modified; otherwise returns
  if (!this->Buckets)
  ...
  if (this->LargeIds)
  {
    return static_cast<BucketList2D<svtkIdType>*>(this->Buckets)
...
//-----------------------------------------------------------------------------
void svtkStaticPointLocator2D::FindClosestNPoints(int N, const double x[3], svtkIdList* result)
{
  this->BuildLocator(); // will subdivide if modified; otherwise returns
  if (!this->Buckets)
  {
  ...
  if (this->LargeIds)
  {
    return static_cast<BucketList2D<svtkIdType>*>(this->Buckets)->FindClosestNPoints(N, x, result);
  }
  else
  {
...
//-----------------------------------------------------------------------------
void svtkStaticPointLocator2D::FindPointsWithinRadius(double R, const double x[3], svtkIdList* result)
{
  this->BuildLocator(); // will subdivide if modified; otherwise returns
  if (!this->Buckets)
  {
...
double svtkStaticPointLocator2D::FindCloseNBoundedPoints(int N, const double x[3], svtkIdList* result)
{
  this->BuildLocator(); // will subdivide if modified; otherwise returns
  if (!this->Buckets)
  {
...
int svtkStaticPointLocator2D::IntersectWithLine(double a0[3], double a1[3], double tol, double& t,
  double lineX[3], double ptX[3], svtkIdType& ptId)
{
  this->BuildLocator(); // will subdivide if modified; otherwise returns
  if (!this->Buckets)
...
//-----------------------------------------------------------------------------
void svtkStaticPointLocator2D::GenerateRepresentation(int level, svtkPolyData* pd)
{
  this->BuildLocator(); // will subdivide if modified; otherwise returns
  if (!this->Buckets)
  {
  ...
  if (this->LargeIds)
  {
    return static_cast<BucketList2D<svtkIdType>*>(this->Buckets)->GenerateRepresentation(level, pd);
  }
  else
  {
...
//-----------------------------------------------------------------------------
// Given a position x, return the id of the point closest to it.
svtkIdType svtkStaticPointLocator2D::GetNumberOfPointsInBucket(svtkIdType bNum)
{
  if (this->LargeIds)
  {
    return static_cast<BucketList2D<svtkIdType>*>(this->Buckets)->GetNumberOfIds(bNum);
  }
  else
  {
...
//-----------------------------------------------------------------------------
// Given a position x, return the id of the point closest to it.
void svtkStaticPointLocator2D::GetBucketIds(svtkIdType bNum, svtkIdList* bList)
{
  if (this->LargeIds)
  {
    return static_cast<BucketList2D<svtkIdType>*>(this->Buckets)->GetIds(bNum, bList);
  }
  else
  {
...
void svtkStaticPointLocator2D::MergePoints(double tol, svtkIdType* pointMap)
{
  this->BuildLocator(); // will subdivide if modified; otherwise returns
  if (!this->Buckets)
  {
  ...
  if (this->LargeIds)
  {
    return static_cast<BucketList2D<svtkIdType>*>(this->Buckets)->MergePoints(tol, pointMap);
  }
  else
  {
...
//-----------------------------------------------------------------------------
void svtkStaticPointLocator2D::GetBucketIndices(const double* x, int ij[2]) const
{
  this->Buckets->GetBucketIndices(x, ij);
}

//-----------------------------------------------------------------------------
svtkIdType svtkStaticPointLocator2D::GetBucketIndex(const double* x) const
{
  return this->Buckets->GetBucketIndex(x);
}

//-----------------------------------------------------------------------------
void svtkStaticPointLocator2D::PrintSelf(ostream& os, svtkIndent indent)
{
