...
int TestVector(int, char*[])
{
  ...
  if (sizeof(vec2i) != sizeof(arr2i))
  {
    ...
    cerr << "svtkVector2i should be the same size as int[2]." << endl
         << "sizeof(vec2i) = " << sizeof(vec2i) << endl
         << "sizeof(int[2]) = " << sizeof(arr2i) << endl;
    ++retVal;
  }

  svtkVector<float, 3> vector3f;
  ...
  // Test out svtkVector3i and ensure the various access methods are the same
  svtkVector3i vec3i(0, 6, 9);
  ...
  // Test out casting...
  svtkVector<float, 3> castVec = vec3i.Cast<float>();
  svtkVector3d castVecd(castVec.Cast<double>().GetData());
  ...
  svtkVector3d normy(1, 2, 3);
  svtkVector3d normed = normy.Normalized();
  double dotted = normy.Dot(normed);
  if (!svtkMathUtilities::FuzzyCompare(dotted, 3.74166, 0.0001))
  {
    cerr << "The dot product of " << normy << " and " << normed << " was " << dotted
         << ", expected 3.74166." << endl;
    ++retVal;
  }
  if (!normed.Compare(svtkVector3d(0.267261, 0.534522, 0.801784), 0.0001))
  {
    cerr << "Error svtkVector3d::Normalized() failed: " << normed << endl;
    ++retVal;
  }
  normy.Normalize();
  if (!normy.Compare(normed, 0.0001))
  {
    cerr << "Error svtkVector3d::Normalize() failed: " << normy << endl;
  }
  ...
  if (!svtkMathUtilities::FuzzyCompare(normy.Dot(normed), 1.0, 0.0001))
  {
  ...
  if (!normy.Cross(normed).Compare(svtkVector3d(0, 0, 0), 0.0001))
  {
    cerr << normy << " cross " << normed << " expected to be 0, got " << normy.Cross(normed)
         << endl;
    ++retVal;
  }
  if (!normy.Cross(svtkVector3d(0, 1, 0)).Compare(svtkVector3d(-0.801784, 0, 0.267261), 0.0001))
  {
