...
int TestPolyhedronContouring(int argc, char* argv[])
{
...

  {
    ...
    cf->SetInputArrayToProcess(
      0, 0, 0, svtkDataObject::FIELD_ASSOCIATION_POINTS, "AirVolumeFraction");
    cf->SetInputData(grid);
    cf->SetValue(0, 0.5);
    cf->Update();

    svtkPolyData* polys = cf->GetOutput();
    ...
    svtkUnstructuredGrid* clip = cd->GetOutput();
    ...
    svtkCell* clipCell0 = clip->GetCell(0);
    ...
    svtkCell* clipCell1 = clip->GetCell(1);
    ...
    svtkCell* clipCell = clip->GetCell(0);
  ...
  // contouring code
  {
    ...
    svtkNew<svtkUnstructuredGrid> p;
    p->SetPoints(pts);
    p->Allocate(1);

    svtkIdType faceStream[] = { 6, 8, 3, 4, 5, 9, 10, 4, 8, 3, 6, 11, 6, 3, 6, 0, 7, 5, 4, 4, 9, 5,
      7, 12, 4, 10, 9, 12, 13, 4, 13, 12, 1, 2, 4, 12, 7, 0, 1, 5, 8, 11, 2, 13, 10, 5, 11, 6, 0, 1,
      2 };

    p->InsertNextCell(SVTK_POLYHEDRON, 9, faceStream);
    ...
    svtkPolyData* result = cf->GetOutput();
    ...
    svtkCell* contour = result->GetCell(0);
    ...
    svtkUnstructuredGrid* cell_12851 = r->GetOutput();
    ...
    svtkPolyData* cell_12851_contour = cf->GetOutput();
  ...
  // is now solved in svtkPolyhedron::TriangulatePolyhedralFaces
  {
    svtkNew<svtkPoints> pnts;
    ...
    svtkNew<svtkUnstructuredGrid> ba;
    ba->SetPoints(pnts);
    ...
    svtkIdType faceStream[] = { 5, 4, 8, 1, 3, 9, 4, 9, 3, 2, 0, 5, 4, 8, 1, 7, 10, 5, 1, 7, 5, 2, 3,

      4, 0, 2, 5, 6, 4, 9, 0, 6, 11, 4, 4, 9, 11, 10, 5, 10, 7, 5, 6, 11 };

    ba->InsertNextCell(SVTK_POLYHEDRON, 8, faceStream);
    ...
    svtkCell* contourCell = result->GetCell(0);
