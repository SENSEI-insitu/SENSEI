...
{
  svtkSmartPointer<svtkPolygon> polygon = svtkSmartPointer<svtkPolygon>::New();
  ...
  {
    svtkIdType idTypeArray[4] = { 0, 1, 2, 3 };

    svtkSmartPointer<svtkIdTypeArray> idArray = svtkSmartPointer<svtkIdTypeArray>::New();
    ...
    convex &= svtkPolygon::IsConvex(polygon->GetPoints(), polygon->GetNumberOfPoints(), idTypeArray);
    convex &= svtkPolygon::IsConvex(idArray, polygon->GetPoints());
    convex &= svtkPolygon::IsConvex(polygon->GetPoints());
  ...
  // Next, create a nonconvex element and test it.
  {
    ...
    nonconvex &=
      !svtkPolygon::IsConvex(polygon->GetPoints(), polygon->GetNumberOfPoints(), idTypeArray);
    nonconvex &= !svtkPolygon::IsConvex(idArray, polygon->GetPoints());
    nonconvex &= !svtkPolygon::IsConvex(polygon->GetPoints());
  ...
  // Test Normal : void svtkPolygon::ComputeNormal (int numPts, double *pts, double n[3])
  double normal[3];
  double points[12];
  for (int i = 0; i < polygon->GetNumberOfPoints(); i++)
  {
  ...
  svtkPolygon::ComputeNormal(polygon->GetNumberOfPoints(), points, normal);
  ...
  // Test Normal : void svtkPolygon::ComputeNormal(svtkIdTypeArray *ids, svtkPoints *p, double n[3])
  svtkSmartPointer<svtkIdTypeArray> idArray = svtkSmartPointer<svtkIdTypeArray>::New();
  for (int i = 0; i < polygon->GetNumberOfPoints(); i++)
  {
    idArray->InsertNextValue(i);
  }
  svtkPolygon::ComputeNormal(idArray, polygon->GetPoints(), normal);
  if (!fuzzyCompare(normal[0], 0.0) || !fuzzyCompare(normal[1], 0.0) ||
    !fuzzyCompare(normal[2], 1.0))
  {
    cerr << "ERROR: The normal (" << normal[0] << ", " << normal[1] << ", " << normal[2]
  ...
  {
    svtkSmartPointer<svtkPolygon> polygon1 = svtkSmartPointer<svtkPolygon>::New();
    ...
    svtkSmartPointer<svtkPolygon> polygon2 = svtkSmartPointer<svtkPolygon>::New();
    ...
    /*
      int svtkPolygon::IntersectPolygonWithPolygon(int npts, double *pts,double bounds[6],
                                                           int npts2, double *pts2,
                                                           double bounds2[6], double tol2,
                                                           double x[3])
    */
    double intersection[3];
    int intersect = svtkPolygon::IntersectPolygonWithPolygon(
