...
int TimePointLocators(int, char*[])
{
  ...
  {
    points->SetPoint(i, svtkMath::Random(-1, 1), svtkMath::Random(-1, 1), svtkMath::Random(-1, 1));
  }
  ...
  svtkPoints* qPoints = svtkPoints::New();
  qPoints->SetDataTypeToDouble();
  qPoints->SetNumberOfPoints(nQ);
  svtkMath::RandomSeed(314159);
  for (int i = 0; i < nQ; ++i)
  {
    qPoints->SetPoint(i, svtkMath::Random(-1, 1), svtkMath::Random(-1, 1), svtkMath::Random(-1, 1));
  }

  svtkIdList* closest = svtkIdList::New();
  ...
  uniformLocator = svtkPointLocator::New();
  uniformLocator->SetDataSet(polydata);
  uniformLocator->BuildLocator();
  timer->StartTimer();
  for (int i = 0; i < nQ; ++i)
  ...
  svtkStaticPointLocator* staticLocator = svtkStaticPointLocator::New();
  timer->StartTimer();
  ...
  staticLocator = svtkStaticPointLocator::New();
  staticLocator->SetDataSet(polydata);
  staticLocator->BuildLocator();
  timer->StartTimer();
  for (int i = 0; i < nQ; ++i)
  ...
  kdTreeLocator = svtkKdTreePointLocator::New();
  kdTreeLocator->SetDataSet(polydata);
  kdTreeLocator->BuildLocator();
  timer->StartTimer();
  for (int i = 0; i < nQ; ++i)
  ...
  octreeLocator = svtkOctreePointLocator::New();
  octreeLocator->SetDataSet(polydata);
  octreeLocator->BuildLocator();
  timer->StartTimer();
  for (int i = 0; i < nQ; ++i)
