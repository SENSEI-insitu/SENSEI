...
svtkSmartPointer<svtkSignedCharArray> NewArray(svtkIdType numVals)
{
  std::random_device rd;  // Will be used to obtain a seed for the random number engine
  std::mt19937 gen(rd()); // Standard mersenne_twister_engine seeded with rd()
  std::uniform_int_distribution<> dis(0, 1);

  auto array = svtkSmartPointer<svtkSignedCharArray>::New();
...
static inline bool get(svtkSignedCharArray* array, svtkIdType cc)
{
  return array->GetTypedComponent(cc, 0) != 0;
}

template <typename Functor>
void ValidateIternal(svtkSignedCharArray* result, const Functor& f)
{
  for (svtkIdType cc = 0; cc < 1024; ++cc)
  {
    bool val = f(cc);
...
template <typename Functor>
void Validate(svtkSelection* expr, const char* exprstr,
  const std::vector<svtkSignedCharArray*>& arrays, const Functor& f)
{
...
void Validate(svtkSelection* expr, const char* exprstr, const MapType& arrays, const Functor& f)
{
  expr->SetExpression(exprstr);
...
{
  svtkNew<svtkSelection> expr;
  svtkNew<svtkSelectionNode> aItem;
  svtkNew<svtkSelectionNode> bItem;
  svtkNew<svtkSelectionNode> cItem;
  svtkNew<svtkSelectionNode> dItem;
  svtkNew<svtkSelectionNode> eItem;
  svtkNew<svtkSelectionNode> fItem;
  svtkNew<svtkSelectionNode> gItem;
  ...
  std::map<std::string, svtkSmartPointer<svtkSignedCharArray> > arrays;
  std::vector<svtkSignedCharArray*> arrays_ptrs(expr->GetNumberOfNodes(), nullptr);
  ...
  Validate(expr, "A & (B | (C & D))", arrays_ptrs, [&](svtkIdType cc) {
  ...
  Validate(expr, "(A & B) | (C | (D & E) ) | (C & D)", arrays_ptrs, [&](svtkIdType cc) {
    auto a = get(arrays["A"], cc);
    auto b = get(arrays["B"], cc);
    auto c = get(arrays["C"], cc);
    auto d = get(arrays["D"], cc);
  ...
  Validate(expr, "", arrays, [&](svtkIdType cc) {
    auto a = get(arrays["A"], cc);
    auto b = get(arrays["B"], cc);
    auto c = get(arrays["C"], cc);
    auto d = get(arrays["D"], cc);
