...
// .NAME
// .SECTION Description
// this program tests svtkRectilinearGrid
...
int TestORG(ostream& strm)
{
  int i, j, k;
  // actual test
  strm << "Testing svtkRectilinearGrid" << endl;
  svtkRectilinearGrid* rg0D = svtkRectilinearGrid::New();
  svtkRectilinearGrid* rg1Dx = svtkRectilinearGrid::New();
  svtkRectilinearGrid* rg1Dy = svtkRectilinearGrid::New();
  svtkRectilinearGrid* rg1Dz = svtkRectilinearGrid::New();
  svtkRectilinearGrid* rg2Dxy = svtkRectilinearGrid::New();
  svtkRectilinearGrid* rg2Dxz = svtkRectilinearGrid::New();
  svtkRectilinearGrid* rg2Dyz = svtkRectilinearGrid::New();
  svtkRectilinearGrid* rg3D = svtkRectilinearGrid::New();

  svtkDoubleArray* xdata = svtkDoubleArray::New();
  svtkDoubleArray* ydata = svtkDoubleArray::New();
  svtkDoubleArray* zdata = svtkDoubleArray::New();
  ...
  // Test shallow copy
  svtkRectilinearGrid* scrg3D = svtkRectilinearGrid::New();
  scrg3D->ShallowCopy(rg3D);
  strm << "ShallowCopy(rg3D):" << *scrg3D;

  // Test deep copy
  svtkRectilinearGrid* dcrg3D = svtkRectilinearGrid::New();
  ...
  svtkCell* cell3D = rg3D->GetCell(cellId);
  strm << "cell3D: " << *cell3D;
  ...
  j = 15;
  cellId = j * 19 + i;
  svtkCell* cell2D = rg2Dxy->GetCell(cellId);
  strm << "cell2D: " << *cell2D;
  ...
  i = 10;
  cellId = i;
  svtkCell* cell1D = rg1Dx->GetCell(i);
  strm << "cell1D: " << *cell1D;
  ...
  cellId = 0;
  svtkCell* cell0D = rg0D->GetCell(0);
  strm << "cell0D: " << *cell0D;
  ...
  if (cell3D == nullptr)
  {
    std::cerr << "svtkRectilinearGrid::GetCell returned nullptr instead of a valid cell.\n";
    return 1;
  }
  if (cell3D->GetCellType() != SVTK_VOXEL)
  {
    std::cerr << "svtkRectilinearGrid::GetCell returned the wrong cell type.\n"
              << "Expected: " << SVTK_VOXEL << " Returned: " << cell3D->GetCellType() << '\n';
    return 1;
  }
  strm << "cell3D: " << *cell3D;
  rg3D->GetCellPoints(cellId, ids);
  ...
  if (cell2D->GetCellType() != SVTK_PIXEL)
  {
    std::cerr << "svtkRectilinearGrid::GetCell returned the wrong cell type.\n"
              << "Expected: " << SVTK_PIXEL << " Returned: " << cell2D->GetCellType() << '\n';
    return 1;
  }
  double bounds[6];
  ...
  dummyCell = nullptr;
  svtkCell* found = rg2Dxy->FindAndGetCell(point3D, dummyCell, 0, 0, subId, pcoords, weights);
  if (found == nullptr)
  {
...
int otherRectilinearGrid(int, char*[])
{
  ...
  return TestORG(svtkmsg_with_warning_C4701);
}
