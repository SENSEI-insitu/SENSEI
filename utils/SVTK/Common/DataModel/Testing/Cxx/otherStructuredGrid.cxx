...
// .NAME
// .SECTION Description
// this program tests svtkStructuredGrid
...
int TestOSG(ostream& strm)
{
  int i, j, k;
  // actual test
  strm << "Testing svtkStructuredGrid" << endl;
  svtkStructuredGrid* sg0D = svtkStructuredGrid::New();
  svtkStructuredGrid* sg1Dx = svtkStructuredGrid::New();
  svtkStructuredGrid* sg1Dy = svtkStructuredGrid::New();
  svtkStructuredGrid* sg1Dz = svtkStructuredGrid::New();
  svtkStructuredGrid* sg2Dxy = svtkStructuredGrid::New();
  svtkStructuredGrid* sg2Dxz = svtkStructuredGrid::New();
  svtkStructuredGrid* sg2Dyz = svtkStructuredGrid::New();
  svtkStructuredGrid* sg3D = svtkStructuredGrid::New();

  svtkPoints* xyzpoints = svtkPoints::New();
  for (k = 0; k < 20; k++)
  {
    for (j = 0; j < 20; j++)
    {
  ...
  svtkPoints* xypoints = svtkPoints::New();
  for (j = 0; j < 20; j++)
  {
    for (i = 0; i < 20; i++)
    {
  ...
  svtkPoints* xzpoints = svtkPoints::New();
  for (k = 0; k < 20; k++)
  {
    for (i = 0; i < 20; i++)
    {
  ...
  svtkPoints* yzpoints = svtkPoints::New();
  for (k = 0; k < 20; k++)
  {
    for (j = 0; j < 20; j++)
    {
  ...
  svtkPoints* xpoints = svtkPoints::New();
  for (i = 0; i < 20; i++)
  {
  ...
  svtkPoints* ypoints = svtkPoints::New();
  for (j = 0; j < 20; j++)
  {
  ...
  svtkPoints* zpoints = svtkPoints::New();
  for (k = 0; k < 20; k++)
  {
  ...
  svtkPoints* onepoints = svtkPoints::New();
  for (k = 0; k < 1; k++)
  {
  ...
  svtkShortArray* shortScalars3D = svtkShortArray::New();
  ...
  svtkShortArray* shortScalars2D = svtkShortArray::New();
  ...
  svtkShortArray* shortScalars1D = svtkShortArray::New();
  ...
  svtkShortArray* shortScalars0D = svtkShortArray::New();
  ...
  // Test shallow copy
  svtkStructuredGrid* scsg3D = svtkStructuredGrid::New();
  scsg3D->ShallowCopy(sg3D);
  strm << "ShallowCopy(sg3D):" << *scsg3D;

  // Test deep copy
  svtkStructuredGrid* dcsg3D = svtkStructuredGrid::New();
  ...
  svtkCell* cell3D = sg3D->GetCell(cellId);
  strm << "cell3D: " << *cell3D;
  ...
  j = 15;
  cellId = j * 19 + i;
  svtkCell* cell2D = sg2Dxy->GetCell(cellId);
  strm << "cell2D: " << *cell2D;
  ...
  i = 10;
  cellId = i;
  svtkCell* cell1D = sg1Dx->GetCell(i);
  strm << "cell1D: " << *cell1D;
  ...
  cellId = 0;
  svtkCell* cell0D = sg0D->GetCell(0);
  strm << "cell0D: " << *cell0D;
  ...
  if (cell3D == nullptr)
  {
    std::cerr << "svtkStructuredGrid::GetCell returned nullptr instead of a valid cell.\n";
    return 1;
  }
  if (cell3D->GetCellType() != SVTK_HEXAHEDRON)
  {
    std::cerr << "svtkStructuredGrid::GetCell returned the wrong cell type.\n"
              << "Expected: " << SVTK_HEXAHEDRON << " Returned: " << cell3D->GetCellType() << '\n';
    return 1;
  }
  strm << "cell3D: " << *cell3D;
  sg3D->GetCellPoints(cellId, ids);
  ...
  if (cell2D->GetCellType() != SVTK_QUAD)
  {
    std::cerr << "svtkStructuredGrid::GetCell returned the wrong cell type.\n"
              << "Expected: " << SVTK_QUAD << " Returned: " << cell2D->GetCellType() << '\n';
    return 1;
  }
  double bounds[6];
  ...
  cell2D->GetBounds(bounds);
  if (!svtkMathUtilities::FuzzyCompare(bounds[2], bounds[3]))
  {
  ...
  cell2D->GetBounds(bounds);
  if (!svtkMathUtilities::FuzzyCompare(bounds[0], bounds[1]))
  {
  ...
  if (cell1D->GetCellType() != SVTK_LINE)
  {
    std::cerr << "svtkStructuredGrid::GetCell returned the wrong cell type.\n"
              << "Expected: " << SVTK_LINE << " Returned: " << cell1D->GetCellType() << '\n';
    return 1;
  }
  ...
  else if (!svtkMathUtilities::FuzzyCompare(bounds[4], bounds[5]))
  {
  ...
  else if (!svtkMathUtilities::FuzzyCompare(bounds[2], bounds[3]))
  {
  ...
  if (cell0D->GetCellType() != SVTK_VERTEX)
  {
    std::cerr << "svtkStructuredGrid::GetCell returned the wrong cell type.\n"
              << "Expected: " << SVTK_VERTEX << " Returned: " << cell0D->GetCellType() << '\n';
    return 1;
  }
  strm << "cell0D: " << *cell0D;
  sg0D->GetCellPoints(cellId, ids);
  ...
  svtkGenericCell* gcell3D = svtkGenericCell::New();
  svtkGenericCell* gcell2D = svtkGenericCell::New();
  svtkGenericCell* gcell1D = svtkGenericCell::New();
  svtkGenericCell* gcell0D = svtkGenericCell::New();
  i = 10;
  j = 15;
  k = 7;
  ...
  dummyCell = nullptr;
  svtkCell* found = sg2Dxy->FindAndGetCell(point3D, dummyCell, 0, 0, subId, pcoords, weights);
  if (found == nullptr)
  {
...
int otherStructuredGrid(int, char*[])
{
  ...
  return TestOSG(svtkmsg_with_warning_C4701);
}
