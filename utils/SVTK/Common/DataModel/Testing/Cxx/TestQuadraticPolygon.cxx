...
class svtkQuadraticPolygonTest : public svtkQuadraticPolygon
{
public:
  static svtkQuadraticPolygonTest* New();
  svtkTypeMacro(svtkQuadraticPolygonTest, svtkQuadraticPolygon);
...
svtkStandardNewMacro(svtkQuadraticPolygonTest);

int TestQuadraticPolygon(int, char*[])
{
  svtkNew<svtkQuadraticPolygonTest> test;
  int result = test->TestAll();
  cout << ((result == EXIT_SUCCESS) ? "SUCCESS" : "FAILURE") << endl;
  return result;
}

int svtkQuadraticPolygonTest::TestAll()
{
...
bool svtkQuadraticPolygonTest::IsClose(double v1, double v2)
{
  return (v1 < v2 ? (v2 - v1 < this->Tolerance) : (v1 - v2 < this->Tolerance));
}

bool svtkQuadraticPolygonTest::IsClose(double* point1, double* point2)
{
  return (svtkMath::Distance2BetweenPoints(point1, point2) < this->Tolerance * this->Tolerance);
}

void svtkQuadraticPolygonTest::InitializeSquare()
{
...
void svtkQuadraticPolygonTest::InitializeSquareWithQuadraticEdge()
{
  this->InitializeSquare();
  this->GetPoints()->SetPoint(5, 3.0, 1.0, 0.0);
}

int svtkQuadraticPolygonTest::TestGetSet()
{
  if (this->GetCellType() != SVTK_QUADRATIC_POLYGON)
  {
    cerr << "ERROR:  quadratic polygon type is " << this->GetCellType() << ", should be "
         << SVTK_QUADRATIC_POLYGON << endl;
    return EXIT_FAILURE;
  }
...
int svtkQuadraticPolygonTest::TestGetPermutations()
{
  // reference permutation
  svtkIdType temp[] = { 0, 2, 4, 6, 1, 3, 5, 7 };
  svtkNew<svtkIdList> permutationToPolygonRef;
  ...
  svtkNew<svtkIdList> permutationToPolygon;
  svtkQuadraticPolygon::GetPermutationToPolygon(8, permutationToPolygon);

  // reference permutation
  svtkIdType temp2[] = { 0, 4, 1, 5, 2, 6, 3, 7 };
  svtkNew<svtkIdList> permutationFromPolygonRef;
  ...
  // computed permutation
  svtkNew<svtkIdList> permutationFromPolygon;
  svtkQuadraticPolygon::GetPermutationFromPolygon(8, permutationFromPolygon);
...
int svtkQuadraticPolygonTest::TestInitializePolygon()
{
  // reference permutation
  svtkIdType temp[] = { 0, 2, 4, 6, 1, 3, 5, 7 };
  svtkNew<svtkIdList> permutationToPolygonRef;
  ...
  svtkPolygon* polygon = this->Polygon;
...
int svtkQuadraticPolygonTest::TestIntersectWithLine()
{
  ...
  {
    cerr << "ERROR:  svtkQuadraticPolygon::IntersectWithLine returns point (" << x[0] << "," << x[1]
         << "," << x[2] << ")"
  ...
  if (!intersect)
  {
    cerr << "ERROR:  svtkQuadraticPolygon::IntersectWithLine returns " << intersect
...
int svtkQuadraticPolygonTest::TestInterpolateFunctions()
{
...
int svtkQuadraticPolygonTest::TestInterpolateFunctionsUsingMVC()
{
  int nbPoints = this->GetNumberOfPoints();
