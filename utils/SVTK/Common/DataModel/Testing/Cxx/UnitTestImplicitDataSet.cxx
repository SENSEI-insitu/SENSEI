...
static svtkSmartPointer<svtkImageData> MakeVolume(int, int, int);
...
int UnitTestImplicitDataSet(int, char*[])
{
  ...
  // Create a volume
  svtkSmartPointer<svtkImageData> aVolume = MakeVolume(dim, dim, dim);

  // Test empty print
  std::cout << "Testing empty Print...";
  svtkSmartPointer<svtkImplicitDataSet> impVol = svtkSmartPointer<svtkImplicitDataSet>::New();
  ...
  impVol->AddObserver(svtkCommand::ErrorEvent, errorObserver);
  ...
  for (int k = 0; k < dim; k++)
  {
    for (int j = 0; j < dim; j++)
    {
      for (int i = 0; i < dim; i++)
      {
        ...
        else if (!svtkMathUtilities::FuzzyCompare(0.0, n[0], tol) ||
          !svtkMathUtilities::FuzzyCompare(0.0, n[1], tol) ||
          !svtkMathUtilities::FuzzyCompare(1.0, n[2], tol))
        {
          std::cout << "For " << xyz[0] << ", " << xyz[1] << ", " << xyz[2] << " expected "
                    << "0, 0, 1"
...
svtkSmartPointer<svtkImageData> MakeVolume(int dimx, int dimy, int dimz)
{
  svtkSmartPointer<svtkImageData> aVolume = svtkSmartPointer<svtkImageData>::New();
  aVolume->SetDimensions(dimx, dimy, dimz);
  aVolume->AllocateScalars(SVTK_FLOAT, 1);
