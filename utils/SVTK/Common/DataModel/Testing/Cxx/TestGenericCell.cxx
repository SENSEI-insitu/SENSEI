...
int TestGenericCell(int, char*[])
{
  ...
  cell->AddObserver(svtkCommand::ErrorEvent, errorObserver);
  for (int i = 0; i < SVTK_NUMBER_OF_CELL_TYPES; ++i)
  {
    ...
    if (cell->GetCellType() != SVTK_EMPTY_CELL && cell->GetCellType() != SVTK_POLY_VERTEX // FIXME
      && cell->GetCellType() != SVTK_POLY_LINE                                           // FIXME
      && cell->GetCellType() != SVTK_TRIANGLE_STRIP                                      // FIXME
      && cell->GetCellType() != SVTK_POLYGON                                             // FIXME
      && cell->GetCellType() != SVTK_CONVEX_POINT_SET                                    // FIXME
      && cell->GetCellType() != SVTK_POLYHEDRON)                                         // FIXME
    {
      double m[3] = { 0., 0., 0. };
    ...
    {
      svtkCell* c = cell->GetEdge(e);
      c->Print(cout);
    }
    for (int f = 0; f < numFaces; ++f)
    {
      svtkCell* c = cell->GetFace(f);
      c->Print(cout);
    }
    if (cell->GetCellType() != i && cell->GetCellType() != SVTK_EMPTY_CELL)
    {
      ++rval;
    }
  }
