...
template <typename T>
int TestCell(const SVTKCellType cellType, svtkSmartPointer<T> cell);
...
int TestPentagonalPrism(int, char*[])
{
  ...
  results["PentagonalPrism"] =
    TestCell<svtkPentagonalPrism>(SVTK_PENTAGONAL_PRISM, MakePentagonalPrism());
  results["HexagonalPrism"] =
    TestCell<svtkHexagonalPrism>(SVTK_HEXAGONAL_PRISM, MakeHexagonalPrism());
...
template <typename T>
int TestCell(const SVTKCellType cellType, svtkSmartPointer<T> aCell)
{
  int status = 0;
