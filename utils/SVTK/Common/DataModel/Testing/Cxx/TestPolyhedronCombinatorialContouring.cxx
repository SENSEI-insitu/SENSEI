...
void BuildCaseGrid(int aCase, svtkUnstructuredGrid* grid, svtkIdList* faceStream)
{
  faceStream->Reset();
  ...
  grid->InsertNextCell(SVTK_POLYHEDRON, FACES, faceStream->GetPointer(0));
}

void BuildPoints(svtkPoints* pts)
{
...
int TestPolyhedronCombinatorialContouring(int svtkNotUsed(argc), char* svtkNotUsed(argv)[])
{
  svtkNew<svtkPoints> pts;
  BuildPoints(pts);

  svtkNew<svtkUnstructuredGrid> g;
  g->Allocate(1);
  svtkNew<svtkIdList> ptIds;

  svtkNew<svtkXMLPolyDataWriter> pw;
  ...
  for (int i = 1; i < nCases; ++i)
  {
    ...
    svtkNew<svtkClipDataSet> clipLess, clipMore;
    ...
    clipLess->Update();
    svtkUnstructuredGrid* less = clipLess->GetOutput();
    ...
    clipMore->Update();
    svtkUnstructuredGrid* more = clipMore->GetOutput();
