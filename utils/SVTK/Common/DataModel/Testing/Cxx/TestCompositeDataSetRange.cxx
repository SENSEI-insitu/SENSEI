...
bool TestCopy(svtkCompositeDataSet* src)
{
  // Clone dataset:
  auto dst = svtk::TakeSmartPointer(src->NewInstance());

  // Create tree structure:
  dst->CopyStructure(src);

  { // Copy dataset pointer into new datset:
    const auto srcRange = svtk::Range(src);
    const auto dstRange = svtk::Range(dst);
...
bool TestConfig(svtkCompositeDataSet* cds, svtk::CompositeDataSetOptions opts)
{
  using Opts = svtk::CompositeDataSetOptions;

  auto refIter = svtk::TakeSmartPointer(cds->NewIterator());
...
bool TestOptions(svtkCompositeDataSet* cds)
{
  using Opts = svtk::CompositeDataSetOptions;
...
//                                P                   // depth 5
//
svtkSmartPointer<svtkCompositeDataSet> CreateDataSet()
{
