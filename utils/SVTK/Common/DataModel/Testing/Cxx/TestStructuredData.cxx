...
int TestGetNumNodesAndCells()
{
  ...
  int dims[3];
  svtkStructuredData::GetDimensionsFromExtent(ext, dims);
  ...
  // Compute expected number of nodes. Note, we cast dims to svtkIdType to
  // ensure the compiler will generate a 32x32=64 multiply instruction.
  svtkIdType nNodesExpected = static_cast<svtkIdType>(dims[0]) * static_cast<svtkIdType>(dims[1]) *
    static_cast<svtkIdType>(dims[2]);

  if (svtkStructuredData::GetNumberOfPoints(ext) != nNodesExpected)
  {
    std::cerr << "ERROR: GetNumberOfNodes(ext) failed!\n";
    std::cerr << "val=" << svtkStructuredData::GetNumberOfPoints(ext) << "\n";
    std::cerr << "expected=" << nNodesExpected << "\n";
    return EXIT_FAILURE;
  }

  svtkStructuredData::GetCellDimensionsFromExtent(ext, dims);
  ...
  // Compute expected number of nodes. Note, we cast dims to svtkIdType to
  // ensure the compiler will generate a 32x32=64 multiply instruction.
  svtkIdType nCellsExpected = static_cast<svtkIdType>(dims[0]) * static_cast<svtkIdType>(dims[1]) *
    static_cast<svtkIdType>(dims[2]);

  if (svtkStructuredData::GetNumberOfCells(ext) != nCellsExpected)
  {
    std::cerr << "ERROR: GetNumberOfNodes(ext) failed!\n";
    std::cerr << "val=" << svtkStructuredData::GetNumberOfCells(ext) << "\n";
  ...
  for (int i = ext[0]; i <= ext[1]; ++i)
  {
    for (int j = ext[2]; j <= ext[3]; ++j)
    {
      for (int k = ext[4]; k <= ext[5]; ++k)
      {
        ...
        ijk[1] = j;
        ijk[2] = k;
        svtkIdType idx = svtkStructuredData::ComputePointIdForExtent(ext, ijk);

        svtkStructuredData::ComputePointStructuredCoordsForExtent(idx, ext, myijk);
  ...
  for (int i = 0; i < dim[0] - 1; ++i)
  {
    for (int j = 0; j < dim[1] - 1; ++j)
    {
      for (int k = 0; k < dim[2] - 1; ++k)
      {
        ...
        int ijk[3];
        svtkIdType id = svtkStructuredData::ComputeCellId(dim, pos);

        svtkStructuredData::ComputeCellStructuredCoords(id, dim, ijk);
  ...
  for (int i = 0; i < dim[0]; ++i)
  {
    for (int j = 0; j < dim[1]; ++j)
    {
      for (int k = 0; k < dim[2]; ++k)
      {
        ...
        int ijk[3];
        svtkIdType id = svtkStructuredData::ComputePointId(dim, pos);

        svtkStructuredData::ComputePointStructuredCoords(id, dim, ijk);
