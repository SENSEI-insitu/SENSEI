...
// .NAME
// .SECTION Description
// this program tests svtkUniformGrid
...
int TestOUG(ostream& strm)
{
  int i, j, k;
  // actual test
  strm << "Testing svtkUniformGrid" << endl;
  svtkUniformGrid* ug0D = svtkUniformGrid::New();
  svtkUniformGrid* ug1Dx = svtkUniformGrid::New();
  svtkUniformGrid* ug1Dy = svtkUniformGrid::New();
  svtkUniformGrid* ug1Dz = svtkUniformGrid::New();
  svtkUniformGrid* ug2Dxy = svtkUniformGrid::New();
  svtkUniformGrid* ug2Dxz = svtkUniformGrid::New();
  svtkUniformGrid* ug2Dyz = svtkUniformGrid::New();
  svtkUniformGrid* ug3D = svtkUniformGrid::New();
  ...
  // Test shallow copy
  svtkUniformGrid* scug3D = svtkUniformGrid::New();
  scug3D->ShallowCopy(ug3D);
  strm << "ShallowCopy(ug3D):" << *scug3D;

  // Test deep copy
  svtkUniformGrid* dcug3D = svtkUniformGrid::New();
  ...
  svtkCell* cell3D = ug3D->GetCell(cellId);
  strm << "cell3D: " << *cell3D;
  ...
  j = 15;
  cellId = j * 19 + i;
  svtkCell* cell2D = ug2Dxy->GetCell(cellId);
  strm << "cell2D: " << *cell2D;
  ...
  i = 10;
  cellId = i;
  svtkCell* cell1D = ug1Dx->GetCell(i);
  strm << "cell1D: " << *cell1D;
  ...
  cellId = 0;
  svtkCell* cell0D = ug0D->GetCell(0);
  strm << "cell0D: " << *cell0D;
  ...
  if (cell3D == nullptr)
  {
    std::cerr << "svtkUniformGrid::GetCell returned nullptr instead of a valid cell.\n";
    return 1;
  }
  if (cell3D->GetCellType() != SVTK_VOXEL)
  {
    std::cerr << "svtkUniformGrid::GetCell returned the wrong cell type.\n"
  ...
  dummyCell = nullptr;
  svtkCell* found = ug2Dxy->FindAndGetCell(point3D, dummyCell, 0, 0, subId, pcoords, weights);
  if (found == nullptr)
  {
...
int otherUniformGrid(int, char*[])
{
  ...
  return TestOUG(svtkmsg_with_warning_C4701);
}
