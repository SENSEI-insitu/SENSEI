...
{
  svtkSmartPointer<svtkPoints> pts;
  svtkLagrangeInterpolation::AppendHexahedronCollocationPoints(pts, order);
...
bool SumShapeFunctions(const int order[N], T tab, U world, V lpd)
{
  ...
  for (svtkIdType rr = 0; rr < nr; ++rr)
  {
    ...
    svtkVector3d cp; // Lagrange control point
    cp[2] = 0.0;
...
int LagrangeInterpolation(int argc, char* argv[])
{
  ...
  int order5[1] = { 2 };
  ok &= testEqual(svtkLagrangeInterpolation::NumberOfIntervals<1>(order5), 2,
    "NumberOfIntervals: 1-D order 2 has 2 intervals");
  pts = CreatePrismaticPoints<1>(order5, svtkLagrangeInterpolation::AppendCurveCollocationPoints);
  tab = CreateShapeFunctionTable<1>(order5, pts, svtkLagrangeInterpolation::Tensor1ShapeFunctions);
  // tab->Dump(10,-1);
  // Dump(pts);
  world->DeepCopy(pts);
  ...
  /*
  svtkNew<svtkXMLPolyDataWriter> wri;
  ...
  */
  ...
  pts =
    CreatePrismaticPoints<3>(order222, svtkLagrangeInterpolation::AppendHexahedronCollocationPoints);
  tab = CreateShapeFunctionTable<3>(order222, pts, svtkLagrangeInterpolation::Tensor3ShapeFunctions);
  world->DeepCopy(pts);
  ...
  */
  ...
  ok &= testEqual(svtkLagrangeInterpolation::NumberOfIntervals<3>(order333), 27,
    "NumberOfIntervals: 3-D order 3 has 27 intervals");
  pts =
    CreatePrismaticPoints<3>(order333, svtkLagrangeInterpolation::AppendHexahedronCollocationPoints);
  tab = CreateShapeFunctionTable<3>(order333, pts, svtkLagrangeInterpolation::Tensor3ShapeFunctions);
  ok &= TestShapeFunctionImage(tab, argc, argv, "LagrangeInterpolation_333hex.png");

  int order531[3] = { 5, 3, 1 };
  pts =
    CreatePrismaticPoints<3>(order531, svtkLagrangeInterpolation::AppendHexahedronCollocationPoints);
  tab = CreateShapeFunctionTable<3>(order531, pts, svtkLagrangeInterpolation::Tensor3ShapeFunctions);
  ok &= TestShapeFunctionImage(tab, argc, argv, "LagrangeInterpolation_531hex.png");

  int order44[2] = { 4, 4 };
  ok &= testEqual(svtkLagrangeInterpolation::NumberOfIntervals<2>(order44), 16,
    "NumberOfIntervals: 2-D order 4 has 16 intervals");
  pts = CreatePrismaticPoints<2>(
    order44, svtkLagrangeInterpolation::AppendQuadrilateralCollocationPoints);

  pts = CreatePrismaticPoints<3>(order333, svtkLagrangeInterpolation::AppendWedgeCollocationPoints);
  tab = CreateShapeFunctionTable<3>(order333, pts, svtkLagrangeInterpolation::WedgeShapeFunctions);
  world->DeepCopy(pts);
