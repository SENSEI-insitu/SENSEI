...
namespace
{
svtkSmartPointer<svtkLagrangeTetra> CreateTetra(int nPoints)
{
  svtkSmartPointer<svtkLagrangeTetra> t = svtkSmartPointer<svtkLagrangeTetra>::New();
...
int TestInterpolationFunction(svtkSmartPointer<svtkLagrangeTetra> cell, double eps = SVTK_EPSILON)
{
...
void InterpolateDerivsNumeric(svtkSmartPointer<svtkLagrangeTetra> cell, double pcoords[3],
...
int TestInterpolationDerivs(svtkSmartPointer<svtkLagrangeTetra> cell, double eps = SVTK_EPSILON)
{
  int numPts = cell->GetNumberOfPoints();
...
#ifdef DEBUG_POINTS
  {
    svtkSmartPointer<svtkLagrangeTetra> tet = CreateTetra(10);
...
int TestLagrangeTetra(int argc, char* argv[])
{
  ...
  static const svtkIdType nIntersections = 50;
  static const svtkIdType nClippedElems[11] = { 0, 6, 9, 33, 45, 85, 91, 26 };

  svtkIdType nPointsForOrder[8] = { -1, 4, 10, 20, 35, 56, 84, 15 };

  for (svtkIdType order = 1; order <= 7; ++order)
  {
    svtkSmartPointer<svtkLagrangeTetra> t = CreateTetra(nPointsForOrder[order]);
    ...
    if (t->GetPoints()->GetNumberOfPoints() != 15)
    {
      ...
      {
        svtkIdType bindex[4] = { static_cast<svtkIdType>(t->GetPoints()->GetPoint(i)[0] * order + .5),
          static_cast<svtkIdType>(t->GetPoints()->GetPoint(i)[1] * order + .5),
          static_cast<svtkIdType>(t->GetPoints()->GetPoint(i)[2] * order + .5), 0 };
