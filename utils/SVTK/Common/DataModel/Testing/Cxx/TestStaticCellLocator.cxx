...
// .NAME Test of svtkStaticCellLocator::FindClosestPoint
// .SECTION Description
// this program tests the FindClosestPoint method

#include "svtkCellLocator.h" // used as reference
...
{
  auto source = svtkSmartPointer<svtkCylinderSource>::New();
  source->SetCapping(1);
  ...
  auto static_loc = svtkSmartPointer<svtkStaticCellLocator>::New();
  static_loc->SetDataSet(source->GetOutput());
  static_loc->AutomaticOn();
  static_loc->BuildLocator();

  auto ref_loc = svtkSmartPointer<svtkCellLocator>::New();
