...
void DrawTriangles(double* p1, double* q1, double* r1, double* p2, double* q2, double* r2)
{
  ...
  svtkSmartPointer<svtkTriangle> triangle1 = svtkSmartPointer<svtkTriangle>::New();
  ...
  svtkSmartPointer<svtkDoubleArray> pointIds = svtkSmartPointer<svtkDoubleArray>::New();
...
static const int SVTK_YES_INTERSECTION = 1;

typedef svtkMinimalStandardRandomSequence svtkRandom;
...
void ProjectPointOntoPlane(double* o, double* n, double* p)
{
  ...
  double pMinuso[3];
  svtkMath::Subtract(p, o, pMinuso);

  double dot = svtkMath::Dot(pMinuso, n);
...
void GeneratePointInPlane(svtkRandom* seq, double* o, double* n, double* p)
{
...
void GeneratePointInHalfPlane(
  svtkRandom* seq, double* o, double* n, double* o2, double* n2, double* p)
{
  ...
  double pMinuso2[3];
  svtkMath::Subtract(p, o2, pMinuso2);

  if (svtkMath::Dot(n2, pMinuso2) < 0.)
  {
...
void GenerateTriangleInPlane(
  svtkRandom* seq, double* o, double* n, double* t1, double* t2, double* t3)
{
  do
  {
  ...
  } while (svtkTriangle::TriangleArea(t1, t2, t3) < EPSILON);
}

void GenerateTriangleInHalfPlane(
  svtkRandom* seq, double* o, double* n, double* o2, double* n2, double* t1, double* t2, double* t3)
{
  do
  {
...
void RandomSphere(svtkRandom* seq, const double radius, const double* offset, double* value)
{
  // Generate a point on a sphere.

  seq->Next();
  double theta = 2. * svtkMath::Pi() * seq->GetValue();
  seq->Next();
  double phi = svtkMath::Pi() * seq->GetValue();
  value[0] = radius * cos(theta) * sin(phi) + offset[0];
  value[1] = radius * sin(theta) * sin(phi) + offset[1];
  value[2] = radius * cos(phi) + offset[2];
}

int TestNegativeResult(svtkRandom* seq, unsigned nTests)
{
  ...
  for (unsigned test = 0; test < nTests; test++)
  {
    ...
    int returnValue = svtkTriangle::TrianglesIntersect(t1[0], t1[1], t1[2], t2[0], t2[1], t2[2]);

    if (returnValue != SVTK_NO_INTERSECTION)
    {
...
int TestCoplanarNegativeResult(svtkRandom* seq, unsigned nTests)
{
  double origin[3] = { 0., 0., 0. };
...
void ProjectAlongRay(svtkRandom* seq, double* x0, double* x1, double* p)
{
...
void GenerateOverlappingSegments(
  svtkRandom* seq, double* p1, double* p2, double* x1, double* x2, double* y1, double* y2)
{
...
void RandomPoint(svtkRandom* seq, double* p)
{
...
int TestPositiveResult(svtkRandom* seq, unsigned nTests)
{
  ...
  for (unsigned test = 0; test < nTests; test++)
  {
    ...
    if (returnValue != SVTK_YES_INTERSECTION)
    {
      std::cout << "Triangle " << TriangleToString(t1[0], t1[1], t1[2]) << std::endl;
...
int TestCoplanarPositiveResult(svtkRandom* seq, unsigned nTests)
{
  ...
  for (unsigned test = 0; test < nTests; test++)
  {
    ...
    if (svtkTriangle::TriangleArea(t1[0], t1[1], t1[2]) <
      svtkTriangle::TriangleArea(t2[0], t2[1], t2[2]))
    {
...
int TestReciprocalResult(svtkRandom* seq, unsigned nTests)
{
  ...
  for (unsigned test = 0; test < nTests; test++)
  {
    ...
    {
      int returnValue1 =
        svtkTriangle::TrianglesIntersect(&p[0][0], &p[1][0], &p[2][0], &p[3][0], &p[4][0], &p[5][0]);
      int returnValue2 =
        svtkTriangle::TrianglesIntersect(&p[3][0], &p[4][0], &p[5][0], &p[0][0], &p[1][0], &p[2][0]);
...
int TestIssue17092()
{
  ...
  // https://gitlab.kitware.com/svtk/svtk/issues/17092. It was fixed here:
  // https://gitlab.kitware.com/svtk/svtk/merge_requests/3886
...
int TestMR4529()
{
  ...
  // reported here: https://gitlab.kitware.com/svtk/svtk/merge_requests/4529
...
int TestTriangleIntersection(svtkRandom* seq, unsigned nTests)
{
...
{
  svtkRandom* sequence = svtkRandom::New();

  sequence->SetSeed(2);

  const unsigned nTest = 1.e5;

  std::cout << "Testing svtkTriangle::TriangleIntersection" << std::endl;
