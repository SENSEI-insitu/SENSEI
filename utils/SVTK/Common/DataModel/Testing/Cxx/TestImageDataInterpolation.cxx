...
inline svtkImageData* GetGrid(int dims[3], double origin[3], double h[3])
{
  ...
  svtkPointData* PD = image->GetPointData();
...
inline svtkPoints* GetReceivePoints(svtkImageData* img, svtkIdList* donorCellList)
{
  svtkPoints* rcvPoints = svtkPoints::New();
  ...
  {
    // Get cell
    svtkCell* myCell = img->GetCell(cellIdx);
...
inline double InterpolateValue(svtkImageData* img, svtkCell* c, double* w)
{
  ...
  assert("pre: point data is nullptr" && (PD != nullptr));
  svtkDataArray* dataArray = PD->GetArray("Fx");
  assert("pre: data array is nullptr" && (dataArray != nullptr));

  std::cout << "W:[";
  std::cout.flush();
  double val = 0.0;
  svtkIdType numNodes = c->GetNumberOfPoints();
  for (svtkIdType nodeIdx = 0; nodeIdx < numNodes; ++nodeIdx)
  {
    std::cout << w[nodeIdx] << " ";
    std::cout.flush();
    svtkIdType meshNodeIdx = c->GetPointId(nodeIdx);
...
{
  svtkImageData* grid = GetGrid(dims, origin, h);
  assert("pre: grid is nullptr" && (grid != nullptr));

  std::cout << "NUMBER OF CELLS:  " << grid->GetNumberOfCells() << std::endl;
  std::cout.flush();

  svtkIdList* donors = svtkIdList::New();
  svtkPoints* pnts = GetReceivePoints(grid, donors);
  ...
  {
    pnts->GetPoint(idx, x);
    svtkIdType cellIdx = grid->FindCell(x, nullptr, 0, 0.0, subId, pcoords, weights);
    ...
    svtkCell* donorCell = grid->GetCell(cellIdx);
