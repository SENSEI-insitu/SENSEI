...
// .NAME
// .SECTION Description
// this program tests svtkImageData
...
int TestOID(ostream& strm)
{
  int i, j, k;
  // actual test
  strm << "Testing svtkImageData" << endl;
  svtkImageData* id0D = svtkImageData::New();
  svtkImageData* id1Dx = svtkImageData::New();
  svtkImageData* id1Dy = svtkImageData::New();
  svtkImageData* id1Dz = svtkImageData::New();
  svtkImageData* id2Dxy = svtkImageData::New();
  svtkImageData* id2Dxz = svtkImageData::New();
  svtkImageData* id2Dyz = svtkImageData::New();
  svtkImageData* id3D = svtkImageData::New();
  ...
  // Test shallow copy
  svtkImageData* scid3D = svtkImageData::New();
  scid3D->ShallowCopy(id3D);
  strm << "ShallowCopy(id3D):" << *scid3D;

  // Test deep copy
  svtkImageData* dcid3D = svtkImageData::New();
  ...
  svtkCell* cell3D = id3D->GetCell(cellId);
  strm << "cell3D: " << *cell3D;
  ...
  j = 15;
  cellId = j * 19 + i;
  svtkCell* cell2D = id2Dxy->GetCell(cellId);
  strm << "cell2D: " << *cell2D;
  ...
  i = 10;
  cellId = i;
  svtkCell* cell1D = id1Dx->GetCell(i);
  strm << "cell1D: " << *cell1D;
  ...
  cellId = 0;
  svtkCell* cell0D = id0D->GetCell(0);
  strm << "cell0D: " << *cell0D;
  ...
  if (cell3D == nullptr)
  {
    std::cerr << "svtkImageData::GetCell returned nullptr instead of a valid cell.\n";
    return 1;
  }
  if (cell3D->GetCellType() != SVTK_VOXEL)
  {
    std::cerr << "svtkImageData::GetCell returned the wrong cell type.\n"
  ...
  dummyCell = nullptr;
  svtkCell* found = id2Dxy->FindAndGetCell(point3D, dummyCell, 0, 0, subId, pcoords, weights);
  if (found == nullptr)
  {
...
int otherImageData(int, char*[])
{
  ...
  return TestOID(svtkmsg_with_warning_C4701);
}
