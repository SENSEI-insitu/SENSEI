...
sideArray = svtk.svtkUnsignedCharArray()
...
def nonOrientedGeometryCursor( htg, waited ):
    nb = 0
    cursor = svtk.svtkHyperTreeGridNonOrientedGeometryCursor()
    for treeId in range(htg.GetMaxNumberOfTrees()):
...
def nonOrientedVonNeumannSuperCursor( htg, waited ):
    nb = 0
    cursor = svtk.svtkHyperTreeGridNonOrientedVonNeumannSuperCursor()
    for treeId in range(htg.GetMaxNumberOfTrees()):
...
def nonOrientedMooreSuperCursor( htg, waited ):
    nb = 0
    cursor = svtk.svtkHyperTreeGridNonOrientedMooreSuperCursor()
    for treeId in range(htg.GetMaxNumberOfTrees()):
