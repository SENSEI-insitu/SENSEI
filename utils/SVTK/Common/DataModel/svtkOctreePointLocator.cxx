/*=========================================================================
  ...
  Module:    svtkOctreePointLocator.cxx
...
#include "svtkOctreePointLocatorNode.h"
...
svtkStandardNewMacro(svtkOctreePointLocator);

// helper class for ordering the points in
// svtkOctreePointLocator::FindClosestNPoints()
namespace
{
class OrderPoints
{
...
private:
  ...
  std::map<float, std::list<svtkIdType> > dist2ToIds;
};
}

//----------------------------------------------------------------------------
svtkOctreePointLocator::svtkOctreePointLocator()
{
  this->FudgeFactor = 0;
  this->MaxWidth = 0.0;
  this->MaxLevel = 20;
...
//----------------------------------------------------------------------------
void svtkOctreePointLocator::DeleteAllDescendants(svtkOctreePointLocatorNode* octant)
{
  if (octant->GetChild(0))
  {
    for (int i = 0; i < 8; i++)
    {
      svtkOctreePointLocatorNode* child = octant->GetChild(i);
      svtkOctreePointLocator::DeleteAllDescendants(child);
    }
...
//----------------------------------------------------------------------------
void svtkOctreePointLocator::GetBounds(double* bounds)
{
  if (this->Top)
  {
...
//----------------------------------------------------------------------------
double* svtkOctreePointLocator::GetBounds()
{
  double* bounds = svtkAbstractPointLocator::GetBounds();
  if (this->Top)
  {
    this->Top->GetBounds(bounds);
  }
...
//----------------------------------------------------------------------------
void svtkOctreePointLocator::GetRegionBounds(int leafNodeId, double bounds[6])
{
  if ((leafNodeId < 0) || (leafNodeId >= this->NumberOfLeafNodes))
  {
    svtkErrorMacro("Invalid region.");
    return;
  }

  svtkOctreePointLocatorNode* node = this->LeafNodeList[leafNodeId];

  node->GetBounds(bounds);
}

//----------------------------------------------------------------------------
void svtkOctreePointLocator::GetRegionDataBounds(int leafNodeId, double bounds[6])
{
  if ((leafNodeId < 0) || (leafNodeId >= this->NumberOfLeafNodes))
  {
    svtkErrorMacro("Invalid region.");
...
//----------------------------------------------------------------------------
void svtkOctreePointLocator::SetDataBoundsToSpatialBounds(svtkOctreePointLocatorNode* octant)
{
  ...
  if (octant->GetChild(0))
  {
    for (int i = 0; i < 8; i++)
    {
      svtkOctreePointLocator::SetDataBoundsToSpatialBounds(octant->GetChild(i));
    }
  }
}

//----------------------------------------------------------------------------
int svtkOctreePointLocator::DivideTest(int size, int level)
{
...
//----------------------------------------------------------------------------
void svtkOctreePointLocator::DivideRegion(svtkOctreePointLocatorNode* node, int* ordering, int level)
{
  ...
  svtkDataSet* ds = this->GetDataSet();
...
//----------------------------------------------------------------------------
void svtkOctreePointLocator::BuildLocator()
{
  if (!this->GetDataSet())
  {
    svtkErrorMacro("Must set a valid data set first.");
  }
  ...
  if (numPoints < 1)
  {
    svtkErrorMacro(<< "No points to build from.");
    return;
  }

  if (numPoints >= SVTK_INT_MAX)
  {
    ...
    svtkErrorMacro("Intentional 64 bit error - time to rewrite code.");
    return;
  }

  svtkDebugMacro(<< "Creating octree");
  ...
  svtkOctreePointLocatorNode* node = this->Top = svtkOctreePointLocatorNode::New();
  ...
  {
    this->FreeSearchStructure();
    svtkErrorMacro("svtkOctreePointLocator::BuildLocatorFromPoints - memory allocation");
    return;
  }
  ...
  int index = 0;
  this->LeafNodeList = new svtkOctreePointLocatorNode*[this->NumberOfLeafNodes];
  this->BuildLeafNodeList(this->Top, index);
  this->BuildTime.Modified();
}

//----------------------------------------------------------------------------
void svtkOctreePointLocator::BuildLeafNodeList(svtkOctreePointLocatorNode* node, int& index)
{
  if (node->GetChild(0))
  {
    for (int i = 0; i < 8; i++)
...
//----------------------------------------------------------------------------
svtkIdType svtkOctreePointLocator::FindClosestPoint(const double x[3])
{
  double dist2(0);
...
//----------------------------------------------------------------------------
svtkIdType svtkOctreePointLocator::FindClosestPoint(double x, double y, double z, double& dist2)
{
  this->BuildLocator();
  ...
  if (regionId < 0)
  {
    ...
    closePointId = static_cast<svtkIdType>(this->LocatorIds[closeId]);
...
//----------------------------------------------------------------------------
svtkIdType svtkOctreePointLocator::FindClosestPointWithinRadius(
...
//----------------------------------------------------------------------------
svtkIdType svtkOctreePointLocator::FindClosestPointInRegion(int regionId, double* x, double& dist2)
{
  return this->FindClosestPointInRegion(regionId, x[0], x[1], x[2], dist2);
}

//----------------------------------------------------------------------------
svtkIdType svtkOctreePointLocator::FindClosestPointInRegion(
  int regionId, double x, double y, double z, double& dist2)
{
  if (!this->LocatorPoints)
  {
    ...
    svtkErrorMacro("svtkOctreePointLocator::FindClosestPointInRegion - must build locator first");
    return -1;
  }
...
//----------------------------------------------------------------------------
int svtkOctreePointLocator::_FindClosestPointInRegion(
...
//----------------------------------------------------------------------------
int svtkOctreePointLocator::FindClosestPointInSphere(
  double x, double y, double z, double radius, int skipRegion, double& dist2)
{
  ...
  std::stack<svtkOctreePointLocatorNode*> regions;
  regions.push(this->Top);
  while (!regions.empty())
  {
    svtkOctreePointLocatorNode* region = regions.top();
    regions.pop();
    if (region->GetChild(0))
    {
      for (int i = 0; i < 8; i++)
      {
        svtkOctreePointLocatorNode* child = region->GetChild(i);
  ...
  {
    originalId = static_cast<svtkIdType>(this->LocatorIds[localCloseId]);
  }
  return originalId;
}

//----------------------------------------------------------------------------
void svtkOctreePointLocator::FindPointsWithinRadius(
  double radius, const double x[3], svtkIdList* result)
{
  result->Reset();
  this->BuildLocator();
...
//----------------------------------------------------------------------------
void svtkOctreePointLocator::FindPointsWithinRadius(
  svtkOctreePointLocatorNode* node, double radiusSquared, const double x[3], svtkIdList* result)
{
  double b[6];
  node->GetBounds(b);
  ...
  if (node->GetChild(0) == nullptr)
  {
    ...
    svtkIdType numPoints = node->GetNumberOfPoints();
...
//----------------------------------------------------------------------------
void svtkOctreePointLocator::FindClosestNPoints(int N, const double x[3], svtkIdList* result)
{
  result->Reset();
  if (N <= 0)
  {
  ...
  svtkOctreePointLocatorNode* node = this->Top;
  svtkOctreePointLocatorNode* startingNode = nullptr;
  if (!node->ContainsPoint(x[0], x[1], x[2], 0))
  {
    // point is not in the region
    int numPoints = node->GetNumberOfPoints();
    svtkOctreePointLocatorNode* prevNode = node;
    while (node->GetChild(0) && numPoints > N)
    {
      prevNode = node;
      svtkOctreePointLocatorNode* nextNode = node->GetChild(0);
  ...
  {
    svtkOctreePointLocatorNode* parentOfNext = startingNode->GetChild(0);
    svtkOctreePointLocatorNode* next = parentOfNext->GetChild(0);
    while (next)
    {
      parentOfNext = next;
  ...
  double bounds[6];
  node = this->Top;
  std::queue<svtkOctreePointLocatorNode*> nodesToBeSearched;
  ...
  while (!nodesToBeSearched.empty())
  {
    ...
    if (node->GetChild(0))
    {
      for (int j = 0; j < 8; j++)
      {
        svtkOctreePointLocatorNode* child = node->GetChild(j);
...
//----------------------------------------------------------------------------
svtkIdTypeArray* svtkOctreePointLocator::GetPointsInRegion(int leafNodeId)
{
  if ((leafNodeId < 0) || (leafNodeId >= this->NumberOfLeafNodes))
  {
    svtkErrorMacro("svtkOctreePointLocator::GetPointsInRegion invalid leaf node ID");
    return nullptr;
  }
  ...
  if (!this->LocatorIds)
  {
    ...
    svtkErrorMacro("svtkOctreePointLocator::GetPointsInRegion build locator first");
    return nullptr;
  }
...
//----------------------------------------------------------------------------
void svtkOctreePointLocator::FreeSearchStructure()
{
  if (this->Top)
  {
    svtkOctreePointLocator::DeleteAllDescendants(this->Top);
    this->Top->Delete();
    this->Top = nullptr;
  }
  delete[] this->LeafNodeList;
...
//
void svtkOctreePointLocator::GenerateRepresentation(int level, svtkPolyData* pd)
{
  if (this->Top == nullptr)
  {
    svtkErrorMacro("svtkOctreePointLocator::GenerateRepresentation no tree");
    return;
  }

  std::list<svtkOctreePointLocatorNode*> nodesAtLevel;
  // queue of nodes to be examined and what level each one is at
  std::queue<std::pair<svtkOctreePointLocatorNode*, int> > testNodes;
  testNodes.push(std::make_pair(this->Top, 0));
  while (!testNodes.empty())
  {
    svtkOctreePointLocatorNode* node = testNodes.front().first;
  ...
  const svtkIdType npoints = 8 * static_cast<svtkIdType>(nodesAtLevel.size());
  const svtkIdType npolys = 6 * static_cast<svtkIdType>(nodesAtLevel.size());
  ...
  for (std::list<svtkOctreePointLocatorNode*>::iterator it = nodesAtLevel.begin();
       it != nodesAtLevel.end(); ++it)
  {
    svtkOctreePointLocator::AddPolys(*it, pts, polys);
  }
...
//----------------------------------------------------------------------------
void svtkOctreePointLocator::AddPolys(
  svtkOctreePointLocatorNode* node, svtkPoints* pts, svtkCellArray* polys)
{
  svtkIdType ids[8];
  svtkIdType idList[4];
  double x[3];
...
//----------------------------------------------------------------------------
int svtkOctreePointLocator::FindRegion(svtkOctreePointLocatorNode* node, float x, float y, float z)
{
  return svtkOctreePointLocator::FindRegion(
    node, static_cast<double>(x), static_cast<double>(y), static_cast<double>(z));
}

//----------------------------------------------------------------------------
int svtkOctreePointLocator::FindRegion(svtkOctreePointLocatorNode* node, double x, double y, double z)
{
  ...
  for (int i = 0; i < 8; i++)
  {
    int regionId = svtkOctreePointLocator::FindRegion(node->GetChild(i), x, y, z);
    if (regionId >= 0)
    {
      return regionId;
    }
...
//----------------------------------------------------------------------------
int svtkOctreePointLocator::GetRegionContainingPoint(double x, double y, double z)
{
  return svtkOctreePointLocator::FindRegion(this->Top, x, y, z);
}

//----------------------------------------------------------------------------
void svtkOctreePointLocator::FindPointsInArea(double* area, svtkIdTypeArray* ids, bool clearArray)
{
  if (clearArray)
  {
    ids->Reset();
...
//----------------------------------------------------------------------------
void svtkOctreePointLocator::FindPointsInArea(
  svtkOctreePointLocatorNode* node, double* area, svtkIdTypeArray* ids)
{
  double b[6];
  node->GetBounds(b);
...
//----------------------------------------------------------------------------
void svtkOctreePointLocator::AddAllPointsInRegion(
  svtkOctreePointLocatorNode* node, svtkIdTypeArray* ids)
{
  int regionLoc = node->GetMinID();
  svtkIdType numPoints = node->GetNumberOfPoints();
  for (svtkIdType i = 0; i < numPoints; i++)
...
//----------------------------------------------------------------------------
void svtkOctreePointLocator::AddAllPointsInRegion(svtkOctreePointLocatorNode* node, svtkIdList* ids)
{
  int regionLoc = node->GetMinID();
  svtkIdType numPoints = node->GetNumberOfPoints();
  for (svtkIdType i = 0; i < numPoints; i++)
...
//----------------------------------------------------------------------------
void svtkOctreePointLocator::PrintSelf(ostream& os, svtkIndent indent)
{
