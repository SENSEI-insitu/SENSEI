...
svtkStandardNewMacro(svtkCylinder);

//----------------------------------------------------------------------------
// Construct cylinder radius of 0.5.
svtkCylinder::svtkCylinder()
{
...
double svtkCylinder::EvaluateFunction(double x[3])
{
  ...
  double proj = svtkMath::Dot(this->Axis, x2C);

  // return distance^2 - R^2
  return ((svtkMath::Dot(x2C, x2C) - proj * proj) - this->Radius * this->Radius);
}
...
void svtkCylinder::EvaluateGradient(double x[3], double g[3])
{
...
void svtkCylinder::SetAxis(double ax, double ay, double az)
{
  double axis[3];
  axis[0] = ax;
  axis[1] = ay;
...
// axis vector.
void svtkCylinder::SetAxis(double a[3])
{
  // If axis length is zero, then don't change it
  if (svtkMath::Normalize(a) < DBL_EPSILON)
  {
    return;
  }
...
//----------------------------------------------------------------------------
void svtkCylinder::PrintSelf(ostream& os, svtkIndent indent)
{
