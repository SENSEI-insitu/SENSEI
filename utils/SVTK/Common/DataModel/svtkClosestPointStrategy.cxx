/*=========================================================================
  ...
  Module:    svtkClosestPointStrategy.cxx
...
//----------------------------------------------------------------------------
svtkStandardNewMacro(svtkClosestPointStrategy);

//----------------------------------------------------------------------------
svtkClosestPointStrategy::svtkClosestPointStrategy()
{
  ...
  this->NearPointIds = svtkIdList::New();
...
//-----------------------------------------------------------------------------
void svtkClosestPointStrategy::SetPointLocator(svtkAbstractPointLocator* pL)
{
...
//-----------------------------------------------------------------------------
int svtkClosestPointStrategy::Initialize(svtkPointSet* ps)
{
  ...
  // method.
  svtkAbstractPointLocator* psPL = ps->GetPointLocator();
  if (psPL == nullptr)
  {
    if (this->PointLocator != nullptr && this->OwnsLocator)
    {
...
// reallocated.
svtkIdType FindCellWalk(svtkClosestPointStrategy* self, svtkPointSet* ps, double x[3], svtkCell* cell,
  svtkGenericCell* gencell, svtkIdType cellId, double tol2, int& subId, double pcoords[3],
  double* weights, std::set<svtkIdType>& visitedCells, svtkIdList* ptIds, svtkIdList* neighbors)
{
  const int SVTK_MAX_WALK = 12;
  for (int walk = 0; walk < SVTK_MAX_WALK; walk++)
  {
...
//-----------------------------------------------------------------------------
svtkIdType FindCellWalk(svtkClosestPointStrategy* self, svtkPointSet* ps, double x[3],
  svtkGenericCell* gencell, svtkIdList* cellIds, double tol2, int& subId, double pcoords[3],
  double* weights, std::set<svtkIdType>& visitedCells, svtkIdList* ptIds, svtkIdList* neighbors)
{
  svtkIdType numCellIds = cellIds->GetNumberOfIds();
  for (svtkIdType i = 0; i < numCellIds; i++)
  {
    svtkIdType cellId = cellIds->GetId(i);
    svtkIdType foundCell = FindCellWalk(self, ps, x, nullptr, gencell, cellId, tol2, subId, pcoords,
...
//-----------------------------------------------------------------------------
svtkIdType svtkClosestPointStrategy::FindCell(double x[3], svtkCell* cell, svtkGenericCell* gencell,
  svtkIdType cellId, double tol2, int& subId, double pcoords[3], double* weights)
{
  ...
  svtkIdType ptId = this->PointLocator->FindClosestPoint(x);
  if (ptId < 0)
    return -1;
  ...
  svtkIdType i, numPts = this->NearPointIds->GetNumberOfIds();
  for (i = 0; i < numPts; i++)
  {
...
//----------------------------------------------------------------------------
void svtkClosestPointStrategy::PrintSelf(ostream& os, svtkIndent indent)
{
