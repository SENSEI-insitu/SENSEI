 ...
 *
 * svtkSpline interpolates a set of data points (i.e., interpolation means
 * that the spline passes through the points).  svtkSpline is an abstract
 * class: its subclasses svtkCardinalSpline and svtkKochanekSpline do the
 ...
 * each of the x-y-z coordinates. Fortunately, the svtkParametricSpline
 ...
 * svtkCardinalSpline::Evaluate()).  Since these splines are 1D, a point in
 ...
 *
 * @sa
 * svtkCardinalSpline svtkKochanekSpline svtkParametricSpline
...
class SVTKCOMMONDATAMODEL_EXPORT svtkSpline : public svtkObject
{
public:
  svtkTypeMacro(svtkSpline, svtkObject);
   ...
   */
  svtkSetMacro(ClampValue, svtkTypeBool);
  svtkGetMacro(ClampValue, svtkTypeBool);
  svtkBooleanMacro(ClampValue, svtkTypeBool);
  //@}
   ...
   */
  virtual void DeepCopy(svtkSpline* s);

protected:
  svtkSpline();
  ~svtkSpline() override;

  svtkMTimeType ComputeTime;
  svtkTypeBool ClampValue;
  double* Intervals;
  double* Coefficients;
  int LeftConstraint;
  double LeftValue;
...
private:
  svtkSpline(const svtkSpline&) = delete;
  void operator=(const svtkSpline&) = delete;
};
