...
svtkStandardNewMacro(svtkKdNode);
svtkCxxSetObjectMacro(svtkKdNode, Left, svtkKdNode);
svtkCxxSetObjectMacro(svtkKdNode, Right, svtkKdNode);
svtkCxxSetObjectMacro(svtkKdNode, Up, svtkKdNode);

// ----------------------------------------------------------------------------
svtkKdNode::svtkKdNode()
{
...
// ----------------------------------------------------------------------------
void svtkKdNode::SetBounds(double x1, double x2, double y1, double y2, double z1, double z2)
{
  this->Min[0] = x1;
  this->Max[0] = x2;
  this->Min[1] = y1;
...
// ----------------------------------------------------------------------------
void svtkKdNode::SetMinBounds(const double* b)
{
  this->Min[0] = b[0];
  this->Min[1] = b[1];
  this->Min[2] = b[2];
...
// ----------------------------------------------------------------------------
void svtkKdNode::SetMaxBounds(const double* b)
{
  this->Max[0] = b[0];
  this->Max[1] = b[1];
  this->Max[2] = b[2];
...
// ----------------------------------------------------------------------------
void svtkKdNode::SetMinDataBounds(const double* b)
{
  this->MinVal[0] = b[0];
  this->MinVal[1] = b[1];
  this->MinVal[2] = b[2];
...
// ----------------------------------------------------------------------------
void svtkKdNode::SetMaxDataBounds(const double* b)
{
  this->MaxVal[0] = b[0];
  this->MaxVal[1] = b[1];
  this->MaxVal[2] = b[2];
...
// ----------------------------------------------------------------------------
void svtkKdNode::GetBounds(double* b) const
{
  b[0] = this->Min[0];
  b[1] = this->Max[0];
  b[2] = this->Min[1];
...
// ----------------------------------------------------------------------------
void svtkKdNode::SetDataBounds(double x1, double x2, double y1, double y2, double z1, double z2)
{
  this->MinVal[0] = x1;
  this->MaxVal[0] = x2;
  this->MinVal[1] = y1;
...
// ----------------------------------------------------------------------------
void svtkKdNode::SetDataBounds(float* v)
{
  int x;
  double newbounds[6];
...
// ----------------------------------------------------------------------------
void svtkKdNode::GetDataBounds(double* b) const
{
  b[0] = this->MinVal[0];
  b[1] = this->MaxVal[0];
  b[2] = this->MinVal[1];
...
// ----------------------------------------------------------------------------
double svtkKdNode::GetDivisionPosition()
{
  if (this->Dim == 3)
  {
    svtkErrorMacro("Called GetDivisionPosition() on a leaf node.");
    return 0.0;
  }

  svtkKdNode* left = this->GetLeft();
  if (!left)
  {
    svtkErrorMacro("Called GetDivisionPosition() on a leaf node.");
    return 0.0;
...
//
double svtkKdNode::GetDistance2ToBoundary(double x, double y, double z, int useDataBounds = 0)
{
...
//
double svtkKdNode::GetDistance2ToBoundary(
...
double svtkKdNode::GetDistance2ToInnerBoundary(double x, double y, double z)
{
  return this->_GetDistance2ToBoundary(x, y, z, nullptr, 1, 0);
}

double svtkKdNode::_GetDistance2ToBoundary(double x, double y, double z, // from this point
  ...
  int useDataBounds = 0) // use bounds of data within region instead
{
  ...
  if (withinX && withinY && withinZ) // point is inside the box
  {
    ...
    {
      int first = 1;
      minDistance = SVTK_FLOAT_MAX; // Suppresses warning message.
  ...
  else if (withinX || withinY || withinZ) // point is closest to an edge
  {
    ...
    minDistance = svtkMath::Distance2BetweenPoints(pt3, edgePt);
  ...
  else // point is closest to a corner
  {
    ...
    minDistance = svtkMath::Distance2BetweenPoints(pt3, cornerPt);
...
// ----------------------------------------------------------------------------
void svtkKdNode::AddChildNodes(svtkKdNode* left, svtkKdNode* right)
{
...
// ----------------------------------------------------------------------------
void svtkKdNode::DeleteChildNodes()
{
  if (this->GetLeft())
  {
...
//----------------------------------------------------------------------------
int svtkKdNode::IntersectsSphere2(
  double x, double y, double z, double rSquared, int useDataBounds = 0)
{
  svtkTypeBool center = this->ContainsPoint(x, y, z, useDataBounds);
...
// ----------------------------------------------------------------------------
svtkTypeBool svtkKdNode::ContainsPoint(double x, double y, double z, int useDataBounds = 0)
{
  double *min, *max;
...
// ----------------------------------------------------------------------------
int svtkKdNode::IntersectsRegion(svtkPlanesIntersection* pi, int useDataBounds)
{
...
// ----------------------------------------------------------------------------
int svtkKdNode::IntersectsCell(svtkCell* cell, int useDataBounds, int cellRegion, double* bounds)
{
  svtkIdType i;
  ...
  if (dim == 1) // lines
  {
    ...
    for (i = 0; i < npts - 1; i++)
    {
      ...
      char result = svtkBox::IntersectBox(regionBounds, p1, dir, x, t);
  ...
  else if (dim == 2) // polygons
  {
    ...
    if (cell->GetCellType() == SVTK_TRIANGLE_STRIP)
    {
      svtkPoints* triangle = svtkPoints::New();
      ...
      for (i = 2; i < npts; i++)
      {
        ...
        intersects = svtkPlanesIntersection::PolygonIntersectsBBox(regionBounds, triangle);
    ...
    {
      intersects = svtkPlanesIntersection::PolygonIntersectsBBox(regionBounds, pts);
    }
  }
  else if (dim == 3) // 3D cells
  {
    svtkPlanesIntersection* pi = svtkPlanesIntersection::Convert3DCell(cell);
...
// ----------------------------------------------------------------------------
void svtkKdNode::PrintNode(int depth)
{
  if ((depth < 0) || (depth > 19))
  {
    depth = 19;
...
// ----------------------------------------------------------------------------
void svtkKdNode::PrintVerboseNode(int depth)
{
  int i;
...
// ----------------------------------------------------------------------------
void svtkKdNode::PrintSelf(ostream& os, svtkIndent indent)
{
