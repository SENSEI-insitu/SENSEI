/*=========================================================================
  ...
  Module:    svtkHigherOrderCurve.cxx
...
svtkHigherOrderCurve::svtkHigherOrderCurve()
{
  this->Approx = nullptr;
  this->Order[0] = 1;
...
svtkHigherOrderCurve::~svtkHigherOrderCurve() = default;

void svtkHigherOrderCurve::PrintSelf(ostream& os, svtkIndent indent)
{
...
void svtkHigherOrderCurve::Initialize() {}

int svtkHigherOrderCurve::CellBoundary(
...
int svtkHigherOrderCurve::EvaluatePosition(const double x[3], double closestPoint[3], int& subId,
  double pcoords[3], double& minDist2, double weights[])
{
  ...
  svtkIdType nseg = svtkHigherOrderInterpolation::NumberOfIntervals<1>(this->GetOrder());
  for (int subCell = 0; subCell < nseg; ++subCell)
  {
    svtkLine* approx = this->GetApproximateLine(subCell, nullptr, nullptr);
...
void svtkHigherOrderCurve::EvaluateLocation(
...
void svtkHigherOrderCurve::Contour(double value, svtkDataArray* cellScalars,
...
void svtkHigherOrderCurve::Clip(double value, svtkDataArray* cellScalars,
...
int svtkHigherOrderCurve::IntersectWithLine(
  const double* p1, const double* p2, double tol, double& t, double* x, double* pcoords, int& subId)
{
  ...
  for (int i = 0; i < nseg; ++i)
  {
    svtkLine* approx = this->GetApproximateLine(i);
...
int svtkHigherOrderCurve::Triangulate(int svtkNotUsed(index), svtkIdList* ptIds, svtkPoints* pts)
{
  ptIds->Reset();
  pts->Reset();
...
void svtkHigherOrderCurve::Derivatives(int svtkNotUsed(subId), const double svtkNotUsed(pcoords)[3],
  const double* svtkNotUsed(values), int svtkNotUsed(dim), double* svtkNotUsed(derivs))
{
  ...
  svtkErrorMacro("Derivatives() is not implemented for svtkHigherOrderCurve.");
}

void svtkHigherOrderCurve::SetParametricCoords()
{
  if (!this->PointParametricCoordinates)
  {
  ...
  if (static_cast<int>(this->PointParametricCoordinates->GetNumberOfPoints()) != this->GetOrder(1))
  {
    ...
    svtkHigherOrderInterpolation::AppendCurveCollocationPoints(
      this->PointParametricCoordinates, this->Order);
  }
}

double* svtkHigherOrderCurve::GetParametricCoords()
{
...
double svtkHigherOrderCurve::GetParametricDistance(const double pcoords[3])
{
...
const int* svtkHigherOrderCurve::GetOrder()
{
...
svtkLine* svtkHigherOrderCurve::GetApprox()
{
  if (!this->Approx)
  {
    this->Approx = svtkSmartPointer<svtkLine>::New();
 ...
 */
void svtkHigherOrderCurve::PrepareApproxData(
  svtkPointData* pd, svtkCellData* cd, svtkIdType cellId, svtkDataArray* cellScalars)
{
  ...
  svtkIdType npts = this->Order[1];
  svtkIdType nele = this->Order[0];
...
bool svtkHigherOrderCurve::SubCellCoordinatesFromId(svtkVector3i& ijk, int subId)
{
  ijk[1] = ijk[2] = 0;
 ...
 * up to date.
 */
bool svtkHigherOrderCurve::SubCellCoordinatesFromId(int& i, int subId)
{
  if (subId < 0)
  {
    return false;
 ...
 */
int svtkHigherOrderCurve::PointIndexFromIJK(int i, int svtkNotUsed(j), int svtkNotUsed(k))
{
 ...
 */
bool svtkHigherOrderCurve::TransformApproxToCellParams(int subCell, double* pcoords)
{
  svtkVector3i ijk;
