 ...
 *
 * svtkXMLDataElement is used by svtkXMLDataParser to represent an XML
 ...
 * traversal of an input XML file by svtkXMLReader and its subclasses.
 *
 * @sa
 * svtkXMLDataParser
 */

#ifndef svtkXMLDataElement_h
#define svtkXMLDataElement_h
...
class SVTKCOMMONDATAMODEL_EXPORT svtkXMLDataElement : public svtkObject
{
public:
  svtkTypeMacro(svtkXMLDataElement, svtkObject);
  void PrintSelf(ostream& os, svtkIndent indent) override;
  static svtkXMLDataElement* New();
   ...
   */
  svtkGetStringMacro(Id);
  svtkSetStringMacro(Id);
  //@}
  ...
  svtkGetStringMacro(CharacterData);
  //@}
   ...
   * such as SVTK_FLOAT or SVTK_UNSIGNED_LONG.
   */
   ...
   */
  svtkGetMacro(NumberOfAttributes, int);
  //@}
   ...
   */
  svtkXMLDataElement* GetParent();
  void SetParent(svtkXMLDataElement* parent);
  //@}

  /**
   * Get root of the XML tree this element is part of.
   */
  virtual svtkXMLDataElement* GetRoot();
   ...
   */
  svtkXMLDataElement* GetNestedElement(int index);

  /**
   * Add nested element
   */
  void AddNestedElement(svtkXMLDataElement* element);

  /**
   * Remove nested element.
   */
  virtual void RemoveNestedElement(svtkXMLDataElement*);
   ...
   * the grand-children.
   */
  svtkXMLDataElement* FindNestedElement(const char* id);
  svtkXMLDataElement* FindNestedElementWithName(const char* name);
  svtkXMLDataElement* FindNestedElementWithNameAndId(const char* name, const char* id);
  svtkXMLDataElement* FindNestedElementWithNameAndAttribute(
   ...
   */
  svtkXMLDataElement* LookupElementWithName(const char* name);

  /**
   * Lookup the element with the given id, starting at this scope.
   */
  svtkXMLDataElement* LookupElement(const char* id);

  //@{
  /**
   * Set/Get the offset from the beginning of the XML document to this element.
   */
  svtkGetMacro(XMLByteIndex, svtkTypeInt64);
  svtkSetMacro(XMLByteIndex, svtkTypeInt64);
  //@}
   ...
   */
  virtual int IsEqualTo(svtkXMLDataElement* elem);
   ...
   */
  virtual void DeepCopy(svtkXMLDataElement* elem);

  //@{
  /**
   * Get/Set the internal character encoding of the attributes.
   * Default type is SVTK_ENCODING_UTF_8.
   * Note that a svtkXMLDataParser has its own AttributesEncoding ivar. If
   * this ivar is set to something other than SVTK_ENCODING_NONE, it will be
   * used to set the attribute encoding of each svtkXMLDataElement
   * created by this svtkXMLDataParser.
   */
  svtkSetClampMacro(AttributeEncoding, int, SVTK_ENCODING_NONE, SVTK_ENCODING_UNKNOWN);
  svtkGetMacro(AttributeEncoding, int);
  //@}
   ...
   */
  void PrintXML(ostream& os, svtkIndent indent);
   ...
   */
  svtkGetMacro(CharacterDataWidth, int);
  svtkSetMacro(CharacterDataWidth, int);
  //@}

protected:
  svtkXMLDataElement();
  ~svtkXMLDataElement() override;
  ...
  svtkGetMacro(InlineDataPosition, svtkTypeInt64);
  svtkSetMacro(InlineDataPosition, svtkTypeInt64);
  // The offset into the XML stream where the inline data begins.
  svtkTypeInt64 InlineDataPosition;
  // The offset into the XML stream where the element begins.
  svtkTypeInt64 XMLByteIndex;
  ...
  svtkXMLDataElement** NestedElements;
  // The parent of this element.
  svtkXMLDataElement* Parent;

  // Internal utility methods.
  svtkXMLDataElement* LookupElementInScope(const char* id);
  svtkXMLDataElement* LookupElementUpScope(const char* id);
  static int IsSpace(char c);
  void PrintCharacterData(ostream& os, svtkIndent indent);
  static void PrintWithEscapedData(ostream& os, const char* data);

  friend class svtkXMLDataParser;
  friend class svtkXMLMaterialParser;

private:
  svtkXMLDataElement(const svtkXMLDataElement&) = delete;
  void operator=(const svtkXMLDataElement&) = delete;
};

//----------------------------------------------------------------------------
inline void svtkXMLDataElement::AddCharacterData(const char* data, size_t length)
{
