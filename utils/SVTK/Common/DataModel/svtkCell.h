 ...
 *
 * svtkCell is an abstract class that specifies the interfaces for data cells.
 ...
 * svtkPolyData, svtkUnstructuredGrid), and in some cases, the datasets are
 * implicitly composed of cells (e.g., svtkStructuredPoints).
 *
 * @warning
 * The \#define SVTK_CELL_SIZE is a parameter used to construct cells and provide
 ...
 *
 * @sa
 * svtkHexahedron svtkLine svtkPixel svtkPolyLine svtkPolyVertex
 * svtkPolygon svtkQuad svtkTetra svtkTriangle
 * svtkTriangleStrip svtkVertex svtkVoxel svtkWedge svtkPyramid
 */

#ifndef svtkCell_h
#define svtkCell_h

#define SVTK_CELL_SIZE 512
#define SVTK_TOL 1.e-05 // Tolerance for geometric calculation

#include "svtkCommonDataModelModule.h" // For export macro
#include "svtkObject.h"

#include "svtkCellType.h" // Needed to define cell types
...
class SVTKCOMMONDATAMODEL_EXPORT svtkCell : public svtkObject
{
public:
  svtkTypeMacro(svtkCell, svtkObject);
   ...
   */
  void Initialize(int npts, const svtkIdType* pts, svtkPoints* p);
   ...
   */
  void Initialize(int npts, svtkPoints* p);
   ...
   * and connectivity list information.  Most cells in SVTK are implicit
   * cells.
   */
   ...
   * Most cells in SVTK are implicit cells.
   */
  virtual int IsExplicitCell() { return 0; }

  /**
   * Determine whether the cell requires explicit face representation, and
   * methods for setting and getting the faces (see svtkPolyhedron for example
   * usage of these methods).
   */
  virtual int RequiresExplicitFaceRepresentation() { return 0; }
  virtual void SetFaces(svtkIdType* svtkNotUsed(faces)) {}
  virtual svtkIdType* GetFaces() { return nullptr; }
   ...
   */
  svtkIdType GetNumberOfPoints() { return this->PointIds->GetNumberOfIds(); }
   ...
   */
  svtkIdList* GetPointIds() { return this->PointIds; }

  /**
   * For cell point i, return the actual point id.
   */
  svtkIdType GetPointId(int ptId) SVTK_EXPECTS(0 <= ptId && ptId < GetPointIds()->GetNumberOfIds())
  {
   ...
   */
  virtual svtkCell* GetEdge(int edgeId) = 0;

  /**
   * Return the face cell from the faceId of the cell.
   */
  virtual svtkCell* GetFace(int faceId) = 0;
   ...
   */
  virtual int CellBoundary(int subId, const double pcoords[3], svtkIdList* pts) = 0;
   ...
   */
  virtual void Contour(double value, svtkDataArray* cellScalars, svtkIncrementalPointLocator* locator,
    svtkCellArray* verts, svtkCellArray* lines, svtkCellArray* polys, svtkPointData* inPd,
    svtkPointData* outPd, svtkCellData* inCd, svtkIdType cellId, svtkCellData* outCd) = 0;
   ...
   * cellId refers to the cell from which the cell data is copied.)
   */
  virtual void Clip(double value, svtkDataArray* cellScalars, svtkIncrementalPointLocator* locator,
    svtkCellArray* connectivity, svtkPointData* inPd, svtkPointData* outPd, svtkCellData* inCd,
    svtkIdType cellId, svtkCellData* outCd, int insideOut) = 0;
   ...
   */
  virtual int Triangulate(int index, svtkIdList* ptIds, svtkPoints* pts) = 0;
   ...
   * topology varies depending on the data (e.g., svtkConvexPointSet).
   ...
   */
  virtual double* GetParametricCoords() SVTK_SIZEHINT(3 * GetNumberOfPoints());
   ...
   */
  virtual void InterpolateFunctions(const double svtkNotUsed(pcoords)[3], double* svtkNotUsed(weight))
  {
  }
  virtual void InterpolateDerivs(const double svtkNotUsed(pcoords)[3], double* svtkNotUsed(derivs)) {}
...
private:
  svtkCell(const svtkCell&) = delete;
  void operator=(const svtkCell&) = delete;
};
