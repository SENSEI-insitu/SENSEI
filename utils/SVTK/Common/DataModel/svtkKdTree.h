 ...
 *
 *
 *     Given one or more svtkDataSets, create a load balancing
 ...
 *
 *     If the points were derived from cells, svtkKdTree
 ...
 *     from one or more svtkPoints objects.  This can be followed by
 ...
 *
 * @sa
 *      svtkLocator svtkCellLocator svtkPKdTree
 */
...
class SVTKCOMMONDATAMODEL_EXPORT svtkKdTree : public svtkLocator
{
public:
  svtkTypeMacro(svtkKdTree, svtkLocator);
  void PrintSelf(ostream& os, svtkIndent indent) override;

  static svtkKdTree* New();

  //@{
  /**
   * Turn on timing of the k-d tree build
   */
  svtkBooleanMacro(Timing, svtkTypeBool);
  svtkSetMacro(Timing, svtkTypeBool);
  svtkGetMacro(Timing, svtkTypeBool);
  //@}
   ...
   */
  svtkSetMacro(MinCells, int);
  svtkGetMacro(MinCells, int);
  //@}
  ...
  svtkGetMacro(NumberOfRegionsOrLess, int);
  svtkSetMacro(NumberOfRegionsOrLess, int);
  ...
  svtkGetMacro(NumberOfRegionsOrMore, int);
  svtkSetMacro(NumberOfRegionsOrMore, int);
  ...
  /**
   * Get a svtkBSPCuts object, a general object representing an axis-
   * aligned spatial partitioning.  Used by svtkBSPIntersections.
   */
   ...
   */
  void SetDataSet(svtkDataSet* set) override;
   ...
   */
  virtual void AddDataSet(svtkDataSet* set);
  ...
  virtual void RemoveDataSet(svtkDataSet* set);
   ...
   */
  svtkDataSet* GetDataSet(int n);

  /**
   * Return the 0'th data set.  For compatibility with the superclass'
   * interface.
   */
  svtkDataSet* GetDataSet() override { return this->GetDataSet(0); }

  //@{
  /**
   * Return a collection of all the data sets.
   */
  svtkGetObjectMacro(DataSets, svtkDataSetCollection);
  //@}
   ...
   * set does not exist.
   */
  int GetDataSetIndex(svtkDataSet* set);
   ...
   */
  svtkGetMacro(NumberOfRegions, int);
  //@}
  ...
  void CreateCellLists(svtkDataSet* set, int* regionReqList, int reqListSize);
   ...
   */
  svtkSetMacro(IncludeRegionBoundaryCells, svtkTypeBool);
  svtkGetMacro(IncludeRegionBoundaryCells, svtkTypeBool);
  svtkBooleanMacro(IncludeRegionBoundaryCells, svtkTypeBool);
  //@}
   ...
   * to svtkKdTree's memory, so don't free it.
   */
  svtkIdList* GetCellList(int regionID);
   ...
   */
  svtkIdList* GetBoundaryCellList(int regionID);
   ...
   */
  svtkIdType GetCellLists(
    svtkIntArray* regions, int set, svtkIdList* inRegionCells, svtkIdList* onBoundaryCells);
  svtkIdType GetCellLists(
   ...
   */
  int GetRegionContainingCell(svtkDataSet* set, svtkIdType cellID);
  int GetRegionContainingCell(int set, svtkIdType cellID);
  int GetRegionContainingCell(svtkIdType cellID);
  //@}
   ...
   * svtkKdTree uses this list, so don't delete it.
   */
  ...
  int MinimalNumberOfConvexSubRegions(svtkIntArray* regionIdList, double** convexRegionBounds);

  /**
   * Given a direction of projection (typically obtained with
   * svtkCamera::GetDirectionOfProjection()), this method, creates a list of the
  ...
  /**
   * Given a camera position (typically obtained with svtkCamera::GetPosition()),
   ...
   * one or more svtkPoints objects instead of from the cells of
   * a svtkDataSet.  This build would normally be followed by
   ...
   * float arrays.
   */
  void BuildLocatorFromPoints(svtkPointSet* pointset);
  void BuildLocatorFromPoints(svtkPoints* ptArray);
  void BuildLocatorFromPoints(svtkPoints** ptArray, int numPtArrays);
  //@}
   ...
   * point in the first svtkPoints object, and increase by 1 for subsequent
   * points and subsequent svtkPoints objects.

   * You must have called BuildLocatorFromPoints() before calling this.
   * You are responsible for deleting the returned array.
   */
  svtkIdTypeArray* BuildMapForDuplicatePoints(float tolerance);
   ...
   */
  svtkIdType FindClosestPoint(double* x, double& dist2);
   ...
   */
  svtkIdType FindClosestPointWithinRadius(double radius, const double x[3], double& dist2);
   ...
   */
  svtkIdTypeArray* GetPointsInRegion(int regionId);
   ...
   */
  void GenerateRepresentation(int* regionList, int len, svtkPolyData* pd);
   ...
   */
  svtkBooleanMacro(GenerateRepresentationUsingDataBounds, svtkTypeBool);
  svtkSetMacro(GenerateRepresentationUsingDataBounds, svtkTypeBool);
  svtkGetMacro(GenerateRepresentationUsingDataBounds, svtkTypeBool);
  //@}

  /**
   * Print timing of k-d tree build
   */
  virtual void PrintTiming(ostream& os, svtkIndent indent);
   ...
   * the k-d tree.
   */
  virtual int NewGeometry(svtkDataSet** sets, int numDataSets);
  ...
  static svtkKdNode* CopyTree(svtkKdNode* kd);
...
protected:
  ...
  svtkBSPIntersections* BSPCalculator;
  int UserDefinedCuts;

  void SetCalculator(svtkKdNode* kd);

  int ProcessUserDefinedCuts(double* bounds);

  void SetCuts(svtkBSPCuts* cuts, int userDefined);
  ...
  svtkKdNode* Top;
  svtkKdNode** RegionList; // indexed by region ID

  svtkTimerLog* TimerLog;

  static void DeleteAllDescendants(svtkKdNode* nd);

  void BuildRegionList();
  virtual int SelectCutDirection(svtkKdNode* kd);
  void SetActualLevel() { this->Level = svtkKdTree::ComputeLevel(this->Top); }
  ...
  void GetRegionsAtLevel(int level, svtkKdNode** nodes);

  /**
   * Adds to the svtkIntArray the list of region IDs of all leaf
   * nodes in the given node.
   */

  static void GetLeafNodeIds(svtkKdNode* node, svtkIntArray* ids);
  ...
  void ComputeCellCenter(svtkDataSet* set, int cellId, float* center);
  void ComputeCellCenter(svtkDataSet* set, int cellId, double* center);
  ...
  float* ComputeCellCenters(svtkDataSet* set);

  svtkDataSetCollection* DataSets;

  /**
   * Modelled on svtkAlgorithm::UpdateProgress().
   * Update the progress when building the locator.
   * Fires svtkCommand::ProgressEvent.
   */
   ...
   */
  svtkSetClampMacro(Progress, double, 0.0, 1.0);
...
protected:
  ...
  static void _SetNewBounds(svtkKdNode* kd, double* b, int* fixDim);
  static void CopyChildNodes(svtkKdNode* to, svtkKdNode* from);
  static void CopyKdNode(svtkKdNode* to, svtkKdNode* from);
  static void SetDataBoundsToSpatialBounds(svtkKdNode* kd);
  static void ZeroNumberOfPoints(svtkKdNode* kd);

  // Recursive helper for public FindPointsWithinRadius
  void FindPointsWithinRadius(svtkKdNode* node, double R2, const double x[3], svtkIdList* ids);

  // Recursive helper for public FindPointsWithinRadius
  void AddAllPointsInRegion(svtkKdNode* node, svtkIdList* ids);

  // Recursive helper for public FindPointsInArea
  void FindPointsInArea(svtkKdNode* node, double* area, svtkIdTypeArray* ids);

  // Recursive helper for public FindPointsInArea
  void AddAllPointsInRegion(svtkKdNode* node, svtkIdTypeArray* ids);

  int DivideRegion(svtkKdNode* kd, float* c1, int* ids, int nlevels);

  void DoMedianFind(svtkKdNode* kd, float* c1, int* ids, int d1, int d2, int d3);

  void SelfRegister(svtkKdNode* kd);

  struct _cellList
  {
    svtkDataSet* dataSet; // cell lists for which data set
    int* regionIds;      // nullptr if listing all regions
    int nRegions;
    svtkIdList** cells;
    svtkIdList** boundaryCells;
    svtkIdList* emptyList;
  };

  void InitializeCellLists();
  svtkIdList* GetList(int regionId, svtkIdList** which);

  void ComputeCellCenter(svtkCell* cell, double* center, double* weights);

  void GenerateRepresentationDataBounds(int level, svtkPolyData* pd);
  void _generateRepresentationDataBounds(
    svtkKdNode* kd, svtkPoints* pts, svtkCellArray* polys, int level);

  void GenerateRepresentationWholeSpace(int level, svtkPolyData* pd);
  void _generateRepresentationWholeSpace(
    svtkKdNode* kd, svtkPoints* pts, svtkCellArray* polys, int level);

  void AddPolys(svtkKdNode* kd, svtkPoints* pts, svtkCellArray* polys);
  ...
  int _ViewOrderRegionsInDirection(
    svtkIntArray* IdsOfInterest, const double dop[3], svtkIntArray* orderedList);

  static int __ViewOrderRegionsInDirection(svtkKdNode* node, svtkIntArray* list,
    svtkIntArray* IdsOfInterest, const double dir[3], int nextId);

  int _ViewOrderRegionsFromPosition(
    svtkIntArray* IdsOfInterest, const double pos[3], svtkIntArray* orderedList);

  static int __ViewOrderRegionsFromPosition(svtkKdNode* node, svtkIntArray* list,
    svtkIntArray* IdsOfInterest, const double pos[3], int nextId);

  static int __ConvexSubRegions(int* ids, int len, svtkKdNode* tree, svtkKdNode** nodes);
  static int FoundId(svtkIntArray* idArray, int id);
  ...
  static void __printTree(svtkKdNode* kd, int depth, int verbose);
  ...
  static int ComputeLevel(svtkKdNode* kd);
  static int SelfOrder(int id, svtkKdNode* kd);
  static int findRegion(svtkKdNode* node, float x, float y, float z);
  static int findRegion(svtkKdNode* node, double x, double y, double z);

  static svtkKdNode** _GetRegionsAtLevel(int level, svtkKdNode** nodes, svtkKdNode* kd);

  static void AddNewRegions(svtkKdNode* kd, float* c1, int midpt, int dim, double coord);
  ...
  svtkTypeBool IncludeRegionBoundaryCells;
  double CellBoundsCache[6]; // to optimize IntersectsCell()

  svtkTypeBool GenerateRepresentationUsingDataBounds;
  ...
  svtkDataSet** LastInputDataSets;
  unsigned long* LastDataSetObserverTags;
  int* LastDataSetType;
  double* LastInputDataInfo;
  double* LastBounds;
  svtkIdType* LastNumPoints;
  svtkIdType* LastNumCells;

  svtkBSPCuts* Cuts;
  double Progress;

  svtkKdTree(const svtkKdTree&) = delete;
  void operator=(const svtkKdTree&) = delete;
};
#endif
