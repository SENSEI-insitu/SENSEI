 ...
 *
 * In SVTK, spatial-temporal data is defined in terms of a dataset. The
 * dataset consists of geometry (e.g., points), topology (e.g., cells), and
 * attributes (e.g., scalars, vectors, etc.) svtkGenericDataSet is an abstract
 * class defining this abstraction.
 *
 * Since svtkGenericDataSet provides a general interface to manipulate data,
 ...
 * SVTK/GenericFiltering/ subdirectory.
 *
 * Unlike the svtkDataSet class, svtkGenericDataSet provides a more flexible
 * interface including support for iterators. svtkGenericDataSet is also
 * designed to interface SVTK to external simulation packages without the
 * penalty of copying memory (see SVTK/GenericFiltering/README.html) for
 * more information. Thus svtkGenericDataSet plays a central role in the
 * adaptor framework.
 *
 ...
 * by the adaptor framework (see svtkGenericAdaptorCell for more information).
 *
 * @sa
 * svtkGenericAdaptorCell svtkDataSet
 */

#ifndef svtkGenericDataSet_h
#define svtkGenericDataSet_h
...
class SVTKCOMMONDATAMODEL_EXPORT svtkGenericDataSet : public svtkDataObject
{
public:
  //@{
  /**
   * Standard SVTK type and print macros.
   */
  svtkTypeMacro(svtkGenericDataSet, svtkDataObject);
   ...
   * \post positive_result: result>=0
   */
  virtual svtkIdType GetNumberOfCells(int dim = -1) = 0;
   ...
   */
  SVTK_NEWINSTANCE
  virtual svtkGenericCellIterator* NewCellIterator(int dim = -1) = 0;
   ...
   */
  SVTK_NEWINSTANCE
  virtual svtkGenericCellIterator* NewBoundaryIterator(int dim = -1, int exteriorOnly = 0) = 0;
   ...
   */
  SVTK_NEWINSTANCE
  virtual svtkGenericPointIterator* NewPointIterator() = 0;
  ...
  virtual int FindCell(
    double x[3], svtkGenericCellIterator*& cell, double tol2, int& subId, double pcoords[3]) = 0;
   ...
   * \pre p_exists: p!=0
   */
  virtual void FindPoint(double x[3], svtkGenericPointIterator* p) = 0;
   ...
   */
  svtkGetObjectMacro(Attributes, svtkGenericAttributeCollection);
  //@}
   ...
   */
  svtkDataSetAttributes* GetAttributes(int type) override
  {
   ...
   */
  virtual void SetTessellator(svtkGenericCellTessellator* tessellator);
  svtkGetObjectMacro(Tessellator, svtkGenericCellTessellator);
  //@}
   ...
   */
  virtual svtkIdType GetEstimatedSize() = 0;

  //@{
  /**
   * Retrieve an instance of this class from an information object.
   */
  static svtkGenericDataSet* GetData(svtkInformation* info);
  static svtkGenericDataSet* GetData(svtkInformationVector* v, int i = 0);
  //@}
...
private:
  svtkGenericDataSet(const svtkGenericDataSet&) = delete;
  void operator=(const svtkGenericDataSet&) = delete;
};
