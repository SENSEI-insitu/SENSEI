/*=========================================================================
  ...
  Module:    svtkOrderedTriangulator.cxx
...
svtkStandardNewMacro(svtkOrderedTriangulator);
...
{
  void* operator new(size_t size, svtkHeap* heap) { return heap->AllocateMemory(size); }
  void operator delete(void*, svtkHeap*) {}
  ...
  void ComputePseudoNormal()
  {
    ...
    svtkMath::Cross(v10, v20, this->Normal);
    this->N2 = svtkMath::Dot(this->Normal, this->Normal);
  }
...
struct svtkOTMesh
{
  ...
  ~svtkOTMesh() { this->EdgeTable->Delete(); }
  ...
  svtkEdgeTable* EdgeTable;        // Edges used to create triangulation of cavity
  double Tolerance2;              // Used to control error
  svtkHeap* Heap;                  // Many allocations occur in efficient heap
...
struct OTTemplate
{
  svtkIdType NumberOfTetras;
  svtkIdType* Tetras;

  OTTemplate(svtkIdType numberOfTetras, svtkHeap* heap)
  {
    this->NumberOfTetras = numberOfTetras;
    this->Tetras =
      static_cast<svtkIdType*>(heap->AllocateMemory(sizeof(svtkIdType) * numberOfTetras * 4));
  }
  void* operator new(size_t size, svtkHeap* heap) { return heap->AllocateMemory(size); }
  void operator delete(void*, svtkHeap*) {}
};
...
struct svtkOTTemplates : public std::map<int, TemplateList*>
{
};
typedef std::map<int, TemplateList*>::iterator TemplatesIterator;

//------------------------------------------------------------------------
svtkOrderedTriangulator::svtkOrderedTriangulator()
{
  // In place news (using allocators) are done here
  this->Heap = svtkHeap::New();
  this->Heap->SetBlockSize(500000);

  this->Mesh = new svtkOTMesh(this->Heap);
  ...
  this->Templates = new svtkOTTemplates;
  this->TemplateHeap = svtkHeap::New();
...
//------------------------------------------------------------------------
void svtkOrderedTriangulator::InitTriangulation(
...
//------------------------------------------------------------------------
void svtkOrderedTriangulator::InitTriangulation(double bounds[6], int numPts)
{
  this->Heap->Reset();
  this->Mesh->Reset();
  this->NumberOfPoints = 0;
...
void svtkOrderedTriangulator::Initialize()
{
  double length;
  double center[3];
  double radius2;
...
svtkIdType svtkOrderedTriangulator::InsertPoint(svtkIdType id, double x[3], double p[3], int type)
{
  svtkIdType idx = this->NumberOfPoints++;
  if (idx >= this->MaximumNumberOfPoints)
  {
    svtkErrorMacro(<< "Trying to insert more points than specified max="
...
//------------------------------------------------------------------------
// Add a point to the list of points to be triangulated.
svtkIdType svtkOrderedTriangulator::InsertPoint(
  svtkIdType id, svtkIdType sortid, double x[3], double p[3], int type)
{
  svtkIdType idx = this->NumberOfPoints++;
  if (idx >= this->MaximumNumberOfPoints)
  {
    svtkErrorMacro(<< "Trying to insert more points than specified");
    return idx;
  }
...
// Add a point to the list of points to be triangulated.
svtkIdType svtkOrderedTriangulator::InsertPoint(
  svtkIdType id, svtkIdType sortid, svtkIdType sortid2, double x[3], double p[3], int type)
{
  svtkIdType idx = this->NumberOfPoints++;
  if (idx >= this->MaximumNumberOfPoints)
  {
...
void svtkOrderedTriangulator::UpdatePointType(svtkIdType internalId, int type)
{
...
//------------------------------------------------------------------------
double* svtkOrderedTriangulator::GetPointPosition(svtkIdType internalId)
{
  assert("pre: valid_range" && internalId >= 0 && internalId < this->NumberOfPoints);
...
//------------------------------------------------------------------------
double* svtkOrderedTriangulator::GetPointLocation(svtkIdType internalId)
{
  assert("pre: valid_range" && internalId >= 0 && internalId < this->NumberOfPoints);
...
//------------------------------------------------------------------------
svtkIdType svtkOrderedTriangulator::GetPointId(svtkIdType internalId)
{
  assert("pre: valid_range" && internalId >= 0 && internalId < this->NumberOfPoints);
...
{
#ifdef _WIN32_WCE
  static int __cdecl svtkSortOnIds(const void* val1, const void* val2)
#else
  static int svtkSortOnIds(const void* val1, const void* val2)
#endif
  {
    svtkIdType sortId1 = static_cast<const OTPoint*>(val1)->SortId;
    svtkIdType sortId2 = static_cast<const OTPoint*>(val2)->SortId;
...
{
#ifdef _WIN32_WCE
  static int __cdecl svtkSortOnTwoIds(const void* val1, const void* val2)
#else
  static int svtkSortOnTwoIds(const void* val1, const void* val2)
#endif
  {
    svtkIdType sortId1 = static_cast<const OTPoint*>(val1)->SortId2;
    svtkIdType sortId2 = static_cast<const OTPoint*>(val2)->SortId2;
...
inline static svtkTypeBool IsAPoint(OTTetra* t, svtkIdType id)
{
...
static void AssignNeighbors(OTTetra* t1, OTTetra* t2)
{
  ...
  switch (index)
  {
    ...
    case 7:
      ...
      break;
    default:
      svtkGenericWarningMacro(<< "Really bad");
  }
...
OTTetra* svtkOTMesh::CreateTetra(OTPoint* p, OTFace* face)
{
  OTTetra* tetra = new (this->Heap) OTTetra;
  this->Tetras.push_front(tetra);
  tetra->Radius2 = svtkTetra::Circumsphere(
...
int svtkOTMesh::CreateInsertionCavity(OTPoint* p, OTTetra* initialTet, double[4])
{
...
int svtkOTMesh::ClassifyTetras()
{
  TetraListIterator t;
  svtkIdType numInsideTetras = 0;
...
//------------------------------------------------------------------------
// Used to debug (writes a SVTK file representing the current insertion cavity).
void svtkOTMesh::DumpInsertionCavity(double x[3])
{
  OTFace* face;
  FaceListIterator fptr;

  cout << "# svtk DataFile Version 3.0\n";
...
OTTetra* svtkOTMesh::WalkToTetra(OTTetra* tetra, double x[3], int depth, double bc[4])
{
  int neg = 0;
  int j, numNeg;
  double negValue;
...
void svtkOrderedTriangulator::Triangulate()
{
  OTPoint* p;
  int i;
  svtkIdType ptId;
  ...
  if (!this->PreSorted)
  {
    ...
    {
      qsort(
        this->Mesh->Points.GetPointer(0), this->NumberOfPoints, sizeof(OTPoint), svtkSortOnTwoIds);
    }
    else
    {
      qsort(this->Mesh->Points.GetPointer(0), this->NumberOfPoints, sizeof(OTPoint), svtkSortOnIds);
    }
  }
  ...
  for (ptId = 0, p = this->Mesh->Points.GetPointer(0); ptId < this->NumberOfPoints; ++p, ++ptId)
  {
    ...
    {
      svtkDebugMacro(<< "Point not in tetrahedron");
      continue;
    }
...
void svtkOrderedTriangulator::TemplateTriangulate(int cellType, int numPts, int numEdges)
{
  this->CellType = cellType;
  if (!this->UseTemplates || cellType != SVTK_HEXAHEDRON)
  {
    this->Triangulate();
    return;
  }
...
svtkIdType svtkOrderedTriangulator::GetTetras(int classification, svtkUnstructuredGrid* ugrid)
{
  // Create the points
  //
  int i;
  svtkIdType numTetras = 0;
  PointListIterator p;
  ...
  for (t = this->Mesh->Tetras.begin(); t != this->Mesh->Tetras.end(); ++t)
  {
    ...
    if (tetra->Type == classification || classification == OTTetra::All)
    {
      ...
      ugrid->InsertNextCell(SVTK_TETRA, 4, pts);
    }
  } // for all tetras
...
//------------------------------------------------------------------------
// Add the tetras classified as specified to an unstructured grid
svtkIdType svtkOrderedTriangulator::AddTetras(int classification, svtkCellArray* outConnectivity)
{
  TetraListIterator t;
  OTTetra* tetra;
  svtkIdType numTetras = 0;
...
svtkIdType svtkOrderedTriangulator::AddTetras(int classification, svtkIncrementalPointLocator* locator,
  svtkCellArray* outConnectivity, svtkPointData* inPD, svtkPointData* outPD, svtkCellData* inCD,
  svtkIdType cellId, svtkCellData* outCD)
{
  ...
  for (t = this->Mesh->Tetras.begin(); t != this->Mesh->Tetras.end(); ++t)
  {
    ...
    if (tetra->Type == classification || classification == OTTetra::All)
    {
      ...
      result++;
      svtkIdType newCellId = outConnectivity->InsertNextCell(4, pts);
...
void svtkOrderedTriangulator::InitTetraTraversal()
{
...
int svtkOrderedTriangulator::GetNextTetra(
  int classification, svtkTetra* tet, svtkDataArray* cellScalars, svtkDoubleArray* tetScalars)
{
...
// point coordinates.
svtkIdType svtkOrderedTriangulator::AddTetras(int classification, svtkIdList* ptIds, svtkPoints* pts)
{
  TetraListIterator t;
  OTTetra* tetra;
  svtkIdType numTetras = 0;
...
//------------------------------------------------------------------------
// Add the tetras classified as specified to an unstructured grid
svtkIdType svtkOrderedTriangulator::AddTetras(int classification, svtkUnstructuredGrid* ugrid)
...
svtkIdType svtkOrderedTriangulator::AddTriangles(svtkCellArray* tris)
{
  ...
  {
    tetra = *t;
    tetra->CurrentPointId = SVTK_INT_MAX; // mark visited
    for (i = 0; i < 4; i++)
    {
      if (tetra->Neighbors[i] && tetra->Neighbors[i]->CurrentPointId != SVTK_INT_MAX &&
...
svtkIdType svtkOrderedTriangulator::AddTriangles(svtkIdType id, svtkCellArray* tris)
{
  svtkIdType numTris = 0;
  int i;
...
// cell maximum (this is due to the use of the svtkHexahedron). Any point can
...
// a template id.
inline TemplateIDType svtkOrderedTriangulator::ComputeTemplateIndex()
{
...
void svtkOrderedTriangulator::AddTemplate()
{
  ...
  {
    svtkGenericWarningMacro(<< "Template found when it should not have been");
  }
  else
  {
    ...
    TetraListIterator t;
    OTTetra* tetra;
    svtkIdType* clist = otplate->Tetras;
    for (t = this->Mesh->Tetras.begin(); t != this->Mesh->Tetras.end(); ++t)
    {
...
// could not be used.
int svtkOrderedTriangulator::TemplateTriangulation()
{
  TemplatesIterator titer = this->Templates->find(this->CellType);
  if (titer != this->Templates->end()) // something found
  {
    ...
    if (tlistIter != tlist->end()) // something found
    {
      ...
      svtkIdType numTets = tets->NumberOfTetras;
      svtkIdType* clist = tets->Tetras;
      OTTetra* tetra;
      for (i = 0; i < numTets; i++)
      {
...
//------------------------------------------------------------------------
void svtkOrderedTriangulator::PrintSelf(ostream& os, svtkIndent indent)
{
