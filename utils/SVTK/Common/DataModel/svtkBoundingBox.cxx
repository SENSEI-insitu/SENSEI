...
// ---------------------------------------------------------------------------
void svtkBoundingBox::AddPoint(double px, double py, double pz)
{
  double p[3];
  p[0] = px;
  p[1] = py;
...
// ---------------------------------------------------------------------------
void svtkBoundingBox::AddPoint(double p[3])
{
  int i;
  for (i = 0; i < 3; i++)
  {
...
// ---------------------------------------------------------------------------
void svtkBoundingBox::AddBox(const svtkBoundingBox& bbox)
{
  double bds[6];
  bbox.GetBounds(bds);
  this->AddBounds(bds);
...
// ---------------------------------------------------------------------------
void svtkBoundingBox::AddBounds(const double bounds[6])
{
  bool this_valid = (this->IsValid() != 0);
  bool other_valid = (svtkBoundingBox::IsValid(bounds) != 0);
...
// ---------------------------------------------------------------------------
void svtkBoundingBox::SetBounds(
...
// ---------------------------------------------------------------------------
void svtkBoundingBox::SetMinPoint(double x, double y, double z)
{
  this->MinPnt[0] = x;
  if (x > this->MaxPnt[0])
  {
...
// ---------------------------------------------------------------------------
void svtkBoundingBox::SetMaxPoint(double x, double y, double z)
{
  this->MaxPnt[0] = x;
  if (x < this->MinPnt[0])
  {
...
// ---------------------------------------------------------------------------
void svtkBoundingBox::Inflate(double delX, double delY, double delZ)
{
  this->MinPnt[0] -= delX;
  this->MaxPnt[0] += delX;
  this->MinPnt[1] -= delY;
...
// ---------------------------------------------------------------------------
void svtkBoundingBox::Inflate(double delta)
{
...
// ---------------------------------------------------------------------------
int svtkBoundingBox::IntersectBox(const svtkBoundingBox& bbox)
{
...
// ---------------------------------------------------------------------------
int svtkBoundingBox::Intersects(const svtkBoundingBox& bbox) const
{
...
// ---------------------------------------------------------------------------
int svtkBoundingBox::Contains(const svtkBoundingBox& bbox) const
{
...
// ---------------------------------------------------------------------------
double svtkBoundingBox::GetMaxLength() const
{
  double l[3];
  this->GetLengths(l);
  if (l[0] > l[1])
...
// ---------------------------------------------------------------------------
double svtkBoundingBox::GetDiagonalLength() const
{
...
void svtkBoundingBox::Scale(double sx, double sy, double sz)
{
  if (this->IsValid())
  {
    if (sx >= 0.0)
...
// ---------------------------------------------------------------------------
void svtkBoundingBox::Scale(double s[3])
{
  this->Scale(s[0], s[1], s[2]);
}

// ---------------------------------------------------------------------------
void svtkBoundingBox::ScaleAboutCenter(double s)
{
...
void svtkBoundingBox::ScaleAboutCenter(double sx, double sy, double sz)
{
  if (this->IsValid())
  {
    double center[3];
...
// ---------------------------------------------------------------------------
void svtkBoundingBox::ScaleAboutCenter(double s[3])
{
...
svtkIdType svtkBoundingBox::ComputeDivisions(svtkIdType totalBins, double bounds[6], int divs[3]) const
{
  ...
  for (int i = 0; i < 3; ++i)
  {
    divs[i] = (nonZero[i] ? svtkMath::Floor(f * lengths[i] / totLen) : 1);
  ...
  return static_cast<svtkIdType>(divs[0]) * divs[1] * divs[2];
}
...
bool svtkBoundingBox::IntersectPlane(double origin[3], double normal[3])
{
    ...
    for (int ix = 0; ix <= 1; ix++)
    {
      for (int iy = 0; iy <= 1; iy++)
      {
        for (int iz = 0; iz <= 1; iz++)
        {
          ...
          d[++index] = svtkPlane::Evaluate(normal, origin, x);
        }
      }
    }
  }
...
struct FastBounds
{
  ...
  double* Bounds;
  svtkSMPThreadLocal<std::array<double, 6> > LocalBounds;
  ...
  void Initialize()
  {
    ...
    localBds[0] = SVTK_DOUBLE_MAX;
    localBds[2] = SVTK_DOUBLE_MAX;
    localBds[4] = SVTK_DOUBLE_MAX;

    localBds[1] = SVTK_DOUBLE_MIN;
    localBds[3] = SVTK_DOUBLE_MIN;
    localBds[5] = SVTK_DOUBLE_MIN;
  }

  void operator()(svtkIdType beginPtId, svtkIdType endPtId)
  {
    std::array<double, 6>& localBds = this->LocalBounds.Local();
    const auto tuples = svtk::DataArrayTupleRange<3>(this->Points, beginPtId, endPtId);
...
// Hooks into dispatcher svtkArrayDispatch by providing a callable generic
struct BoundsWorker
{
  template <typename PointsT>
  void operator()(PointsT* pts, const unsigned char* ptUses, double* bds)
  {
    svtkIdType numPts = pts->GetNumberOfTuples();
    FastBounds<PointsT> fastBds(pts, ptUses, bds);
    svtkSMPTools::For(0, numPts, fastBds);
  }
};
...
// ---------------------------------------------------------------------------
// Fast computing of bounding box from svtkPoints.
void svtkBoundingBox::ComputeBounds(svtkPoints* pts, double bounds[6])
{
  return svtkBoundingBox::ComputeBounds(pts, nullptr, bounds);
}

// ---------------------------------------------------------------------------
// Fast computing of bounding box from svtkPoints and optional array that marks
