...
svtkStandardNewMacro(svtkAMRInformation);
...
public:
  ...
  void GetBoxesInIntersectingBins(const svtkAMRBox& box, std::set<unsigned int>& boxes)
  {
    boxes.clear();
...
svtkAMRInformation::svtkAMRInformation()
  : NumBlocks(1, 0)
{
  this->Refinement = svtkSmartPointer<svtkIntArray>::New();
...
svtkAMRInformation::~svtkAMRInformation() = default;

void svtkAMRInformation::PrintSelf(ostream& os, svtkIndent indent)
{
  ...
  for (unsigned int levelIdx = 0; levelIdx < this->GetNumberOfLevels(); levelIdx++)
  {
    ...
    {
      const svtkAMRBox& box = this->GetAMRBox(levelIdx, dataIdx);
      os << indent;
...
bool svtkAMRInformation::Audit()
{
  ...
  for (int d = 0; d < 3; d++)
  {
    if (d != emptyDimension)
    {
      ...
      {
        svtkErrorMacro("Bound min does not match origin at dimension "
  ...
  {
    svtkErrorMacro("Refinement levels wrong " << this->Refinement->GetNumberOfTuples());
  }
  ...
  for (unsigned int i = 0; i < this->GetNumberOfLevels(); i++)
  {
    ...
    for (int d = 0; d < 3; d++)
    {
      if (h[d] < 0)
      {
        svtkErrorMacro("Invalid spacing at level " << i << endl);
      }
    }
  }
  ...
  {
    const svtkAMRBox& box = this->Boxes[i];
    if (box.IsInvalid())
    {
    ...
    if (!valid)
    {
      svtkErrorMacro("Invalid AMRBox. Wrong dimension");
    }
  }
...
void svtkAMRInformation::Initialize(int numLevels, const int* blocksPerLevel)
{
  if (numLevels < 0)
  {
    svtkErrorMacro("Number of levels must be at least 0: " << numLevels);
    return;
  }
  // allocate boxes
  ...
  this->Spacing = svtkSmartPointer<svtkDoubleArray>::New();
...
unsigned int svtkAMRInformation::GetNumberOfDataSets(unsigned int level) const
{
...
void svtkAMRInformation::AllocateBoxes(unsigned int n)
{
  this->Boxes.clear();
...
void svtkAMRInformation::SetAMRBox(unsigned int level, unsigned int id, const svtkAMRBox& box)
{
...
int svtkAMRInformation::GetAMRBlockSourceIndex(int index)
{
  return this->SourceIndex->GetValue(index);
}

void svtkAMRInformation::SetAMRBlockSourceIndex(int index, int sourceId)
{
  if (!this->SourceIndex)
  {
    this->SourceIndex = svtkSmartPointer<svtkIntArray>::New();
...
void svtkAMRInformation::ComputeIndexPair(unsigned int index, unsigned int& level, unsigned int& id)
{
  this->GenerateBlockLevel();
  level = this->BlockLevel->GetValue(static_cast<svtkIdType>(index));
  id = index - this->NumBlocks[level];
}

void svtkAMRInformation::GetOrigin(double o[3])
{
  for (int i = 0; i < 3; ++i)
  {
    o[i] = this->Origin[i];
...
double* svtkAMRInformation::GetOrigin()
{
  if (!this->HasValidOrigin())
  {
    svtkErrorMacro("Invalid Origin");
  }
  return this->Origin;
}

void svtkAMRInformation::SetOrigin(const double* origin)
{
  for (int d = 0; d < 3; d++)
  {
...
int svtkAMRInformation::GetRefinementRatio(unsigned int level) const
{
  return this->Refinement->GetValue(level);
}

void svtkAMRInformation::SetRefinementRatio(unsigned int level, int refRatio)
{
...
bool svtkAMRInformation::HasRefinementRatio()
{
  return this->Refinement &&
    static_cast<unsigned int>(this->Refinement->GetNumberOfTuples()) == this->GetNumberOfLevels();
}

void svtkAMRInformation::GenerateRefinementRatio()
{
  this->Refinement->SetNumberOfTuples(this->GetNumberOfLevels());
  ...
  if (numLevels == 1)
  {
    ...
    // svtkOverlappingAMR requirement.
...
bool svtkAMRInformation::HasChildrenInformation()
{
  return !this->AllChildren.empty();
}

unsigned int* svtkAMRInformation::GetParents(
...
unsigned int* svtkAMRInformation::GetChildren(
...
void svtkAMRInformation::PrintParentChildInfo(unsigned int level, unsigned int index)
{
...
void svtkAMRInformation::GenerateParentChildInformation()
{
  if (!this->HasRefinementRatio())
  {
...
bool svtkAMRInformation::HasValidOrigin()
{
  return this->Origin[0] != DBL_MAX && this->Origin[1] != DBL_MAX && this->Origin[2] != DBL_MAX;
}

bool svtkAMRInformation::HasValidBounds()
{
  return this->Bounds[0] != DBL_MAX && this->Bounds[1] != DBL_MAX && this->Bounds[2] != DBL_MAX;
}

void svtkAMRInformation::SetGridDescription(int description)
{
  if (description < SVTK_SINGLE_POINT || description > SVTK_EMPTY)
  {
    svtkErrorMacro("Invalid grid description for a svtkUniformGrid.");
    return;
  }
  this->GridDescription = description;
}

void svtkAMRInformation::SetSpacing(unsigned int level, const double* h)
{
  ...
  for (unsigned int i = 0; i < 3; i++)
  {
    ...
    {
      svtkWarningMacro("Inconsistent spacing: " << spacing[i] << " != " << h[i]);
    }
  }
  this->Spacing->SetTuple(level, h);
}

void svtkAMRInformation::GenerateBlockLevel()
{
  if (this->BlockLevel)
  {
    return;
  }
  this->BlockLevel = svtkSmartPointer<svtkUnsignedIntArray>::New();

  this->BlockLevel->SetNumberOfValues(static_cast<svtkIdType>(this->GetTotalNumberOfBlocks()));
...
void svtkAMRInformation::GetBounds(unsigned int level, unsigned int id, double* bb)
{
  const svtkAMRBox& box = this->Boxes[this->GetIndex(level, id)];
  svtkAMRBox::GetBounds(box, this->Origin, this->Spacing->GetTuple(level), bb);
}

const svtkAMRBox& svtkAMRInformation::GetAMRBox(unsigned int level, unsigned int id) const
{
  return this->Boxes[this->GetIndex(level, id)];
}

void svtkAMRInformation::GetSpacing(unsigned int level, double spacing[3])
{
  return this->Spacing->GetTuple(level, spacing);
}

void svtkAMRInformation::CalculateParentChildRelationShip(unsigned int level,
  ...
  std::vector<std::vector<unsigned int> >& parents)
{
  ...
  int extents[6] = { SVTK_INT_MAX, -SVTK_INT_MAX, SVTK_INT_MAX, -SVTK_INT_MAX, SVTK_INT_MAX,
    -SVTK_INT_MAX };
  ...
  {
    svtkAMRBox box = this->GetAMRBox(level - 1, id);
    if (!box.IsInvalid())
    {
  ...
  {
    svtkAMRBox box = this->GetAMRBox(level - 1, i);
    if (!box.IsInvalid())
    {
      unsigned int minbin[3];
      unsigned int maxbin[3];
  ...
  {
    const svtkAMRBox& box = this->GetAMRBox(level, i);
    if (!box.IsInvalid())
    {
      ...
      {
        svtkAMRBox potentialParent = this->GetAMRBox(level - 1, *iter);
...
bool svtkAMRInformation::FindCell(double q[3], unsigned int level, unsigned int id, int& cellIdx)
{
  double h[3];
  this->GetSpacing(level, h);

  const svtkAMRBox& box = this->GetAMRBox(level, id);
  double gbounds[6];
  ...
  int ijk[3];
  double pcoords[3];
  int status = svtkAMRBox::ComputeStructuredCoordinates(box, this->Origin, h, q, ijk, pcoords);
  if (status == 1)
  {
    int dims[3];
    box.GetNumberOfNodes(dims);
    cellIdx = svtkStructuredData::ComputeCellId(dims, ijk);
    return true;
  }
  return false;
}

bool svtkAMRInformation::GetCoarsenedAMRBox(
  unsigned int level, unsigned int id, svtkAMRBox& box) const
{
...
bool svtkAMRInformation::operator==(const svtkAMRInformation& other)
{
  ...
  {
    for (svtkIdType i = 0; i < this->SourceIndex->GetNumberOfTuples(); i++)
    {
  ...
  for (svtkIdType i = 0; i < this->Spacing->GetNumberOfTuples(); i++)
  {
...
bool svtkAMRInformation::GetOrigin(unsigned int level, unsigned int id, double* origin)
{
  const svtkAMRBox& box = this->Boxes[this->GetIndex(level, id)];
  svtkAMRBox::GetBoxOrigin(box, this->Origin, this->Spacing->GetTuple(level), origin);
  return true;
}

void svtkAMRInformation::UpdateBounds(const int level, const int id)
{
  double bb[6];
  svtkAMRBox::GetBounds(
...
void svtkAMRInformation::DeepCopy(svtkAMRInformation* other)
{
...
bool svtkAMRInformation::FindGrid(double q[3], unsigned int& level, unsigned int& gridId)
{
  if (!this->HasChildrenInformation())
  {
...
bool svtkAMRInformation::FindGrid(double q[3], int level, unsigned int& gridId)
{
