/*=========================================================================
  ...
  Module:    svtkPointsProjectedHull.cxx
...
svtkStandardNewMacro(svtkPointsProjectedHull);
...
#define SVTK_ISLEFT(P0, P1, P2)                                                                     \
  (((P1)[0] - (P0)[0]) * ((P2)[1] - (P0)[1]) - ((P2)[0] - (P0)[0]) * ((P1)[1] - (P0)[1]))

svtkPointsProjectedHull::svtkPointsProjectedHull()
{
  this->InitFlags();
}
...
void svtkPointsProjectedHull::Initialize()
{
  this->ClearAllocations();
  this->InitFlags();

  svtkPoints::Initialize();
}

void svtkPointsProjectedHull::Update()
{
  this->ClearAllocations();
  this->InitFlags();
}

void svtkPointsProjectedHull::InitFlags()
{
  int i;
...
void svtkPointsProjectedHull::ClearAllocations()
{
  int i;
  for (i = 0; i < 3; i++)
  {
...
#define SVTK_GETCCWHULL(which, dim)                                                                 \
  int svtkPointsProjectedHull::GetCCWHull##which(float* pts, int len)                               \
  {                                                                                                \
    int i;                                                                                         \
  ...
  int svtkPointsProjectedHull::GetCCWHull##which(double* pts, int len)                              \
  {                                                                                                \
...
#define SVTK_GETSIZECCWHULL(which, dim)                                                             \
  int svtkPointsProjectedHull::GetSizeCCWHull##which()                                              \
  {                                                                                                \
    if ((this->HullSize[dim] == 0) || (this->GetMTime() > this->HullTime[dim]))                    \
    {                                                                                              \
      GrahamScanAlgorithm(dim);                                                                    \
...
SVTK_GETSIZECCWHULL(X, 0);
SVTK_GETSIZECCWHULL(Y, 1);
SVTK_GETSIZECCWHULL(Z, 2);

#define SVTK_RECTANGLEINTERSECTION(which, dim)                                                      \
  int svtkPointsProjectedHull::RectangleIntersection##which(svtkPoints* R)                           \
  {                                                                                                \
    double bounds[6];                                                                              \
    R->Modified();                                                                                 \
    R->GetBounds(bounds);                                                                          \
  ...
  int svtkPointsProjectedHull::RectangleIntersection##which(                                        \
...
SVTK_RECTANGLEINTERSECTION(X, 0);
SVTK_RECTANGLEINTERSECTION(Y, 1);
SVTK_RECTANGLEINTERSECTION(Z, 2);
...
int svtkPointsProjectedHull::RectangleIntersection(
...
extern "C"
{
  int svtkPointsProjectedHullIncrVertAxis(const void* p1, const void* p2);
  int svtkPointsProjectedHullCCW(const void* p1, const void* p2);
}

int svtkPointsProjectedHull::GrahamScanAlgorithm(int dir)
{
  ...
  for (svtkIdType i = 0; i < this->Npts; ++i)
  {
    hullPts[i * 2] = this->Pts[i * 3 + horizAxis];
    hullPts[i * 2 + 1] = this->Pts[i * 3 + vertAxis];
  }

  qsort(hullPts, this->Npts, sizeof(double) * 2, svtkPointsProjectedHullIncrVertAxis);

  int firstId = 0;

  for (svtkIdType i = 1; i < this->Npts; ++i)
  {
  ...
  // projection, the svtkPointsProjectedHullCCW sort will fail.

  svtkIdType dups = 0;

  for (svtkIdType j = 1, i = 1; j < this->Npts; ++j)
  {
  ...
  svtkIdType nHullPts = this->Npts - dups;
  ...
  qsort(hullPts + 2, nHullPts - 1, sizeof(double) * 2, svtkPointsProjectedHullCCW);
  ...
  for (svtkIdType i = 2; i < nHullPts; ++i)
  {
    svtkIdType newpos = PositionInHull(hullPts, hullPts + top * 2, hullPts + i * 2);
  ...
  for (svtkIdType i = 1; i < nHullPts; ++i)
  {
    if (hullPts[2 * i] < x0)
    {
      x0 = hullPts[2 * i];
...
double svtkPointsProjectedHull::Distance(double* p1, double* p2)
{
  return (p1[0] - p2[0]) * (p1[0] - p2[0]) + (p1[1] - p2[1]) * (p1[1] - p2[1]);
}

int svtkPointsProjectedHull::RemoveExtras(double* pts, int n)
{
  ...
  for (i = 1; i < n; i++)
  {
    ...
    {
      where = SVTK_ISLEFT(pts, pts + prev * 2, pts + i * 2);
...
svtkIdType svtkPointsProjectedHull::PositionInHull(double* base, double* top, double* pt)
{
  double *p1, *p2;
  double where;
...
void svtkPointsProjectedHull::GetPoints()
{
  delete[] this->Pts;
...
int svtkPointsProjectedHull::RectangleBoundingBoxIntersection(
  double hmin, double hmax, double vmin, double vmax, int dim)
{
...
int svtkPointsProjectedHull::OutsideHorizontalLine(
...
int svtkPointsProjectedHull::OutsideVerticalLine(
...
int svtkPointsProjectedHull::OutsideLine(
  double hmin, double hmax, double vmin, double vmax, double* p0, double* p1, double* insidePt)
{
  ...
  double ip = SVTK_ISLEFT(p0, p1, insidePt);
  double rp;
...
int svtkPointsProjectedHull::RectangleOutside(
  double hmin, double hmax, double vmin, double vmax, int dir)
{
  svtkIdType npts = this->HullSize[dir];
...
int svtkPointsProjectedHull::RectangleOutside1DPolygon(
  double hmin, double hmax, double vmin, double vmax, int dir)
{
...
extern "C"
{
  ...
  int svtkPointsProjectedHullCCW(const void* p1, const void* p2)
  {
    ...
    double val = SVTK_ISLEFT(firstPt, a, b);
...
void svtkPointsProjectedHull::PrintSelf(ostream& os, svtkIndent indent)
{
