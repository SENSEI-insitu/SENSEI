...
#define SCALAR_FUNC(proc, function, math)                                                          \
  ...
  {                                                                                                \
    ...
    svtkSmartPointer<svtkFunctionParser> parser = svtkSmartPointer<svtkFunctionParser>::New();         \
    ...
    {                                                                                              \
      double value = svtkMath::Random(low, hi);                                                     \
...
int UnitTestFunctionParser(int, char*[])
{
  ...
  svtkSmartPointer<svtkFunctionParser> functionParser = svtkSmartPointer<svtkFunctionParser>::New();
...
int TestVectors()
{
  ...
  {
    double x0 = svtkMath::Random(-1.0, 1.0);
    double x1 = svtkMath::Random(-1.0, 1.0);
    double x2 = svtkMath::Random(-1.0, 1.0);
    parser->SetVectorVariableValue("x", x0, x1, x2);

    double y0 = svtkMath::Random(-1.0, 1.0);
    double y1 = svtkMath::Random(-1.0, 1.0);
    double y2 = svtkMath::Random(-1.0, 1.0);
  ...
  for (unsigned int i = 0; i < 10; ++i)
  {
    ...
    for (int j = 0; j < 3; ++j)
    {
      if (!svtkMathUtilities::FuzzyCompare(a[j], b[j], std::numeric_limits<double>::epsilon() * 1.0))
      {
    ...
    if (!svtkMathUtilities::FuzzyCompare(dot, 0.0, std::numeric_limits<double>::epsilon() * 1.0))
    {
    ...
    if (!svtkMathUtilities::FuzzyCompare(mag, 1.0, std::numeric_limits<double>::epsilon() * 2.0))
    {
  ...
  svtkMTimeType before = parser->GetMTime();
  parser->InvalidateFunction();
  svtkMTimeType after = parser->GetMTime();
...
int TestMinMax()
{
  ...
  for (unsigned int i = 0; i < 1000; ++i)
  {
    double value = svtkMath::Random(-1000.0, 1000.0);
    parser->SetScalarVariableValue("x", value);
...
int TestScalarLogic()
{
  ...
  for (unsigned int i = 0; i < 1000; ++i)
  {
    double x = svtkMath::Random(-1000.0, 1000.0);
    double y = svtkMath::Random(-1000.0, 1000.0);
...
int TestVectorLogic()
{
  ...
  for (unsigned int i = 0; i < 1000; ++i)
  {
    ...
    double v1 = svtkMath::Random(-1000.0, 1000.0);
    double v2 = svtkMath::Random(-1000.0, 1000.0);
    double v3 = svtkMath::Random(-1000.0, 1000.0);
    double w1 = svtkMath::Random(-1000.0, 1000.0);
    double w2 = svtkMath::Random(-1000.0, 1000.0);
    double w3 = svtkMath::Random(-1000.0, 1000.0);
...
int TestMiscFunctions()
{
  ...
  for (unsigned int i = 0; i < 1000; ++i)
  {
    double x = svtkMath::Random(0.0, 10.0);
    double y = svtkMath::Random(0.0, 2.0);
    parser->SetScalarVariableValue("x", x);
    parser->SetScalarVariableValue("y", y);
    parser->SetFunction("x ^ y");
    double result = parser->GetScalarResult();
  ...
  for (unsigned int i = 0; i < 1000; ++i)
  {
    double x = svtkMath::Random(-10.0, 10.0);
    double y = svtkMath::Random(-10.0, 10.0);
    parser->SetScalarVariableValue("x", x);
    parser->SetScalarVariableValue("y", y);
    parser->SetFunction("x / y");
    double result = parser->GetScalarResult();
...
int TestErrors()
{
  ...
  parser->AddObserver(svtkCommand::ErrorEvent, errorObserver);
