 ...
 *
 * svtkContourValues is a general class to manage the creation, generation,
...
#ifndef svtkContourValues_h
#define svtkContourValues_h
...
class SVTKCOMMONMISC_EXPORT svtkContourValues : public svtkObject
{
public:
  /**
   * Construct object with a single contour value at 0.0.
   */
  static svtkContourValues* New();

  svtkTypeMacro(svtkContourValues, svtkObject);
   ...
   * Copy contours.
   */
  void DeepCopy(svtkContourValues* other);

protected:
  svtkContourValues();
  ~svtkContourValues() override;

  svtkDoubleArray* Contours;

private:
  svtkContourValues(const svtkContourValues&) = delete;
  void operator=(const svtkContourValues&) = delete;
};
