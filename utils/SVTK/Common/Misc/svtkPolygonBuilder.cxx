...
svtkPolygonBuilder::svtkPolygonBuilder() = default;

void svtkPolygonBuilder::InsertTriangle(const svtkIdType* abc)
{
  ...
  svtkIdType key = abc[0] + abc[1] + abc[2];
  ...
  if (found != Tris.end())
  {
    ...
    for (Triangles::iterator it = tris.begin(); !duplicate && it != tris.end(); ++it)
    {
      ...
      for (int i = 0; i < 3; ++i)
      {
        svtkIdType ta = tri[(i + 0) % 3];
        svtkIdType tb = tri[(i + 1) % 3];
        svtkIdType tc = tri[(i + 2) % 3];
...
void svtkPolygonBuilder::GetPolygons(svtkIdListCollection* polys)
{
  ...
  {
    svtkIdList* poly = svtkIdList::New();

    EdgeMap::iterator edgeIt = this->Edges.begin();
    Edge edge = *(edgeIt);

    svtkIdType firstVtx = edge.first;
