/*=========================================================================
  ...
  Module:    svtkThinPlateSplineTransform.cxx
...
svtkStandardNewMacro(svtkThinPlateSplineTransform);

//------------------------------------------------------------------------
// some dull matrix things

inline double** svtkNewMatrix(int rows, int cols)
{
...
//------------------------------------------------------------------------
inline void svtkDeleteMatrix(double** m)
{
  delete[] * m;
  delete[] m;
}
...
//------------------------------------------------------------------------
inline void svtkZeroMatrix(double** m, int rows, int cols)
{
  for (int i = 0; i < rows; i++)
  {
...
//------------------------------------------------------------------------
inline void svtkMatrixMultiply(
...
//------------------------------------------------------------------------
inline void svtkMatrixTranspose(double** a, double** b, int rows, int cols)
{
  for (int i = 0; i < rows; i++)
  {
    for (int j = 0; j < cols; j++)
...
//------------------------------------------------------------------------
svtkThinPlateSplineTransform::svtkThinPlateSplineTransform()
{
...
svtkThinPlateSplineTransform::~svtkThinPlateSplineTransform()
{
  ...
  if (this->MatrixW)
  {
    svtkDeleteMatrix(this->MatrixW);
    this->MatrixW = nullptr;
  }
}

//------------------------------------------------------------------------
void svtkThinPlateSplineTransform::SetSourceLandmarks(svtkPoints* source)
{
...
//------------------------------------------------------------------------
void svtkThinPlateSplineTransform::SetTargetLandmarks(svtkPoints* target)
{
...
//------------------------------------------------------------------------
svtkMTimeType svtkThinPlateSplineTransform::GetMTime()
{
  svtkMTimeType result = this->svtkWarpTransform::GetMTime();
  svtkMTimeType mtime;
...
//------------------------------------------------------------------------
void svtkThinPlateSplineTransform::InternalUpdate()
{
  ...
  {
    svtkErrorMacro("Update: Source and Target Landmarks contain a different number of points");
    return;
  }

  const svtkIdType N = this->SourceLandmarks->GetNumberOfPoints();
  const int D = 3; // dimensions

  // the output weights matrix
  double** W = svtkNewMatrix(N + D + 1, D);
  ...
  if (N >= 3)
  {
    ...
    // the input matrices
    double** L = svtkNewMatrix(N + D + 1, N + D + 1);
    double** X = svtkNewMatrix(N + D + 1, D);

    // build L
    // will leave the bottom-right corner with zeros
    svtkZeroMatrix(L, N + D + 1, N + D + 1);
    ...
    double** V = svtkNewMatrix(N + D + 1, N + D + 1);
    double** w = svtkNewMatrix(N + D + 1, N + D + 1);
    double** U = L; // reuse the space
    svtkMath::JacobiN(L, N + D + 1, values, V);
    svtkMatrixTranspose(V, U, N + D + 1, N + D + 1);
    ...
    svtkMatrixMultiply(U, X, W, N + D + 1, N + D + 1, N + D + 1, D);
    svtkMatrixMultiply(w, W, X, N + D + 1, N + D + 1, N + D + 1, D);
    svtkMatrixMultiply(V, X, W, N + D + 1, N + D + 1, N + D + 1, D);
    ...
    if (this->RegularizeBulkTransform)
    {
      ...
      if (fabs(svtkMath::Determinant3x3((double(*)[3]) * A)) < 1e-16)
      {
        for (i = 0; i < 3; i++)
        {
  ...
  else // (N < 3)
  {
    ...
    if (N == 2)
    {
      ...
      if (rs == 0.0)
      {
        svtkWarningMacro("Source landmarks coincide, "
                        "refusing to do infinite scale");
      }
      else if (rt == 0.0)
      {
        svtkWarningMacro("Target landmarks coincide, "
      ...
      else
      {
        ...
        else // rotation by 180 degrees
        {
          ...
          svtkMath::Perpendiculars(ds, dt, nullptr, 0);
          f = sin(theta / 2);
          x = dt[0] * f;
          y = dt[1] * f;
          z = dt[2] * f;
...
template <class T>
inline void svtkThinPlateSplineForwardTransformPoint(svtkThinPlateSplineTransform* self, double** W,
  int N, double (*phi)(double), const T point[3], T output[3])
{
  ...
  svtkPoints* sourceLandmarks = self->GetSourceLandmarks();
...
void svtkThinPlateSplineTransform::ForwardTransformPoint(const double point[3], double output[3])
{
  svtkThinPlateSplineForwardTransformPoint(
    this, this->MatrixW, this->NumberOfPoints, this->BasisFunction, point, output);
}

void svtkThinPlateSplineTransform::ForwardTransformPoint(const float point[3], float output[3])
{
  svtkThinPlateSplineForwardTransformPoint(
    this, this->MatrixW, this->NumberOfPoints, this->BasisFunction, point, output);
}
...
template <class T>
inline void svtkThinPlateSplineForwardTransformDerivative(svtkThinPlateSplineTransform* self,
...
void svtkThinPlateSplineTransform::ForwardTransformDerivative(
  const double point[3], double output[3], double derivative[3][3])
{
  svtkThinPlateSplineForwardTransformDerivative(
...
//------------------------------------------------------------------------
void svtkThinPlateSplineTransform::PrintSelf(ostream& os, svtkIndent indent)
{
...
//----------------------------------------------------------------------------
svtkAbstractTransform* svtkThinPlateSplineTransform::MakeTransform()
{
  return svtkThinPlateSplineTransform::New();
}

//----------------------------------------------------------------------------
void svtkThinPlateSplineTransform::InternalDeepCopy(svtkAbstractTransform* transform)
{
  svtkThinPlateSplineTransform* t = (svtkThinPlateSplineTransform*)transform;
...
static double svtkRBFr(double r)
{
  return r;
}

// calculate both phi(r) its derivative wrt r
static double svtkRBFDRr(double r, double& dUdr)
{
  dUdr = 1;
  return r;
}
...
static double svtkRBFr2logr(double r)
{
  if (r != 0.0)
  {
    return r * r * log(r);
...
// calculate both phi(r) its derivative wrt r
static double svtkRBFDRr2logr(double r, double& dUdr)
{
  if (r)
  {
    double tmp = log(r);
...
//------------------------------------------------------------------------
void svtkThinPlateSplineTransform::SetBasis(int basis)
{
  if (basis == this->Basis)
  {
    return;
  ...
  switch (basis)
  {
    case SVTK_RBF_CUSTOM:
      break;
    case SVTK_RBF_R:
      this->BasisFunction = &svtkRBFr;
      this->BasisDerivative = &svtkRBFDRr;
      break;
    case SVTK_RBF_R2LOGR:
      this->BasisFunction = &svtkRBFr2logr;
      this->BasisDerivative = &svtkRBFDRr2logr;
      break;
    default:
      svtkErrorMacro(<< "SetBasisFunction: Unrecognized basis function");
      break;
  }
...
//------------------------------------------------------------------------
const char* svtkThinPlateSplineTransform::GetBasisAsString()
{
  switch (this->Basis)
  {
    case SVTK_RBF_CUSTOM:
