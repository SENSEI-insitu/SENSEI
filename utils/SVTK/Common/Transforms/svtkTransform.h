 ...
 *
 * A svtkTransform can be used to describe the full range of linear (also
 ...
 * matrix.  When you create a new svtkTransform, it is always initialized
 ...
 * @sa
 * svtkPerspectiveTransform svtkGeneralTransform svtkMatrix4x4
 * svtkTransformCollection svtkTransformFilter svtkTransformPolyDataFilter
 * svtkImageReslice
 */

#ifndef svtkTransform_h
#define svtkTransform_h

#include "svtkCommonTransformsModule.h" // For export macro
#include "svtkLinearTransform.h"

#include "svtkMatrix4x4.h" // Needed for inline methods

class SVTKCOMMONTRANSFORMS_EXPORT svtkTransform : public svtkLinearTransform
{
public:
  static svtkTransform* New();
  svtkTypeMacro(svtkTransform, svtkLinearTransform);
   ...
   */
  void SetMatrix(svtkMatrix4x4* matrix) { this->SetMatrix(*matrix->Element); }
   ...
   */
  void Concatenate(svtkMatrix4x4* matrix) { this->Concatenate(*matrix->Element); }
   ...
   */
  void Concatenate(svtkLinearTransform* transform);
  ...
  //@{
  /**
   * Get one of the concatenated transformations as a svtkAbstractTransform.
   ...
   */
  svtkLinearTransform* GetConcatenatedTransform(int i)
  {
    svtkAbstractTransform* t;
    ...
    return static_cast<svtkLinearTransform*>(t);
  }
  //@}
   ...
   */
  static void GetOrientation(double orient[3], svtkMatrix4x4* matrix);
  ...
  double* GetPosition() SVTK_SIZEHINT(3)
  {
  ...
  double* GetScale() SVTK_SIZEHINT(3)
  {
   ...
   * matrix.
   */
  void GetInverse(svtkMatrix4x4* inverse);
   ...
   */
  void GetTranspose(svtkMatrix4x4* transpose);
   ...
   */
  void SetInput(svtkLinearTransform* input);
  svtkLinearTransform* GetInput() { return this->Input; }
  //@}
   ...
   * it is experimental.
   */
  int CircuitCheck(svtkAbstractTransform* transform) override;

  // Return an inverse transform which will always update itself
  // to match this transform.
  svtkAbstractTransform* GetInverse() { return svtkLinearTransform::GetInverse(); }
...
protected:
  svtkTransform();
  ~svtkTransform() override;

  void InternalDeepCopy(svtkAbstractTransform* t) override;

  void InternalUpdate() override;

  svtkLinearTransform* Input;
  svtkTransformConcatenation* Concatenation;
  svtkTransformConcatenationStack* Stack;

  // this allows us to check whether people have been fooling
  // around with our matrix
  svtkMTimeType MatrixUpdateMTime;
...
private:
  svtkTransform(const svtkTransform&) = delete;
  void operator=(const svtkTransform&) = delete;
};
