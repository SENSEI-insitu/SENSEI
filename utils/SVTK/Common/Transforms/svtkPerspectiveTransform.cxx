/*=========================================================================
  ...
  Module:    svtkPerspectiveTransform.cxx
...
svtkStandardNewMacro(svtkPerspectiveTransform);

//----------------------------------------------------------------------------
svtkPerspectiveTransform::svtkPerspectiveTransform()
{
  this->Input = nullptr;
...
//----------------------------------------------------------------------------
void svtkPerspectiveTransform::PrintSelf(ostream& os, svtkIndent indent)
{
  this->Update();
  ...
  if (this->GetNumberOfConcatenatedTransforms() != 0)
  {
    ...
    for (int i = 0; i < n; i++)
    {
      svtkHomogeneousTransform* t = this->GetConcatenatedTransform(i);
...
//----------------------------------------------------------------------------
void svtkPerspectiveTransform::Concatenate(svtkHomogeneousTransform* transform)
{
...
//----------------------------------------------------------------------------
void svtkPerspectiveTransform::SetInput(svtkHomogeneousTransform* input)
{
  if (this->Input == input)
  {
    return;
...
//----------------------------------------------------------------------------
int svtkPerspectiveTransform::CircuitCheck(svtkAbstractTransform* transform)
{
  if (this->svtkHomogeneousTransform::CircuitCheck(transform) ||
...
//----------------------------------------------------------------------------
svtkAbstractTransform* svtkPerspectiveTransform::MakeTransform()
{
  return svtkPerspectiveTransform::New();
}

//----------------------------------------------------------------------------
svtkMTimeType svtkPerspectiveTransform::GetMTime()
{
...
//----------------------------------------------------------------------------
void svtkPerspectiveTransform::InternalDeepCopy(svtkAbstractTransform* gtrans)
{
  svtkPerspectiveTransform* transform = static_cast<svtkPerspectiveTransform*>(gtrans);
  ...
  // defer to superclass
  this->svtkHomogeneousTransform::InternalDeepCopy(transform);
}

//----------------------------------------------------------------------------
void svtkPerspectiveTransform::InternalUpdate()
{
...
void svtkPerspectiveTransform::AdjustViewport(double oldXMin, double oldXMax, double oldYMin,
...
void svtkPerspectiveTransform::AdjustZBuffer(
...
void svtkPerspectiveTransform::Ortho(
...
// to [-1,+1], [-1,+1], [-1,+1].
// From the OpenGL Programmer's guide, 2nd Ed.
void svtkPerspectiveTransform::Frustum(
  double xmin, double xmax, double ymin, double ymax, double znear, double zfar)
{
  double matrix[4][4];
...
void svtkPerspectiveTransform::Perspective(double angle, double aspect, double znear, double zfar)
{
  double ymax = tan(svtkMath::RadiansFromDegrees(angle) / 2) * znear;
  double ymin = -ymax;
...
// paper, though!
void svtkPerspectiveTransform::Shear(double dxdz, double dydz, double zplane)
{
  double matrix[4][4];
  svtkMatrix4x4::Identity(*matrix);
...
void svtkPerspectiveTransform::Stereo(double angle, double focaldistance)
{
  double dxdz = tan(svtkMath::RadiansFromDegrees(angle));

  this->Shear(dxdz, 0.0, focaldistance);
}

//----------------------------------------------------------------------------
void svtkPerspectiveTransform::SetupCamera(
  const double position[3], const double focalPoint[3], const double viewUp[3])
{
  ...
  svtkMath::Normalize(viewPlaneNormal);

  // orthogonalize viewUp and compute viewSideways
  svtkMath::Cross(viewUp, viewPlaneNormal, viewSideways);
  svtkMath::Normalize(viewSideways);
  svtkMath::Cross(viewPlaneNormal, viewSideways, orthoViewUp);
  ...
  svtkMatrix4x4::MultiplyPoint(*matrix, delta, delta);
...
void svtkPerspectiveTransform::SetupCamera(double p0, double p1, double p2, double fp0, double fp1,
