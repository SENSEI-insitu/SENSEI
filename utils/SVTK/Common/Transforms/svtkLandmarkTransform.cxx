/*=========================================================================
  ...
  Module:    svtkLandmarkTransform.cxx
...
svtkStandardNewMacro(svtkLandmarkTransform);

//----------------------------------------------------------------------------
svtkLandmarkTransform::svtkLandmarkTransform()
{
  this->Mode = SVTK_LANDMARK_SIMILARITY;
...
//----------------------------------------------------------------------------
void svtkLandmarkTransform::PrintSelf(ostream& os, svtkIndent indent)
{
...
void svtkLandmarkTransform::InternalUpdate()
{
  svtkIdType i;
  int j;
  ...
  */
  ...
  const svtkIdType N_PTS = this->SourceLandmarks->GetNumberOfPoints();
  ...
  for (pt = 0; pt < N_PTS; pt++)
  {
    ...
    for (i = 0; i < 3; i++)
    {
      ...
      if (this->Mode == SVTK_LANDMARK_AFFINE)
      {
        AAT[i][0] += a[i] * a[0];
        AAT[i][1] += a[i] * a[1];
        AAT[i][2] += a[i] * a[2];
  ...
  {
    // AAT = (a.a^t)^-1
    svtkMath::Invert3x3(AAT, AAT);

    // M = (a.a^t)^-1 . a.b^t
    svtkMath::Multiply3x3(AAT, M, M);
  ...
  else
  {
    ...
    svtkMath::JacobiN(N, 4, eigenvalues, eigenvectors);
    ...
    if (this->Mode != SVTK_LANDMARK_RIGIDBODY)
...
//------------------------------------------------------------------------
svtkMTimeType svtkLandmarkTransform::GetMTime()
{
...
//------------------------------------------------------------------------
void svtkLandmarkTransform::SetSourceLandmarks(svtkPoints* source)
{
...
//------------------------------------------------------------------------
void svtkLandmarkTransform::SetTargetLandmarks(svtkPoints* target)
{
...
//----------------------------------------------------------------------------
void svtkLandmarkTransform::Inverse()
{
  svtkPoints* tmp1 = this->SourceLandmarks;
  svtkPoints* tmp2 = this->TargetLandmarks;
...
//----------------------------------------------------------------------------
svtkAbstractTransform* svtkLandmarkTransform::MakeTransform()
{
  return svtkLandmarkTransform::New();
}

//----------------------------------------------------------------------------
void svtkLandmarkTransform::InternalDeepCopy(svtkAbstractTransform* transform)
{
  svtkLandmarkTransform* t = (svtkLandmarkTransform*)transform;
