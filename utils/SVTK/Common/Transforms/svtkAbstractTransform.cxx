/*=========================================================================
  ...
  Module:    svtkAbstractTransform.cxx
...
//----------------------------------------------------------------------------
svtkAbstractTransform::svtkAbstractTransform()
{
  this->MyInverse = nullptr;
  this->DependsOnInverse = 0;
  this->InUnRegister = 0;
  this->UpdateMutex = new svtkSimpleCriticalSection;
  this->InverseMutex = new svtkSimpleCriticalSection;
}
...
//----------------------------------------------------------------------------
void svtkAbstractTransform::PrintSelf(ostream& os, svtkIndent indent)
{
...
//----------------------------------------------------------------------------
void svtkAbstractTransform::TransformNormalAtPoint(
  const double point[3], const double in[3], double out[3])
{
  ...
  svtkMath::Transpose3x3(matrix, matrix);
  svtkMath::LinearSolve3x3(matrix, in, out);
...
//----------------------------------------------------------------------------
void svtkAbstractTransform::TransformVectorAtPoint(
  const double point[3], const double in[3], double out[3])
{
  this->Update();
  ...
  this->InternalTransformDerivative(point, coord, matrix);
  svtkMath::Multiply3x3(matrix, in, out);
}
...
void svtkAbstractTransform::TransformPoints(svtkPoints* in, svtkPoints* out)
{
  this->Update();

  double point[3];
  svtkIdType i;
  svtkIdType n = in->GetNumberOfPoints();
...
void svtkAbstractTransform::TransformPointsNormalsVectors(svtkPoints* inPts, svtkPoints* outPts,
  ...
  int nOptionalVectors, svtkDataArray** inVrsArr, svtkDataArray** outVrsArr)
{
  ...
  for (i = 0; i < n; i++)
  {
    ...
    if (inVrs)
    {
      ...
      svtkMath::Multiply3x3(matrix, coord, coord);
    ...
    if (inNms)
    {
      ...
      svtkMath::Transpose3x3(matrix, matrix);
      svtkMath::LinearSolve3x3(matrix, coord, coord);
      svtkMath::Normalize(coord);
...
//----------------------------------------------------------------------------
svtkAbstractTransform* svtkAbstractTransform::GetInverse()
{
...
//----------------------------------------------------------------------------
void svtkAbstractTransform::SetInverse(svtkAbstractTransform* transform)
{
  ...
  {
    svtkErrorMacro("SetInverse: requires a " << this->GetClassName() << ", a "
  ...
  {
    svtkErrorMacro("SetInverse: this would create a circular reference.");
    return;
  }
...
//----------------------------------------------------------------------------
void svtkAbstractTransform::DeepCopy(svtkAbstractTransform* transform)
{
  ...
  if (!transform->IsA(this->GetClassName()))
  {
    svtkErrorMacro("DeepCopy: can't copy a " << transform->GetClassName() << " into a "
                                            << this->GetClassName() << ".");
    return;
  }
  ...
  if (transform->CircuitCheck(this))
  {
    svtkErrorMacro("DeepCopy: this would create a circular reference.");
    return;
  }
...
//----------------------------------------------------------------------------
void svtkAbstractTransform::Update()
{
  ...
  {
    svtkDebugMacro("Updating transformation from its inverse");
    this->InternalDeepCopy(this->MyInverse);
    this->Inverse();
    svtkDebugMacro("Calling InternalUpdate on the transformation");
    this->InternalUpdate();
  }
...
//----------------------------------------------------------------------------
int svtkAbstractTransform::CircuitCheck(svtkAbstractTransform* transform)
{
  return (
...
svtkMTimeType svtkAbstractTransform::GetMTime()
{
  svtkMTimeType mtime = this->svtkObject::GetMTime();
  if (this->DependsOnInverse)
  {
    svtkMTimeType inverseMTime = this->MyInverse->GetMTime();
...
// inverse.
void svtkAbstractTransform::UnRegister(svtkObjectBase* o)
{
  if (this->InUnRegister)
  { // we don't want to go into infinite recursion...
    svtkDebugMacro(<< "UnRegister: circular reference eliminated");
    --this->ReferenceCount;
    return;
  }
  ...
  { // break the cycle
    svtkDebugMacro(<< "UnRegister: eliminating circular reference");
    this->InUnRegister = 1;
...
//----------------------------------------------------------------------------
//----------------------------------------------------------------------------
// All of the following methods are for svtkTransformConcatenation
//----------------------------------------------------------------------------
//----------------------------------------------------------------------------

//----------------------------------------------------------------------------
// A very, very minimal transformation
class svtkSimpleTransform : public svtkHomogeneousTransform
{
public:
  svtkTypeMacro(svtkSimpleTransform, svtkHomogeneousTransform);
  static svtkSimpleTransform* New() { SVTK_STANDARD_NEW_BODY(svtkSimpleTransform); }
  svtkAbstractTransform* MakeTransform() override { return svtkSimpleTransform::New(); }
...
protected:
  svtkSimpleTransform() = default;
  svtkSimpleTransform(const svtkSimpleTransform&);
  svtkSimpleTransform& operator=(const svtkSimpleTransform&);
};

//----------------------------------------------------------------------------
svtkTransformConcatenation::svtkTransformConcatenation()
{
...
svtkTransformConcatenation::~svtkTransformConcatenation()
{
  if (this->NumberOfTransforms > 0)
  {
    ...
    {
      svtkTransformPair* tuple = &this->TransformList[i];
...
//----------------------------------------------------------------------------
void svtkTransformConcatenation::Concatenate(svtkAbstractTransform* trans)
{
  ...
  svtkTransformPair* transList = this->TransformList;
  ...
  if (this->NumberOfTransforms > this->MaxNumberOfTransforms)
  {
    ...
    transList = new svtkTransformPair[nMax];
    for (int i = 0; i < n; i++)
    {
...
//----------------------------------------------------------------------------
void svtkTransformConcatenation::Concatenate(const double elements[16])
{
  ...
  if (this->PreMultiplyFlag)
  {
    if (this->PreMatrix == nullptr)
    {
      ...
      svtkSimpleTransform* mtrans = svtkSimpleTransform::New();
      this->Concatenate(mtrans);
      mtrans->Delete();
      this->PreMatrixTransform = mtrans;
      this->PreMatrix = mtrans->GetMatrix();
    }
    svtkMatrix4x4::Multiply4x4(*this->PreMatrix->Element, elements, *this->PreMatrix->Element);
  ...
  else
  {
    ...
    svtkMatrix4x4::Multiply4x4(elements, *this->PostMatrix->Element, *this->PostMatrix->Element);
...
//----------------------------------------------------------------------------
void svtkTransformConcatenation::Translate(double x, double y, double z)
{
...
//----------------------------------------------------------------------------
void svtkTransformConcatenation::Rotate(double angle, double x, double y, double z)
{
...
//----------------------------------------------------------------------------
void svtkTransformConcatenation::Scale(double x, double y, double z)
{
...
//----------------------------------------------------------------------------
void svtkTransformConcatenation::Inverse()
{
  // invert the matrices
  if (this->PreMatrix)
  {
  ...
  svtkMatrix4x4* tmp = this->PreMatrix;
  svtkAbstractTransform* tmp2 = this->PreMatrixTransform;
...
//----------------------------------------------------------------------------
void svtkTransformConcatenation::Identity()
{
...
//----------------------------------------------------------------------------
svtkAbstractTransform* svtkTransformConcatenation::GetTransform(int i)
{
  // we walk through the list in reverse order if InverseFlag is set
  if (this->InverseFlag)
  {
    int j = this->NumberOfTransforms - i - 1;
    svtkTransformPair* tuple = &this->TransformList[j];
...
//----------------------------------------------------------------------------
svtkMTimeType svtkTransformConcatenation::GetMaxMTime()
{
  svtkMTimeType result = 0;
  svtkMTimeType mtime;
...
//----------------------------------------------------------------------------
void svtkTransformConcatenation::DeepCopy(svtkTransformConcatenation* concat)
{
  ...
  if (this->MaxNumberOfTransforms < concat->NumberOfTransforms)
  {
    ...
    svtkTransformPair* newList = new svtkTransformPair[newMax];
  ...
  svtkSimpleTransform* oldPreMatrixTransform = nullptr;
  svtkSimpleTransform* oldPostMatrixTransform = nullptr;

  if (this->PreMatrix)
  {
    svtkTransformPair* tuple;
    if (this->InverseFlag)
    {
    ...
    oldPreMatrixTransform = static_cast<svtkSimpleTransform*>(this->PreMatrixTransform);
    this->PreMatrixTransform = nullptr;
    this->PreMatrix = nullptr;
  }
  ...
  if (this->PostMatrix)
  {
    ...
    oldPostMatrixTransform = static_cast<svtkSimpleTransform*>(this->PostMatrixTransform);
    this->PostMatrixTransform = nullptr;
    this->PostMatrix = nullptr;
  }
  ...
  for (; i < n; i++)
  {
    svtkTransformPair* pair = &this->TransformList[i];
    svtkTransformPair* pair2 = &concat->TransformList[i];
  ...
  if (concat->PreMatrix)
  {
    ...
    svtkTransformPair* pair = &this->TransformList[i];
    svtkSimpleTransform* mtrans;

    if (concat->InverseFlag == this->InverseFlag)
    {
      mtrans = (oldPreMatrixTransform ? oldPreMatrixTransform : svtkSimpleTransform::New());
      oldPreMatrixTransform = nullptr;
    }
    else
    {
      mtrans = (oldPostMatrixTransform ? oldPostMatrixTransform : svtkSimpleTransform::New());
...
//----------------------------------------------------------------------------
void svtkTransformConcatenation::PrintSelf(ostream& os, svtkIndent indent)
{
...
//----------------------------------------------------------------------------
//----------------------------------------------------------------------------
// All of the following methods are for svtkTransformConcatenationStack
//----------------------------------------------------------------------------
//----------------------------------------------------------------------------

//----------------------------------------------------------------------------
svtkTransformConcatenationStack::svtkTransformConcatenationStack()
{
  this->StackSize = 0;
...
//----------------------------------------------------------------------------
void svtkTransformConcatenationStack::Pop(svtkTransformConcatenation** concat)
{
...
//----------------------------------------------------------------------------
void svtkTransformConcatenationStack::Push(svtkTransformConcatenation** concat)
{
  // check stack size and grow if necessary
  if ((this->Stack - this->StackBottom) == this->StackSize)
  {
    int newStackSize = this->StackSize + 10;
    svtkTransformConcatenation** newStackBottom = new svtkTransformConcatenation*[newStackSize];
  ...
  *concat = svtkTransformConcatenation::New();
  (*concat)->DeepCopy(*(this->Stack - 1));
}

//----------------------------------------------------------------------------
void svtkTransformConcatenationStack::DeepCopy(svtkTransformConcatenationStack* stack)
{
  ...
  for (int i = m; i < n; i++)
  {
    *this->Stack++ = svtkTransformConcatenation::New();
  }
