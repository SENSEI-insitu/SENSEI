 ...
 *
 * svtkLinearTransform provides a generic interface for linear
 * (affine or 12 degree-of-freedom) geometric transformations.
 * @sa
 * svtkTransform svtkIdentityTransform
 */

#ifndef svtkLinearTransform_h
#define svtkLinearTransform_h

#include "svtkCommonTransformsModule.h" // For export macro
#include "svtkHomogeneousTransform.h"

class SVTKCOMMONTRANSFORMS_EXPORT svtkLinearTransform : public svtkHomogeneousTransform
{
public:
  svtkTypeMacro(svtkLinearTransform, svtkHomogeneousTransform);
   ...
   */
  double* TransformNormal(double x, double y, double z) SVTK_SIZEHINT(3)
  {
    return this->TransformDoubleNormal(x, y, z);
  }
  double* TransformNormal(const double normal[3]) SVTK_SIZEHINT(3)
  {
   ...
   * Use this if you are programming in python or Java.
   */
  float* TransformFloatNormal(float x, float y, float z) SVTK_SIZEHINT(3)
  {
  ...
  float* TransformFloatNormal(const float normal[3]) SVTK_SIZEHINT(3)
  {
   ...
   * Use this if you are programming in python or Java.
   */
  double* TransformDoubleNormal(double x, double y, double z) SVTK_SIZEHINT(3)
  {
  ...
  double* TransformDoubleNormal(const double normal[3]) SVTK_SIZEHINT(3)
  {
    return this->TransformDoubleNormal(normal[0], normal[1], normal[2]);
  }
  //@}
   ...
   * Use this if you are programming in python or Java.
   */
  double* TransformVector(double x, double y, double z) SVTK_SIZEHINT(3)
  {
    return this->TransformDoubleVector(x, y, z);
  }
  double* TransformVector(const double normal[3]) SVTK_SIZEHINT(3)
  {
   ...
   * Use this if you are programming in python or Java.
   */
  float* TransformFloatVector(float x, float y, float z) SVTK_SIZEHINT(3)
  {
    this->InternalFloatPoint[0] = x;
    this->InternalFloatPoint[1] = y;
    this->InternalFloatPoint[2] = z;
  ...
  float* TransformFloatVector(const float vec[3]) SVTK_SIZEHINT(3)
  {
   ...
   * Use this if you are programming in python or Java.
   */
  double* TransformDoubleVector(double x, double y, double z) SVTK_SIZEHINT(3)
  {
    this->InternalDoublePoint[0] = x;
    this->InternalDoublePoint[1] = y;
    this->InternalDoublePoint[2] = z;
  ...
  double* TransformDoubleVector(const double vec[3]) SVTK_SIZEHINT(3)
  {
   ...
   * results to outNms.
   */
  virtual void TransformNormals(svtkDataArray* inNms, svtkDataArray* outNms);

  /**
   * Apply the transformation to a series of vectors, and append the
   * results to outVrs.
   */
  virtual void TransformVectors(svtkDataArray* inVrs, svtkDataArray* outVrs);
   ...
   * svtkLinearTransform.
   */
  svtkLinearTransform* GetLinearInverse()
  {
    return static_cast<svtkLinearTransform*>(this->GetInverse());
  }
...
protected:
  svtkLinearTransform() {}
  ~svtkLinearTransform() override {}

private:
  svtkLinearTransform(const svtkLinearTransform&) = delete;
  void operator=(const svtkLinearTransform&) = delete;
};
