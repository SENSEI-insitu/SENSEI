...
#ifndef svtkTimerLog_h
#define svtkTimerLog_h
...
class SVTKCOMMONSYSTEM_EXPORT svtkTimerLog : public svtkObject
{
public:
  static svtkTimerLog* New();

  svtkTypeMacro(svtkTimerLog, svtkObject);
   ...
   */
  static void SetLogging(int v) { svtkTimerLog::Logging = v; }
  static int GetLogging() { return svtkTimerLog::Logging; }
  static void LoggingOn() { svtkTimerLog::SetLogging(1); }
  static void LoggingOff() { svtkTimerLog::SetLogging(0); }
  ...
  static void FormatAndMarkEvent(const char* EventString, ...) SVTK_FORMAT_PRINTF(1, 2);
#endif
  ...
  static svtkTimerLogEntry::LogEntryType GetEventType(int i);
  //@}
...
protected:
  ...
  static std::vector<svtkTimerLogEntry> TimerLog;
   ...
   * Record a timing event and capture wall time and cpu ticks.
   */
  static void MarkEventInternal(const char* EventString, svtkTimerLogEntry::LogEntryType type,
    svtkTimerLogEntry* entry = nullptr);
  ...
  static svtkTimerLogEntry* GetEvent(int i);

  static void DumpEntry(ostream& os, int index, double time, double deltatime, int tick,
    int deltatick, const char* event);

private:
  svtkTimerLog(const svtkTimerLog&) = delete;
  void operator=(const svtkTimerLog&) = delete;
};

/**
 * Helper class to log time within scope
 */
class svtkTimerLogScope
{
public:
  svtkTimerLogScope(const char* eventString)
  {
    if (eventString)
    {
      this->EventString = eventString;
    }
    svtkTimerLog::MarkStartEvent(eventString);
  }

  ~svtkTimerLogScope() { svtkTimerLog::MarkEndEvent(this->EventString.c_str()); };

protected:
  std::string EventString;

private:
  svtkTimerLogScope(const svtkTimerLogScope&) = delete;
  void operator=(const svtkTimerLogScope&) = delete;
};

//
// Set built-in type.  Creates member Set"name"() (e.g., SetVisibility());
//
#define svtkTimerLogMacro(string)                                                                   \
  {                                                                                                \
    svtkTimerLog::FormatAndMarkEvent(                                                               \
