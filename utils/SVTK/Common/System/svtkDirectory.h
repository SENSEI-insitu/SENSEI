 ...
 *
 * svtkDirectory provides a portable way of finding the names of the files
 * in a system directory.  It also provides methods of manipulating directories.
 *
 * @warning
 * svtkDirectory works with windows and unix only.
 */

#ifndef svtkDirectory_h
#define svtkDirectory_h
...
class SVTKCOMMONSYSTEM_EXPORT svtkDirectory : public svtkObject
{
public:
  //@{
  /**
   ...
   */
  svtkTypeMacro(svtkDirectory, svtkObject);
  //@}

  /**
   * Create a new svtkDirectory object.
   */
  static svtkDirectory* New();
   ...
   */
  const char* GetFile(svtkIdType index);
...
private:
  svtkDirectory(const svtkDirectory&) = delete;
  void operator=(const svtkDirectory&) = delete;
};
