...
class SVTKCOMMONSYSTEM_EXPORT svtkSocket : public svtkObject
{
public:
  svtkTypeMacro(svtkSocket, svtkObject);
   ...
   * Returns 1 on success, 0 on error and raises svtkCommand::ErrorEvent.
   */
   ...
   * svtkCommand::ErrorEvent is raised.
   */
   ...
   */
  svtkGetMacro(SocketDescriptor, int);
...
protected:
  ...
  friend class svtkSocketCollection;
...
private:
  svtkSocket(const svtkSocket&) = delete;
  void operator=(const svtkSocket&) = delete;
};
