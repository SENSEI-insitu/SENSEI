...
/**
 * @class   svtkSparseArray
 * @brief   Sparse, independent coordinate storage for N-way arrays.
 *
 *
 * svtkSparseArray is a concrete svtkArray implementation that stores values using
 ...
 *
 * In addition to the value retrieval and update methods provided by svtkTypedArray,
 * svtkSparseArray provides methods to:
 *
 ...
 *
 * @sa
 * svtkArray, svtkTypedArray, svtkDenseArray
 *
 * @par Thanks:
 * Developed by Timothy M. Shead (tshead@sandia.gov) at Sandia National Laboratories.
 */

#ifndef svtkSparseArray_h
#define svtkSparseArray_h
...
template <typename T>
class svtkSparseArray : public svtkTypedArray<T>
{
public:
  svtkTemplateTypeMacro(svtkSparseArray<T>, svtkTypedArray<T>);
  static svtkSparseArray<T>* New();
   ...
   * The supplied svtkArraySort object controls which dimensions are sorted, and in what
   * order, and should contain one-or-more sort dimensions, up to the number of dimensions
   * stored in the array.
   */
  void Sort(const svtkArraySort& sort);
   ...
   */
  void SetExtents(const svtkArrayExtents& extents);
  ...
  void AddValue(const svtkArrayCoordinates& coordinates, const T& value);
  //@}
...
protected:
  svtkSparseArray();
  ~svtkSparseArray() override;

private:
  svtkSparseArray(const svtkSparseArray&) = delete;
  void operator=(const svtkSparseArray&) = delete;
  ...
  typedef svtkSparseArray<T> ThisT;
...
#include "svtkSparseArray.txx"

#endif
// SVTK-HeaderTest-Exclude: svtkSparseArray.h
