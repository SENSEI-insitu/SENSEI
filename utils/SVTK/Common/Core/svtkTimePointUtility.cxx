/*=========================================================================
  ...
  Module:    svtkTimePointUtility.cxx
...
const int svtkTimePointUtility::MILLIS_PER_SECOND = 1000;
const int svtkTimePointUtility::MILLIS_PER_MINUTE = 60000;
const int svtkTimePointUtility::MILLIS_PER_HOUR = 3600000;
const int svtkTimePointUtility::MILLIS_PER_DAY = 86400000;
const int svtkTimePointUtility::SECONDS_PER_MINUTE = 60;
const int svtkTimePointUtility::SECONDS_PER_HOUR = 3600;
const int svtkTimePointUtility::SECONDS_PER_DAY = 86400;
const int svtkTimePointUtility::MINUTES_PER_HOUR = 60;
const int svtkTimePointUtility::MINUTES_PER_DAY = 1440;
const int svtkTimePointUtility::HOURS_PER_DAY = 24;

svtkStandardNewMacro(svtkTimePointUtility);

void svtkTimePointUtility::PrintSelf(ostream& os, svtkIndent indent)
{
  this->Superclass::PrintSelf(os, indent);
}

svtkTypeUInt64 svtkTimePointUtility::DateToTimePoint(int year, int month, int day)
{
  if (year < 0)
  {
    ++year;
...
svtkTypeUInt64 svtkTimePointUtility::TimeToTimePoint(int hour, int minute, int second, int millis)
{
  return MILLIS_PER_HOUR * hour + MILLIS_PER_MINUTE * minute + MILLIS_PER_SECOND * second + millis;
}

svtkTypeUInt64 svtkTimePointUtility::DateTimeToTimePoint(
...
void svtkTimePointUtility::GetDate(svtkTypeUInt64 time, int& year, int& month, int& day)
{
  int y, m, d;
...
void svtkTimePointUtility::GetTime(
  svtkTypeUInt64 time, int& hour, int& minute, int& second, int& millis)
{
...
void svtkTimePointUtility::GetDateTime(svtkTypeUInt64 time, int& year, int& month, int& day,
...
int svtkTimePointUtility::GetYear(svtkTypeUInt64 time)
{
  int year, month, day;
  GetDate(time, year, month, day);
  return year;
...
int svtkTimePointUtility::GetMonth(svtkTypeUInt64 time)
{
  int year, month, day;
  GetDate(time, year, month, day);
  return month;
...
int svtkTimePointUtility::GetDay(svtkTypeUInt64 time)
{
  int year, month, day;
  GetDate(time, year, month, day);
  return day;
...
int svtkTimePointUtility::GetHour(svtkTypeUInt64 time)
{
  return static_cast<int>(time % MILLIS_PER_DAY) / MILLIS_PER_HOUR;
}

int svtkTimePointUtility::GetMinute(svtkTypeUInt64 time)
{
  return static_cast<int>(time % MILLIS_PER_HOUR) / MILLIS_PER_MINUTE;
}

int svtkTimePointUtility::GetSecond(svtkTypeUInt64 time)
{
  return static_cast<int>(time % MILLIS_PER_MINUTE) / MILLIS_PER_SECOND;
}

int svtkTimePointUtility::GetMillisecond(svtkTypeUInt64 time)
{
  return static_cast<int>(time % MILLIS_PER_SECOND);
}

svtkTypeUInt64 svtkTimePointUtility::ISO8601ToTimePoint(const char* cstr, bool* ok)
{
  bool formatValid = true;
  svtkTypeUInt64 value = 0;

  svtkStdString str(cstr);
  ...
  if (str.length() == 19 || str.length() == 23)
  {
    ...
    for (svtkStdString::size_type c = 0; c < str.length(); c++)
    {
      if (c == 4 || c == 7)
      {
        if (str.at(c) != '-')
...
const char* svtkTimePointUtility::TimePointToISO8601(svtkTypeUInt64 time, int format)
{
  ...
  {
    svtkGenericWarningMacro(<< "Format undefined.");
    return nullptr;
  }
