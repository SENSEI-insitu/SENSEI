...
svtkStandardNewMacro(svtkConditionVariable);
...
int pthread_cond_init(svtkConditionType* cv, const pthread_condattr_t*)
{
  *cv = 0;
  return 0;
}

int pthread_cond_destroy(svtkConditionType* cv)
{
  if (*cv)
    return EBUSY;
  return 0;
...
int pthread_cond_signal(svtkConditionType* cv)
{
  *cv = 1;
  return 0;
}

int pthread_cond_broadcast(svtkConditionType* cv)
{
  *cv = 1;
  return 0;
}

int pthread_cond_wait(svtkConditionType* cv, svtkMutexType* lock)
{
  *lock = 0;
  while (!*cv)
    ;
...
#endif // ! defined(SVTK_USE_PTHREADS) && ! defined(SVTK_USE_WIN32_THREADS)
...
int pthread_cond_wait(pthread_cond_t* cv, svtkMutexType* externalMutex)
{
...
svtkSimpleConditionVariable::svtkSimpleConditionVariable()
{
  ...
  switch (result)
  {
    case EINVAL:
    {
      svtkGenericWarningMacro("Invalid condition variable attributes.");
    }
    break;
    case ENOMEM:
    {
      svtkGenericWarningMacro("Not enough memory to create a condition variable.");
    }
    break;
    case EAGAIN:
    {
      svtkGenericWarningMacro("Temporarily not enough memory to create a condition variable.");
    }
    break;
  }
}
...
svtkSimpleConditionVariable::~svtkSimpleConditionVariable()
{
  ...
  switch (result)
  {
    case EINVAL:
    {
      svtkGenericWarningMacro("Could not destroy condition variable (invalid value)");
    }
    break;
    case EBUSY:
    {
      svtkGenericWarningMacro("Could not destroy condition variable (locked by another thread)");
    }
    break;
  }
}

void svtkSimpleConditionVariable::Signal()
{
  pthread_cond_signal(&this->ConditionVariable);
}

void svtkSimpleConditionVariable::Broadcast()
{
  pthread_cond_broadcast(&this->ConditionVariable);
}

int svtkSimpleConditionVariable::Wait(svtkSimpleMutexLock& lock)
{
  return pthread_cond_wait(&this->ConditionVariable, &lock.MutexLock);
}

void svtkConditionVariable::PrintSelf(ostream& os, svtkIndent indent)
{
