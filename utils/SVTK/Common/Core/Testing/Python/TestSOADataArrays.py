...
# method svtkSOADataArrayTemplate.keys().
types = ['char', 'int8', 'uint8', 'int16', 'uint16', 'int32', 'uint32',
         'int64', 'uint64', 'float32', 'float64']
formats = ['c', 'b', 'B', 'h', 'H', 'i', 'I', 'q', 'Q', 'f', 'd']

numarraytypes = 1
if hasattr(svtk, "svtkScaledSOADataArrayTemplate"):
    numarraytypes = 2
...
for i in range(numarraytypes):
    for T in types:
        ...
        if i == 0:
            a = svtk.svtkSOADataArrayTemplate[T]()
        else:
            a = svtk.svtkScaledSOADataArrayTemplate[T]()
