"""Test buffer protocol for SVTK arrays
...
lsize = svtk.SVTK_SIZEOF_LONG
idsize = svtk.SVTK_SIZEOF_ID_TYPE
if idsize == 8:
    idchar = 'q'
else:
    idchar = 'i'
...
class TestBuffer(Testing.svtkTest):
    ...
    def testBufferShared(self):
        """Test the special buffer_shared() check that SVTK provides."""
        a = bytearray(b'hello')
        self.assertEqual(svtk.buffer_shared(a, a), True)
        b = bytearray(b'hello')
        self.assertEqual(svtk.buffer_shared(a, b), False)
        ...
        b = svtk.svtkFloatArray()
        b.SetVoidArray(a, 6, True)
        self.assertEqual(svtk.buffer_shared(a, b), True)

        c = svtk.svtkFloatArray()
        c.DeepCopy(a)
        self.assertEqual(svtk.buffer_shared(a, c), False)

        if sys.hexversion >= 0x02070000:
            m = memoryview(a)
            self.assertEqual(svtk.buffer_shared(a, m), True)
