...
noShutdown = set()
noObserver = set()
# Is a svtkAlgorithm but EmptyInput failed.
isVtkAlgorithm = set()
...
def GetSVTKClassGroups(svtkClasses, subStr):
    ...
    :return: a set of all the SVTK classes that are readers.
    '''
    res = set()
...
def TestOne(cname):
    ...
    try:
        ...
        # If thread creation moves away from the svtkGeoSource constructor, then
        # this ShutDown call will not be necessary...
        #
        if b.IsA("svtkGeoSource"):
            TryShutdown(b, e)
...
def ProgramOptions():
    desc = """
    %prog Tests each SVTK class for empty input using various data structures.
    """
