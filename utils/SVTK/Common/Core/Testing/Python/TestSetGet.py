...
'''
svtk.svtkObject.GlobalWarningDisplayOff()

def RedirectSVTKMessages():
    '''
    Can be used to redirect SVTK related error messages to a
    file.
    '''
    log = svtk.svtkFileOutputWindow()
    log.SetFlush(1)
    log.AppendOff()
...
"svtkDistributedDataFilter",  # core dump
"svtkDataEncoder",  # Use after free error.
"svtkWebApplication",  # Thread issues - calls svtkDataEncoder
"svtkView",  # svtkView.SetRepresentation(None) fails

"svtkGenericAttributeCollection",  # Assert error
"svtkOverlappingAMR",
...
# core dump in release mode, issue is svtkChartMatrix.GetChart() & svtkScatterPlotMatrix.SetActivePlot()
"svtkChartMatrix",
...
class ErrorObserver:
   '''
   See: http://public.kitware.com/pipermail/svtkusers/2012-June/074703.html
   '''
   def __init__(self):
...
def GetSVTKClasses():
    '''
    :return: a set of all the SVTK classes.
    '''
    # This pattern will get the name and type of the member in the svtk classes.
    pattern = r'\<svtkclass (.*)\.(.*)\>'
    regEx = re.compile(pattern)
    svtkClasses = inspect.getmembers(
                    svtk, inspect.isclass and not inspect.isabstract)
    res = set()
    for name, obj in svtkClasses:
            result = re.match(regEx, repr(obj))
            if result:
                res.add(result.group(2))
    return res

def GetSVTKClassGroups(svtkClasses, subStr):
    '''
    :param: svtkClasses - the set of SVTK classes
    :param: subStr - the substring for the SVTK class to match e.g Readers
    :return: a set of all the SVTK classes that are e.g. Readers.
    '''
    res = set()
    for obj in list(svtkClasses):
...
def FilterClasses(allClasses, filter):
    ...
    :return: a set of all the SVTK classes that do not have the substrings
     in their names.
    '''
    res = allClasses
    for f in filter:
        c = GetSVTKClassGroups(allClasses, f)
        res = res - c
    return res
...
def TestOne(cname):
    ...
    try:
        ...
        methods = [method for method in dir(getattr(svtk, cname))
                    if callable(getattr(getattr(svtk, cname), method))]
...
def BatchTest(svtkClasses, batchNo, batchSize):
    '''
...
def ProgramOptions():
    desc = """
    %prog Tests each SVTK class for Set(Get()) where Get() has no parameters.
    """
...
def main(argv=None):
    ...
    if classNames:
        ...
        svtkClasses = GetSVTKClasses()
#         filter = ['Reader', 'Writer', 'Array_I', 'Qt']
#         svtkClasses = FilterClasses(svtkClasses, filter)
        svtkClasses = svtkClasses - classExceptions
        TestSetGet(svtkClasses)
#         In Windows
