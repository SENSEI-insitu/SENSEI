...
class TerminationCrashTestCase:
    ...
    careful in DECREFing a Python callback via svtkPythonCommand.
    ...
    class which contains a svtk object which is being observed by that
    ...
    the same python object which contains the svtk object. There is also
    no error if a svtk object is subclassed in python and that subclass
