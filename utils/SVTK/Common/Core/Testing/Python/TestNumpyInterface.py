...
elev2 = svtk.svtkElevationFilter()
...
assert algs.make_point_mask_from_NaNs(ds1, elev_copy)[1] == svtk.svtkDataSetAttributes.DUPLICATEPOINT | svtk.svtkDataSetAttributes.HIDDENPOINT

cell_array = numpy.zeros(ds1.GetNumberOfCells())
cell_array[1] = numpy.nan
assert algs.make_cell_mask_from_NaNs(ds1, cell_array)[1] == svtk.svtkDataSetAttributes.HIDDENCELL

g3 = svtk.svtkMultiBlockDataGroupFilter()
...
compare(randomVec[dsa.SVTKCompositeDataArray([(slice(None, None, None), slice(0,2,None)), 2])].Arrays[0] - npa[:, 0:2], 1E-6)
...
comp_array1 = dsa.SVTKCompositeDataArray([int_array1, int_array2])
comp_array2 = dsa.SVTKCompositeDataArray([int_array2, int_array1])
comp_array3 = dsa.SVTKCompositeDataArray([int_array2, dsa.NoneArray])
...
assert algs.all(algs.bitwise_or(comp_array1, comp_array3) == dsa.SVTKCompositeDataArray([algs.bitwise_or(int_array1, int_array2), int_array2]))

ssource = svtk.svtkSphereSource()
ssource.Update()

output = ssource.GetOutput()

fd = svtk.svtkFloatArray()
