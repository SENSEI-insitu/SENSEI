...
// Define this to run benchmarking tests on some svtkDataArray methods:
#undef BENCHMARK
// #define BENCHMARK
...
int TestDataArray(int, char*[])
{
  ...
  if (range[0] != SVTK_DOUBLE_MAX || range[1] != SVTK_DOUBLE_MIN)
  {
  ...
  farray->InsertNextTuple1(svtkMath::Inf());
  farray->InsertNextTuple1(svtkMath::NegInf());
  farray->InsertNextTuple1(svtkMath::Nan());
  farray->GetRange(range, 0);
  if (range[0] != svtkMath::NegInf() || range[1] != svtkMath::Inf())
  {
    cerr << "Getting range (" << range[0] << "-" << range[1]
  ...
  if (!svtkMathUtilities::FuzzyCompare(range[0], 0.0) ||
    !svtkMathUtilities::FuzzyCompare(range[1], 9.0))
  {
    cerr << "Getting finite range (" << range[0] << "-" << range[1]
         << ") of array containing infinity and NaN" << std::endl;
    farray->Delete();
...
void benchmark()
{
  ...
  svtkNew<svtkDoubleArray> double1;
  svtkNew<svtkDoubleArray> double2;
  svtkNew<svtkDoubleArray> double3;
  svtkNew<svtkIntArray> int1;
  svtkNew<svtkIntArray> int2;
  svtkNew<svtkIntArray> int3;
  double time;

  double1->SetNumberOfComponents(4);
  double1->SetNumberOfTuples(2500000);

  for (svtkIdType i = 0; i < 10000000; ++i)
  {
