...
static svtkStdString testTuplesString[] = {
  /*
   ...
   The svtkObject ModificationTime counter is used as a
...
static bool CheckUniques(
  svtkAbstractArray* arr, int* uniqueSizeBds, double uncertainty, double minProminence)
{
  bool ok = true;
  if (!arr)
  {
  ...
  svtkNew<svtkVariantArray> uniques;
  for (int c = 0; c < nc; ++c)
  {
...
int TestArrayUniqueValueDetection(int svtkNotUsed(argc), char* svtkNotUsed(argv)[])
{
  bool ok = true;
  svtkNew<svtkDoubleArray> darr;
  svtkNew<svtkIntArray> iarr;
  svtkNew<svtkStringArray> sarr;
