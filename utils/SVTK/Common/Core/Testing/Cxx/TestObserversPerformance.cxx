...
// .SECTION Description
// Probe the speed of svtkObject::AddObserver, svtkObject::InvokeEvent and
// svtkObject::RemoveObserver
...
//------------------------------------------------------------------------------
class svtkSimpleCommand : public svtkCommand
{
public:
  static svtkSimpleCommand* New() { return new svtkSimpleCommand(); }
  svtkTypeMacro(svtkSimpleCommand, svtkCommand);

  void Execute(svtkObject*, unsigned long, void*) override { this->MTime.Modified(); }

protected:
  static svtkTimeStamp MTime;
};

svtkTimeStamp svtkSimpleCommand::MTime;
...
double StressInvoke(const int observerCount, const int eventCount, const int invokeCount)
{
  ...
  std::vector<svtkSmartPointer<svtkSimpleCommand> > observers;
  svtkNew<svtkTimerLog> totalTimer;
  svtkNew<svtkTimerLog> addTimer;
  svtkNew<svtkTimerLog> invokeTimer;
  svtkNew<svtkTimerLog> removeTimer;
  ...
  for (int observerIDX = 0; observerIDX < observerCount; observerIDX += eventCount)
  {
    ...
    {
      svtkNew<svtkSimpleCommand> observer;
