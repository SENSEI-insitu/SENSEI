...
void svtkSystemInformationPrintFile(const char* name, ostream& os)
{
  os << "================================================================\n";
...
#ifdef _WIN32
  svtksys::ifstream fin(name, ios::in | ios::binary);
#else
  svtksys::ifstream fin(name, ios::in);
#endif
...
int TestSystemInformation(int argc, char* argv[])
{
  ...
  const char* files[] = { "CMakeCache.txt", "CMakeFiles/CMakeError.log",
    "Common/Core/svtkConfigure.h", "Common/Core/svtkToolkits.h", "SVTKConfig.cmake",
  ...
  for (const char** f = files; *f; ++f)
  {
    ...
    svtkSystemInformationPrintFile(fname.c_str(), cout);
  }
