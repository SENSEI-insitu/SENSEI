...
int TestArrayAPISparse(int svtkNotUsed(argc), char* svtkNotUsed(argv)[])
{
  try
  {
    // Create an 3D array ...
    ...
    // Verify that the array contains all zeros ...
    {
      ...
      for (int i = extents[0].GetBegin(); i != extents[0].GetEnd(); ++i)
      {
        for (int j = extents[1].GetBegin(); j != extents[1].GetEnd(); ++j)
        {
          ...
          {
            test_expression(array->GetValue(svtkArrayCoordinates(i, j, k)) == 0);
          }
        }
      }
    }
    ...
    svtkSmartPointer<svtkSparseArray<double> > deep_copy;
    deep_copy.TakeReference(svtkSparseArray<double>::SafeDownCast(array->DeepCopy()));
    ...
    array->Resize(svtkArrayExtents(2, 3, 4));
    array->SetValue(0, 1, 2, 1.1);
    array->SetValue(svtkArrayCoordinates(1, 2, 3), 2.2);
    array->SetValue(svtkArrayCoordinates(0, 1, 1), 3.3);

    test_expression(array->GetSize() == 24);
    test_expression(array->GetNonNullSize() == 3);
    test_expression(array->GetValue(svtkArrayCoordinates(0, 1, 2)) == 1.1);
    test_expression(array->GetValue(0, 1, 2) == 1.1);
    test_expression(array->GetValue(svtkArrayCoordinates(1, 2, 3)) == 2.2);
    test_expression(array->GetValue(svtkArrayCoordinates(0, 1, 1)) == 3.3);
    ...
    array->SetExtents(svtkArrayExtents(2, 3, 4));
    test_expression(array->GetExtents() == svtkArrayExtents(2, 3, 4));

    // Verify the sort
    array->Clear();
    array->Resize(svtkArrayExtents(5));
    ...
    svtkArraySort arraySort;
    arraySort.SetDimensions(1);
    arraySort[0] = 0;
    array->Sort(arraySort);
    const svtkSparseArray<double>::CoordinateT* coords = array->GetCoordinateStorage(0);
    ...
    // dimension API's
    svtkSmartPointer<svtkSparseArray<double> > array1D =
      svtkSmartPointer<svtkSparseArray<double> >::New();
    array1D->Resize(svtkArrayExtents::Uniform(1, 0));
    array1D->SetValue(0, 1.0);
    array1D->AddValue(0, 2.0);
    test_expression(!array1D->GetValue(1));
    test_expression(array1D->GetExtents() == svtkArrayExtents(0));

    std::vector<svtkSparseArray<double>::CoordinateT> coords1DU = array1D->GetUniqueCoordinates(0);
    test_expression(!(coords1DU.size() == array1D->GetNonNullSize()));

    svtkArrayCoordinates coord1D;
    ...
    test_expression(array1D->GetExtents() == svtkArrayExtents(10));

    array1D->ReserveStorage(1000);
    test_expression(array1D->GetNonNullSize() == 1000);

    svtkSmartPointer<svtkSparseArray<double> > array2D =
      svtkSmartPointer<svtkSparseArray<double> >::New();
    array2D->Resize(svtkArrayExtents::Uniform(2, 0));
    array2D->SetValue(0, 0, 2.0);
    array2D->AddValue(0, 0, 2.0);
    test_expression(array2D->GetExtents() == svtkArrayExtents(0, 0));
    ...
    test_expression(array2D->GetExtents() == svtkArrayExtents(10, 9));

    svtkSmartPointer<svtkSparseArray<double> > array3D =
      svtkSmartPointer<svtkSparseArray<double> >::New();
    array3D->Resize(svtkArrayExtents::Uniform(3, 0));
    array3D->SetValue(0, 0, 0, 0.0);
    array3D->AddValue(0, 0, 0, 3.0);
    test_expression(array3D->GetExtents() == svtkArrayExtents(0, 0, 0));
    ...
    test_expression(array3D->GetExtents() == svtkArrayExtents(10, 9, 8));
    ...
    array1D->AddObserver(svtkCommand::ErrorEvent, errorObserver);
    array2D->AddObserver(svtkCommand::ErrorEvent, errorObserver);
    array3D->AddObserver(svtkCommand::ErrorEvent, errorObserver);

    // Extent array dimension mismatch
    array1D->SetExtents(svtkArrayExtents(2, 3, 4));
    test_expression(errorObserver->GetErrorMessage().find("Extent-array dimension mismatch.") !=
      std::string::npos);
    errorObserver->Clear();

    // Index mismatch errors
    svtkArrayCoordinates coordND;
    ...
    std::vector<svtkSparseArray<double>::CoordinateT> badcoords = array1D->GetUniqueCoordinates(5);
