...
{
  static constexpr svtk::ComponentIdType NumComps = 3;
  static constexpr svtk::TupleIdType NumTuples = 12;

  void operator()()
  {
    svtkNew<ArrayType> array;
    auto da = static_cast<svtkDataArray*>(array);
    array->SetNumberOfComponents(NumComps);

    this->TestEmptyRange(svtk::DataArrayTupleRange(array));
    this->TestEmptyRange(svtk::DataArrayTupleRange(da));
    this->TestEmptyRange(svtk::DataArrayTupleRange<NumComps>(array));
    this->TestEmptyRange(svtk::DataArrayTupleRange<NumComps>(da));

    array->SetNumberOfTuples(this->NumTuples);

    this->TestEmptyRange(svtk::DataArrayTupleRange(array, 4, 4));
    this->TestEmptyRange(svtk::DataArrayTupleRange(da, 4, 4));
    this->TestEmptyRange(svtk::DataArrayTupleRange<NumComps>(array, 4, 4));
    this->TestEmptyRange(svtk::DataArrayTupleRange<NumComps>(da, 4, 4));

    FillTupleRangeIota(svtk::DataArrayTupleRange<NumComps>(array));
    ...
    { // Full, dynamic-size, real typed range
      ...
      DispatchRangeTests<ArrayType, svtk::detail::DynamicTupleSize>(range, array, 0, NumTuples);
    }
    { // Full, dynamic-size, generic-typed range
      auto range = svtk::DataArrayTupleRange(da);
      DispatchRangeTests<svtkDataArray, svtk::detail::DynamicTupleSize>(range, array, 0, NumTuples);
    }
    { // Full, fixed-size, real typed range
      auto range = svtk::DataArrayTupleRange<NumComps>(array);
      DispatchRangeTests<ArrayType, NumComps>(range, array, 0, NumTuples);
    }
    { // Full, fixed-size, generic-typed range
      auto range = svtk::DataArrayTupleRange<NumComps>(da);
      DispatchRangeTests<svtkDataArray, NumComps>(range, array, 0, NumTuples);
    }
    { // Partial, dynamic-size, real typed range
      auto range = svtk::DataArrayTupleRange(array, pStart, pEnd);
      DispatchRangeTests<ArrayType, svtk::detail::DynamicTupleSize>(range, array, pStart, pEnd);
    }
    { // Partial, dynamic-size, generic-typed range
      auto range = svtk::DataArrayTupleRange(da, pStart, pEnd);
      DispatchRangeTests<svtkDataArray, svtk::detail::DynamicTupleSize>(range, array, pStart, pEnd);
    }
    { // Partial, fixed-size, real typed range
      auto range = svtk::DataArrayTupleRange<NumComps>(array, pStart, pEnd);
      DispatchRangeTests<ArrayType, NumComps>(range, array, pStart, pEnd);
    }
    { // Partial, fixed-size, generic-typed range
      auto range = svtk::DataArrayTupleRange<NumComps>(da, pStart, pEnd);
      DispatchRangeTests<svtkDataArray, NumComps>(range, array, pStart, pEnd);
    }
  }
  ...
  template <typename RangeArrayType, svtk::ComponentIdType RangeTupleSize, typename Range>
  void DispatchRangeTests(
    Range range, RangeArrayType* array, svtk::TupleIdType start, svtk::TupleIdType end)
  {
    {
  ...
  template <typename RangeArrayType, svtk::ComponentIdType RangeTupleSize, typename Range>
  void TestRange(Range& range, RangeArrayType* array, svtk::TupleIdType start, svtk::TupleIdType end)
  {
  ...
  void TestTypes(Range& range)
  {
    ...
    CHECK_TYPEDEF(typename Range::ComponentType, svtk::GetAPIType<RangeArrayType>);
    CHECK_TYPEDEF(typename Range::size_type, svtk::TupleIdType);
    ...
    CHECK_TYPEDEF(svtk::TupleIdType, decltype(range.GetBeginTupleId()));
    CHECK_TYPEDEF(svtk::TupleIdType, decltype(range.GetEndTupleId()));
...
struct UnitTestTupleReferenceAPI
{
  ...
  void TestTypes(Range& range)
  {
    ...
    CHECK_TYPEDEF(svtk::GetAPIType<RangeArrayType>, typename Ref::value_type);
    CHECK_TYPEDEF(typename Ref::size_type, svtk::ComponentIdType);
  ...
  void TestIndexing(Range& range)
  {
    ...
    // exist in memory -- when the ArrayType is svtkDataArray, the doubles
  ...
  void TestArrayAccess(Range& range)
  {
    ...
    using VectorType = svtkVector<typename Range::ComponentType, NumComps>;
  ...
  template <typename Range>
  static auto GetTestingIterator(Range& range, svtk::TupleIdType tupleOffset = 0)
...
struct UnitTestComponentIteratorAPI
{
  ...
  // (-2, 2).
  template <typename Range>
  static auto GetTestingIterRange(Range& range, svtk::TupleIdType tupleOffset = 0)
    -> decltype(range.begin())
  {
    return (range.begin() + (range.size() / 2)) + tupleOffset;
  }
  ...
  template <typename Range>
  static auto GetTestingIter(Range& range, svtk::TupleIdType tupleOffset = 0)
...
{
  static constexpr svtk::ComponentIdType NumComps = 9;
  static constexpr svtk::TupleIdType NumTuples = 5;
  ...
  template <typename Range>
  auto GetTestCompRef(Range& range, svtk::ComponentIdType offset)
...
struct UnitTestEdgeCases
{
  ...
  void operator()()
  {
    ...
    DispatchTupleCompat<svtkSOADataArrayTemplate<float>, svtkAOSDataArrayTemplate<float> >();

    std::cerr << "AOS<float> <--> SOA<float>\n";
    DispatchTupleCompat<svtkAOSDataArrayTemplate<float>, svtkSOADataArrayTemplate<float> >();

    std::cerr << "SOA<double> <--> AOS<float>\n";
    DispatchTupleCompat<svtkSOADataArrayTemplate<double>, svtkAOSDataArrayTemplate<float> >();

    std::cerr << "AOS<float> <--> SOA<double>\n";
    DispatchTupleCompat<svtkAOSDataArrayTemplate<float>, svtkSOADataArrayTemplate<double> >();

    std::cerr << "SOA<int> <--> AOS<float>\n";
    DispatchTupleCompat<svtkSOADataArrayTemplate<int>, svtkAOSDataArrayTemplate<float> >();

    std::cerr << "AOS<float> <--> SOA<int>\n";
    DispatchTupleCompat<svtkAOSDataArrayTemplate<float>, svtkSOADataArrayTemplate<int> >();

#ifdef SVTK_USE_SCALED_SOA_ARRAYS
    std::cerr << "ScaleSOA<float> <--> AOS<float>\n";
    DispatchTupleCompat<svtkScaledSOADataArrayTemplate<float>, svtkAOSDataArrayTemplate<float> >();

    std::cerr << "AOS<float> <--> ScaleSOA<float>\n";
    DispatchTupleCompat<svtkAOSDataArrayTemplate<float>, svtkScaledSOADataArrayTemplate<float> >();

    std::cerr << "ScaleSOA<double> <--> AOS<float>\n";
    DispatchTupleCompat<svtkScaledSOADataArrayTemplate<double>, svtkAOSDataArrayTemplate<float> >();

    std::cerr << "AOS<float> <--> ScaleSOA<double>\n";
    DispatchTupleCompat<svtkAOSDataArrayTemplate<float>, svtkScaledSOADataArrayTemplate<double> >();

    std::cerr << "ScaleSOA<int> <--> AOS<float>\n";
    DispatchTupleCompat<svtkScaledSOADataArrayTemplate<int>, svtkAOSDataArrayTemplate<float> >();

    std::cerr << "AOS<float> <--> ScaleSOA<int>\n";
    DispatchTupleCompat<svtkAOSDataArrayTemplate<float>, svtkScaledSOADataArrayTemplate<int> >();
#endif
  }
  ...
  static void TestSpecializations()
  {
    ...
    // These should use the objects in svtkDataArrayTupleRange_AOS.h, which
    // end up using ValueType* pointers for component iterators.
    TestAOSSpecialization<svtkAOSDataArrayTemplate<float> >();
    TestAOSSpecialization<svtkFloatArray>();
#endif
  }
  ...
  {
    using ValueType = svtk::GetAPIType<ArrayType>;
    using RangeType = decltype(svtk::DataArrayTupleRange(std::declval<ArrayType*>()));
  ...
  {
    svtkNew<ArrayT1> storage1;
    svtkNew<ArrayT2> storage2;
    this->PrepArray(static_cast<ArrayT1*>(storage1));
    this->PrepArray(static_cast<ArrayT2*>(storage2));

    ArrayT1* a1 = storage1;
    ArrayT2* a2 = storage2;
    svtkDataArray* da1 = a1;
    svtkDataArray* da2 = a2;

    // Generate ranges:
    // - derived and svtkDataArray pointers
    // - dynamic and fixed tuple sizes
    // - mutable and const tuple sizes

    auto aRange1 = svtk::DataArrayTupleRange(a1);
    auto aRange2 = svtk::DataArrayTupleRange(a2);
    auto daRange1 = svtk::DataArrayTupleRange(da1);
    auto daRange2 = svtk::DataArrayTupleRange(da2);

    auto aFixedRange1 = svtk::DataArrayTupleRange<NumComps>(a1);
    auto aFixedRange2 = svtk::DataArrayTupleRange<NumComps>(a2);
    auto daFixedRange1 = svtk::DataArrayTupleRange<NumComps>(da1);
    auto daFixedRange2 = svtk::DataArrayTupleRange<NumComps>(da2);
...
{
  std::cerr << "AOS:\n";
  RunTestsForArray<svtkAOSDataArrayTemplate<float> >();
  std::cerr << "SOA:\n";
  RunTestsForArray<svtkSOADataArrayTemplate<float> >();
#ifdef SVTK_USE_SCALED_SOA_ARRAYS
  std::cerr << "ScaleSOA:\n";
  RunTestsForArray<svtkScaledSOADataArrayTemplate<float> >();
#endif
  std::cerr << "svtkFloatArray:\n";
  RunTestsForArray<svtkFloatArray>();
