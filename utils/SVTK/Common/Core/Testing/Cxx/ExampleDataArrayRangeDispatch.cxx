...
// will work with all svtkDataArrays.
//
// ArrayDispatch provides a mechanism to automatically downcast a svtkDataArray
// object to a subclass (such as svtkFloatArray, svtkSOADataArrayTemplate<int>,
// etc). Using a subclass instead of a svtkDataArray allows the array data
// to be accessed directly in memory, while using the svtkDataArray API requires
// virtual methods and type conversions.
//
// The svtk::DataArrayRange utilities provide STL-style iterators that abstract
// the details of reading/writing values in a svtkDataArray. When used with a
// specific subclass of svtkDataArray, optimized memory access are used. When
// used with a svtkDataArray, the virtual interface is used. This allows a
...
struct FillImpl
{
  ...
  void operator()(ArrayType* array) const
  {
    ...
    auto range = svtk::DataArrayValueRange<1>(array);
...
struct CopyImpl
{
  ...
  {
    // The svtk::GetAPIType<ArrayType> helper provides the API type of the
    // data array (e.g. `float` for svtkFloatArray, `double` for the virtual
    // svtkDataArray API).
    using SrcValueType = svtk::GetAPIType<SrcArray>;
    using DstValueType = svtk::GetAPIType<DstArray>;

    // Create range object for the arrays. These work whether the arrays are
    // downcasted AOS arrays, SOA array, etc or plain svtkDataArrays.
    const auto srcRange = svtk::DataArrayValueRange<1>(src);
    auto dstRange = svtk::DataArrayValueRange<1>(dst);
...
// Create an AOS<float> array and return a plain svtkDataArray pointer.
svtkDataArray* CreateArray()
{
  svtkNew<svtkFloatArray> aosArray;
...
{
  // Create the array. The auto type here is svtkSmartPointer<svtkDataArray>.
  auto array = svtk::TakeSmartPointer(CreateArray());

  // Attempt to downcast the array to a known type and call the FillImpl
  // functor with the downcasted array:
  FillImpl fillFunctor;
  if (!svtkArrayDispatch::Dispatch::Execute(array, fillFunctor))
  {
    ...
    // can reuse the functor with the svtkDataArray pointer as a fallback:
    fillFunctor(array.Get());
  }

  // SumImpl:
  SumImpl sumFunctor;
  if (!svtkArrayDispatch::Dispatch::Execute(array, sumFunctor))
  {
  ...
  // CopyImpl:
  {
    ...
    // The auto type here is svtkSmartPointer<svtkDataArray>.
    auto intArray = svtk::TakeSmartPointer(svtkDataArray::CreateDataArray(SVTK_INT));
    ...
    // documented in the svtkArrayDispatch header.
    CopyImpl copyFunctor;
    if (!svtkArrayDispatch::Dispatch2::Execute(array, intArray, copyFunctor))
    {
      copyFunctor(array.Get(), intArray.Get());
    }

    // Check the arrays are equal using the svtkDataArray double interface via
    // range iterators and std::equal:
    const auto srcRange = svtk::DataArrayValueRange<1>(array);
    const auto dstRange = svtk::DataArrayValueRange<1>(intArray);
