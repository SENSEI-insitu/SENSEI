...
=========================================================================*/
// Test some generic features of svtkLookupTable
...
#define TestVector4d(t1, t2)                                                                       \
  TestAssert((svtkMathUtilities::FuzzyCompare<double>(t1[0], t2[0]) &&                              \
    svtkMathUtilities::FuzzyCompare<double>(t1[1], t2[1]) &&                                        \
    svtkMathUtilities::FuzzyCompare<double>(t1[2], t2[2]) &&                                        \
    svtkMathUtilities::FuzzyCompare<double>(t1[3], t2[3])))
...
public:
  ...
  void Callback(svtkObject*, unsigned long e, void*) { this->EventId = e; }
...
int TestLookupTable(int, char*[])
{
  ...
  TestAssert(table->GetIndex(svtkMath::Nan()) == -1);
  ...
  svtkMath::HSVToRGB(table->GetHueRange()[0], table->GetSaturationRange()[0],
    table->GetValueRange()[0], &rgba[0], &rgba[1], &rgba[2]);
  rgba[3] = table->GetAlphaRange()[0];

  svtkLookupTable::GetColorAsUnsignedChars(rgba, expected);
  ...
  svtkMath::HSVToRGB(table->GetHueRange()[1], table->GetSaturationRange()[1],
  ...
  unsigned long observerId =
    table->AddObserver(svtkCommand::ErrorEvent, &observer, &errorObserver::Callback);
