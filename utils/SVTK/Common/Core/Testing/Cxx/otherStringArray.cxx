...
int doStringArrayTest(ostream& strm, int size)
{
  ...
  svtkStringArray* ptr = svtkStringArray::New();
  svtkStdString* strings = new svtkStdString[SIZE];
  for (int i = 0; i < SIZE; ++i)
  {
    char buf[1024];
    snprintf(buf, sizeof(buf), "string entry %d", i);
    strings[i] = svtkStdString(buf);
  }
  ...
  strm << "\tGetValue...";
  svtkStdString value = ptr->GetValue(123);
  ...
  strm << "\tsvtkAbstractArray::GetTuples(svtkIdList)...";
  svtkIdList* indices = svtkIdList::New();
  indices->InsertNextId(10);
  indices->InsertNextId(20);
  indices->InsertNextId(314);

  svtkStringArray* newValues = svtkStringArray::New();
  ...
  strm << "\tsvtkAbstractArray::GetTuples(svtkIdType, svtkIdType)...";
  newValues->SetNumberOfValues(3);
  ...
  strm << "\tsvtkAbstractArray::InsertTuple...";
