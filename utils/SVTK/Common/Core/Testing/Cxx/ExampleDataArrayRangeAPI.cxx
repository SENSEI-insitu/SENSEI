...
// This file provides some examples of how to use the svtkDataArrayRange
...
void TestTupleRangeAPI(ArrayT* someArray)
{
  ...
  // explicit start/end values to svtk::DataArrayTupleRange:
  {
    auto subRange = svtk::DataArrayTupleRange(someArray, 2, 8);
  ...
  // this can be passed as a template argument to svtk::DataArrayTupleRange.
  // This will enable additional compiler optimizations to improve performance.
  {
    auto optimizedRange = svtk::DataArrayTupleRange<4>(someArray);
  ...
  {
    auto optimizedSubRange = svtk::DataArrayTupleRange<4>(someArray, 2, 8);
  ...
  auto range = svtk::DataArrayTupleRange(someArray);
  ...
  {
    auto fullRange = svtk::DataArrayTupleRange(someArray);
...
{
  // ValueRanges emulate walking a svtkDataArray object using
  // svtkAOSDataArrayTemplate<T>::GetPointer(). That is, ValueRange provides
  ...
  // familiar to experienced SVTK developers. The ValueRange uses ValueIterators
  // and ValueReferences.

  // A ValueRange can be restricted to a subset of the array's data by passing
  // explicit start/end value indices to svtk::DataArrayValueRange:
  {
    auto subRange = svtk::DataArrayValueRange(someArray, 3, 19);
  ...
  // If the exact number of components in a tuple is known at compile-time,
  // this can be passed as a template argument to svtk::DataArrayValueRange.
  ...
  {
    auto optimizedRange = svtk::DataArrayValueRange<4>(someArray);
    // Accessing data in optimizedRange will be more efficient as variables
    // like array strides are made available to the compiler.
    std::cout << "Values: ";
  ...
  // Both tuple size and subrange information can be used simultaneously:
  {
    auto optimizedSubRange = svtk::DataArrayValueRange<4>(someArray, 3, 19);
    std::cout << "Values: ";
  ...
  // If tuple size is unknown and the range should encompass the full length of
  // the array, simply pass in the array with no template arguments:
  auto range = svtk::DataArrayValueRange(someArray);
  std::cout << "Values: ";
  ...
  // spans a portion of the original range:
  {
    auto fullRange = svtk::DataArrayValueRange(someArray);
    // Arguments are (beginOffset, endOffset)
    auto range_2_thru_8 = fullRange.GetSubRange(2, 8);
    // Arguments are relative to the current range:
    auto range_3_thru_6 = range_2_thru_8.GetSubRange(1, 4);
...
int ExampleDataArrayRangeAPI(int, char*[])
{
  ...
  { // Fill with data we don't care about:
    auto range = svtk::DataArrayValueRange<4>(aosArray);
    std::iota(range.begin(), range.end(), 0.f);
  }

  svtkNew<svtkSOADataArrayTemplate<float> > soaArray;
  soaArray->DeepCopy(aosArray);

  // Some svtkDataArray pointers to show that these ranges work with the generic
  // svtkDataArray API:
  svtkDataArray* daAos = aosArray;
  svtkDataArray* daSoa = soaArray;
