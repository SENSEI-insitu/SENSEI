...
static svtkMTimeType MTimeValues[Target];
...
static int Total = 0;
static svtkTypeInt64 Total64 = 0;
#endif

SVTK_THREAD_RETURN_TYPE MyFunction(void*)
{
  svtkNew<svtkObject> AnObject;
...
SVTK_THREAD_RETURN_TYPE MyFunction2(void*)
{
  for (int i = 0; i < Target / NumThreads; i++)
  {
    --TotalAtomic;
...
SVTK_THREAD_RETURN_TYPE MyFunction3(void*)
{
  for (int i = 0; i < Target / NumThreads; i++)
  {
...
SVTK_THREAD_RETURN_TYPE MyFunction4(void*)
{
  for (int i = 0; i < Target / NumThreads; i++)
  {
    TotalAtomic++;
...
int TestAtomic(int, char*[])
{
  ...
  svtkMTimeType *from = MTimeValues, *to = MTimeValues + Target;
  std::sort(from, to);
