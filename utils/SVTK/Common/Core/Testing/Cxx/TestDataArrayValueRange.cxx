...
struct UnitTestValueRangeAPI
{
  ...
  static constexpr svtk::ValueIdType NumValues = NumTuples * NumComps;
  ...
  void operator()()
  {
    ...
    this->TestEmptyRange(svtk::DataArrayValueRange(array));
    this->TestEmptyRange(svtk::DataArrayValueRange(da));
    this->TestEmptyRange(svtk::DataArrayValueRange<NumComps>(array));
    this->TestEmptyRange(svtk::DataArrayValueRange<NumComps>(da));

    array->SetNumberOfTuples(this->NumTuples);

    this->TestEmptyRange(svtk::DataArrayValueRange(array, 4, 4));
    this->TestEmptyRange(svtk::DataArrayValueRange(da, 4, 4));
    this->TestEmptyRange(svtk::DataArrayValueRange<NumComps>(array, 4, 4));
    this->TestEmptyRange(svtk::DataArrayValueRange<NumComps>(da, 4, 4));

    FillValueRangeIota(svtk::DataArrayValueRange<NumComps>(array));

    auto pStart = static_cast<svtk::ValueIdType>(NumTuples / 4 * NumComps + 1);
    auto pEnd = static_cast<svtk::ValueIdType>(3 * NumTuples / 4 * NumComps + 2);

    { // Full, dynamic-size, real typed range
      auto range = svtk::DataArrayValueRange(array);
      DispatchRangeTests<ArrayType, svtk::detail::DynamicTupleSize>(range, array, 0, NumValues);
    }
    { // Full, dynamic-size, generic-typed range
      auto range = svtk::DataArrayValueRange(da);
      DispatchRangeTests<svtkDataArray, svtk::detail::DynamicTupleSize>(range, array, 0, NumValues);
    }
    { // Full, fixed-size, real typed range
      auto range = svtk::DataArrayValueRange<NumComps>(array);
      DispatchRangeTests<ArrayType, NumComps>(range, array, 0, NumValues);
    }
    { // Full, fixed-size, generic-typed range
      auto range = svtk::DataArrayValueRange<NumComps>(da);
      DispatchRangeTests<svtkDataArray, NumComps>(range, array, 0, NumValues);
    }
    { // Partial, dynamic-size, real typed range
      auto range = svtk::DataArrayValueRange(array, pStart, pEnd);
  ...
  void DispatchRangeTests(
    Range range, RangeArrayType* array, svtk::ValueIdType start, svtk::ValueIdType end)
  {
    {
  ...
  template <typename RangeArrayType, svtk::ComponentIdType RangeTupleSize, typename Range>
  void TestRange(Range& range, RangeArrayType* array, svtk::ValueIdType start, svtk::ValueIdType end)
  {
  ...
  void TestTypes(Range& range)
  {
    ...
    CHECK_TYPEDEF(typename Range::ValueType, svtk::GetAPIType<RangeArrayType>);
    CHECK_TYPEDEF(typename Range::ValueType, svtk::GetAPIType<RangeArrayType>);
    CHECK_TYPEDEF(typename Range::size_type, svtk::ValueIdType);
    ...
    CHECK_TYPEDEF(svtk::ValueIdType, decltype(range.GetBeginValueId()));
    CHECK_TYPEDEF(svtk::ValueIdType, decltype(range.GetEndValueId()));
...
struct UnitTestValueIteratorAPI
{
  static constexpr svtk::ComponentIdType NumComps = 3;
  static constexpr svtk::TupleIdType NumTuples = 12;
  static constexpr svtk::ValueIdType NumValues = NumComps * NumTuples;
...
struct UnitTestValueReferenceAPI
{
  ...
  template <typename Range>
  auto GetTestRef(Range& range, svtk::ValueIdType offset) -> decltype(std::declval<Range>()[0])
  {
...
struct UnitTestEdgeCases
{
  ...
  void operator()()
  {
    ...
    DispatchValueCompat<svtkSOADataArrayTemplate<float>, svtkAOSDataArrayTemplate<float> >();

    std::cerr << "AOS<float> <--> SOA<float>\n";
    DispatchValueCompat<svtkAOSDataArrayTemplate<float>, svtkSOADataArrayTemplate<float> >();

    std::cerr << "SOA<double> <--> AOS<float>\n";
    DispatchValueCompat<svtkSOADataArrayTemplate<double>, svtkAOSDataArrayTemplate<float> >();

    std::cerr << "AOS<float> <--> SOA<double>\n";
    DispatchValueCompat<svtkAOSDataArrayTemplate<float>, svtkSOADataArrayTemplate<double> >();

    std::cerr << "SOA<int> <--> AOS<float>\n";
    DispatchValueCompat<svtkSOADataArrayTemplate<int>, svtkAOSDataArrayTemplate<float> >();

    std::cerr << "AOS<float> <--> SOA<int>\n";
    DispatchValueCompat<svtkAOSDataArrayTemplate<float>, svtkSOADataArrayTemplate<int> >();

#ifdef SVTK_USE_SCALED_SOA_ARRAYS
    std::cerr << "ScaleSOA<float> <--> AOS<float>\n";
    DispatchValueCompat<svtkScaledSOADataArrayTemplate<float>, svtkAOSDataArrayTemplate<float> >();

    std::cerr << "AOS<float> <--> ScaleSOA<float>\n";
    DispatchValueCompat<svtkAOSDataArrayTemplate<float>, svtkScaledSOADataArrayTemplate<float> >();

    std::cerr << "ScaleSOA<double> <--> AOS<float>\n";
    DispatchValueCompat<svtkScaledSOADataArrayTemplate<double>, svtkAOSDataArrayTemplate<float> >();

    std::cerr << "AOS<float> <--> ScaleSOA<double>\n";
    DispatchValueCompat<svtkAOSDataArrayTemplate<float>, svtkScaledSOADataArrayTemplate<double> >();

    std::cerr << "ScaleSOA<int> <--> AOS<float>\n";
    DispatchValueCompat<svtkScaledSOADataArrayTemplate<int>, svtkAOSDataArrayTemplate<float> >();

    std::cerr << "AOS<float> <--> ScaleSOA<int>\n";
    DispatchValueCompat<svtkAOSDataArrayTemplate<float>, svtkScaledSOADataArrayTemplate<int> >();
#endif
  }
  ...
  static void TestAOSSpecialization()
  {
    ...
    using RangeType = decltype(svtk::DataArrayValueRange(std::declval<ArrayType*>()));
    using ValueIterType = decltype(std::declval<RangeType>().begin());

    // Sanity Check:
    static_assert(svtk::IsAOSDataArray<ArrayType>::value, "Not AOS type?");
  ...
  void DispatchValueCompat()
  {
    ...
    auto aRange1 = svtk::DataArrayValueRange(a1);
    auto aRange2 = svtk::DataArrayValueRange(a2);
    auto daRange1 = svtk::DataArrayValueRange(da1);
    auto daRange2 = svtk::DataArrayValueRange(da2);

    auto aFixedRange1 = svtk::DataArrayValueRange<NumComps>(a1);
    auto aFixedRange2 = svtk::DataArrayValueRange<NumComps>(a2);
    auto daFixedRange1 = svtk::DataArrayValueRange<NumComps>(da1);
    auto daFixedRange2 = svtk::DataArrayValueRange<NumComps>(da2);
