...
// .NAME
// .SECTION Description
// This program tests the svtkMersenneTwister class.
...
//----------------------------------------------------------------------------
// Construct two instances of svtkMersenneTwister, each with <nThreads>
...
int ThreadCheck(std::size_t nThreads, std::size_t nValues)
{
  ...
  svtkNew<svtkMersenneTwister> seq1, seq2;
  typedef svtkMersenneTwister::SequenceId SequenceId;
  ...
  for (std::size_t i = 0; i < nThreads; ++i)
  {
    for (std::size_t j = 0; j < nValues; ++j)
    {
      if (fabs(values1[i][j] - values2[i][j]) > SVTK_DBL_EPSILON)
      {
...
//----------------------------------------------------------------------------
// Construct an instance of svtkMersenneTwister and initialize two sequences,
...
{
  svtkNew<svtkMersenneTwister> seq;
  svtkMersenneTwister::SequenceId id0 = 0;
  seq->InitializeSequence(id0, 0);
  svtkMersenneTwister::SequenceId id1 = seq->InitializeNewSequence(0);
  svtkNew<svtkMersenneTwister> seq2;
  ...
  for (int i = 0; i < 10; i++)
  {
    if (fabs(seq->GetValue(id0) - expectedValues[i]) > SVTK_DBL_EPSILON)
    {
      std::cerr << "Sequence seeded with seed 0 has changed." << std::endl;
      return EXIT_FAILURE;
    }
    if (fabs(seq->GetValue(id0) - seq->GetValue(id1)) < SVTK_DBL_EPSILON)
    {
      std::cerr << "Sequence 0 seeded with seed 0 has produced the same value as "
                << "sequence 1 seeded with seed 0." << std::endl;
      return EXIT_FAILURE;
    }
    if (fabs(seq->GetValue(id0) - seq2->GetValue(id0)) < SVTK_DBL_EPSILON)
    {
      std::cerr << "Sequence 0 seeded with seed 0 has produced the same value as "
...
int SequenceCheck()
{
  ...
  pool->SetSize(SVTK_SEQUENCE_TEST_SIZE);
  pool->SetNumberOfComponents(1);
  pool->SetChunkSize(SVTK_SEQUENCE_TEST_SIZE / 7);
