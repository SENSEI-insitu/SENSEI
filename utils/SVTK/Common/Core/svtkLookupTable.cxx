...
const svtkIdType svtkLookupTable::REPEATED_LAST_COLOR_INDEX = 0;
const svtkIdType svtkLookupTable::BELOW_RANGE_COLOR_INDEX = 1;
const svtkIdType svtkLookupTable::ABOVE_RANGE_COLOR_INDEX = 2;
const svtkIdType svtkLookupTable::NAN_COLOR_INDEX = 3;
const svtkIdType svtkLookupTable::NUMBER_OF_SPECIAL_COLORS = NAN_COLOR_INDEX + 1;

svtkStandardNewMacro(svtkLookupTable);

// Construct with range=(0,1); and hsv ranges set up for rainbow color table
// (from red to blue).
svtkLookupTable::svtkLookupTable(int sze, int ext)
{
  ...
  this->Ramp = SVTK_RAMP_SCURVE;
  this->Scale = SVTK_SCALE_LINEAR;

  this->OpaqueFlag = 1;
}

//----------------------------------------------------------------------------
svtkLookupTable::~svtkLookupTable()
{
...
int svtkLookupTable::IsOpaque()
{
  if (this->OpaqueFlagBuildTime < this->GetMTime())
  {
    ...
    svtkIdType size = this->Table->GetNumberOfTuples();
    svtkIdType i = 0;
...
int svtkLookupTable::IsOpaque(svtkAbstractArray* scalars, int colorMode, int component)
{
...
//----------------------------------------------------------------------------
void svtkLookupTable::SetTableRange(const double r[2])
{
...
// range value.
void svtkLookupTable::SetTableRange(double rmin, double rmax)
{
  if (this->Scale == SVTK_SCALE_LOG10 && ((rmin > 0 && rmax < 0) || (rmin < 0 && rmax > 0)))
  {
    svtkErrorMacro("Bad table range for log scale: [" << rmin << ", " << rmax << "]");
    return;
  }
  if (rmax < rmin)
  {
    svtkErrorMacro("Bad table range: [" << rmin << ", " << rmax << "]");
    return;
  }
...
void svtkLookupTable::SetScale(int scale)
{
  if (this->Scale == scale)
  {
    return;
...
int svtkLookupTable::Allocate(int sz, int ext)
{
...
void svtkLookupTable::ForceBuild()
{
  svtkIdType maxIndex = this->NumberOfColors - 1;
  ...
  double rgba[4];
  for (svtkIdType i = 0; i <= maxIndex; i++)
  {
    ...
    svtkMath::HSVToRGB(hue, sat, val, &rgba[0], &rgba[1], &rgba[2]);
    rgba[3] = alpha;
    ...
    switch (this->Ramp)
    {
      case SVTK_RAMP_SCURVE:
      {
        c_rgba[0] = static_cast<unsigned char>(
          127.5 * (1.0 + cos((1.0 - static_cast<double>(rgba[0])) * svtkMath::Pi())));
        c_rgba[1] = static_cast<unsigned char>(
          127.5 * (1.0 + cos((1.0 - static_cast<double>(rgba[1])) * svtkMath::Pi())));
        c_rgba[2] = static_cast<unsigned char>(
          127.5 * (1.0 + cos((1.0 - static_cast<double>(rgba[2])) * svtkMath::Pi())));
        c_rgba[3] = static_cast<unsigned char>(alpha * 255.0);
        /* same code, but with rounding for correctness
        c_rgba[0] = static_cast<unsigned char>
          (127.5*(1.0 + cos((1.0 - rgba[0])*svtkMath::Pi())) + 0.5);
        c_rgba[1] = static_cast<unsigned char>
          (127.5*(1.0 + cos((1.0 - rgba[1])*svtkMath::Pi())) + 0.5);
        c_rgba[2] = static_cast<unsigned char>
          (127.5*(1.0 + cos((1.0 - rgba[2])*svtkMath::Pi())) + 0.5);
        c_rgba[3] = static_cast<unsigned char>(alpha*255.0 + 0.5);
        */
      }
      break;
      case SVTK_RAMP_LINEAR:
      {
...
//----------------------------------------------------------------------------
void svtkLookupTable::BuildSpecialColors()
{
  // Append 4 "special" colors (repeated last, below range, above range, NaN) to table here.
  svtkIdType numberOfColors = this->GetTable()->GetNumberOfTuples();
  ...
  // Floating point math in svtkLinearIndexLookupMain may result in an off-by-one,
  // and having an extra copy of the last color lets us avoid a test in that very hot function.
  unsigned char* tptr = table + 4 * (numberOfColors + svtkLookupTable::REPEATED_LAST_COLOR_INDEX);
  if (numberOfColors > 0)
  {
  ...
  {
    svtkLookupTable::GetColorAsUnsignedChars(this->GetAboveRangeColor(), color);
    tptr[0] = color[0];
    tptr[1] = color[1];
    tptr[2] = color[2];
    tptr[3] = color[3];
  ...
  // Below range color
  tptr = table + 4 * (numberOfColors + svtkLookupTable::BELOW_RANGE_COLOR_INDEX);
  if (this->GetUseBelowRangeColor() || numberOfColors == 0)
  {
    svtkLookupTable::GetColorAsUnsignedChars(this->GetBelowRangeColor(), color);
    tptr[0] = color[0];
    tptr[1] = color[1];
    tptr[2] = color[2];
    tptr[3] = color[3];
  ...
  // Above range color
  tptr = table + 4 * (numberOfColors + svtkLookupTable::ABOVE_RANGE_COLOR_INDEX);
  ...
  // Always use NanColor
  svtkLookupTable::GetColorAsUnsignedChars(this->GetNanColor(), color);
  tptr = table + 4 * (numberOfColors + svtkLookupTable::NAN_COLOR_INDEX);
  tptr[0] = color[0];
  tptr[1] = color[1];
  tptr[2] = color[2];
  tptr[3] = color[3];
...
void svtkLookupTable::GetColor(double v, double rgb[3])
{
...
double svtkLookupTable::GetOpacity(double v)
{
  const unsigned char* rgb8 = this->MapValue(v);
...
inline void svtkLookupTableLogRange(const double range[2], double logRange[2])
{
  double rmin = range[0];
  double rmax = range[1];
  ...
  if ((rmin <= 0 && rmax >= 0) || (rmin >= 0 && rmax <= 0))
  {
    ...
    if (rmax == 0)
    {
      rmax = (rmin < 0 ? -SVTK_DBL_MIN : SVTK_DBL_MIN);
    }
    if (rmin == 0)
    {
      rmin = (rmax < 0 ? -SVTK_DBL_MIN : SVTK_DBL_MIN);
    }
  }
...
static double svtkApplyLogScaleMain(double v, const double range[2], const double logRange[2])
{
...
//----------------------------------------------------------------------------
template <class T>
double svtkApplyLogScale(T v, const double range[2], const double logRange[2])
{
  return svtkApplyLogScaleMain(v, range, logRange);
}

//----------------------------------------------------------------------------
// Apply log to a float value (NaN values pass through)
inline double svtkApplyLogScale(float v, const double range[2], const double logRange[2])
{
  if (svtkMath::IsNan(v))
  {
    return v;
...
inline double svtkApplyLogScale(double v, const double range[2], const double logRange[2])
{
  if (svtkMath::IsNan(v))
  {
    return v;
...
inline svtkIdType svtkLinearIndexLookupMain(double v, const TableParameters& p)
{
  svtkIdType index;
  ...
  if (v < p.Range[0])
  {
    index = p.NumColors + svtkLookupTable::BELOW_RANGE_COLOR_INDEX;
  }
  else if (v > p.Range[1])
  {
    index = p.NumColors + svtkLookupTable::ABOVE_RANGE_COLOR_INDEX;
  }
  else
  {
    ...
    index = static_cast<svtkIdType>(dIndex);
...
// integer variant
template <class T>
inline svtkIdType svtkLinearLookup(T v, const TableParameters& p)
{
  // First convert from integer to double.
  double dv = static_cast<double>(v);
  return svtkLinearIndexLookupMain(dv, p);
}

//----------------------------------------------------------------------------
// double variant
inline svtkIdType svtkLinearLookup(double v, const TableParameters& p)
{
  // If Nan, use the special NaN color.
  if (svtkMath::IsNan(v))
  {
    svtkIdType maxIndex = p.NumColors + svtkLookupTable::NAN_COLOR_INDEX;
    return maxIndex;
  }

  return svtkLinearIndexLookupMain(v, p);
}

//----------------------------------------------------------------------------
// float variant
inline svtkIdType svtkLinearLookup(float v, const TableParameters& p)
{
  // Convert from float to double, then call the double variant.
  double dv = static_cast<double>(v);
  return svtkLinearLookup(dv, p);
}

//----------------------------------------------------------------------------
inline void svtkLookupShiftAndScale(
  const double range[2], double numColors, double& shift, double& scale)
{
  shift = -range[0];
  double rangeDelta = range[1] - range[0];
  if (rangeDelta < SVTK_DBL_MIN * numColors)
  {
...
//----------------------------------------------------------------------------
void svtkLookupTable::GetLogRange(const double range[2], double log_range[2])
{
  svtkLookupTableLogRange(range, log_range);
}

//----------------------------------------------------------------------------
double svtkLookupTable::ApplyLogScale(double v, const double range[2], const double log_range[2])
{
  return svtkApplyLogScale(v, range, log_range);
}

//----------------------------------------------------------------------------
// Given a scalar value v, return an index into the lookup table
svtkIdType svtkLookupTable::GetIndex(double v)
{
  ...
  if (this->Scale == SVTK_SCALE_LOG10)
  { // handle logarithmic scale
    double logRange[2];
    svtkLookupTableLogRange(this->TableRange, logRange);
    svtkLookupShiftAndScale(logRange, p.NumColors, p.Shift, p.Scale);
    v = svtkApplyLogScale(v, this->TableRange, logRange);
    p.Range[0] = logRange[0];
    p.Range[1] = logRange[1];
  }
  else
  { // plain old linear
    svtkLookupShiftAndScale(this->TableRange, p.NumColors, p.Shift, p.Scale);
    p.Range[0] = this->TableRange[0];
    p.Range[1] = this->TableRange[1];
  }

  svtkIdType index = svtkLinearIndexLookupMain(v, p);
...
void svtkLookupTable::SetTable(svtkUnsignedCharArray* table)
{
  if (table != this->Table && table != nullptr)
  {
    ...
    {
      svtkErrorMacro(<< "Number of components in given table (" << table->GetNumberOfComponents()
...
//----------------------------------------------------------------------------
void svtkLookupTable::GetColorAsUnsignedChars(const double colorIn[4], unsigned char colorOut[4])
{
...
//----------------------------------------------------------------------------
unsigned char* svtkLookupTable::GetNanColorAsUnsignedChars()
{
...
const unsigned char* svtkLookupTable::MapValue(double v)
{
  svtkIdType index = this->GetIndex(v);
  if (index < 0)
  {
...
//----------------------------------------------------------------------------
template <class T>
void svtkLookupTableMapData(svtkLookupTable* self, T* input, unsigned char* output, int length,
  int inIncr, int outFormat, TableParameters& p)
{
  ...
  svtkUnsignedCharArray* lookupTable = self->GetTable();
  ...
  if (alpha >= 1.0) // no blending required
  {
    if (self->GetScale() == SVTK_SCALE_LOG10)
    {
      double val;
      double logRange[2];
      svtkLookupTableLogRange(range, logRange);
      svtkLookupShiftAndScale(logRange, p.NumColors, p.Shift, p.Scale);
      p.Range[0] = logRange[0];
      p.Range[1] = logRange[1];
      ...
      if (outFormat == SVTK_RGBA)
      {
        while (--i >= 0)
        {
          ...
          svtkIdType idx = svtkLinearLookup(val, p);
          cptr = table + 4 * idx;
    ...
    {
      svtkLookupShiftAndScale(range, p.NumColors, p.Shift, p.Scale);
      p.Range[0] = range[0];
      p.Range[1] = range[1];
      if (outFormat == SVTK_RGBA)
      {
        while (--i >= 0)
        {
          svtkIdType idx = svtkLinearLookup(*input, p);
          cptr = table + 4 * idx;
          memcpy(output, cptr, 4);
          input += inIncr;
          output += 4;
...
//----------------------------------------------------------------------------
template <class T>
void svtkLookupTableIndexedMapData(svtkLookupTable* self, const T* input, unsigned char* output,
  int length, int inIncr, int outFormat)
{
  ...
  svtkLookupTable::GetColorAsUnsignedChars(self->GetNanColor(), nanColor);
...
//----------------------------------------------------------------------------
void svtkLookupTable::MapScalarsThroughTable2(void* input, unsigned char* output, int inputDataType,
  ...
  if (this->IndexedLookup)
  {
    switch (inputDataType)
    {
      ...
      break;
        ...
        svtkTemplateMacro(svtkLookupTableIndexedMapData(
          this, static_cast<SVTK_TT*>(input), output, numberOfValues, inputIncrement, outputFormat));

      case SVTK_STRING:
        svtkLookupTableIndexedMapData(this, static_cast<svtkStdString*>(input), output,
          numberOfValues, inputIncrement, outputFormat);
        break;

      default:
        svtkErrorMacro(<< "MapScalarsThroughTable2: Unknown input ScalarType");
        return;
    }
  }
  else
  {
    ...
    switch (inputDataType)
    {
      ...
      break;
        ...
        svtkTemplateMacro(svtkLookupTableMapData(this, static_cast<SVTK_TT*>(input), output,
...
void svtkLookupTable::SetNumberOfTableValues(svtkIdType number)
{
...
void svtkLookupTable::SetTableValue(svtkIdType indx, const double rgba[4])
{
  // Check the index to make sure it is valid
  if (indx < 0)
  {
    svtkErrorMacro("Can't set the table value for negative index " << indx);
    return;
  }
...
void svtkLookupTable::SetTableValue(svtkIdType indx, double r, double g, double b, double a)
{
...
void svtkLookupTable::GetTableValue(svtkIdType indx, double rgba[4])
{
...
// Return an RGBA color value for the given index into the lookup table. Color
// components are expressed as [0,1] double values.
double* svtkLookupTable::GetTableValue(svtkIdType indx)
{
...
//----------------------------------------------------------------------------
void svtkLookupTable::PrintSelf(ostream& os, svtkIndent indent)
{
  this->Superclass::PrintSelf(os, indent);

  os << indent << "TableRange: (" << this->TableRange[0] << ", " << this->TableRange[1] << ")\n";
  os << indent << "Scale: " << (this->Scale == SVTK_SCALE_LOG10 ? "Log10\n" : "Linear\n");
  ...
  os << indent << "Ramp: " << (this->Ramp == SVTK_RAMP_SCURVE ? "SCurve\n" : "Linear\n");
...
//----------------------------------------------------------------------------
void svtkLookupTable::DeepCopy(svtkScalarsToColors* obj)
{
  if (!obj)
  {
    return;
  ...
  svtkLookupTable* lut = svtkLookupTable::SafeDownCast(obj);

  if (!lut)
  {
    svtkErrorMacro("Cannot DeepCopy a " << obj->GetClassName() << " into a svtkLookupTable.");
    return;
  }
...
//----------------------------------------------------------------------------
svtkIdType svtkLookupTable::GetNumberOfAvailableColors()
{
  return this->Table->GetNumberOfTuples();
}

//----------------------------------------------------------------------------
void svtkLookupTable::GetIndexedColor(svtkIdType idx, double rgba[4])
{
  svtkIdType n = this->GetNumberOfAvailableColors();
  if (n > 0 && idx >= 0)
  {
...
//----------------------------------------------------------------------------
void svtkLookupTable::ResizeTableForSpecialColors()
{
  svtkIdType neededColors = this->NumberOfColors + svtkLookupTable::NUMBER_OF_SPECIAL_COLORS;
