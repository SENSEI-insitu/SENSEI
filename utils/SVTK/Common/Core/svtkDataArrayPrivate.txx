/*=========================================================================
  ...
  Module:    svtkDataArrayPrivate.txx
...
=========================================================================*/
#ifndef svtkDataArrayPrivate_txx
#define svtkDataArrayPrivate_txx
...
protected:
  ...
  svtkSMPThreadLocal<std::array<APIType, 2 * NumComps> > TLRange;
...
public:
  void Initialize()
  {
    ...
    {
      range[j] = svtkTypeTraits<APIType>::Max();
      range[j + 1] = svtkTypeTraits<APIType>::Min();
      this->ReducedRange[j] = svtkTypeTraits<APIType>::Max();
      this->ReducedRange[j + 1] = svtkTypeTraits<APIType>::Min();
    }
  }
  void Reduce()
  {
...
template <int NumComps, typename ArrayT, typename APIType = typename svtk::GetAPIType<ArrayT> >
...
public:
  ...
  // Help svtkSMPTools find Initialize() and Reduce()
  void Initialize() { MinAndMaxT::Initialize(); }
  void Reduce() { MinAndMaxT::Reduce(); }
  void operator()(svtkIdType begin, svtkIdType end)
  {
    const auto tuples = svtk::DataArrayTupleRange<NumComps>(this->Array, begin, end);
...
template <typename ArrayT, typename APIType = typename svtk::GetAPIType<ArrayT> >
...
public:
  ...
  void operator()(svtkIdType begin, svtkIdType end)
  {
    const auto tuples = svtk::DataArrayTupleRange(this->Array, begin, end);
    auto& range = MinAndMaxT::TLRange.Local();
    for (const auto tuple : tuples)
    {
...
struct ComputeScalarRange
{
  ...
  bool operator()(ArrayT* array, RangeValueType* ranges, AllValues)
  {
    ...
    svtkSMPTools::For(0, array->GetNumberOfTuples(), minmax);
...
protected:
  ArrayT* Array;
  svtkIdType NumComps;
  svtkSMPThreadLocal<std::vector<APIType> > TLRange;
...
bool DoComputeScalarRange(ArrayT* array, RangeValueType* ranges, ValueType tag)
{
  ...
  {
    ranges[j] = svtkTypeTraits<RangeValueType>::Max();
    ranges[j + 1] = svtkTypeTraits<RangeValueType>::Min();
  }
...
{
  range[0] = svtkTypeTraits<RangeValueType>::Max();
  range[1] = svtkTypeTraits<RangeValueType>::Min();

  // do this after we make sure range is max to min
  const svtkIdType numTuples = array->GetNumberOfTuples();
  if (numTuples == 0)
  {
    return false;
  }
  ...
  svtkSMPTools::For(0, numTuples, MinAndMax);
...
} // end namespace svtkDataArrayPrivate
#endif
// SVTK-HeaderTest-Exclude: svtkDataArrayPrivate.txx
