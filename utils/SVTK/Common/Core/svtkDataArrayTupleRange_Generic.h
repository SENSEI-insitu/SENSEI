/*=========================================================================
  ...
  Module:    svtkDataArrayTupleRange_Generic.h
...
#ifndef svtkDataArrayTupleRange_Generic_h
#define svtkDataArrayTupleRange_Generic_h
...
public:
    ...
    , ComponentId{ comp }
  {
    ...
    SVTK_ITER_ASSERT(numComps.value > 0, "Invalid number of components.");
    SVTK_ITER_ASSERT(
      tuple >= 0 && tuple <= array->GetNumberOfTuples(), "Invalid tuple accessed by iterator.");
    SVTK_ITER_ASSERT(comp >= 0 && comp <= array->GetNumberOfComponents(),
  ...
  operator APIType() const noexcept
  {
    ...
    SVTK_ITER_ASSUME(this->Array->GetNumberOfComponents() == this->NumComps.value);
...
public:
  ...
  SVTK_ITER_INLINE ComponentReference operator=(const ComponentReference<OArray, OSize>& o) noexcept
  ...
  friend SVTK_ITER_INLINE void swap(ComponentReference lhs, ComponentReference rhs) noexcept
  ...
  template <typename OArray, ComponentIdType OSize>
  friend SVTK_ITER_INLINE void swap(
  ...
  friend SVTK_ITER_INLINE void swap(ComponentReference lhs, APIType& rhs) noexcept
  {
    APIType tmp = std::move(static_cast<APIType>(lhs));
    lhs = std::move(rhs);
    rhs = std::move(tmp);
  ...
  friend SVTK_ITER_INLINE void swap(APIType& lhs, ComponentReference rhs) noexcept
  {
...
#define SVTK_REF_OP_OVERLOADS(Op, ImplOp)                                                           \
  friend SVTK_ITER_INLINE ComponentReference operator Op(ComponentReference lhs, APIType val)       \
    noexcept                                                                                       \
  {                                                                                                \
  ...
  friend SVTK_ITER_INLINE ComponentReference operator Op(                                           \
  ...
  friend SVTK_ITER_INLINE APIType& operator Op(APIType& lhs, ComponentReference val) noexcept       \
  {                                                                                                \
    const APIType newVal = lhs ImplOp val;                                                         \
    lhs = newVal;                                                                                  \
    return lhs;                                                                                    \
...
public:
    ...
    , ComponentId(comp)
  {
    SVTK_ITER_ASSERT(array != nullptr, "Invalid array.");
    SVTK_ITER_ASSERT(numComps.value > 0, "Invalid number of components.");
    SVTK_ITER_ASSERT(tupleId >= 0 && tupleId <= array->GetNumberOfTuples(),
      "Const component iterator at invalid tuple id.");
    SVTK_ITER_ASSERT(comp >= 0 && comp <= this->NumComps.value,
  ...
  {
    ++this->ComponentId;
    SVTK_ITER_ASSERT(this->ComponentId >= 0 && this->ComponentId <= this->NumComps.value,
      "Const component iterator at invalid component id.");
    return *this;
  }
...
#define SVTK_TMP_MAKE_OPERATOR(OP)                                                                  \
  ...
  {                                                                                                \
    ...
    SVTK_ITER_ASSERT(lhs.TupleId == rhs.TupleId, "Mismatched tuple ids in iterator comparison.");   \
    SVTK_ITER_ASSUME(lhs.NumComps.value > 0);                                                       \
    SVTK_ITER_ASSUME(lhs.NumComps.value == rhs.NumComps.value);                                     \
...
#undef SVTK_TMP_MAKE_OPERATOR
  ...
  friend SVTK_ITER_INLINE ConstComponentIterator operator+(
    ...
    const ConstComponentIterator& it1, const ConstComponentIterator& it2) noexcept
  {
    ...
    SVTK_ITER_ASSERT(it1.TupleId == it2.TupleId,
...
public:
  ...
  ComponentIterator& operator++() noexcept // prefix
  {
    ...
    SVTK_ITER_ASSERT(this->Ref.ComponentId >= 0 && this->Ref.ComponentId <= this->Ref.NumComps.value,
...
#define SVTK_TMP_MAKE_OPERATOR(OP)                                                                  \
  ...
  {                                                                                                \
    ...
    SVTK_ITER_ASSUME(lhs.GetNumComps().value > 0);                                                  \
    SVTK_ITER_ASSUME(lhs.GetNumComps().value == rhs.GetNumComps().value);                           \
...
#undef SVTK_TMP_MAKE_OPERATOR
  ...
  friend SVTK_ITER_INLINE ComponentIterator operator+(
  ...
  {
    SVTK_ITER_ASSERT(it1.GetArray() == it2.GetArray(),
      "Cannot do math with component iterators from different "
      "arrays.");
    SVTK_ITER_ASSERT(it1.GetTupleId() == it2.GetTupleId(),
      "Cannot do math with component iterators from different "
      "tuples.");
    return it1.GetComponentId() - it2.GetComponentId();
  }

  friend SVTK_ITER_INLINE void swap(ComponentIterator& lhs, ComponentIterator& rhs) noexcept
  {
    // Different arrays may use different iterator implementations.
    SVTK_ITER_ASSERT(
...
public:
  ...
  SVTK_ITER_INLINE bool operator!=(const ConstTupleReference<OArrayT, OSize>& o) const noexcept
  {
    return !(*this == o);
  }
...
protected:
  ...
  void CopyReference(const ConstTupleReference& o) noexcept
  {
    ...
    SVTK_ITER_ASSERT(this->Array == o.Array, "Cannot copy reference objects between arrays.");
    this->NumComps = o.NumComps;
    this->TupleId = o.TupleId;
  }
...
template <typename ArrayType, ComponentIdType TupleSize>
TupleRange<ArrayType, TupleSize> DeclareTupleRangeSpecialization(svtkDataArray*);
...
#endif // __SVTK_WRAP__
#endif // svtkDataArrayTupleRange_Generic_h

// SVTK-HeaderTest-Exclude: svtkDataArrayTupleRange_Generic.h
