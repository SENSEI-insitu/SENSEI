...
#ifndef svtkVariant_h
#define svtkVariant_h

#include "svtkCommonCoreModule.h" // For export macro
#include "svtkObject.h"           // For svtkObject's warning support
#include "svtkSetGet.h"           // For svtkNotUsed macro
#include "svtkStdString.h"
#include "svtkSystemIncludes.h" // To define ostream
#include "svtkType.h"           // To define type IDs and SVTK_TYPE_USE_* flags
#include "svtkUnicodeString.h"

//
// The following should be eventually placed in svtkSetGet.h
//

// This is same as extended template macro with an additional case for SVTK_VARIANT
#define svtkExtraExtendedTemplateMacro(call)                                                        \
  svtkExtendedTemplateMacro(call);                                                                  \
  svtkTemplateMacroCase(SVTK_VARIANT, svtkVariant, call)

// This is same as Iterator Template macro with an additional case for SVTK_VARIANT
#define svtkExtendedArrayIteratorTemplateMacro(call)                                                \
  svtkArrayIteratorTemplateMacro(call);                                                             \
  svtkArrayIteratorTemplateMacroCase(SVTK_VARIANT, svtkVariant, call)
...
class svtkVariant;
struct svtkVariantLessThan;

SVTKCOMMONCORE_EXPORT ostream& operator<<(ostream& os, const svtkVariant& val);

class SVTKCOMMONCORE_EXPORT svtkVariant
{
public:
  /**
   ...
   * Copy constructor.
   */
  svtkVariant(const svtkVariant& other);

  /**
   * Create a bool variant. Internally store it as char.
   */
  svtkVariant(bool value);

  /**
   * Create a char variant.
   */
  svtkVariant(char value);

  /**
   * Create an unsigned char variant.
   */
  svtkVariant(unsigned char value);

  /**
   * Create a signed char variant.
   */
  svtkVariant(signed char value);

  /**
   * Create a short variant.
   */
  svtkVariant(short value);

  /**
   * Create an unsigned short variant.
   */
  svtkVariant(unsigned short value);

  /**
   * Create an integer variant.
   */
  svtkVariant(int value);

  /**
   * Create an unsigned integer variant.
   */
  svtkVariant(unsigned int value);

  /**
   * Create an long variant.
   */
  svtkVariant(long value);

  /**
   * Create an unsigned long variant.
   */
  svtkVariant(unsigned long value);

  /**
   * Create a long long variant.
   */
  svtkVariant(long long value);

  /**
   * Create an unsigned long long variant.
   */
  svtkVariant(unsigned long long value);

  /**
   * Create a float variant.
   */
  svtkVariant(float value);

  /**
   * Create a double variant.
   */
  svtkVariant(double value);

  /**
   * Create a string variant from a const char*.
   */
  svtkVariant(const char* value);

  /**
   * Create a string variant from a std string.
   */
  svtkVariant(svtkStdString value);

  /**
   * Create a Unicode string variant
   */
  svtkVariant(const svtkUnicodeString& value);

  /**
   * Create a svtkObjectBase variant.
   */
  svtkVariant(svtkObjectBase* value);

  /**
   * Create a variant of a specific type.
   */
  svtkVariant(const svtkVariant& other, unsigned int type);

  /**
   * Copy the value of one variant into another.
   */
  svtkVariant& operator=(const svtkVariant& other);
  ...
  /**
   * Get whether the variant is a SVTK object pointer.
   */
  bool IsSVTKObject() const;

  /**
   * Get whether the variant is a SVTK array (i.e. a subclass of svtkAbstractArray).
   */
  bool IsArray() const;
   ...
   */
  svtkUnicodeString ToUnicodeString() const;
   ...
   * Fail if it holds a SVTK object which is not an array.
   */
  ...
  svtkTypeInt64 ToTypeInt64(bool* valid) const;
  svtkTypeInt64 ToTypeInt64() const { return this->ToTypeInt64(nullptr); }
  svtkTypeUInt64 ToTypeUInt64(bool* valid) const;
  svtkTypeUInt64 ToTypeUInt64() const { return this->ToTypeUInt64(nullptr); }
  //@}

  /**
   * Return the SVTK object, or nullptr if not of that type.
   */
  svtkObjectBase* ToSVTKObject() const;

  /**
   * Return the array, or nullptr if not of that type.
   */
  svtkAbstractArray* ToArray() const;
   ...
   * svtkVariantStrictEquality comparator;
   * bool variantsEqual = comparator(firstVariant, secondVariant);
   */
  bool IsEqual(const svtkVariant& other) const;
   ...
   * * svtkObject pointers will be converted to an unsigned integer of
   * appropriate size.  If both variants contain svtkObjects then
   ...
   * -- for example, when using svtkVariants as keys in STL containers
   * -- you can use the functors described at the bottom of this file.

   * The actual definitions of these operators are in
   * svtkVariantInlineOperators.cxx.
   */
  bool operator==(const svtkVariant& other) const;
  bool operator!=(const svtkVariant& other) const;
  bool operator<(const svtkVariant& other) const;
  bool operator>(const svtkVariant& other) const;
  bool operator<=(const svtkVariant& other) const;
  ...
  friend SVTKCOMMONCORE_EXPORT ostream& operator<<(ostream& os, const svtkVariant& val);

private:
  template <typename T>
  T ToNumeric(bool* valid, T* svtkNotUsed(ignored)) const;

  union {
    svtkStdString* String;
    svtkUnicodeString* UnicodeString;
    float Float;
    double Double;
    char Char;
    ...
    svtkObjectBase* SVTKObject;
  } Data;

  unsigned char Valid;
  unsigned char Type;

  friend struct svtkVariantLessThan;
  friend struct svtkVariantEqual;
  friend struct svtkVariantStrictWeakOrder;
  friend struct svtkVariantStrictEquality;
};

#include "svtkVariantInlineOperators.h" // needed for operator== and company

// A STL-style function object so you can compare two variants using
// comp(s1,s2) where comp is an instance of svtkVariantStrictWeakOrder.
...
struct SVTKCOMMONCORE_EXPORT svtkVariantLessThan
{
public:
  bool operator()(const svtkVariant& s1, const svtkVariant& s2) const;
};

struct SVTKCOMMONCORE_EXPORT svtkVariantEqual
{
public:
  bool operator()(const svtkVariant& s1, const svtkVariant& s2) const;
};

struct SVTKCOMMONCORE_EXPORT svtkVariantStrictWeakOrder
{
public:
  bool operator()(const svtkVariant& s1, const svtkVariant& s2) const;
};
...
struct SVTKCOMMONCORE_EXPORT svtkVariantStrictEquality
{
public:
  bool operator()(const svtkVariant& s1, const svtkVariant& s2) const;
};
...
#endif
// SVTK-HeaderTest-Exclude: svtkVariant.h
