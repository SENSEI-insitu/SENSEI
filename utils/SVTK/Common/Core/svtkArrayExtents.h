 ...
 * coordinate ranges along each dimension for svtkArray.
 *
 *
 * svtkArrayExtents describes the number of dimensions and coordinate
 * ranges along each dimension of an N-way collection of values.  It is
 * used to retrieve and update the extents of a svtkArray object.
 *
 * Conceptually, svtkArrayExtents is a collection of svtkArrayRange
 ...
 *
 * @sa
 * svtkArray, svtkArrayRange, svtkArrayCoordinates
 *
 * @par Thanks:
...
#ifndef svtkArrayExtents_h
#define svtkArrayExtents_h
...
public:
   ...
   * svtkArrayExtents(svtkArrayRange(0, i)).
   */
  explicit svtkArrayExtents(const CoordinateT i);
  /**
   * Create one-dimensional extents.
   */
  explicit svtkArrayExtents(const svtkArrayRange& i);

  /**
   * Create zero-based two-dimensional extents.  This constructor is shorthand for
   * svtkArrayExtents(svtkArrayRange(0, i), svtkArrayRange(0, j)).
   */
  svtkArrayExtents(const CoordinateT i, const CoordinateT j);
  /**
   * Create two-dimensional extents.
   */
  svtkArrayExtents(const svtkArrayRange& i, const svtkArrayRange& j);
   ...
   * svtkArrayRange(0, k)).
   */
  svtkArrayExtents(const CoordinateT i, const CoordinateT j, const CoordinateT k);
  /**
   * Create three-dimensional extents.
   */
  svtkArrayExtents(const svtkArrayRange& i, const svtkArrayRange& j, const svtkArrayRange& k);

  /**
   * Create n-dimensional extents with extent [0, m) along each dimension.
   * This is useful for creating e.g: a square matrix.
   */
  static svtkArrayExtents Uniform(DimensionT n, CoordinateT m);

  /**
   * Grow the number of dimensions by one, specifying the extent
   * of the new dimension.
   */
  void Append(const svtkArrayRange& extent);
   ...
   */
  svtkArrayRange& operator[](DimensionT i);

  /**
   * Accesses the extent of the i-th dimension.
   */
  const svtkArrayRange& operator[](DimensionT i) const;

  /**
   * Get the extent of the i-th dimension.
   */
  svtkArrayRange GetExtent(DimensionT i) const;

  /**
   * Set the extent of the i-th dimension.
   */
  void SetExtent(DimensionT i, const svtkArrayRange&);

  /**
   * Equality comparison
   */
  bool operator==(const svtkArrayExtents& rhs) const;
   ...
   * sizes match.
   */
  bool SameShape(const svtkArrayExtents& rhs) const;
   ...
   */
  void GetLeftToRightCoordinatesN(SizeT n, svtkArrayCoordinates& coordinates) const;
   ...
   */
  void GetRightToLeftCoordinatesN(SizeT n, svtkArrayCoordinates& coordinates) const;
   ...
   * dimensions.
   */
  bool Contains(const svtkArrayExtents& extents) const;
   ...
   */
  bool Contains(const svtkArrayCoordinates& coordinates) const;

  SVTKCOMMONCORE_EXPORT friend ostream& operator<<(ostream& stream, const svtkArrayExtents& rhs);

private:
  std::vector<svtkArrayRange> Storage;
};

#endif
// SVTK-HeaderTest-Exclude: svtkArrayExtents.h
