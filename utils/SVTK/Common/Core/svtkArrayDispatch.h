...
=========================================================================*/
/**
 * @class   svtkArrayDispatch
 * @brief   svtkDataArray code generator/dispatcher.
 *
 * svtkArrayDispatch implements a mechanism for generating optimized code for
 * multiple subclasses of svtkDataArray at once. Using a TypeList based
 * approach (see svtkTypeList), a templated worker implementation is generated
 * for a restricted or unrestricted set of svtkDataArray subclasses.
 *
 ...
 * the encapsulation-breaking svtkDataArray::GetVoidPointer method. Not all
 * subclasses of svtkDataArray use the memory layout assumed by GetVoidPointer;
 * calling this method on, e.g. a svtkSOADataArrayTemplate will trigger a deep
 * copy of the array data into an AOS buffer. This is very inefficient and
 * should be avoided.
 *
 * The svtkDataArrayRange.h utilities are worth mentioning here, as they allow
 * svtkArrayDispatch workers to operate on selected concrete subclasses for
 * 'fast paths', yet fallback to using the slower svtkDataArray API for uncommon
 ...
 *
 * These dispatchers extend the basic functionality of svtkTemplateMacro with
 ...
 *   can be restricted. For example, if only svtkDoubleArray or svtkFloatArray
 ...
 * - Application-wide array restrictions: If a SVTK application uses only a few
 ...
 *   Example usecase: A filter that creates either svtkFloatArray or
 *   svtkDoubleArray depending on configuration can use this to restrict itself
 *   to only these specific types.
 *   Note that these should not be used for operating on filter inputs, instead
 *   use DispatchByValueType, which also considers variations in svtkDataArray
 ...
 *   svtkDataArray subclasses and respects the application-wide array
 *   restrictions.
 *
 ...
 *
 * @code
 * using MyDispatcher = svtkArrayDispatch::SomeDispatcher<...>;
 * MyWorker worker;
 ...
 *   {
 *     // Do work using svtkGenericDataArray API...
 *   }
 * };
 * @endcode
 *
 ...
 *   {
 *     using SrcType = svtk::GetAPIType<ArraySrc>;
 *     using DstType = svtk::GetAPIType<ArrayDst>;
 *
 *     const auto srcRange = svtk::DataArrayValueRange(srcArray);
 *     auto dstRange = svtk::DataArrayValueRange(dstArray);
 *
 ...
 * if (!svtkArrayDispatch::Dispatch2::Execute(src, dst, ScaleArray, 3))
 * {
 ...
 *
 * @sa
 * svtkDataArrayAccessor
 */

#ifndef svtkArrayDispatch_h
#define svtkArrayDispatch_h

#include "svtkArrayDispatchArrayList.h"
 ...
 */
typedef svtkTypeList::Create<double, float> Reals;

/**
 * A Typelist containing all integral ValueTypes.
 */
typedef svtkTypeList::Unique<
  svtkTypeList::Create<char, int, long, long long, short, signed char, unsigned char, unsigned int,
    unsigned long, unsigned long long, unsigned short, svtkIdType> >::Result Integrals;

/**
 * A Typelist containing all standard SVTK array ValueTypes.
 */
typedef svtkTypeList::Append<Reals, Integrals>::Result AllTypes;

//------------------------------------------------------------------------------
/**
 * Dispatch a single array against all array types in the application-wide
 * svtkArrayDispatch::Arrays list.
 * The entry point is:
 * bool Dispatch::Execute(svtkDataArray *array, Worker &worker).
 */
struct Dispatch;
 ...
 * template parameter.
 * The entry point is:
 * bool DispatchByArray<...>::Execute(svtkDataArray *array, Worker &worker).
 */
...
/**
 * Dispatch a single array against all array types in the application-wide
 * svtkArrayDispatch::Arrays list with the added restriction that the array
 * must have a type that appears the ValueTypeList TypeList.
 * The entry point is:
 * bool DispatchByValueType<...>::Execute(svtkDataArray *array, Worker &worker).
 */
 ...
 * svtkArrayDispatch::Arrays list.
 * The entry point is:
 * bool Dispatch2::Execute(svtkDataArray *array, svtkDataArray *array2,
 * Worker &worker).
 */
struct Dispatch2;
 ...
 * All application-wide arrays in svtkArrayDispatch::Arrays are used.
 * The entry point is:
 * bool Dispatch2SameValueType::Execute(
 * svtkDataArray *a1, svtkDataArray *a2, Worker &worker).
 */
 ...
 * If all application-wide arrays are desired, use svtkArrayDispatch::Arrays for
 * the first two template parameters.
 * The entry point is:
 * bool Dispatch2ByArray<...>::Execute(svtkDataArray *a1, svtkDataArray *a2,
 * Worker &worker).
 */
 ...
 * ValueType-filtered versions of the application-wide svtkArrayDispatch::Arrays
 * TypeList. The first array's ValueType must be in the ValueTypeList1 TypeList,
 * and the second's must be in ValueTypeList2.
 * If all types are to be considered, use svtkArrayDispatch::AllTypes for the
 * first two template parameters.
 * The entry point is:
 * bool Dispatch2ByValueType<...>::Execute(svtkDataArray *a1, svtkDataArray *a2,
 * Worker &worker).
 */
...
/**
 * Dispatch two arrays, restricting the valid code paths to use only array types
 * found in application-wide svtkArrayDispatch::Arrays TypeList that have a
 ...
 * svtkArrayDispatch::Arrays list.
 * The entry point is:
 * bool Dispatch3::Execute(svtkDataArray *array1, svtkDataArray *array2,
 * svtkDataArray *array3, Worker &worker).
 */
struct Dispatch3;
 ...
 * svtkDataArray *a1, svtkDataArray *a2, svtkDataArray *a3, Worker &worker).
 */
 ...
 * The entry point is:
 * bool Dispatch3ByArray::Execute<...>(svtkDataArray *a1, svtkDataArray *a2,
 * svtkDataArray *a3, Worker &worker).
 */
 ...
 * The entry point is:
 * bool Dispatch3ByValueType<...>::Execute(svtkDataArray *a1, svtkDataArray *a2,
 * svtkDataArray *a3, Worker &worker).
 */
...
/**
 * Dispatch three arrays, restricting the valid code paths to use only array
 * types found in application-wide svtkArrayDispatch::Arrays TypeList that have a
 * ValueType contained in the ValueTypeList TypeList. This dispatcher also
 * enforces that all arrays have the same ValueType.
 * If all types are to be considered, use svtkArrayDispatch::AllTypes for the
 * first three template parameters.
...
#include "svtkArrayDispatch.txx"

#endif // svtkArrayDispatch_h
// SVTK-HeaderTest-Exclude: svtkArrayDispatch.h
