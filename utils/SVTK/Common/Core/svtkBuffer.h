...
/**
 * @class   svtkBuffer
 * @brief   internal storage class used by svtkSOADataArrayTemplate,
 * svtkAOSDataArrayTemplate, and others.
 *
 * svtkBuffer makes it easier to keep data pointers in svtkDataArray subclasses.
 * This is an internal class and not intended for direct use expect when writing
 * new types of svtkDataArray subclasses.
 */
...
#include "svtkObjectFactory.h" // New() implementation

#include <algorithm> // for std::min and std::copy

template <class ScalarTypeT>
class svtkBuffer : public svtkObject
{
public:
  svtkTemplateTypeMacro(svtkBuffer<ScalarTypeT>, svtkObject);
  typedef ScalarTypeT ScalarType;

  static svtkBuffer<ScalarTypeT>* New();
...
protected:
  ...
  ~svtkBuffer() override { this->SetBuffer(nullptr, 0); }
...
private:
  svtkBuffer(const svtkBuffer&) = delete;
  void operator=(const svtkBuffer&) = delete;
};

template <class ScalarT>
inline svtkBuffer<ScalarT>* svtkBuffer<ScalarT>::New()
{
  SVTK_STANDARD_NEW_BODY(svtkBuffer<ScalarT>);
}

//------------------------------------------------------------------------------
template <typename ScalarT>
void svtkBuffer<ScalarT>::SetBuffer(typename svtkBuffer<ScalarT>::ScalarType* array, svtkIdType size)
{
...
//------------------------------------------------------------------------------
template <typename ScalarT>
void svtkBuffer<ScalarT>::SetFreeFunction(bool noFreeFunction, void (*deleteFunction)(void*))
{
  if (noFreeFunction)
  {
...
//------------------------------------------------------------------------------
template <typename ScalarT>
bool svtkBuffer<ScalarT>::Allocate(svtkIdType size)
{
  // release old memory.
...
//------------------------------------------------------------------------------
template <typename ScalarT>
bool svtkBuffer<ScalarT>::Reallocate(svtkIdType newsize)
{
  if (newsize == 0)
  {
...
#endif
// SVTK-HeaderTest-Exclude: svtkBuffer.h
