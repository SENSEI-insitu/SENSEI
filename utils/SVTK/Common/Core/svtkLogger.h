...
/**
 * @class svtkLogger
 * @brief logging framework for use in SVTK and in applications based on SVTK
 *
 * svtkLogger acts as the entry point to SVTK's logging framework. The
 * implementation uses the loguru (https://github.com/emilk/loguru). svtkLogger
 ...
 * `svtkLogger::Init(argv, argc)`. This is totally optional but useful to
 ...
 * changed by calling `svtkLogger::SetStderrVerbosity`.
 *
 * In additional to logging to `stderr`, one can accumulate logs to one or more files using
 * `svtkLogger::LogToFile`. Each log file can be given its own verbosity level.
 *
 ...
 * that, use `svtkLogger::SetThreadName`. Calling `svtkLogger::Init` will set the name
 * for the main thread.
 *
 * @section Logging Logging
 *
 * svtkLogger provides several macros (again, based on `loguru`) that can be
 ...
 * e.g. `svtkVLog` vs `svtkLog`. A macro with the `V` prefix takes a fully
 * qualified verbosity enum e.g. `svtkLogger::VERBOSITY_INFO` or
 * `svtkLogger::VERBOSITY_0`, while the non-`V` variant takes the verbosity
 * name e.g. `INFO` or `0`.
 *
 ...
 *  svtkLogger::LogToFile("everything.log", svtkLogger::APPEND, svtkLogger::VERBOSITY_MAX);
 *
 *  // Only log INFO, WARNING, ERROR to "latest_readable.log":
 *  svtkLogger::LogToFile("latest_readable.log", svtkLogger::TRUNCATE, svtkLogger::VERBOSITY_INFO);
 *
 *  // Only show most relevant things on stderr:
 *  svtkLogger::SetStderrVerbosity(svtkLogger::VERBOSITY_1);
 *
 *  // add a line to log using the verbosity name.
 *  svtkLogF(INFO, "I'm hungry for some %.3f!", 3.14159);
 *  svtkLogF(0, "same deal");
 *
 *  // add a line to log using the verbosity enum.
 *  svtkVLogF(svtkLogger::VERBOSITY_INFO, "I'm hungry for some %.3f!", 3.14159);
 *  svtkVLogF(svtkLogger::VERBOSITY_0, "same deal");
 *
 *  // to add an identifier for a svtkObjectBase or subclass
 *  svtkLogF(INFO, "The object is %s", svtkLogIdentifier(svtkobject));
 *
 *  // add a line conditionally to log if the condition succeeds:
 *  svtkLogIfF(INFO, ptr == nullptr, "ptr is nullptr (some number: %.3f)", *  3.14159);
 *
 *  svtkLogScopeF(INFO, "Will indent all log messages within this scope.");
 *  // in a function, you may use svtkLogScopeFunction(INFO)
 *
 *  // scope can be explicitly started and closed by svtkLogStartScope (or
 *  // svtkLogStartScopef) and svtkLogEndScope
 *  svtkLogStartScope(INFO, "id-used-as-message");
 *  svtkLogStartScopeF(INFO, "id", "message-%d", 1);
 *  svtkLogEndScope("id");
 *  svtkLogEndScope("id-used-as-message");
 *
 *  // alternatively, you can use streams instead of printf-style
 *  svtkLog(INFO, "I'm hungry for some " << 3.14159 << "!");
 *  svtkLogIF(INFO, ptr == nullptr, "ptr is " << "nullptr");
 *
 * @endcode
 *
 * @section LoggingAndLegacyMacros Logging and SVTK error macros
 *
 * SVTK has long supported multiple macros to report errors, warnings and debug
 * messages through `svtkErrorMacro`, `svtkWarningMacro`, `svtkDebugMacro`, etc.
 * In addition to performing the traditional message reporting via
 * `svtkOutputWindow`, these macros also log to the logging sub-system with
 * appropriate verbosity levels.
 *
 * To avoid the svtkLogger and svtkOutputWindow both posting the message to the
 * standard output streams, svtkOutputWindow now supports an ability to specify
 * terminal display mode, via `svtkOutputWindow::SetDisplayMode`. If display mode
 * is `svtkOutputWindow::DEFAULT` then the output window will not
 * post messages originating from the standard error/warning/debug macros to the
 * standard output if SVTK is built with logging support. If SVTK is not built
 * with logging support, then svtkOutputWindow will post the messages to the
 * standard output streams, unless disabled explicitly.
 *
 * @section Callbacks Custom callbacks/handlers for log messages
 *
 * svtkLogger supports ability to register callbacks to call on each logged
 * message. This is useful to show the messages in application specific
 * viewports, e.g. a special message widget.
 *
 * To register a callback use `svtkLogger::AddCallback` and to remove a callback
 * use `svtkLogger::RemoveCallback` with the id provided when registering the
 * callback.
 *
 */
...
#include "svtkSetGet.h" // needed for macros
...
#define SVTK_PRINTF_LIKE(fmtarg, firstvararg)                                                       \
  __attribute__((__format__(__printf__, fmtarg, firstvararg)))
#define SVTK_FORMAT_STRING_TYPE const char*
#elif defined(_MSC_VER)
#define SVTK_PRINTF_LIKE(fmtarg, firstvararg)
#define SVTK_FORMAT_STRING_TYPE _In_z_ _Printf_format_string_ const char*
#else
#define SVTK_PRINTF_LIKE(fmtarg, firstvararg)
#define SVTK_FORMAT_STRING_TYPE const char*
#endif

class SVTKCOMMONCORE_EXPORT svtkLogger : public svtkObjectBase
{
public:
  svtkBaseTypeMacro(svtkLogger, svtkObjectBase);
   ...
   * `svtkLogger::SetStderrVerbosity` before calling `svtkLogger::Init`. That
  ...
  /**
   * Returns a printable string for a svtkObjectBase instance.
   */
  static std::string GetIdentifier(svtkObjectBase* obj);

  /**
   * The message structure that is passed to custom callbacks registered using
   * `svtkLogger::AddCallback`.
   */
  struct Message
  {
  ...
  /**
   * Returns true if SVTK is built with logging support enabled.
   */
   ...
   * val is less than or equal to svtkLogger::VERBOSITY_INVALID, then
   * svtkLogger::VERBOSITY_INVALID is returned. If value is greater than
   * svtkLogger::VERBOSITY_MAX, then svtkLogger::VERBOSITY_MAX is returned.
   */
  static Verbosity ConvertToVerbosity(int value);

  /**
   * Convenience function to convert a string to matching verbosity level.
   * svtkLogger::VERBOSITY_INVALID will be return for invalid strings.
  ...
  static void LogF(Verbosity verbosity, const char* fname, unsigned int lineno,
    SVTK_FORMAT_STRING_TYPE format, ...) SVTK_PRINTF_LIKE(4, 5);
  static void StartScopeF(Verbosity verbosity, const char* id, const char* fname,
    unsigned int lineno, SVTK_FORMAT_STRING_TYPE format, ...) SVTK_PRINTF_LIKE(5, 6);

  class SVTKCOMMONCORE_EXPORT LogScopeRAII
  {
  public:
    LogScopeRAII();
    LogScopeRAII(svtkLogger::Verbosity verbosity, const char* fname, unsigned int lineno,
      SVTK_FORMAT_STRING_TYPE format, ...) SVTK_PRINTF_LIKE(5, 6);
    ~LogScopeRAII();
...
private:
  svtkLogger(const svtkLogger&) = delete;
  void operator=(const svtkLogger&) = delete;
};
 ...
 *
 *     // using printf-style
 *     svtkLogF(INFO, "Hello %s", "world!");
 *     svtkVLogF(svtkLogger::VERBOSITY_INFO, "Hello %s", "world!");
 *
 *     // using streams
 *     svtkLog(INFO, "Hello " << "world!");
 *     svtkVLog(svtkLogger::VERBOSITY_INFO, << "Hello world!");
 *
 */
#define svtkVLogF(level, ...)                                                                       \
  ((level) > svtkLogger::GetCurrentVerbosityCutoff())                                               \
    ? (void)0                                                                                      \
    : svtkLogger::LogF(level, __FILE__, __LINE__, __VA_ARGS__)
#define svtkLogF(verbosity_name, ...) svtkVLogF(svtkLogger::VERBOSITY_##verbosity_name, __VA_ARGS__)
#define svtkVLog(level, x)                                                                          \
  if ((level) <= svtkLogger::GetCurrentVerbosityCutoff())                                           \
  {                                                                                                \
    svtkOStrStreamWrapper::EndlType endl;                                                           \
    svtkOStrStreamWrapper::UseEndl(endl);                                                           \
    svtkOStrStreamWrapper svtkmsg;                                                                   \
    svtkmsg << "" x;                                                                                \
    svtkLogger::Log(level, __FILE__, __LINE__, svtkmsg.str());                                       \
    svtkmsg.rdbuf()->freeze(0);                                                                     \
  }
#define svtkLog(verbosity_name, x) svtkVLog(svtkLogger::VERBOSITY_##verbosity_name, x)
//@}
 ...
 *
 *     // using printf-style
 *     svtkLogIfF(ERROR, ptr == nullptr, "`ptr` cannot be null!");
 *     svtkVLogIfF(svtkLogger::VERBOSITY_ERROR, ptr == nullptr, "`ptr` cannot be null!");
 *
 *     // using streams
 *     svtkLogIf(ERROR, ptr == nullptr, "`ptr` cannot be null!");
 *     svtkVLogIf(svtkLogger::VERBOSITY_ERROR, ptr == nullptr, << "`ptr` cannot be null!");
 *
 */
#define svtkVLogIfF(level, cond, ...)                                                               \
  ((level) > svtkLogger::GetCurrentVerbosityCutoff() || (cond) == false)                            \
    ? (void)0                                                                                      \
    : svtkLogger::LogF(level, __FILE__, __LINE__, __VA_ARGS__)

#define svtkLogIfF(verbosity_name, cond, ...)                                                       \
  svtkVLogIfF(svtkLogger::VERBOSITY_##verbosity_name, cond, __VA_ARGS__)

#define svtkVLogIf(level, cond, x)                                                                  \
  if ((level) <= svtkLogger::GetCurrentVerbosityCutoff() && (cond))                                 \
  {                                                                                                \
    svtkOStrStreamWrapper::EndlType endl;                                                           \
    svtkOStrStreamWrapper::UseEndl(endl);                                                           \
    svtkOStrStreamWrapper svtkmsg;                                                                   \
...
#define svtkLogIf(verbosity_name, cond, x) svtkVLogIf(svtkLogger::VERBOSITY_##verbosity_name, cond, x)
//@}

#define SVTKLOG_CONCAT_IMPL(s1, s2) s1##s2
#define SVTKLOG_CONCAT(s1, s2) SVTKLOG_CONCAT_IMPL(s1, s2)
#define SVTKLOG_ANONYMOUS_VARIABLE(x) SVTKLOG_CONCAT(x, __LINE__)

#define svtkVLogScopeF(level, ...)                                                                  \
  auto SVTKLOG_ANONYMOUS_VARIABLE(msg_context) = ((level) > svtkLogger::GetCurrentVerbosityCutoff()) \
    ? svtkLogger::LogScopeRAII()                                                                    \
    : svtkLogger::LogScopeRAII(level, __FILE__, __LINE__, __VA_ARGS__)

#define svtkLogScopeF(verbosity_name, ...)                                                          \
  svtkVLogScopeF(svtkLogger::VERBOSITY_##verbosity_name, __VA_ARGS__)

#define svtkLogScopeFunction(verbosity_name) svtkLogScopeF(verbosity_name, "%s", __func__)
#define svtkVLogScopeFunction(level) svtkVLogScopeF(level, "%s", __func__)
 ...
 */
#define svtkLogStartScope(verbosity_name, id)                                                       \
  svtkLogger::StartScope(svtkLogger::VERBOSITY_##verbosity_name, id, __FILE__, __LINE__)
#define svtkLogEndScope(id) svtkLogger::EndScope(id)

#define svtkLogStartScopeF(verbosity_name, id, ...)                                                 \
  svtkLogger::StartScopeF(svtkLogger::VERBOSITY_##verbosity_name, id, __FILE__, __LINE__, __VA_ARGS__)

#define svtkVLogStartScope(level, id) svtkLogger::StartScope(level, id, __FILE__, __LINE__)
#define svtkVLogStartScopeF(level, id, ...)                                                         \
  svtkLogger::StartScopeF(level, id, __FILE__, __LINE__, __VA_ARGS__)
//@}

/**
 * Convenience macro to generate an identifier string for any svtkObjectBase subclass.
 ...
 */
#define svtkLogIdentifier(svtkobject) svtkLogger::GetIdentifier(svtkobject).c_str()
