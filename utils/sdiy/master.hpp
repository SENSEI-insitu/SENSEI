...
load_outgoing(int gid__)
{
  ...
  if (out_qr.external != -1)
  {
    ...
    for (size_t i = 0; i < count; ++i)
    {
      BlockID to;
      sdiy::load(bb, to);
      sdiy::load(bb, out_qr.queues[to]);
    }
  }
}

sdiy::Master::ProxyWithLink
sdiy::Master::
proxy(int i) const
{ return ProxyWithLink(Proxy(const_cast<Master*>(this), gid(i)), block(i), link(i)); }

sdiy::Master::IProxyWithLink
sdiy::Master::
