...
struct Serialization<io::detail::GidOffsetCount>
{
    ...
    {
      sdiy::save(bb, x.gid);
      sdiy::save(bb, x.offset);
      sdiy::save(bb, x.count);
    }

    static void         load(BinaryBuffer& bb, GidOffsetCount& x)
    {
      sdiy::load(bb, x.gid);
      sdiy::load(bb, x.offset);
      sdiy::load(bb, x.count);
    }
};
 ...
 */
               ...
               Master::SaveBlock            save = 0)              //!< block save function in case different than or undefined in the master
  {
    ...
    // truncate the file
    if (comm.rank() == 0)
        sdiy::io::utils::truncate(outfilename.c_str(), 0);
    ...
    if (comm.rank() == 0)
    {
      ...
      MemoryBuffer oc_buffer; sdiy::save(oc_buffer, offset_counts);
      ...
      for (unsigned i = 0; i < gathered_offset_count_buffers.size(); ++i)
      {
        ...
        sdiy::load(per_rank_oc_buffer, per_rank_offset_counts);
      ...
      MemoryBuffer bb;
      sdiy::save(bb, all_offset_counts);
      sdiy::save(bb, extra);
      size_t footer_size = bb.size();
      sdiy::save(bb, footer_size);
 ...
 */
                ...
                Master::LoadBlock            load = 0)       //!< load block function in case different than or unefined in the master
    {
        ...
        std::vector<GidOffsetCount>  all_offset_counts;
        sdiy::load(footer, all_offset_counts);
        sdiy::load(footer, extra);
        extra.reset();
        ...
        for (unsigned i = 0; i < gids.size(); ++i)
        {
            if (gids[i] != all_offset_counts[gids[i]].gid)
                get_logger()->warn("gids don't match in sdiy::io::read_blocks(), {} vs {}",
 ...
 */
               ...
               Master::SaveBlock            save = 0)              //!< block save function in case different than or undefined in master
  {
    ...
    if (comm.rank() == 0)
    {
        if (sdiy::io::utils::is_directory(outfilename))
          proceed = true;
        else if (sdiy::io::utils::make_directory(outfilename))
          proceed = true;
    ...
    for (int i = 0; i < (int)master.size(); ++i)
    {
        ...
        ::sdiy::detail::FileBuffer bb(fopen(filename.c_str(), "w"));
 ...
 */
              ...
              Master::LoadBlock            load = 0)    //!< block load function in case different than or undefined in master
  {
    ...
    size_t          size;
    {
        ...
        ::sdiy::detail::FileBuffer bb(fopen(filename.c_str(), "r"));
        ::sdiy::load(bb, size);
        ::sdiy::load(bb, extra);
        extra.reset();
        fclose(bb.file);
