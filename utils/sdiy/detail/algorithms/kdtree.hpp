...
{
    typedef     sdiy::RegularContinuousLink      RCLink;
    typedef     sdiy::ContinuousBounds           Bounds;
    ...
    void        operator()(Block* b, const sdiy::ReduceProxy& srp, const KDTreePartners& partners) const;

    int         divide_gid(int gid, bool lower, int round, int rounds) const;
    void        update_links(Block* b, const sdiy::ReduceProxy& srp, int dim, int round, int rounds, bool wrap, const Bounds& domain) const;
    void        split_to_neighbors(Block* b, const sdiy::ReduceProxy& srp, int dim) const;
    sdiy::Direction
                find_wrap(const Bounds& bounds, const Bounds& nbr_bounds, const Bounds& domain) const;

    void        compute_local_histogram(Block* b, const sdiy::ReduceProxy& srp, int dim) const;
    void        add_histogram(Block* b, const sdiy::ReduceProxy& srp, Histogram& histogram) const;
    void        receive_histogram(Block* b, const sdiy::ReduceProxy& srp,       Histogram& histogram) const;
    void        forward_histogram(Block* b, const sdiy::ReduceProxy& srp, const Histogram& histogram) const;

    void        enqueue_exchange(Block* b, const sdiy::ReduceProxy& srp, int dim, const Histogram& histogram) const;
    void        dequeue_exchange(Block* b, const sdiy::ReduceProxy& srp, int dim) const;
...
struct sdiy::detail::KDTreePartners
{
  ...
  inline bool   active(int round, int gid, const sdiy::Master& m) const
  {
    if (round == (int) rounds())
        return true;
  ...
  inline void   incoming(int round, int gid, std::vector<int>& partners, const sdiy::Master& m) const
  {
    if (round == (int) rounds())
  ...
  inline void   outgoing(int round, int gid, std::vector<int>& partners, const sdiy::Master& m) const
  {
    if (round == (int) rounds())
  ...
  inline void   link_neighbors(int, int gid, std::vector<int>& partners, const sdiy::Master& m) const
  {
    int         lid  = m.lid(gid);
    sdiy::Link*  link = m.link(lid);
  ...
  sdiy::RegularDecomposer<sdiy::DiscreteBounds>   decomposer;

  sdiy::RegularAllReducePartners     histogram;
  sdiy::RegularSwapPartners          swap;
...
template<class Block, class Point>
void
sdiy::detail::KDTreePartition<Block,Point>::
operator()(Block* b, const sdiy::ReduceProxy& srp, const KDTreePartners& partners) const
{
    int dim;
...
void
sdiy::detail::KDTreePartition<Block,Point>::
update_links(Block* b, const sdiy::ReduceProxy& srp, int dim, int round, int rounds, bool wrap, const Bounds& domain) const
{
    int         gid  = srp.gid();
    ...
    // (gid, dir) -> i
    std::map<std::pair<int,sdiy::Direction>, int> link_map;
    for (int i = 0; i < link->size(); ++i)
    ...
    for (int i = 0; i < link->size(); ++i)
    {
        float split; sdiy::Direction dir;
    ...
    for (int i = 0; i < link->size(); ++i)
    {
        sdiy::Direction  dir      = link->direction(i);
        //sdiy::Direction  wrap_dir = link->wrap(i);     // we don't use existing wrap, but restore it from scratch
        if (dir[dim] != 0)
        {
            if ((dir[dim] < 0 && lower) || (dir[dim] > 0 && !lower))
            {
                int nbr_gid = divide_gid(link->target(i).gid, !lower, round, rounds);
                sdiy::BlockID nbr = { nbr_gid, srp.assigner().rank(nbr_gid) };
                ...
                if (wrap)
                    ...
                    new_link.add_wrap(sdiy::Direction());
            }
    ...
    sdiy::BlockID dual = { dual_gid, srp.assigner().rank(dual_gid) };
    ...
    new_link.add_wrap(sdiy::Direction());    // dual block cannot be wrapped

    if (lower)
    {
        sdiy::Direction right;
        right[dim] = 1;
...
void
sdiy::detail::KDTreePartition<Block,Point>::
split_to_neighbors(Block* b, const sdiy::ReduceProxy& srp, int dim) const
{
...
void
sdiy::detail::KDTreePartition<Block,Point>::
compute_local_histogram(Block* b, const sdiy::ReduceProxy& srp, int dim) const
{
    int         lid  = srp.master()->lid(srp.gid());
    RCLink*     link = static_cast<RCLink*>(srp.master()->link(lid));
...
void
sdiy::detail::KDTreePartition<Block,Point>::
add_histogram(Block* b, const sdiy::ReduceProxy& srp, Histogram& histogram) const
{
...
void
sdiy::detail::KDTreePartition<Block,Point>::
receive_histogram(Block* b, const sdiy::ReduceProxy& srp, Histogram& histogram) const
{
...
void
sdiy::detail::KDTreePartition<Block,Point>::
forward_histogram(Block* b, const sdiy::ReduceProxy& srp, const Histogram& histogram) const
{
...
void
sdiy::detail::KDTreePartition<Block,Point>::
enqueue_exchange(Block* b, const sdiy::ReduceProxy& srp, int dim, const Histogram& histogram) const
{
    auto        log = get_logger();
...
void
sdiy::detail::KDTreePartition<Block,Point>::
dequeue_exchange(Block* b, const sdiy::ReduceProxy& srp, int dim) const
{
    int         lid  = srp.master()->lid(srp.gid());
    RCLink*     link = static_cast<RCLink*>(srp.master()->link(lid));
