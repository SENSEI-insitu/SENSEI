...
namespace sdiy
{
  ...
  struct Master::Proxy
  {
    ...
    void                enqueue(const BlockID&  to,                                     //!< target block (gid,proc)
                                ...
                                void (*save)(BinaryBuffer&, const T&) = &::sdiy::save<T> //!< optional serialization function
                               ) const
    ...
    //! previously enqueued so that sdiy knows its size when it is received.
    //! In this case, sdiy will allocate the receive buffer; the user does not need to do so.
    template<class T>
    void                dequeue(int             from,                                   //!< target block gid
                                T&              x,                                      //!< data (eg. STL vector)
                                void (*load)(BinaryBuffer&, T&) = &::sdiy::load<T>       //!< optional serialization function
                               ) const
    ...
    EnqueueIterator<T>  enqueuer(const T& x,
                                 void (*save)(BinaryBuffer&, const T&) = &::sdiy::save<T>) const
 ...
 */
  ...
  struct Master::IProxyWithLink: public Master::ProxyWithLink
  {
      ...
      void enqueue(const BlockID&     to,
              ...
              void (*save)(BinaryBuffer&, const T&) = &::sdiy::save<T>) const
      {
          sdiy::Master::Proxy::enqueue(to, x, save);
      ...
      void enqueue(const BlockID&     to,
              ...
              void (*save)(BinaryBuffer&, const T&) = &::sdiy::save<T>) const
      {
          sdiy::Master::Proxy::enqueue(to, x, n, save);
          master()->icommunicate(iexchange);
      }
      ...
      void dequeue(int                from,
              T&                      x,
              void (*load)(BinaryBuffer&, T&) = &::sdiy::load<T>) const
      {
          master()->icommunicate(iexchange);
          sdiy::Master::Proxy::dequeue(from, x, load);
      }
      ...
      {
          master()->icommunicate(iexchange);
          sdiy::Master::Proxy::dequeue(from, x, n, load);
      }
  };
}
        ...
        void (*save)(BinaryBuffer&, const T&)) const
{
    ...
    if (save == (void (*)(BinaryBuffer&, const T&)) &::sdiy::save<T>)
        sdiy::save(bb, x, n);       // optimized for unspecialized types
    else
        ...
        void (*load)(BinaryBuffer&, T&)) const
{
    ...
    if (load == (void (*)(BinaryBuffer&, T&)) &::sdiy::load<T>)
        sdiy::load(bb, x, n);       // optimized for unspecialized types
    else
        for (size_t i = 0; i < n; ++i)
            load(bb, x[i]);
}
